# OpenAI-Compatible API Server for Ruslan Agent
...
SSE streaming. By exposing this endpoint, ruslan-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  ruslan-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.ruslan/.env`
2. Run `ruslan gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with ruslan-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns ruslan-agent as a model) |
...
Authorization: Bearer ruslan-api-key-here
...
But ruslan-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "ruslan-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured ruslan-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of ruslan-agent's core prompt
...
Once implemented, ruslan-agent works as a drop-in backend for:
