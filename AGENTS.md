# Ruslan Agent - Development Guide

Instructions for AI coding assistants and developers working on the ruslan-agent codebase.

**Never give up on the right solution.**

## What Ruslan Is

Ruslan is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Ruslan ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`ruslan tools`, `ruslan setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `RUSLAN_HOME`. Mocks hide integration bugs.
...
- **New `RUSLAN_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `ruslan tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.ruslan/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `ruslan <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `ruslan webhook`, `ruslan cron`, `ruslan tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.ruslan/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.ruslan/ruslan-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _RUSLAN_CORE_TOOLS list
├── cli.py                # RuslanCLI class — interactive CLI orchestrator (~11k LOC)
├── ruslan_state.py       # SessionDB — SQLite session store (FTS5 search)
├── ruslan_constants.py   # get_ruslan_home(), display_ruslan_home() — profile-aware paths
├── ruslan_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── ruslan_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── ruslan-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `ruslan --tui`
...
**User config:** `~/.ruslan/config.yaml` (settings), `~/.ruslan/.env` (API keys only).
**Logs:** `~/.ruslan/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_ruslan_home()`.
Browse with `ruslan logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Ruslan: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`ruslan_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `RuslanCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.ruslan/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`ruslan_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/ruslan` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `ruslan_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `RuslanCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `ruslan --tui` or `RUSLAN_TUI=1`.
...
npm run dev       # watch mode (rebuilds ruslan-ink + tsx --watch)
npm start         # production
npm run build     # full build (ruslan-ink + tsc)
...
### TUI in the Dashboard (`ruslan dashboard` → `/chat`)

The dashboard embeds the real `ruslan --tui` — **not** a rewrite.  See `ruslan_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `ruslan_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `ruslan --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `ruslan --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@ruslan/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `ruslan serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `RUSLAN_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `ruslan` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `ruslan --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Ruslan built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Ruslan core.
Use the plugin route instead: create `~/.ruslan/plugins/<name>/plugin.yaml`
and `~/.ruslan/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Ruslan tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_RUSLAN_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_RUSLAN_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_ruslan_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `RUSLAN_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_ruslan_home()` for the base directory — never `Path.home() / ".ruslan"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `ruslan_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `ruslan_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `ruslan tools`, `ruslan setup`, most CLI subcommands | `ruslan_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`ruslan_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
ruslan_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.ruslan/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Ruslan gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `ruslan_cli/skin_engine.py`:
...
Users create `~/.ruslan/skins/<name>.yaml`:
...
Ruslan has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.ruslan/plugins/` and pip-installed entry points.

### General plugins (`ruslan_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.ruslan/plugins/`, `./.ruslan/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `ruslan` at startup so
  `ruslan <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(ruslan_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `ruslan <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `ruslan --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `ruslan_cli/main.py`, etc.).
...
into `~/.ruslan/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `ruslan memory setup` / `post_setup()` without
...
`~/.ruslan/plugins/` (or via pip entry points). They register through
...
2. User: `$RUSLAN_HOME/plugins/model-providers/<name>/`
...
[`ruslan-example-plugins`](https://github.com/NousResearch/ruslan-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `ruslan skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.ruslan.tags`, `metadata.ruslan.category`,
`metadata.ruslan.related_skills`, `metadata.ruslan.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.ruslan.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Ruslan tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Ruslan Agent" is the secondary collaborator. If the
   contributor's commit shows "Ruslan Agent" as author (because they
   used Ruslan to draft the skill), replace it with their actual name
...
lives in the `ruslan-agent-dev` skill at
...
`"messaging"`); `_RUSLAN_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `ruslan tools` (the curses UI) or the
...
Ruslan returns a delegation id immediately and the result re-enters the
...
go to `~/.ruslan/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `ruslan_cli/curator.py` wires `ruslan curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.ruslan/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `ruslan cron <verb>`
...
- File lock at `~/.ruslan/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `ruslan kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `ruslan_cli/kanban.py` wires `ruslan kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`ruslan-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `RUSLAN_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Ruslan-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `RUSLAN_BACKGROUND_NOTIFICATIONS` env var):
...
Ruslan supports **profiles** — multiple fully isolated instances, each with its own
`RUSLAN_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `ruslan_cli/main.py` sets
`RUSLAN_HOME` before any module imports. All `get_ruslan_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_ruslan_home()` for all RUSLAN_HOME paths.** Import from `ruslan_constants`.
   NEVER hardcode `~/.ruslan` or `Path.home() / ".ruslan"` in code that reads/writes state.
   ```python
   # GOOD
   from ruslan_constants import get_ruslan_home
   config_path = get_ruslan_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".ruslan" / "config.yaml"
   ```

2. **Use `display_ruslan_home()` for user-facing messages.** Import from `ruslan_constants`.
   This returns `~/.ruslan` for default or `~/.ruslan/profiles/<name>` for profiles.
   ```python
   # GOOD
   from ruslan_constants import display_ruslan_home
   print(f"Config saved to {display_ruslan_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.ruslan/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_ruslan_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_ruslan_home()`,
   not `Path.home() / ".ruslan"`.

4. **Tests that mock `Path.home()` must also set `RUSLAN_HOME`** — since code now uses
   `get_ruslan_home()` (reads env var), not `Path.home() / ".ruslan"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"RUSLAN_HOME": str(tmp_path / ".ruslan")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not RUSLAN_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".ruslan" / "profiles"`, NOT `get_ruslan_home() / "profiles"`.
   This is intentional — it lets `ruslan -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.ruslan` paths
Use `get_ruslan_home()` from `ruslan_constants` for code paths. Use `display_ruslan_home()`
for user-facing print/log messages. Hardcoding `~/.ruslan` breaks profiles — each profile
has its own `RUSLAN_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `ruslan_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `ruslan_cli/curses_ui.py` — see
`ruslan_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `RUSLAN_HOME`.

### Tests must not write to `~/.ruslan/`
The `_isolate_ruslan_home` autouse fixture in `tests/conftest.py` redirects `RUSLAN_HOME` to a temp dir. Never hardcode `~/.ruslan/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_ruslan_home()` resolve within the temp dir.
Use the pattern from `tests/ruslan_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("RUSLAN_HOME", str(home))
    return home
```
...
| HOME / `~/.ruslan/` | Your real config+auth.json                  | Temp dir per test                         |
