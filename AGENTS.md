...
A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `ruslan --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `ruslan-desktop-app-work` skill, not `ruslan-dashboard-work`.
...
[`ruslan-example-plugins`](https://github.com/valldun1/ruslan)
...
- The plugin disables itself in child processes (sentinel envvar
  `RUSLAN_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
