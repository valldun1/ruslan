# Contribuir a Ruslan Agent

¡Gracias por contribuir a Ruslan Agent! Esta guía cubre todo lo que necesitas: configurar tu entorno de desarrollo, entender la arquitectura, decidir qué construir y conseguir que tu PR sea aceptado.
...
2. **Compatibilidad entre plataformas** — macOS, diferentes distribuciones de Linux y WSL2 en Windows. Queremos que Ruslan funcione en todas partes.
...
Las habilidades incluidas (en `skills/`) se envían con cada instalación de Ruslan. Deben ser **ampliamente útiles para la mayoría de los usuarios**:
...
**Ya no aceptamos nuevos proveedores de memoria en este repositorio.** El conjunto de proveedores integrados en `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) está cerrado. Si quieres añadir un nuevo backend de memoria, publícalo como un **repositorio de plugin independiente** que los usuarios instalen en `~/.ruslan/plugins/` (o a través de un entry point de pip).

Los plugins de memoria independientes:

- Implementan el mismo ABC `MemoryProvider` (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown` y opcionalmente `post_setup(ruslan_home, config)` para integración con el asistente de configuración
...
- Se integran con `ruslan memory setup` a través de `post_setup()` — sin necesidad de tocar el código base
...
│
├── ruslan_cli/               # Implementaciones de comandos CLI
...
### Configuración del usuario (almacenada en `~/.ruslan/`)

| Ruta | Propósito |
|------|-----------|
| `~/.ruslan/config.yaml` | Configuración (modelo, terminal, toolsets, compresión, etc.) |
| `~/.ruslan/.env` | Claves API y secretos |
| `~/.ruslan/auth.json` | Credenciales OAuth (Nous Portal) |
| `~/.ruslan/skills/` | Todas las habilidades activas (incluidas + instaladas desde hub + creadas por el agente) |
| `~/.ruslan/memories/` | Memoria persistente (MEMORY.md, USER.md) |
| `~/.ruslan/state.db` | Base de datos de sesiones SQLite |
| `~/.ruslan/sessions/` | Índice de enrutamiento del gateway (`sessions.json`), migas de pan de solicitudes, transcripciones `*.jsonl` del gateway y (opcionalmente) snapshots JSON por sesión cuando `sessions.write_json_snapshots: true` está configurado. Los snapshots por sesión están desactivados por defecto; state.db es canónica. |
| `~/.ruslan/cron/` | Datos de trabajos programados |
| `~/.ruslan/whatsapp/session/` | Credenciales del puente WhatsApp |
...
- **Persistencia de sesión**: Todas las conversaciones se almacenan en SQLite (`ruslan_state.py`) con búsqueda de texto completo y títulos de sesión únicos.
...
(por ejemplo `_RUSLAN_CORE_TOOLS` o un toolset dedicado); de lo contrario la herramienta
...
2. **Las herramientas referenciadas en el cuerpo de SKILL.md deben ser herramientas nativas de Ruslan o servidores MCP que la habilidad espere explícitamente.** Usa los nombres de herramientas en comillas invertidas: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, etc.
...
Ruslan usa un sistema de skins basado en datos — no se necesitan cambios de código para añadir una nueva skin.

**Opción A: Skin de usuario (archivo YAML)**

Crea `~/.ruslan/skins/<nombre>.yaml`:
...
Añade al dict `_BUILTIN_SKINS` en `ruslan_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
...
Ruslan se ejecuta en Linux, macOS y Windows nativo (además de WSL2). Al escribir código
...
Ruslan tiene acceso al terminal. La seguridad importa.
...
2. **Probar manualmente**: Ejecuta `ruslan` y ejercita la ruta de código que cambiaste
