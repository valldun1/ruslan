# Política de Seguridad de Ruslan Agent

Este documento describe el modelo de confianza de Ruslan Agent, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/NousResearch/ruslan-agent/security/advisories/new)
o **security@nousresearch.com**. No abras issues públicos para
vulnerabilidades de seguridad. **Ruslan Agent no opera un programa de
...
- Detalles del entorno (`ruslan version`, SHA del commit, SO, versión de Python).
...
Ruslan Agent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Ruslan Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Ruslan Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Ruslan Agent
...
Ruslan Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
Ruslan Agent admite esto de dos maneras:

- **La propia imagen Docker de Ruslan Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Ruslan Agent
...
Ruslan Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Ruslan Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Ruslan Agent
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Ruslan Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Ruslan Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Ruslan Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de ruslan-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Ruslan Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Ruslan Agent. Los errores en la ruta de
  instalación de habilidades o plugins de Ruslan Agent que impidan al
...
- Ruslan Agent incluye guardias de cadena de suministro para lanzamientos de servidores
