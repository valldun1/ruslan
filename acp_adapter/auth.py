"""ACP auth helpers — detect and advertise Ruslan authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "ruslan-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Ruslan runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Ruslan can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Ruslan.
    ...
    not have Ruslan provider credentials configured yet, so Ruslan always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Ruslan using the currently configured "
