"""CLI entry point for the ruslan-agent ACP adapter.

Loads environment variables from ``~/.ruslan/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from RUSLAN_HOME (default ``~/.ruslan``)."""
    from ruslan_cli.env_loader import load_ruslan_dotenv

    ruslan_home = get_ruslan_home()
    loaded = load_ruslan_dotenv(ruslan_home=ruslan_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", ruslan_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="ruslan-acp",
        description="Run Ruslan Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Ruslan version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Ruslan provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.ruslan/node/ "
...
def _run_setup() -> None:
    from ruslan_cli.main import main as ruslan_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "ruslan", "model"]
        ruslan_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``ruslan postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting ruslan-agent ACP adapter")
