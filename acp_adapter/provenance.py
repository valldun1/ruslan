...
This is an additive Ruslan extension surfaced under ACP ``_meta.ruslan`` so
...
compression rotates the internal Ruslan head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.ruslan.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_ruslan_session_id: The live internal Ruslan DB session id
            (``state.agent.session_id``).
        previous_ruslan_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"ruslan": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_ruslan_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_ruslan_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_ruslan_session_id}
    ...
    rotated = bool(
        ...
        and previous_ruslan_session_id != current_ruslan_session_id
    )
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"ruslan": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"ruslan": {"sessionProvenance": prov}}
