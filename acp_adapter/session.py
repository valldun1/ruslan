"""ACP session manager — maps ACP sessions to Ruslan AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.ruslan/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Ruslan itself is running in WSL.

    Windows ACP clients can launch ``ruslan acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from ruslan_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Ruslan from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["ruslan-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Ruslan agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Ruslan AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Ruslan runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.ruslan/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``RUSLAN_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_ruslan_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=ruslan_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Ruslan daemon's process cwd.
