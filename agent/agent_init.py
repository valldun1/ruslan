...
from ruslan_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: ruslan config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$RUSLAN_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_ruslan_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$RUSLAN_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .ruslan.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / RUSLAN_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.ruslan/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from ruslan_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind RUSLAN_DEV_CREDITS.
    ...
    # both live under ~/.ruslan/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from ruslan_logging import setup_logging, setup_verbose_logging
    setup_logging(ruslan_home=_ra()._ruslan_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that ruslan_logging.setup_logging() does not
        ...
        # handler level inside ruslan_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from ruslan_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from ruslan_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `ruslan model` to "
                        "select a provider, or run `ruslan setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff RUSLAN_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["RUSLAN_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.ruslan/sessions/ alongside gateway sessions
    ruslan_home = get_ruslan_home()
    agent.logs_dir = ruslan_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.ruslan/sessions/session_{sid}.json)
    ...
    try:
        from ruslan_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from ruslan_cli.config import load_config as _load_agent_config
        _agent_cfg = _load_agent_config()
    except Exception:
        _agent_cfg = {}

    # LM Studio can either be explicitly preloaded through LM Studio's
    # management API (the historical Ruslan behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "ruslan"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Ruslan' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "ruslan", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, ruslan, off.",
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from ruslan_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from ruslan_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Ruslan Agent.  Choose a model with at least "
    ...
    # Nous Ruslan 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from ruslan_cli.model_switch import _check_ruslan_model_warning

            _ruslan_warn = _check_ruslan_model_warning(agent.model or "")
            if _ruslan_warn:
                _user_msg = (
                    "⚠ Nous Research Ruslan 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_ruslan_warn)
        except Exception:
            pass
