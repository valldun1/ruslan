...
def _codex_note_to_tool_progress(note: dict) -> tuple[str, str, dict] | None:
    """Map a Codex app-server ``item/started`` notification to a Ruslan
    ...
    surfaced them as Ruslan tool-progress events — so gateways (Telegram, etc.)
...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Ruslan accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    Ruslan' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, Ruslan should still count that turn
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Ruslan state.

    The app-server owns the compacted thread context, so Ruslan should not
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior Ruslan deterministic-fallback
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Ruslan'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to Ruslan' standard prompt flow if a
        ...
        # explicitly opted out of Ruslan approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / RUSLAN_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing Ruslan UI. Defaults (manual/smart/unset) preserve the
        ...
        def _on_codex_event(note: dict) -> None:
            # Bridge Codex app-server item/started notifications to Ruslan
