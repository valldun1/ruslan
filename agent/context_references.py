...
_SENSITIVE_RUSLAN_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    ruslan_home = get_ruslan_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(ruslan_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(ruslan_home / rel for rel in _SENSITIVE_RUSLAN_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Ruslan path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.ruslan/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Ruslan path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
