...
``auth.json``.  They intentionally have no dependency on ``ruslan_cli.auth`` so
...
# Sources Ruslan owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Ruslan-owned OAuth/device-code state)
