...
Ruslan keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Ruslan's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Ruslan)
        ...
        with httpx.Client(timeout=timeout) as client:
            resp = client.post(
                ...
                headers={
                    ...
                    "X-Goog-Api-Client": f"ruslan-agent/{_RUSLAN_VERSION}",
                },
            )
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Ruslan typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Ruslan retry/error classification."""
...
def build_gemini_request(
    *,
    ...
    if max_tokens is not None:
        ...
        # finishReason=MAX_TOKENS, truncating tool calls mid-stream (Ruslan
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.ruslan/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `ruslan setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": f"ruslan-agent/{_RUSLAN_VERSION} (gemini-native)",
            "X-Goog-Api-Client": f"ruslan-agent/{_RUSLAN_VERSION}",
        }
