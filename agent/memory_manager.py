...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_RUSLAN_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Ruslan expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``ruslan_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_ruslan_home()`` themselves.
        """
        if "ruslan_home" not in kwargs:
            from ruslan_constants import get_ruslan_home
            kwargs["ruslan_home"] = str(get_ruslan_home())
        for provider in self._providers:
            try:
