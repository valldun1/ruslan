...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `ruslan backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - ruslan_home (str): The active RUSLAN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.ruslan``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "ruslan").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'ruslan memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], ruslan_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'ruslan memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``ruslan_home`` is the active RUSLAN_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE RUSLAN_HOME.

        ``ruslan backup`` only walks RUSLAN_HOME, so any provider state kept
        ...
        of the archive, and ``ruslan import`` restores them to their original
