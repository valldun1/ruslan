...
to ``<ruslan_home>/moa-traces/<session_id>.jsonl``. The record is the TRUE
...
def _traces_enabled_and_dir() -> Optional[Path]:
    ...
    ``moa.trace_dir`` overrides the default ``<ruslan_home>/moa-traces/``.
    """
    try:
    ...
    if override:
        ...
        base = get_ruslan_home() / "moa-traces"
    return base
