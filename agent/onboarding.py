...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Ruslan starts and finds ``~/.openclaw/``.

    Points users at ``ruslan claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``ruslan claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Ruslan, run "
        "`ruslan claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `ruslan claw cleanup` (renames it to ~/.openclaw.pre-migration — "
