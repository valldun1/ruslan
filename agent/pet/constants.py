...
row taxonomy is inferred from the atlas shape so Ruslan can render both legacy
...
# rows (1536x1872), while older Ruslan/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Ruslan' activity state names. They are not always identical to the
...
# Legacy Ruslan/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Ruslan activity names -> accepted row-name aliases in descending
