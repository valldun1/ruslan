...
Plugins built on Ruslan Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Ruslan
...
:class:`~ruslan_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Ruslan' main config
...
handles every provider, fallback chain, and per-task override Ruslan
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`ruslan_cli.plugins.PluginContext`
