...
Every Ruslan request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Ruslan Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=ruslan-agent",
        "client=ruslan-client-v<__version__>",
    ]

The version is sourced live from ``ruslan_cli.__version__`` so it auto-aligns
...
``ruslan_cli.__version__`` is the canonical source of truth.
"""
...
def _ruslan_version() -> str:
    """Return the current Ruslan release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``ruslan_cli`` cannot be imported (should
...
def ruslan_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=ruslan-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=ruslan-client-v{_ruslan_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Ruslan session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Ruslan conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.ruslan/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Ruslan
    ...
    """
    tags = ["product=ruslan-agent", ruslan_client_tag()]
