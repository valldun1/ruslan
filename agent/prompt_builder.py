...
from ruslan_constants import get_ruslan_home, get_skills_dir, is_wsl
...
_RUSLAN_MD_NAMES = (".ruslan.md", "RUSLAN.md")


def _find_ruslan_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.ruslan.md`` or ``RUSLAN.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Ruslan Agent, an intelligent AI assistant created by Valldun. "
...
RUSLAN_AGENT_HELP_GUIDANCE = (
    "You run on Ruslan Agent (by Valldun). When the user needs help with "
    "Ruslan itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://ruslan.team/docs is your "
...
#
# The ruslan-agent runtime already executes a batch of tool calls
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `ruslan computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Ruslan "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Ruslan WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Ruslan itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Ruslan runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Ruslan itself is running. The host OS, home, and cwd "
                f"of the Ruslan process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Ruslan "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Ruslan
    ...
    extra = (os.getenv("RUSLAN_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_ruslan_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.ruslan/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Ruslan Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `ruslan-agent` skill "
            "first. It has the actual commands (e.g. `ruslan config set …`, `ruslan tools`, "
            "`ruslan setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: ruslan setup, ruslan setup tools, ruslan setup terminal, ruslan status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from RUSLAN_HOME and return its content, or None.
    ...
    """
    try:
        from ruslan_cli.config import ensure_ruslan_home
        ensure_ruslan_home()
    except Exception as e:
        logger.debug("Could not ensure RUSLAN_HOME before loading SOUL.md: %s", e)

    soul_path = get_ruslan_home() / "SOUL.md"
...
def _load_ruslan_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.ruslan.md / RUSLAN.md — walk to git root."""
    ruslan_md_path = _find_ruslan_md(cwd_path)
    if not ruslan_md_path:
        return ""
    try:
        content = ruslan_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = ruslan_md_path.name
        try:
            rel = str(ruslan_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".ruslan.md", context_length=context_length,
            read_path=str(ruslan_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", ruslan_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .ruslan.md / RUSLAN.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from RUSLAN_HOME is independent and always included when present.
    ...
    # SOUL.md from RUSLAN_HOME only — skip when already loaded as identity
    if not skip_soul:
