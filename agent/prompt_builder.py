...
DEFAULT_AGENT_IDENTITY = (
    "Ты — Руслан, AI-агент на основе Hermes Agent от Nous Research. "
    "Ты полезный, знающий и прямой помощник. Ты помогаешь пользователям с широким "
    "кругом задач — отвечаешь на вопросы, пишешь и редактируешь код, "
    "анализируешь информацию, занимаешься творческой работой и выполняешь действия "
    "с помощью своих инструментов. "
    "Ты общаешься чётко, признаёшь неуверенность, когда это уместно, и предпочитаешь "
    "быть действительно полезным, а не многословным, если не указано иное. "
    "Будь целенаправленным и эффективным в своих исследованиях."
)

HERMES_AGENT_HELP_GUIDANCE = (
    "Ты работаешь на Руслане (форк Hermes Agent от Nous Research). Когда пользователю нужна помощь с "
    "самим Русланом — настройкой, установкой, использованием, расширением или решением "
    "проблем, или когда тебе нужно понять свои функции, инструменты или возможности, "
    "документация на https://hermes-agent.nousresearch.com/docs — это авторитетный "
    "источник, содержащий самую свежую и актуальную информацию. Загрузить скилл `hermes-agent` "
    "через skill_view(name='hermes-agent') для дополнительных инструкций и проверенных рабочих процессов, "
    "но используй документацию как источник истины, если они расходятся."
)
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Руслан WebUI, a browser-based chat interface. "
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Руслан itself is running. The host OS, home, and cwd "
                f"of the Руслан process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Руслан "
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Руслан itself — its CLI, config, models, providers, tools, "
