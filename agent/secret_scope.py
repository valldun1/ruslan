...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the RUSLAN_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Ruslan runtime / deployment
    "RUSLAN_HOME", "RUSLAN_PROFILE", "RUSLAN_GATEWAY_LOCK_DIR",
    "RUSLAN_MAX_ITERATIONS", "RUSLAN_MAX_TOKENS", "RUSLAN_API_TIMEOUT",
    "RUSLAN_REDACT_SECRETS", "RUSLAN_NOUS_TIMEOUT_SECONDS",
    "_RUSLAN_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "RUSLAN_KANBAN_DB", "RUSLAN_KANBAN_WORKSPACES_ROOT", "RUSLAN_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "RUSLAN_KANBAN_",
    "RUSLAN_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(ruslan_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(ruslan_home) / ".env")
