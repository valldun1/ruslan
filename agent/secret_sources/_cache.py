...
cache problem must never block Ruslan startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Ruslan home used for cache paths.

    ``home_path`` is whatever ``load_ruslan_dotenv()`` already resolved;
    falling back to ``$RUSLAN_HOME`` / ``~/.ruslan`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<ruslan_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if RUSLAN_HOME is ever made traversable.
            try:
