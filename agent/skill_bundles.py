...
Storage
-------
Bundles live in ``~/.ruslan/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``RUSLAN_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``ruslan bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under RUSLAN_HOME.

    Honors ``RUSLAN_BUNDLES_DIR`` for tests; falls back to
    ``<RUSLAN_HOME>/skill-bundles``.
    """
    override = os.environ.get("RUSLAN_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_ruslan_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `ruslan bundles` CLI subcommand.
# ---------------------------------------------------------------------------
