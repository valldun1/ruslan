...
#
# When a user invokes a /skill (or /bundle), Ruslan expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_ruslan_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``RUSLAN_PLATFORM`` env var and
    ``RUSLAN_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("RUSLAN_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.ruslan.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_ruslan_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.ruslan/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
                ...
                try:
                    ...
                    # with a core Ruslan slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Ruslan command; skipping "
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.ruslan/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``ruslan -s <skill>`` or
    a deployment's ``RUSLAN_TUI_SKILLS`` env var could force-load a skill an
