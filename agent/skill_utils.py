...
from ruslan_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``RUSLAN_KANBAN_TASK`` / ``RUSLAN_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("RUSLAN_KANBAN_TASK") or os.getenv("RUSLAN_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Ruslan Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``ruslan_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``RUSLAN_PLATFORM`` or
            ``RUSLAN_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``ruslan`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.ruslan/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``ruslan`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against RUSLAN_HOME, not cwd
        if not p.is_absolute():
            p = (ruslan_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.ruslan/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.ruslan/skills/`` (including via symlinks) or configured
    ...
    # ~/.ruslan/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Ruslan can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    ruslan = metadata.get("ruslan") or {}
    if not isinstance(ruslan, dict):
        ruslan = {}
    return {
        "fallback_for_toolsets": ruslan.get("fallback_for_toolsets", []),
        "requires_toolsets": ruslan.get("requires_toolsets", []),
        "fallback_for_tools": ruslan.get("fallback_for_tools", []),
        "requires_tools": ruslan.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    ruslan = metadata.get("ruslan")
    if not isinstance(ruslan, dict):
        return []
    raw = ruslan.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Ruslan metadata, VCS, virtualenv/dependency, cache, and skill
