...
upstream prefix cache warm.  See ``ruslan-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``RUSLAN_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("RUSLAN_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Ruslan never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want RUSLAN_HOME persona while keeping
    ...
    # Pointer to the ruslan-agent skill + docs for user questions about Ruslan itself.
    stable_parts.append(RUSLAN_AGENT_HELP_GUIDANCE)
    ...
    # RUSLAN_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Ruslan, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Ruslan profile the agent is running
    # under so it doesn't conflate ~/.ruslan/skills/ (default profile) with
    # ~/.ruslan/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Ruslan profile: default. Other profiles (if any) live "
            "under ~/.ruslan/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Ruslan profile: {active_profile}. This session reads "
            f"and writes ~/.ruslan/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.ruslan/skills/, ~/.ruslan/plugins/, "
            f"~/.ruslan/cron/, ~/.ruslan/memories/ — those belong to a "
    ...
    if platform_key == "telegram" and _default_hint:
        try:
            from ruslan_cli.config import load_config_readonly
    ...
    if not agent.skip_context_files:
        ...
        # (developing Ruslan). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Ruslan never rebuilds or reinjects parts of it
