"""Upload a Ruslan session transcript to Hugging Face as an agent trace.

Ruslan stores sessions in its own SQLite store (``ruslan_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``ruslan trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Ruslan' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "ruslan-traces"
_RUSLAN_VERSION = "ruslan-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Ruslan OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Ruslan' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Ruslan conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.ruslan/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `ruslan trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/ruslan-traces`` dataset. Returns a
