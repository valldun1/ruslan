...
projection into Ruslan' display, approval bridging, transcript projection into
...
"codex_app_server"`. Ruslan' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Ruslan secret — gateway
        ...
        spawn_env = ruslan_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("RUSLAN_KANBAN_TASK"):
            kanban_db = spawn_env.get("RUSLAN_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "RUSLAN_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("RUSLAN_HOME", os.path.expanduser("~/.ruslan")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "ruslan",
        client_title: str = "Ruslan Agent",
