...
Owns one Codex thread per Ruslan session. Drives `turn/start`, consumes
...
# Ruslan' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_RUSLAN_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Ruslan/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Ruslan session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _RUSLAN_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("RUSLAN_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="ruslan",
            client_title="Ruslan Agent",
            client_version=_get_ruslan_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Ruslan' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Ruslan' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own ruslan-tools callback we
            # auto-accept — the user already approved Ruslan' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Ruslan approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Ruslan returns 'once', 'session', 'always', or 'deny'.
...
def _get_ruslan_version() -> str:
    """Best-effort Ruslan version string for codex's userAgent line."""
    try:
