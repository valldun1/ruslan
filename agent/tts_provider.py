...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Ruslan with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.ruslan/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Ruslan
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``ruslan tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``ruslan setup`` for
