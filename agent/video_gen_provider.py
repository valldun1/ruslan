...
as ``kind: backend``) or ``~/.ruslan/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``ruslan tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$RUSLAN_HOME/cache/videos/``, creating parents as needed."""
    from ruslan_constants import get_ruslan_home

    path = get_ruslan_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$RUSLAN_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$RUSLAN_HOME/cache/videos/``.
