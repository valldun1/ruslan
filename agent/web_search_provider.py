...
``kind: backend``) or ``~/.ruslan/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`ruslan_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.ruslan/.env``) so credentials set
    through Ruslan' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``ruslan tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``ruslan tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``ruslan_cli/tools_config.py`` to inject this provider as a
