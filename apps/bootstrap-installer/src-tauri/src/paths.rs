...
//!
//! Mirrors `ruslan_constants.get_ruslan_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\ruslan
//!   macOS:   ~/.ruslan
...
/// Returns the canonical Ruslan home directory, respecting $RUSLAN_HOME if set.
pub fn ruslan_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("RUSLAN_HOME") {
...
pub fn bootstrap_cache_dir() -> PathBuf {
    ruslan_home().join("bootstrap-cache")
}
...
/// RUSLAN_HOME so it survives repo checkout deletion (unlike anything under
/// ruslan-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\ruslan\ruslan-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_ruslan`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under RUSLAN_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves RUSLAN_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    ruslan_home().join(".ruslan-update-in-progress")
}
...
pub fn copy_self_to_ruslan_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to RUSLAN_HOME");
    Ok(())
}
...
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".ruslan-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under RUSLAN_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[ruslan-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("RUSLAN_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_ruslan_home() -> String {
    ruslan_home().to_string_lossy().into_owned()
}
