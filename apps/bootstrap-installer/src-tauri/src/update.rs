...
//!
//! Driven when the installer is launched as `Ruslan-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Ruslan desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `ruslan update`
//!      or repair bootstrap can race locked files),
//!   2. run `ruslan update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in ruslan_cli/main.py),
//!   3. run `ruslan desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `ruslan update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_ruslan) and
...
/// `ruslan update` exit code meaning "another ruslan process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// ruslan_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `ruslan update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let ruslan_home = crate::paths::ruslan_home();
    let install_root = ruslan_home.join("ruslan-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_ruslan`
    ...
    let ruslan = resolve_ruslan(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the ruslan CLI under {}. Is Ruslan installed? \
    ...
    // async on Windows. If it still holds the venv shim, `ruslan update`
    ...
    // ---- stage 2: ruslan update -----------------------------------------
    // Pass --branch so `ruslan update` targets the branch this installer was
    ...
    // without --branch, `ruslan update` switches the checkout to `main` (a
    ...
    // --force skips `ruslan update`'s Windows running-exe guard (which would
    ...
    // time `ruslan update` runs there is no legitimate ruslan.exe to protect,
    // and the guard would only produce a false "Ruslan is still running" stop.
    //
    ...
    // probe only checks the ruslan.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Ruslan windows" message.
    ...
    // Retry-once for the update-boundary crash. `ruslan update` lazily imports
    ...
    // second `ruslan update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Ruslan" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Ruslan is still running. Close all Ruslan windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "ruslan update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::ruslan_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: ruslan desktop --build-only ----------------------------
    // `ruslan update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `ruslan update` already does above, and `ruslan update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `ruslan desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Ruslan manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_ruslan_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Ruslan to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend ruslan.exe (or the desktop Ruslan.exe
            ...
            // force-kill every Ruslan.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Ruslan still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_ruslan();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_ruslan(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Ruslan.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Ruslan.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `ruslan.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `ruslan.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\ruslan.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startRuslan()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// ruslan.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named ruslan.exe.)
fn force_kill_other_ruslan() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `ruslan <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv ruslan shim under an install root, regardless of existence.
fn venv_ruslan(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("ruslan.exe")
    } else {
        install_root.join("venv").join("bin").join("ruslan")
    }
}

/// Resolve the ruslan CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `ruslan` on PATH.
fn resolve_ruslan(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_ruslan(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "ruslan.exe" } else { "ruslan" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let ruslan_home = crate::paths::ruslan_home();
    let mut envs = vec![(
        "RUSLAN_HOME".to_string(),
        ruslan_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        ruslan_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_ruslan_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Ruslan.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.ruslan-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.ruslan-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::ruslan_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_ruslan_is_under_install_root() {
        let root = Path::new("/x/ruslan-agent");
        let shim = venv_ruslan(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_ruslan(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/ruslan-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".ruslan-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Ruslan.app"]),
            Some(PathBuf::from("/Applications/Ruslan.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Ruslan.app");
        let tmp = base.join("Ruslan.app.ruslan-update-new");
        let old = base.join("Ruslan.app.ruslan-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Ruslan.app");
        let tmp = base.join("Ruslan.app.ruslan-update-new"); // intentionally absent
        let old = base.join("Ruslan.app.ruslan-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Ruslan.app");
        let tmp = base.join("Ruslan.app.ruslan-update-new"); // absent
        let old = base.join("Ruslan.app.ruslan-update-old");
        write_marker(&target, "OLD");
