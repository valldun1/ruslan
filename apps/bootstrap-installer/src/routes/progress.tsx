...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Ruslan' : 'Setting up Ruslan Agent'

  const description = isUpdate
    ? 'Ruslan is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Ruslan installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="ruslan-fade-in flex h-full flex-col">
