...
export const $ruslanHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.ruslan/logs/bootstrap-installer.log')
    $ruslanHome.set('~/.ruslan')
  ...
  try {
    const [logPath, ruslanHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_ruslan_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $ruslanHome.set(ruslanHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Ruslan" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Ruslan-Setup.exe --update);
...
export async function launchRuslanDesktop(): Promise<void> {
  ...
  await invoke('launch_ruslan_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Ruslan repository' },
