...
How to build Ruslan Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Ruslan surface can also
...
- A **runtime home change** (switching the underlying `RUSLAN_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Ruslan — do not assume
