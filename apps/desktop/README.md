...
<p align="center">
  <a href="https://github.com/NousResearch/ruslan-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Ruslan Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.
...
Prebuilt installers are built and distributed via [the Ruslan Desktop website.](https://ruslan-agent.nousresearch.com/).
...
```bash
# throwaway RUSLAN_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Ruslan Agent runtime into `RUSLAN_HOME`
(`~/.ruslan`, or `%LOCALAPPDATA%\ruslan` on Windows), using the same layout as a
CLI install.
...
- **Ruslan Agent** runs as a headless `ruslan serve` process and exposes the
...
1. `RUSLAN_DESKTOP_RUSLAN_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `RUSLAN_DESKTOP_RUSLAN`, or `ruslan` on `PATH`
5. a system Python that can import the Ruslan runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Ruslan
...
- 📖 [Documentation](https://ruslan-agent.nousresearch.com/docs/)
