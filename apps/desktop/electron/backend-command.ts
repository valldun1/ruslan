// Backend subcommand routing for the desktop-managed Ruslan process.
//
// The desktop app launches its own headless backend via `ruslan serve` — it
...
// app hasn't updated yet, or an older `ruslan` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Ruslan profile to pin via `--profile`.
 */
 ...
 * `-m ruslan_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `ruslan_cli/subcommands/dashboard.py` source registers
