 ...
 * RUSLAN_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS, clamped to a 45s floor.
 */
...
test('honors a valid RUSLAN_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS override', () => {
  const env = { RUSLAN_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '120000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), 120_000)
})

test('clamps an override below the floor up to the 45s minimum', () => {
  const env = { RUSLAN_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '1000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), MIN_PORT_ANNOUNCE_TIMEOUT_MS)
})

test('rounds a fractional override', () => {
  const env = { RUSLAN_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '60000.7' }
...
test('falls back to the default for malformed / non-positive overrides', () => {
  for (const bad of ['', 'abc', '0', '-5', 'NaN', undefined]) {
    const env = bad === undefined ? {} : { RUSLAN_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: bad }
    assert.equal(
...
test('resolves with the announced port', async () => {
  ...
  child.stdout.emit('data', 'noise before\nRUSLAN_DASHBOARD_READY port=54321\n')
  assert.equal(await p, 54321)
})

test('resolves with a RUSLAN_BACKEND_READY port (headless `serve`)', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'RUSLAN_BACKEND_READY port=43210\n')
...
test('parses the port even when the line arrives split across chunks', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'RUSLAN_DASHBOARD_READY po')
...
test('rejects with the timeout message after the deadline', async () => {
  ...
  await assert.rejects(
    ...
    /Timed out waiting for Ruslan backend port announcement \(20ms\)/
  )
})
...
test('a late announcement after timeout does not throw (listeners torn down)', async () => {
  ...
  assert.doesNotThrow(() => {
    child.stdout.emit('data', 'RUSLAN_DASHBOARD_READY port=9999\n')
  })
})
...
function mkTmpReadyFile() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ruslan-ready-test-'))
