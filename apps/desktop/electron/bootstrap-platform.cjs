 ...
 * null to keep it enabled. `RUSLAN_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options = {}) {
  ...
  const override = String(env.RUSLAN_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()
  if (GPU_OVERRIDE_ON.has(override)) return 'override (RUSLAN_DESKTOP_DISABLE_GPU)'
