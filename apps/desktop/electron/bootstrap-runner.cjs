...
function installedAgentInstallScript(hermesHome) {
  ...
  const candidate = path.join(hermesHome, 'ruslan-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/valldun1/ruslan/${commit}/scripts/${scriptName}`
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...
        RUSLAN_HOME: hermesHome || process.env.RUSLAN_HOME || ''
      }
    }))
...
function buildPosixPinArgs({ installStamp, activeRoot, hermesHome }) {
  const args = ['--dir', activeRoot, '--ruslan-home', hermesHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
