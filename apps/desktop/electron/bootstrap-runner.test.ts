...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ruslan-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/ruslan-runner-test',
    sourceRepoRoot: null,
    ruslanHome: '/tmp/ruslan-runner-test',
    logRoot: '/tmp/ruslan-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'ruslan-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'ruslan-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/ruslan-agent',
      ruslanHome: '/tmp/ruslan'
    }),
    ['--dir', '/tmp/ruslan-agent', '--ruslan-home', '/tmp/ruslan', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/ruslan-agent', '--ruslan-home', '/tmp/ruslan', '--branch', 'main']
  )
})
