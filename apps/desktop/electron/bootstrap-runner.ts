 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Ruslan Agent by spawning
...
function bootstrapCacheDir(ruslanHome) {
  return path.join(ruslanHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.ruslan/ruslan-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(ruslanHome) {
  if (!ruslanHome) {
    return null
  }

  const candidate = path.join(ruslanHome, 'ruslan-agent', 'scripts', installScriptName())
...
function cachedScriptPath(ruslanHome, commit) {
  return path.join(bootstrapCacheDir(ruslanHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/ruslan-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(ruslanHome, installStamp.commit)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(ruslanHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, ruslanHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass RUSLAN_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          RUSLAN_HOME: ruslanHome || process.env.RUSLAN_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, ruslanHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, ruslanHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--ruslan-home', ruslanHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, ruslanHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, ruslanHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, ruslanHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(ruslanHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, ruslanHome, emit })
