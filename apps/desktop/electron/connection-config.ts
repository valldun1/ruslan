 ...
 *     `X-Ruslan-Session-Token` header; WS uses `?token=`.
...
// ruslan_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - ruslan_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - ruslan_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / ruslan #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-ruslan_session_at', '__Secure-ruslan_session_at', 'ruslan_session_at']
const RT_COOKIE_VARIANTS = ['__Host-ruslan_session_rt', '__Secure-ruslan_session_rt', 'ruslan_session_rt']

// The Nous portal (NAS) does NOT use Ruslan gateway session cookies — it is a
...
// Ruslan Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://ruslan.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a ruslan session ACCESS-token cookie
 ...
 * the Ruslan gateway session cookies, so cloud sign-in / discovery liveness
