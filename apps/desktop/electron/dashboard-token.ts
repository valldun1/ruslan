 ...
 *
 * The desktop main process can pass RUSLAN_DASHBOARD_SESSION_TOKEN when it
...
async function fetchPublicText(url, options: any = {}) {
  ...
  if (protocol !== 'http:' && protocol !== 'https:') {
    throw new Error(`Unsupported Ruslan backend URL protocol: ${protocol}`)
  }
  ...
  const res = await fetch(url, { signal: AbortSignal.timeout(timeoutMs) }).catch(error => {
    if (error.name === 'TimeoutError') {
      throw new Error(`Timed out connecting to Ruslan backend after ${timeoutMs}ms`)
    }
...
function extractInjectedDashboardToken(html) {
  const match = /window\.__RUSLAN_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")/.exec(String(html || ''))
 ...
 */
async function adoptServedDashboardToken(baseUrl, spawnToken, { childAlive, label = 'Ruslan backend', ...options }) {
