...
test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_HermesXXXX/ruslan', 'linux', { APPIMAGE: '/home/x/Apps/Ruslan.AppImage' }),
...
test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(
    resolveRemovableAppPath('/opt/ruslan/linux-unpacked/ruslan', 'linux', {}),
    '/opt/ruslan/linux-unpacked'
  )
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    hermesHome: 'C:\\Users\\x\\AppData\\Local\\ruslan'
  })
