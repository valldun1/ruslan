 ...
 * the three user-facing uninstall modes to the `ruslan uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m ruslan_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `ruslan_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'ruslan_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Ruslan.app/Contents/MacOS/Ruslan  → …/Ruslan.app
 *   Windows: …\Ruslan\Ruslan.exe                 → …\Ruslan  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Ruslan.app/Contents/MacOS/Ruslan → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Ruslan.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Ruslan.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Ruslan$/i.test(dir) || /[\\/]ruslan-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/ruslan
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import ruslan_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, ruslanHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export RUSLAN_HOME=${q(ruslanHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import ruslan_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Ruslan install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "RUSLAN_HOME=${String(ruslanHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
