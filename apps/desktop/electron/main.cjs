...
// scripts/install.ps1's $HermesHome and scripts/install.sh's $RUSLAN_HOME.
//
// Defaults:
...
function resolveHermesHome() {
  if (process.env.RUSLAN_HOME) return normalizeHermesHomeRoot(process.env.RUSLAN_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'ruslan-home')
  if (IS_WINDOWS) {
...
const RUSLAN_HOME = resolveHermesHome()
...
// local backend as. When set, startHermes() passes `ruslan --profile <name>
...
// electron-builder derives from the build `appId` (com.valldun1.ruslan) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.valldun1.ruslan')
}
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Valldun'
})
...
function resolveUpdateRoot() {
  const candidates = [
    ...
    isHermesSourceRoot(ACTIVE_RUSLAN_ROOT) ? ACTIVE_RUSLAN_ROOT : null
  ].filter(Boolean)
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater) {
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `ruslan update --branch ${branch}`
        }
      } catch {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message: 'Updating Ruslan — this window will close and the updater will open. Don’t reopen Ruslan yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHermes = path.join(venvBin, IS_WINDOWS ? 'ruslan.exe' : 'ruslan')
...
function resolveHermesCliBinary(updateRoot) {
  const venvHermes = path.join(updateRoot, 'venv', 'bin', 'ruslan')
...
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const ruslan = resolveHermesCliBinary(updateRoot)
  if (!ruslan) {
    emitUpdateProgress({ stage: 'manual', message: 'ruslan update', percent: null })
    return { ok: true, manual: true, command: 'ruslan update', hermesRoot: updateRoot }
  }
  ...
  // `ruslan update` reaps stale `ruslan dashboard` backends (a code update
...
function isBootstrapComplete() {
  ...
  return isHermesSourceRoot(ACTIVE_RUSLAN_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'ruslan_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      hermesHome: RUSLAN_HOME,
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    ...
    env: buildDesktopBackendEnv({
      hermesHome: RUSLAN_HOME,
      pythonPathEntries: [ACTIVE_RUSLAN_ROOT],
      venvRoot: VENV_ROOT
    }),
...
function resolveHermesBackend(dashboardArgs) {
  ...
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Ruslan source at ${overrideRoot}`, dashboardArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Ruslan source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }
  ...
  if (process.env.RUSLAN_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    const hermesOverride = process.env.RUSLAN_DESKTOP_RUSLAN
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Ruslan override under WSL: ${hermesOverride}`)
      }
    } else {
      hermesCommand = findOnPath('ruslan')
    }

    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Ruslan CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        return {
          label: `existing Ruslan CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Ruslan CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('Ruslan recovery was handed off to Ruslan Setup. The desktop will restart when recovery completes.')
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Ruslan install was cancelled.')
  ...
  if (!isHermesSourceRoot(ACTIVE_RUSLAN_ROOT)) {
    throw new Error(
...
const ZOOM_STORAGE_KEY = 'ruslan:desktop:zoomLevel'
...
// Valldun) instead of a static session token. The auth model is
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Ruslan gateway',
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Valldun" instead of a generic
...
ipcMain.handle('ruslan:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('ruslan:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: hermesLog.slice(-200) }))
...
ipcMain.handle('ruslan:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))

ipcMain.handle('ruslan:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Ruslan.')
  }
