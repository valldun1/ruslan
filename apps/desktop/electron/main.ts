...
import { buildDesktopBackendEnv, normalizeRuslanHomeRoot } from './backend-env'
import { canImportRuslanCli, verifyRuslanCli } from './backend-probes'
...
const USER_DATA_OVERRIDE = process.env.RUSLAN_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.RUSLAN_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.RUSLAN_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with RUSLAN_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[ruslan] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[ruslan] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('ruslan:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[ruslan] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[ruslan] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[ruslan] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[ruslan] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// RUSLAN_HOME — the user-facing root for everything Ruslan-related. Mirrors
// scripts/install.ps1's $RuslanHome and scripts/install.sh's $RUSLAN_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\ruslan (matches install.ps1)
//   macOS / Linux: ~/.ruslan (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.ruslan directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\ruslan yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// RUSLAN_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// RUSLAN_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.ruslan / %LOCALAPPDATA%\ruslan.
function resolveRuslanHome() {
  if (process.env.RUSLAN_HOME) {
    return normalizeRuslanHomeRoot(process.env.RUSLAN_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'ruslan-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a RUSLAN_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\ruslan and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('RUSLAN_HOME')

    if (fromRegistry) {
      return normalizeRuslanHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'ruslan')
    const legacy = path.join(app.getPath('home'), '.ruslan')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.ruslan setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.ruslan')
}

const RUSLAN_HOME = resolveRuslanHome()

function ruslanManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_ruslan_node_dirs() in
  // ruslan_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(RUSLAN_HOME, 'node')
...
function pathWithRuslanManagedNode(...entries) {
  return [...ruslanManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_RUSLAN_ROOT — the canonical mutable Ruslan install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_RUSLAN_ROOT = path.join(RUSLAN_HOME, 'ruslan-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_RUSLAN_ROOT, 'venv')
...
// boots, going straight to `resolveRuslanBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_RUSLAN_ROOT (not alongside)
...
// active-profile.json records which Ruslan profile the desktop launches its
// local backend as. When set, startRuslan() passes `ruslan --profile <name>
// dashboard …`, which deterministically pins RUSLAN_HOME (see
// _apply_profile_override in ruslan_cli/main.py) and bypasses the sticky
// ~/.ruslan/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors ruslan_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// ruslanDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under RUSLAN_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by ruslan_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(RUSLAN_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (ruslan_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.RUSLAN_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.RUSLAN_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.RUSLAN_DESKTOP_APP_NAME || 'Ruslan'
...
// 'ruslan:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.ruslan) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.ruslan')
}

// Seed the native About panel with the live Ruslan version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `ruslan update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveRuslanVersion(),
...
const MEDIA_PROTOCOL = 'ruslan-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(ruslan-media://…)`.
...
let mainWindow = null
let ruslanProcess = null
let connectionPromise = null
// True while connection-config:apply soft-rehomes the primary — suppresses the
// backend-exit toast so an intentional kill doesn't look like a crash.
let softRehomeInProgress = false
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by ruslanProcess +
// connectionPromise + startRuslan(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.RUSLAN_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.RUSLAN_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startRuslan() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning ruslan serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Ruslan backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[ruslan] ${line}`)
  ruslanLog.push(...lines)

  if (ruslanLog.length > 300) {
    ruslanLog.splice(0, ruslanLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-ruslan-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('ruslan:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the ruslan:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('ruslan:bootstrap:event', ev)
}
...
// The Tauri updater writes RUSLAN_HOME/.ruslan-update-in-progress for the whole
...
// (`force_kill_other_ruslan`, taskkill /IM ruslan.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(RUSLAN_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Ruslan will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(RUSLAN_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `ruslan` — must not shadow `ruslan.cmd`/`ruslan.exe`.
...
function unwrapWindowsVenvRuslanCommand(command, backendArgs) {
  return resolveVenvRuslanCommand(command, backendArgs, {
...
// spawns `ruslan serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `ruslan` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'ruslan_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, RUSLAN_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isRuslanSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'ruslan_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.RUSLAN_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Ruslan deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Ruslan' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\ruslan\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'ruslan', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'ruslan', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal RUSLAN_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\ruslan\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'ruslan', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'ruslan', 'git', 'bin', 'git.exe'))
  }
...
function recentRuslanLog() {
  return ruslanLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's ruslan root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_RUSLAN_ROOT.
// RUSLAN_DESKTOP_RUSLAN_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.RUSLAN_DESKTOP_RUSLAN_ROOT && path.resolve(process.env.RUSLAN_DESKTOP_RUSLAN_ROOT),
    !IS_PACKAGED && isRuslanSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isRuslanSourceRoot(ACTIVE_RUSLAN_ROOT) ? ACTIVE_RUSLAN_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_RUSLAN_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('ruslan:updates:progress', merged)
  }
}
...
// RUSLAN_HOME/ruslan-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_ruslan_home). That binary owns
// ALL repo mutation — running `ruslan update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'ruslan-setup.exe' : 'ruslan-setup'
  const candidate = path.join(RUSLAN_HOME, name)
...
// Path to the venv shim whose lock decides whether `ruslan update` can write
// fresh entry points. On Windows this is the file the running backend
// `ruslan.exe` holds open; on POSIX it's never mandatory-locked.
function venvRuslanShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'ruslan.exe')
    : path.join(updateRoot, 'venv', 'bin', 'ruslan')
}
...
// only signals the direct child, so on Windows a backend `ruslan.exe` that
// spawned its own grandchildren (a `ruslan` REPL, a pty terminal session, the
...
// `ruslanProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `ruslan.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a ruslan
// REPL, a pty terminal, the gateway) keep `ruslan.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (ruslanProcess && Number.isInteger(ruslanProcess.pid)) {
    pids.push(ruslanProcess.pid)
  }
  ...
  if (ruslanProcess && !ruslanProcess.killed) {
    try {
      ruslanProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvRuslanShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    if (ruslanProcess && Number.isInteger(ruslanProcess.pid)) {
      stragglers.push(ruslanProcess.pid)
    }
...
// `ruslan update`). Instead we spawn the staged Ruslan-Setup binary with
// --update and quit, so it can run `ruslan update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri ruslan-setup. Unlike Windows
      ...
      // whole update itself: `ruslan update` (backend) + `ruslan desktop
    ...
    if (!updater) {
      ...
      // `ruslan desktop`, never the Tauri installer that self-copies
      // ruslan-setup.exe into RUSLAN_HOME). They DO have a working `ruslan`
      ...
      // checkout they're on — bare `ruslan update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `ruslan update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `ruslan update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, ruslanRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Ruslan — this window will close and the updater will open. Don’t reopen Ruslan yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // ruslan.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running ruslan, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Ruslan install open ' +
        '(a second Ruslan window or a terminal running ruslan?). Close it and retry.'
    ...
    // `ruslan update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: RUSLAN_HOME,
      env: {
        ...process.env,
        RUSLAN_HOME,
        PATH: pathWithRuslanManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(RUSLAN_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvRuslan = path.join(venvBin, IS_WINDOWS ? 'ruslan.exe' : 'ruslan')
  ...
  // not just the `ruslan.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvRuslan) || fileExists(path.join(updateRoot, '.ruslan-bootstrap-complete'))
...
// Resolve the ruslan CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `ruslan` on PATH.
function resolveRuslanCliBinary(updateRoot) {
  const venvRuslan = path.join(updateRoot, 'venv', 'bin', 'ruslan')

  if (fileExists(venvRuslan)) {
    return venvRuslan
  }

  return findOnPath('ruslan') || null
}
...
// macOS/Linux in-app update: backend (`ruslan update`) + OS-aware GUI rebuild
// (`ruslan desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const ruslan = resolveRuslanCliBinary(updateRoot)

  if (!ruslan) {
    emitUpdateProgress({ stage: 'manual', message: 'ruslan update', percent: null })

    return { ok: true, manual: true, command: 'ruslan update', ruslanRoot: updateRoot }
  }

  // Put the Ruslan-managed Node and the venv on PATH so `ruslan desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\ruslan\node, not node\bin.
  const env: Record<string, string> = {
    RUSLAN_HOME,
    PATH: pathWithRuslanManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `ruslan update` reaps stale `ruslan serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads RUSLAN_DESKTOP_CHILD_PID and excludes
  ...
  if (ruslanProcess && Number.isInteger(ruslanProcess.pid)) {
    desktopChildPids.push(ruslanProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.RUSLAN_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Ruslan (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(ruslan, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'ruslan update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'ruslan update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(ruslan, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Ruslan to retry.',
  ...
  // Linux in-app update terminal state (#45205). `ruslan desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Ruslan…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `ruslan-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Ruslan to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Ruslan desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Ruslan to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Ruslan.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Ruslan.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Ruslan to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.ruslan-update-new"; then
    rm -rf "$DST.ruslan-update-old" 2>/dev/null || true
    mv "$DST" "$DST.ruslan-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.ruslan-update-new" "$DST"
    rm -rf "$DST.ruslan-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Ruslan to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_RUSLAN_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isRuslanSourceRoot(ACTIVE_RUSLAN_ROOT) &&
    fileExists(venvPython) &&
    canImportRuslanCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_RUSLAN_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `ruslan
...
function resolveWebDist() {
  const override = process.env.RUSLAN_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set RUSLAN_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: ruslan desktop --force-build`
  )
...
function resolveRuslanCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Ruslan.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.RUSLAN_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveRuslanCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_RUSLAN_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Ruslan at ${ACTIVE_RUSLAN_ROOT}`,
    command,
    args: ['-m', 'ruslan_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      ruslanHome: RUSLAN_HOME,
      pythonPathEntries: [ACTIVE_RUSLAN_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_RUSLAN_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveRuslanBackend(backendArgs) {
  // 1. Explicit override -- RUSLAN_DESKTOP_RUSLAN_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.RUSLAN_DESKTOP_RUSLAN_ROOT && path.resolve(process.env.RUSLAN_DESKTOP_RUSLAN_ROOT)

  if (overrideRoot && isRuslanSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Ruslan source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `ruslan` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isRuslanSourceRoot.)
  if (!IS_PACKAGED && isRuslanSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Ruslan source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_RUSLAN_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\ruslan\ruslan-agent (Windows) or ~/.ruslan/ruslan-agent.
  ...
  //    to spawning ruslan. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `ruslan update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `ruslan` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    RUSLAN_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.RUSLAN_DESKTOP_IGNORE_EXISTING !== '1') {
    let ruslanCommand = null
    const ruslanOverride = process.env.RUSLAN_DESKTOP_RUSLAN

    if (ruslanOverride) {
      const resolvedOverride = findOnPath(ruslanOverride)

      if (resolvedOverride) {
        ruslanCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(ruslanOverride, { isWsl: IS_WSL })) {
        ruslanCommand = ruslanOverride
      } else {
        rememberLog(`Ignoring Windows Ruslan override under WSL: ${ruslanOverride}`)
      }
    } else {
      ruslanCommand = findOnPath('ruslan')
    }

    if (ruslanCommand) {
      if (looksLikeDesktopAppBinary(ruslanCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Ruslan CLI: ${ruslanCommand}`)
        ruslanCommand = null
      }
    }

    if (ruslanCommand) {
      const unwrapped = unwrapWindowsVenvRuslanCommand(ruslanCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `ruslan` shim
      ...
      const shellForProbe = isCommandScript(ruslanCommand)

      if (verifyRuslanCli(ruslanCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvRuslanCommand(ruslanCommand, backendArgs) || {
            label: `existing Ruslan CLI at ${ruslanCommand}`,
            command: ruslanCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Ruslan CLI at ${ruslanCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed ruslan_cli module via system Python.
  ...
  if (python) {
    ...
    // having ruslan_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\ruslan\ruslan-agent\venv.
    if (canImportRuslanCli(python)) {
      return {
        kind: 'python',
        label: `installed ruslan_cli module via ${python}`,
        command: python,
        args: ['-m', 'ruslan_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: ruslan_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveRuslanBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Ruslan Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveRuslanBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Ruslan install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Ruslan recovery was handed off to Ruslan Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      ruslanHome: RUSLAN_HOME,
      logRoot: path.join(RUSLAN_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Ruslan install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Ruslan bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(RUSLAN_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startRuslan() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // ruslan:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveRuslanBackend(backend.args))
  }
  ...
  if (!isRuslanSourceRoot(ACTIVE_RUSLAN_ROOT)) {
    throw new Error(
      `Ruslan install at ${ACTIVE_RUSLAN_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Ruslan' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\ruslan\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `ruslan` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Ruslan on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Ruslan.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isRuslanSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Ruslan venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Ruslan at ${ACTIVE_RUSLAN_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Ruslan runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----ruslan-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Ruslan backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Ruslan backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Ruslan backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Ruslan-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('ruslan:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveRuslanCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('ruslan:preview-file-changed', payload)
}
...
async function waitForRuslan(baseUrl, token) {
  ...
  throw new Error(`Ruslan backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('ruslan:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('ruslan:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Ruslan...".
function sendPowerResume() {
  ...
  webContents.send('ruslan:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('ruslan:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('ruslan:window-state-changed', state)
}
...
//
// Hosted Ruslan gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``ruslan_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / ruslan #37247).
//     Both are set as HttpOnly cookies (``ruslan_session_at`` ~15 min,
//     ``ruslan_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:ruslan-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Ruslan Cloud agent…' : 'Sign in to Ruslan gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Ruslan backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Ruslan Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:ruslan-remote-oauth`:
...
// convention (ruslan_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Ruslan surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.RUSLAN_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Ruslan gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for ruslan_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Ruslan Cloud sign-in.'))
...
// Discover the hosted (Ruslan Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Ruslan Cloud. Open Settings → Gateway, choose Ruslan Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Ruslan Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Ruslan Cloud session has expired. Sign in to Ruslan Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Ruslan Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root RUSLAN_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.RUSLAN_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.RUSLAN_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  // The env override forces a plain remote connection. Otherwise reflect the
  // saved mode, preserving 'cloud' (a Ruslan Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted Ruslan Cloud org (slug/id) for a cloud connection, or '' for
    ...
    // scope is never overridden by RUSLAN_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Ruslan Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Ruslan Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (ruslan #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Ruslan gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Ruslan gateway is selected, but no session token is saved. ' +
...
//   2. env override (RUSLAN_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.RUSLAN_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.RUSLAN_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'RUSLAN_DESKTOP_REMOTE_URL is set but RUSLAN_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Ruslan backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.RUSLAN_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Ruslan
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startRuslan())
  ...
  // connect to Ruslan gateway". Mirror the renderer's connect here so the test
...
function resetRuslanConnection({ soft = false } = {}) {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(ruslanProcess)
...
// startRuslan() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetRuslanConnection() nulls ruslanProcess.
  const dying = ruslanProcess && !ruslanProcess.killed ? ruslanProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('ruslan:connection:applied')
}
...
// profile to startRuslan() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startRuslan() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForRuslan(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: ruslanLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited RUSLAN_HOME env (see _apply_profile_override
  // step 3 in ruslan_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveRuslanBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const ruslanCwd = resolveRuslanCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Ruslan backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: ruslanCwd,
        RUSLAN_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        RUSLAN_DESKTOP: '1',
        RUSLAN_WEB_DIST: webDist,
        ...(readyFile ? { RUSLAN_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Ruslan backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Ruslan backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Ruslan backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForRuslan(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Ruslan backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_ruslan_home() recreates the deleted profile directory.
//
...
async function startRuslan() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startRuslan() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Ruslan backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())

    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Ruslan backend at ${remote.baseUrl}`, 24)
      await waitForRuslan(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Ruslan backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.ruslan/active_profile file) and
    // resolves RUSLAN_HOME the same way `ruslan -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Ruslan runtime', 28)
    const backend = await ensureRuntime(resolveRuslanBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const ruslanCwd = resolveRuslanCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Ruslan backend via ${backend.label}`, 84)
    rememberLog(`Starting Ruslan backend via ${backend.label}`)
    ...
    ruslanProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin RUSLAN_HOME for the child so Python's get_ruslan_home()
          // resolves to the SAME location our resolveRuslanHome() picked. Without
          // this pin, Python falls back to ~/.ruslan on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\ruslan, which differs from C:\Users\<u>\.ruslan.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets RUSLAN_HOME via setx; the desktop
    ...
    ruslanProcess.stdout.on('data', rememberLog)
    ruslanProcess.stderr.on('data', rememberLog)
    ...
    ruslanProcess.once('error', error => {
      rememberLog(`Ruslan backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Ruslan backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    ruslanProcess.once('exit', (code, signal) => {
      rememberLog(`Ruslan backend exited (${signal || code})`)
      ruslanProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Ruslan backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Ruslan backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentRuslanLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Ruslan backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(ruslanProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Ruslan backend to become ready', 90)
    await Promise.race([waitForRuslan(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null ruslanProcess when the child dies.
      childAlive: () => ruslanProcess !== null && ruslanProcess.exitCode === null && !ruslanProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Ruslan backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Ruslan is
...
// pushes pet state over IPC (ruslan:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via ruslan:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Ruslan app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Ruslan icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see ruslan:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Ruslan in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('ruslan:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startRuslan().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('ruslan:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Ruslan…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('ruslan:connection:revalidate', async () => {
  if (!connectionPromise) {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetRuslanConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Ruslan backend failed liveness probe; dropping stale connection.')
    resetRuslanConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('ruslan:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('ruslan:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('ruslan:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('ruslan:window:openNewSession', async () => {
...
ipcMain.handle('ruslan:zoom:get', event => {
...
ipcMain.on('ruslan:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('ruslan:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('ruslan:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('ruslan:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('ruslan:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('ruslan:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('ruslan:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('ruslan:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('ruslan:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('ruslan:pet-overlay:control', payload)
})
ipcMain.handle('ruslan:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startRuslan() call restarts the
...
ipcMain.handle('ruslan:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startRuslan() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('ruslan:bootstrap:cancel', async () => {
...
ipcMain.handle('ruslan:boot-progress:get', async () => bootProgressState)
ipcMain.handle('ruslan:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('ruslan:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('ruslan:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('ruslan:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('ruslan:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('ruslan:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('ruslan:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('ruslan:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('ruslan:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on ruslan:connection:applied.
...
ipcMain.handle('ruslan:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('ruslan:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new RUSLAN_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('ruslan:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('ruslan:requestMicrophoneAccess', async () => {
...
ipcMain.handle('ruslan:api', async (_event, request) => {
  ...
  // backend calls ensure_ruslan_home() which recreates the profile directory,
...
ipcMain.handle('ruslan:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('ruslan:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('ruslan:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('ruslan:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('ruslan:readFileText', async (_event, filePath) => {
...
ipcMain.handle('ruslan:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('ruslan:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('ruslan:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('ruslan:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('ruslan:saveClipboardImage', async () => {
...
ipcMain.handle('ruslan:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('ruslan:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('ruslan:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('ruslan:titlebar-theme', (_event, payload) => {
...
ipcMain.on('ruslan:native-theme', (_event, mode) => {
...
ipcMain.on('ruslan:translucency', (_event, payload) => {
...
ipcMain.handle('ruslan:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('ruslan:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveRuslanCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('ruslan:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveRuslanCwd()
}))

ipcMain.handle('ruslan:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('ruslan:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('ruslan:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('ruslan:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('ruslan:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('ruslan:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: ruslanLog.slice(-200) }))
...
function terminalShellCommand() {
  // RUSLAN_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.RUSLAN_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Ruslan' TUI into
  ...
  env.TERM_PROGRAM = 'Ruslan'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a ruslan/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from RUSLAN_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.RUSLAN_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `ruslan:terminal:${id}:${suffix}`
}
...
ipcMain.handle('ruslan:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('ruslan:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('ruslan:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('ruslan:fs:openDir', async (_event, dirPath) => {
...
ipcMain.handle('ruslan:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('ruslan:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('ruslan:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('ruslan:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('ruslan:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('ruslan:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('ruslan:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('ruslan:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('ruslan:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('ruslan:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('ruslan:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('ruslan:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('ruslan:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('ruslan:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('ruslan:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('ruslan:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('ruslan:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('ruslan:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('ruslan:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('ruslan:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('ruslan:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('ruslan:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('ruslan:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('ruslan:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('ruslan:terminal:write', (_event, id, data) => {
...
ipcMain.handle('ruslan:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('ruslan:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('ruslan:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('ruslan:updates:check', async () =>
...
ipcMain.handle('ruslan:updates:apply', async (_event, payload) =>
...
ipcMain.handle('ruslan:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('ruslan:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Ruslan version (the one `release.py` bumps in
// ruslan_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Ruslan version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveRuslanVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'ruslan_cli', '__init__.py')
...
// Re-resolve the live Ruslan version and push it into the native About panel
// just before showing it, so an in-place `ruslan update` is reflected without
...
ipcMain.handle('ruslan:version', async () => ({
  appVersion: resolveRuslanVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  ruslanRoot: resolveUpdateRoot()
}))
...
// the actual removal via `ruslan uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (ruslan_cli/uninstall.py + ruslan_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_RUSLAN_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isRuslanSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'ruslan_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, RUSLAN_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Ruslan agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import ruslan_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_RUSLAN_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `ruslan-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `ruslan-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('ruslan:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('ruslan:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('ruslan:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('ruslan:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// ruslan:// deep links (e.g. ruslan://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const RUSLAN_PROTOCOL = 'ruslan'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${RUSLAN_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // ruslan://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('ruslan:deep-link', payload)
...
ipcMain.handle('ruslan:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${RUSLAN_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(RUSLAN_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(RUSLAN_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `ruslan://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching ruslan:// URL is in our own argv.
