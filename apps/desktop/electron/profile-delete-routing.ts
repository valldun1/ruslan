// Profile-delete routing logic for the `ruslan:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_ruslan_home() and
...
/**
 * Parse a `ruslan:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `ruslan:api` request away from the primary/window backend
 ...
 * ensure_ruslan_home() recreates the directory the delete just removed.
 */
