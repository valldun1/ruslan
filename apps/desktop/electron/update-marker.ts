...
export function markerPath(ruslanHome) {
  return path.join(ruslanHome, '.ruslan-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(ruslanHome)
  let raw
 ...
 *
 * The Tauri-based ruslan-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(ruslanHome, pid, { now = Date.now } = {}) {
  const file = markerPath(ruslanHome)
