 ...
 * Background
 * ----------
 * After `ruslan update` + `ruslan desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything RUSLAN_DESKTOP_* is preserved
// plus RUSLAN_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['RUSLAN_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['RUSLAN_DESKTOP_']
