...
// even though a fresh shell — and the Ruslan CLI — sees it immediately. The
// desktop's RUSLAN_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\ruslan. Reading
