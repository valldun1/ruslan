...
export const ZOOM_STORAGE_KEY = 'ruslan:desktop:zoomLevel'
 ...
 * but forgot to emit 'ruslan:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('ruslan:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
