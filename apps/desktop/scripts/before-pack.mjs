 ...
 * product name (`Ruslan`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Ruslan'
 *
 * This is a hard failure with no obvious cause for the user — `ruslan desktop`
 ...
 * (the mac-unpacked Ruslan.app bundle) and Windows (win-unpacked), so we
