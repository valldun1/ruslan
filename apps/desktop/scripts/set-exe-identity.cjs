#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Ruslan icon + version metadata onto the
// built Ruslan.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Ruslan.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `ruslan desktop`, the installer's
...
// path (which rebuilds via `ruslan desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Ruslan.exe>
//
...
// Stamp the Ruslan icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Ruslan',
      FileDescription: 'Ruslan',
      CompanyName: 'Valldun',
      LegalCopyright: 'Copyright (c) 2026 Valldun'
    }
  })

  console.log('[set-exe-identity] done — Ruslan icon + identity stamped')
}
