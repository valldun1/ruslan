...
// no longer bundles the Ruslan Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Ruslan.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Ruslan'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Ruslan.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Ruslan'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default RUSLAN_HOME for non-sandboxed runs -- matches main.ts's
// resolveRuslanHome(). On Windows it's %LOCALAPPDATA%\ruslan; elsewhere
// it's ~/.ruslan. The fresh-install sandbox launchFresh() sets its own
// RUSLAN_HOME and never touches this.
const DEFAULT_RUSLAN_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'ruslan')
  }
  return path.join(os.homedir(), '.ruslan')
})()
const VENV_ROOT = path.join(DEFAULT_RUSLAN_HOME, 'ruslan-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'ruslan-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.RUSLAN_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Ruslan-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Ruslan-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Ruslan-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Ruslan-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.RUSLAN_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.RUSLAN_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const ruslanHome = path.join(sandbox, 'ruslan-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(ruslanHome, { recursive: true })
  ...
  env.RUSLAN_DESKTOP_CWD = cwd
  env.RUSLAN_DESKTOP_IGNORE_EXISTING = '1'
  env.RUSLAN_DESKTOP_TEST_MODE = 'fresh-install'
  env.RUSLAN_DESKTOP_USER_DATA_DIR = userDataDir
  env.RUSLAN_HOME = ruslanHome
  delete env.RUSLAN_DESKTOP_RUSLAN
  delete env.RUSLAN_DESKTOP_RUSLAN_ROOT
  ...
  console.log(`  RUSLAN_HOME: ${ruslanHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(ruslanHome, 'ruslan-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Ruslan Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of ruslan_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'ruslan-agent', 'ruslan_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Ruslan
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + RUSLAN_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  RUSLAN_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
