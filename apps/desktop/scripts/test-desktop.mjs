...
// Default RUSLAN_HOME for non-sandboxed runs -- matches main.cjs's
// resolveHermesHome(). On Windows it's %LOCALAPPDATA%\ruslan; elsewhere
...
function launchFresh() {
  ...
  const hermesHome = path.join(sandbox, 'ruslan-home')
  ...
  console.log(`  RUSLAN_HOME: ${hermesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hermesHome, 'ruslan-agent', 'venv') }
}
