        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.ruslanDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Ruslan to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.ruslanDesktop?.watchPreviewFile ||
      !window.ruslanDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.ruslanDesktop.onPreviewFileChanged(payload => {
    ...
    void window.ruslanDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.ruslanDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.ruslanDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:ruslan-preview')
