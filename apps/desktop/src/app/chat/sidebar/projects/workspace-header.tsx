...
import type { RuslanGitBranch } from '@/global'
...
const branchActionLabel = (branch: RuslanGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `ruslan/work-<ts>` trees.
...
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<RuslanGitBranch[]>([])
  ...
  const convert = async (branch: RuslanGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
