...
// bug that drives the REAL useGatewayBoot hook + REAL RuslanGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshRuslanConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { ruslanDesktop?: unknown }).ruslanDesktop = fakeDesktop()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForRuslan) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startRuslan()'s remote branch awaits waitForRuslan() for 45s before it
    ...
    ;(window as { ruslanDesktop?: unknown }).ruslanDesktop = desktop
    ...
    // After ~45s waitForRuslan gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Ruslan backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
