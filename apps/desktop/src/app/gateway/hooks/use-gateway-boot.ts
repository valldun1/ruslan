import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@ruslan/shared'
import { useEffect, useRef } from 'react'

import type { RuslanConnection } from '@/global'
import { RuslanGateway } from '@/ruslan'
...
import type { RpcEvent } from '@/types/ruslan'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.ruslanDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: RuslanGateway | null) => void
  refreshRuslanConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: RuslanConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Ruslan...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Ruslan…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Ruslan gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshRuslanConfig().catch(() => undefined)
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startRuslan re-emits progress — ignore so the
    ...
    const gateway = new RuslanGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshRuslanConfig()
