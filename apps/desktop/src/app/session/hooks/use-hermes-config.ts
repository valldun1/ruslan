...
import { getRuslanConfig, getRuslanConfigDefaults } from '@/ruslan'
...
interface RuslanConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useRuslanConfig({ activeSessionIdRef, refreshProjectBranch }: RuslanConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshRuslanConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getRuslanConfig(), getRuslanConfigDefaults().catch(() => ({}))])
  ...
  return { refreshRuslanConfig, sttEnabled, voiceMaxRecordingSeconds }
}
