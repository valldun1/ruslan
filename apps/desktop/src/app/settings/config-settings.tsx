...
import { getElevenLabsVoices, getRuslanConfigSchema, saveRuslanConfig } from '@/ruslan'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, RuslanConfigRecord } from '@/types/ruslan'

import { setRuslanConfigCache, useRuslanConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: RuslanConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<RuslanConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useRuslanConfigRecord()
  ...
  const {
    ...
    queryKey: ['ruslan-config-schema'],
    queryFn: getRuslanConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: RuslanConfigRecord) => {
