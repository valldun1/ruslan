...
const getRuslanConfigRecord = vi.fn()
const saveRuslanConfig = vi.fn()
...
vi.mock('@/ruslan', () => ({
  ...
  getRuslanConfigRecord: () => getRuslanConfigRecord(),
  saveRuslanConfig: (config: unknown) => saveRuslanConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'ruslan-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['ruslan-4', 'ruslan-4-mini'],
        authenticated: true,
        capabilities: { 'ruslan-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'ruslan-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ provider: 'nous', model: 'ruslan-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'nous', model: 'ruslan-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getRuslanConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveRuslanConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getRuslanConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveRuslanConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['ruslan-4'],
          authenticated: true,
          capabilities: { 'ruslan-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'ruslan-4' }]
    })
...
describe('ModelSettings MoA preset editor', () => {
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('nous · ruslan-4')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
