...
import { invalidateRuslanConfig, setRuslanConfigCache, useRuslanConfigRecord } from '../hooks/use-config-record'
...
// Ruslan' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Ruslan default (medium), shown as Medium.
...
// backend now surfaces the full `ruslan model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in ruslan_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useRuslanConfigRecord()
  const setConfig = setRuslanConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `ruslan model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Ruslan runs the flow for you.`}
          </p>
        )}
