...
import { disconnectOAuthProvider, listOAuthProviders } from '@/ruslan'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/ruslan'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.ruslanDesktop?.terminal)
...
//      in ruslan_cli/provider_catalog.py) — the SAME provider identity
//      `ruslan model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Ruslan can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Ruslan can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Ruslan never deletes creds another tool owns behind a silent API call.
