...
import { formatRefValue, ruslanDirectiveFormatter } from './directive-text'
...
describe('ruslanDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = ruslanDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = ruslanDirectiveFormatter.parse('@file:src/main.tsx the entry point')
