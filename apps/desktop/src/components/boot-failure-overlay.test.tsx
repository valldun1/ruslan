...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Ruslan gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.ruslanDesktop
  Object.defineProperty(window, 'ruslanDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'ruslanDesktop', { configurable: true, value: original })
}
