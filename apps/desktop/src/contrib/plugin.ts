 ...
 * `RuslanPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/ruslan'
import { readKey, writeKey } from '@/lib/storage'

import { registry } from './registry'
import type { Contribution } from './types'

export type { PluginRestOptions } from '@/ruslan'
 ...
 *  under `ruslan.plugin.<id>.` — plugins can't read or clobber each other. */
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `ruslan.plugin.${pluginId}.${key}`
