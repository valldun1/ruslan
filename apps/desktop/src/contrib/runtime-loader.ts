 ...
 *   (`@ruslan/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default RuslanPlugin -> register(ctx)
 *
 ...
 * and `<ruslan home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { getStatus } from '@/ruslan'
import { installPluginSdk, sdkImportMap } from '@/sdk/runtime'
import { notifyError } from '@/store/notifications'

import { createPluginContext, type RuslanPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@ruslan/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @ruslan/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: RuslanPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default RuslanPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<ruslan home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    const { ruslan_home } = await getStatus()
    const { entries } = await desktop.readDir(`${ruslan_home}/desktop-plugins`)
