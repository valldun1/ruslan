...
import { getRuslanConfigRecord, type RuslanConfigRecord, saveRuslanConfig } from '@/ruslan'
...
export interface I18nConfigClient {
  getConfig: () => Promise<RuslanConfigRecord>
  saveConfig: (config: RuslanConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.ruslanDesktop?.api) {
      return Promise.resolve({})
    }

    return getRuslanConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.ruslanDesktop?.api) {
    ...
    return saveRuslanConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: RuslanConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: RuslanConfigRecord, locale: Locale): RuslanConfigRecord {
