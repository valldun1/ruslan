...
export const ja = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingRuslanDesktop: 'Ruslan Desktop を起動中…'
    },
    errors: {
  ...
  settings: {
    ...
    appearance: {
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、Ruslan の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。Ruslan を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
  ...
  skills: {
    ...
    skillArchivedMessage: 'ruslan curator restore で復元できます。'
  },
  ...
  starmap: {
    ...
    emptyDesc: 'Ruslan がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには Ruslan を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには Ruslan を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら Ruslan が通知します。',
    ...
    ruslanActiveSessions: (version, count) => `Ruslan ${version} · アクティブセッション ${count}`,
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには Ruslan バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'Ruslan から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Ruslan にコミットと PR を任せる',
      agentShipPrompt:
  ...
  updates: {
    ...
    applyingBody:
      'Ruslan アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に Ruslan を再度開きます。更新中はご自分で Ruslan を開き直さないでください。',
