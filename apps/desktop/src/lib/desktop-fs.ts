import type {
  RuslanConnection,
  RuslanReadDirResult,
  RuslanReadFileTextResult,
  RuslanSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: RuslanConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<RuslanReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<RuslanReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<RuslanReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<RuslanReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: RuslanSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
