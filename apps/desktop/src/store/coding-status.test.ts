...
import type { RuslanRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: RuslanRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<RuslanRepoStatus | null>) {
  ;(window as unknown as { ruslanDesktop?: unknown }).ruslanDesktop = { git: { repoStatus: impl } }
}
