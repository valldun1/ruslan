...
const STORAGE_KEY = 'ruslan:native-notifications'
...
// "Backgrounded" = the user isn't on Ruslan. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.ruslanDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.ruslanDesktop
