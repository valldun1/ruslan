...
import type { OAuthProvider } from '@/types/ruslan'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('ruslan-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('ruslan-desktop-onboarded-v1')).toBe('1')
  })
