 ...
 * app's bounds and stays visible while Ruslan is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'ruslan.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'ruslan.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.ruslanDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.ruslanDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.ruslanDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.ruslanDesktop?.petOverlay?.close()
}
