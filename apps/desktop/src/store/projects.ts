...
import type { RuslanGitBaseBranch, RuslanGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/ruslan'
...
const PROJECT_SCOPE_KEY = 'ruslan.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Ruslan gateway is not connected')
  }
...
// repo, returning where Ruslan should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<RuslanGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<RuslanGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.ruslanDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.ruslanDesktop?.writeClipboard?.(path)
  }
}
