...
import type { RuslanReviewFile, RuslanReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Ruslan' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'ruslan.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'ruslan.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'ruslan.desktop.reviewTreeMode'
const SELECTED_KEY = 'ruslan.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<RuslanReviewFile[]>([])
...
export const $reviewShipInfo = atom<RuslanReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['ruslanDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: RuslanReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.ruslanDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.ruslanDesktop?.openExternal?.(url)
    }
