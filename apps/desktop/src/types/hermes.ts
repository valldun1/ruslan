...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Ruslan doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in ruslan_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `ruslan model` uses — instead of
...
export type RuslanConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `ruslan model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors ruslan_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Ruslan, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `ruslan tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Ruslan); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/ruslan/update/check` — the backend's own update state.
