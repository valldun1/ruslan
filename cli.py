...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚔ РУСЛАН - AI-агент для русскоязычных"
        tiny_line = "⚔ РУСЛАН"
    else:
        agent_name = _skin.get_branding("agent_name", "Руслан") if _skin else "Руслан"
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"Руслан v{_version} ({_release_date})"
    else:
...
class HermesCLI:
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"🛡 {self.model if getattr(self, 'model', None) else 'Руслан'}"
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from hermes_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "🛡 Руслан")
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Руслану нужно минимум {MINIMUM_CONTEXT_LENGTH:,} токенов. Схемы инструментов + системный промпт занимают большой фиксированный префикс.[/]"
            )
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  🛡 Руслан: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  🛡 Руслан: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def show_help(self):
        ...
        _cprint(f"\n  {_DIM}Совет: просто напиши сообщение для чата с Русланом!{_RST}")
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Руслан default (chat_completions)
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Руслан продолжает работать, пока ты не приостановишь, не очистишь или не закончится бюджет "
    ...
    def _handle_busy_command(self, cmd: str):
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter будет ставить в очередь твой ввод, пока Руслан занят."
            elif arg == "steer":
                behavior = "Enter будет направлять твоё сообщение в текущий запуск (после следующего вызова инструмента)."
            else:
                behavior = "Enter будет прерывать текущий запуск, пока Руслан занят."
    ...
    def _handle_update_command(self) -> bool:
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Руслан')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Руслан"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="🛡  Обновить Руслана",
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = "Добро пожаловать в Руслан! Напиши /help для списка команд."
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Руслан")
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Руслану нужен твой ответ", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Руслану нужен твой ответ'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Руслану нужен твой ответ") - 3)) + '╮\n'))
            if not use_compact_chrome:
...
def main(
    ...
    if gateway:
        ...
        print("🇷🇺 Запускаю Руслан (messaging platforms)...")
