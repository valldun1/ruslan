#!/usr/bin/env python3
"""
Ruslan Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Ruslan Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills ruslan-agent-dev,github-auth
...
os.environ["RUSLAN_QUIET"] = "1"  # Our own modules

import yaml

from ruslan_cli.fallback_config import get_fallback_chain
from ruslan_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from ruslan_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from ruslan_cli.pt_input_extras import (
...
from ruslan_cli.banner import _format_context_length, format_banner_version_label
...
load_ruslan_dotenv(ruslan_home=_ruslan_home, project_env=_project_env)
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("RUSLAN_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.ruslan/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If RUSLAN_IGNORE_USER_CONFIG=1 is set (via ``ruslan chat --ignore-user-config``),
    the user config at ``~/.ruslan/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({RUSLAN_HOME}/config.yaml)
    user_config_path = _ruslan_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("RUSLAN_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Ruslan, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Ruslan - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see ruslan_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from ruslan_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and RuslanCLI.__init__ checks "default" first.
            ...
            logger.warning("Не удалось загрузить cli-config.yaml: %s", e)
    ...
    # ruslan_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `ruslan config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (ruslan_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && ruslan` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _RUSLAN_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_RUSLAN_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["RUSLAN_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.ruslan/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from ruslan_logging import setup_logging
...
try:
    from ruslan_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``ruslan --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from ruslan_cli.mcp_startup import wait_for_mcp_discovery
...
from ruslan_cli.banner import build_welcome_banner
from ruslan_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from ruslan_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("RUSLAN_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("RUSLAN_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    try:
        ...
        logger.warning(
            "обнаружение плагина не удалось при отложенном запуске CLI",
            exc_info=True,
        )
    try:
    ...
    try:
        if _active_agent_ref and hasattr(_active_agent_ref, 'shutdown_memory_provider'):
            ...
            if isinstance(_session_msgs, list):
                logger.info(
                    "Очистка CLI вызывает завершение памяти для сессии %s с %d сообщением(ями)",
                ...
                logger.info(
                    "Очистка CLI вызывает завершение памяти для сессии %s без списка сообщений сессии",
                    getattr(_active_agent_ref, "session_id", None) or "<unknown>",
                )
                _active_agent_ref.shutdown_memory_provider()
    except Exception as e:
        logger.warning("Ошибка очистки памяти при завершении CLI: %s", e, exc_info=True)
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.ruslan/ruslan-agent`` clone is updated only by ``ruslan update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("сначала перейдите в репозиторий вашего проекта, затем выполните ruslan -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"ruslan-{short_id}"
    branch_name = f"ruslan/{wt_name}"
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if base_ref != "HEAD":
                logger.warning(
                    "добавление worktree из %s не удалось (%s); повторная попытка с локального HEAD",
        ...
        try:
            ...
            for line in include_file.read_text().splitlines():
                ...
                if not _path_is_within_root(src_resolved, repo_root_resolved):
                    logger.warning("Пропуск записи .worktreeinclude за пределами корня репозитория: %s", entry)
                    continue
                if not _path_is_within_root(dst_resolved, wt_path_resolved):
                    logger.warning("Пропуск записи .worktreeinclude, выходящей за пределы рабочего дерева: %s", entry)
                    continue
                if src.is_file():
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"ruslan pid={os.getpid()}", str(wt_path)],
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`ruslan_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from ruslan_constants import get_ruslan_home as _get_ruslan_home
        _ruslan_home_maint = _get_ruslan_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_ruslan_home_maint / "sessions"
                )
                session_db.set_meta("ghost_session_prune_v1", "1")
                if pruned:
                    logger.info("Удалено %d пустых призрачных сессий TUI", pruned)
        ...
        try:
            if not session_db.get_meta("orphaned_compression_finalize_v1"):
                ...
                if finalized:
                    logger.info(
                        "Завершено %d потерянных сессий сжатия", finalized
                    )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`ruslan_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``ruslan/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("ruslan-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``ruslan/ruslan-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``ruslan -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("ruslan/ruslan-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. RUSLAN_LIGHT / RUSLAN_TUI_LIGHT env (true/false) — explicit override
#   2. RUSLAN_TUI_THEME=light|dark — explicit theme
#   3. RUSLAN_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Ruslan assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("RUSLAN_LIGHT", "RUSLAN_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("RUSLAN_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("RUSLAN_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from ruslan_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_ruslan_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._ruslan_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Ruslan output it's common to display cron schedules verbatim.
    #
...
from ruslan_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_ruslan_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_ruslan_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._ruslan_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_ruslan_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._ruslan_bp_start = now
        ...
        _vt100_mod._ruslan_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``RuslanCLI._busy_command()``.
        """
        yield self

# ASCII Art - RUSLAN-AGENT logo (full width, single line - requires ~95 char terminal)
RUSLAN_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Ruslan Caduceus (compact, fits in left panel)
RUSLAN_SHIELD = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS RUSLAN - AI Agent Framework"
        tiny_line = "⚕ NOUS RUSLAN"
    else:
        agent_name = _skin.get_branding("agent_name", "Ruslan Agent") if _skin else "Ruslan Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("RUSLAN_FAST_STARTUP_BANNER") == "1":
        from ruslan_cli import __release_date__ as _release_date
        from ruslan_cli import __version__ as _version

        version_line = f"Ruslan Agent v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Valldun[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from ruslan_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.ruslan/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.ruslan/config.yaml on first use)
        ...
        logger.error("Не удалось сохранить конфиг: %s", e)
        return False
...
class RuslanCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Ruslan Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: RUSLAN_MAX_TOKENS)
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from ruslan_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("RUSLAN_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("RUSLAN_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("RUSLAN_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `ruslan chat --ignore-rules` in ruslan_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("RUSLAN_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("RUSLAN_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        try:
            ...
            logger.warning("Не удалось инициализировать SessionDB — сессия НЕ будет проиндексирована для поиска: %s", e)
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `ruslan update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Ruslan processes for this RUSLAN_HOME.
        ...
        self._history_file = _ruslan_home / ".ruslan_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from ruslan_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Ruslan'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from ruslan_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from ruslan_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from ruslan_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from ruslan_cli.codex_models import get_codex_model_ids
    ...
    def _expand_paste_references(self, text: str | None) -> str:
        ...
        def _expand_ref(match):
            ...
            try:
                ...
                logger.warning("Файл вставки исчез или нечитаем, возвращается заглушка: %s", path)
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from ruslan_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Ruslan")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - RuslanCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``ruslan doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``ruslan doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Ruslan needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Ruslan LLM (not agentic)
        from ruslan_cli.model_switch import is_nous_ruslan_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_ruslan_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Valldun Ruslan 3 & 4 models are NOT agentic and are not "
                "designed for use with Ruslan Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.ruslan/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from ruslan_cli.clipboard import save_clipboard_image

        img_dir = get_ruslan_home() / "images"
    ...
    def _recover_terminal_input_modes(self, *, reason: str) -> None:
        ...
        logger.warning("Восстановлены режимы ввода терминала после утечки: %s", reason)
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'ruslan setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from ruslan_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_toolsets(self):
        ...
        print()
        print("Совет: используйте 'all' или '*', чтобы включить все наборы инструментов")
        print("Пример: python cli.py --toolsets web,terminal")
        print()
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from ruslan_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from ruslan_cli.main import _relative_time

        print()
        if reason == "history":
            print("(._.) В текущем чате еще нет сообщений — вот недавние сеансы, которые вы можете продолжить:")
        else:
            print("Недавние сеансы:")
        print()
        ...
        print()
        print("Используйте /resume <number>, /resume <session id> или /resume <session title>, чтобы продолжить.")
        print("Пример: /resume 2")
        print()
        return True
    ...
    def show_history(self):
        ...
        if not self.conversation_history:
            if not self._show_recent_sessions(reason="history"):
                print("(._.) История беседы еще отсутствует.")
            return
        ...
        for msg in self.conversation_history:
            ...
            print(f"\n  [Ruslan #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``ruslan sessions list``. Delegates the
        ...
        if getattr(self, "conversation_history", None):
            return False
        try:
            from ruslan_constants import get_ruslan_home as _ghh
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `ruslan sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("RUSLAN_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.ruslan/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``ruslan --resume <id>``
        regardless of whether the user ever runs ``/save``.
        """
        if not self.conversation_history:
            print("(;_;) Нет разговора для сохранения.")
            return

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        saved_dir = get_ruslan_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"ruslan_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: ruslan --resume {self.session_id}")
        except Exception as e:
    ...
    def retry_last(self):
        ...
        """
        if not self.conversation_history:
            print("(._.) Нет сообщений для повторной попытки.")
            return None
        ...
        if last_user_idx is None:
            print("(._.) Не найдено сообщение пользователя для повторной попытки.")
            return None
    ...
    def undo_last(self, n: int = 1, prefill: bool = True):
        ...
        """
        if not self.conversation_history:
            print("(._.) Нет сообщений для отмены.")
            return
        ...
        if not user_indices:
            print("(._.) Не найдено сообщение пользователя для отмены.")
            return
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from ruslan_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Ruslan session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                from ruslan_cli.context_switch_guard import merge_preflight_compression_warning
        ...
        try:
            from ruslan_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `ruslan model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from ruslan_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from ruslan_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        from ruslan_cli.providers import get_label
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from ruslan_cli.commands import resolve_command
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print("Конфигурация платформы обмена сообщениями:")
            print("  " + "-" * 55)
            ...
            print()
            print("Чтобы запустить шлюз:")
            print("python cli.py --gateway")
            print()
            print(f"  Configuration file: {display_ruslan_home()}/config.yaml")
            print()
            ...
            print("DISCORD_BOT_TOKEN=ваш_токен")
            print(f"    2. Or configure settings in {display_ruslan_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in ruslan_cli/commands.py instead of touching every dispatch site.
        from ruslan_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from ruslan_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from ruslan_cli.skin_engine import get_active_skin
            ...
            from ruslan_cli.main import _print_version_info
            ...
            from ruslan_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `ruslan plugins
                ...
                # catalog lives behind `ruslan plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("Пользовательские плагины не установлены.")
                    print("Установите один: ruslan plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_ruslan_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: ruslan plugins list)")
                else:
                    # Loaded-plugin details (tools/hooks/commands counts, errors)
                    # keyed by name, when available.
                    loaded: dict = {}
                    try:
                        from ruslan_cli.plugins import get_plugin_manager
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: ruslan plugins list)")
                    print("Включить/отключить: ruslan plugins enable/disable <name>")
            except Exception as e:
            ...
            # Check for plugin-registered slash commands
                ...
                from ruslan_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from ruslan_cli.commands import COMMANDS
    ...
    def _toggle_verbose(self):
        ...
        from ruslan_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``RUSLAN_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``RUSLAN_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        if not self.conversation_history or len(self.conversation_history) < 4:
            print("(._.) Недостаточно разговора для сжатия (нужно как минимум 4 сообщения).")
            return

        if not self.agent:
            print("(._.) Нет активного агента — сначала отправьте сообщение.")
            return

        if not self.agent.compression_enabled:
            print("(._.) Сжатие отключено в конфиге.")
            return

        from ruslan_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if calls == 0:
            if not self._print_nous_credits_block():
                print("(._.) В этом сеансе ещё не было вызовов API.")
            return
        ...
        print("📊 Использование токенов сеанса")
        print(f"  {'─' * 40}")
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by ruslan_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors RUSLAN_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("Выполните `ruslan portal` для входа, затем /credits.")
            return
        ...
        if not getattr(self, "_app", None):
            ...
            print("Завершите пополнение в браузере — кредиты появятся в /credits в ближайшее время.")
            return
        ...
        if choice == "open":
            ...
            print("🟡 Отменено. Кредиты не добавлены.")
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("Выполните `ruslan portal` для входа, затем /billing.")
            return
    ...
    def _billing_open_portal(self, state):
        ...
        print("Завершите изменения в биллинге в браузере.")
    ...
    def _billing_buy_flow(self, state):
        ...
        if not getattr(self, "_app", None):
            ...
            print("Выполните это в интерактивном CLI для завершения покупки.")
            self._billing_portal_hint(state)
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Valldun to charge your card."
        )
        ...
        if not getattr(self, "_app", None):
            print("Выполните в интерактивном CLI для подтверждения покупки.")
            return
        raw = self._prompt_text_input_modal(
        ...
        from ruslan_cli.nous_billing import (
            BillingError,
        ...
        if not charge_id:
            print("🔴 Не возвращен id платежа; пожалуйста, проверьте портал.")
            return
    ...
    def _billing_render_charge_failed(self, state, reason):
        ...
        if reason == "authentication_required":
            ...
            print("🔴 Ваша карта истекла. Обновите её на портале.")
        elif reason == "card_declined":
            print("🔴 Ваша карта была отклонена. Попробуйте другую карту на портале.")
        else:
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from ruslan_cli.nous_billing import BillingRateLimited
        ...
        if code == "no_payment_method":
            ...
            print("🔴 Терминальный биллинг отключён для этой организации — администратор должен включить его на портале.")
    ...
    def _billing_handle_scope_required(self, state):
        ...
        print()
        print("💳 Биллинг терминала требует дополнительного разрешения (billing:manage).")
        _scope_msg = (
        ...
        if not getattr(self, "_app", None):
            print("Выполните `ruslan portal` и одобрите биллинг терминала, затем повторите попытку.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from ruslan_cli.auth import step_up_nous_billing_scope
            ...
            return
        if granted:
            print("✅ Разрешение на биллинг предоставлено.")
            ...
            if fresh.logged_in and fresh.cli_billing_enabled:
                print("Выполните /billing buy снова, чтобы продолжить.")
            else:
            ...
            print("🟡 Биллинг терминала не был предоставлен (администратор должен отметить галочку).")
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        if card:
            ...
            print("Нет сохраненной карты — сначала настройте ее на портале.")
            self._billing_portal_hint(state)
            return
        if currently_on:
            print(
        ...
        if not getattr(self, "_app", None):
            print("Выполните в интерактивном CLI для настройки автоперезагрузки.")
            self._billing_portal_hint(state)
            return
        ...
        if reload_amt is None or threshold_amt is None or reload_amt <= threshold_amt:
            print("🔴 Сумма пополнения должна быть больше порога.")
            return

        print()
        _ar_consent = (
            f"By confirming, you authorize Valldun to charge {card.masked} "
    ...
    def _billing_auto_reload_disable(self, state):
        ...
        try:
            ...
            self._billing_render_charge_error(state, exc)
            return
        print("✅ Автоперезагрузка отключена.")
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from ruslan_cli.config import get_config_path as _get_config_path
        ...
        print()
        print("🔄 Конфигурация MCP сервера изменена — перезагрузка соединений...")
        ...
        if _reload_thread.is_alive():
            print("⚠️  Перезагрузка MCP превысила время ожидания (30с). Некоторые серверы могли не переподключиться.")
    ...
    def _confirm_destructive_slash(
        ...
        if choice == "always":
            if save_config_value("approvals.destructive_slash_confirm", False):
                print("🔒 Будущие команды /clear, /new, /reset и /undo будут выполняться без подтверждения.")
                print("Повторно включите через `approvals.destructive_slash_confirm: true` в config.yaml.")
            else:
                print("⚠️  Не удалось сохранить отказ — выполняется один раз.")
    ...
    def _confirm_and_reload_mcp(self, cmd_original: str = "") -> None:
        ...
        if raw is None:
            print("🟡 /reload-mcp отменен (нет ввода).")
            return
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if choice is None:
        ...
        if choice == "cancel":
            print("🟡 /reload-mcp отменено. Инструменты MCP не изменены.")
            return

        if choice == "always":
            if save_config_value("approvals.mcp_reload_confirm", False):
                print("🔒 Будущие вызовы /reload-mcp будут выполняться без подтверждения.")
                print("Повторно включите через `approvals.mcp_reload_confirm: true` в config.yaml.")
            else:
                print("⚠️  Не удалось сохранить отказ — перезагрузка один раз.")
    ...
    def _reload_mcp(self):
        ...
        try:
            ...
            if not self._command_running:
                print("🔄 Перезагрузка серверов MCP...")
            ...
            if not connected_servers:
                print("Нет подключённых серверов MCP.")
            else:
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.ruslan/skills/ and queue a note for the
        next user turn.
        ...
        try:
            ...
            if not added and not removed:
                print("Новых навыков не обнаружено.")
            ...
            if added:
                print("➕ Добавленные навыки:")
                for item in added:
                    print(f"  {_fmt_line(item)}")
            if removed:
                print("➖ Удалённые навыки:")
                for item in removed:
                    print(f"  {_fmt_line(item)}")
            print(f"  📚 {total} skill(s) available")
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "ruslan_voice"), exist_ok=True)
            ...
            logger.warning("Воспроизведение TTS не удалось: %s", e)
        ...
        if images:
            ...
            if _img_mode == "native":
                try:
                    ...
                    logging.warning("Не удалось присоединить нативное изображение, возврат к тексту: %s", _img_exc)
                    message = self._preprocess_images_with_vision(
                        message if isinstance(message, str) else "", images
                    )
            else:
        ...
        try:
            ...
            def run_agent():
                ...
                try:
                    ...
                    logging.error("run_conversation выбросил исключение: %s", exc, exc_info=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _ruslan_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard RUSLAN_HOME, so no -p needed.
            try:
            ...
            print(f"  ruslan --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  ruslan -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from ruslan_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from ruslan_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from ruslan_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            from ruslan_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(RUSLAN_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Ruslan migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from ruslan_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in ruslan_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from ruslan_cli.model_switch import resolve_persist_behavior
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Ruslan binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from ruslan_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Ruslan Agent")
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if (lines_hit or chars_hit) and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _ruslan_home / "pastes"
                    paste_dir.mkdir(parents=True, exist_ok=True)
                    paste_file = paste_dir / f"paste_{_paste_counter[0]}_{datetime.now().strftime('%H%M%S')}.txt"
                    paste_file.write_text(pasted_text, encoding="utf-8")
                    logger.info("Свёрнутая вставка #%d: %d строк, %d символов -> %s", _paste_counter[0], line_count + 1, len(pasted_text), paste_file)
        ...
        # Dynamic prompt: shows Ruslan symbol when agent is working,
        ...
        def _on_text_changed(buf):
            ...
            if (lines_hit or chars_hit) and is_paste and not text.startswith('/'):
                ...
                logger.info("Свёрнутая вставка #%d: %d строк, %d символов -> %s (запасной вариант)", _paste_counter[0], line_count + 1, len(text), paste_file)
                _paste_just_collapsed[0] = True
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Ruslan needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Ruslan needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Ruslan needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = RuslanCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_ruslan_osd_patched", False):
                ...
                _pt_renderer._ruslan_osd_patched = True
        except Exception:
            pass
        ...
        def process_loop():
            while not self._should_exit:
                try:
                    ...
                    logger.warning("process_loop: необработанная ошибка (сообщение может быть потеряно): %s", e)
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``RUSLAN_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("RUSLAN_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the ruslan process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: ruslan setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `ruslan sessions list`
...
def _run_kanban_goal_loop_q(cli: "RuslanCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``RUSLAN_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("RUSLAN_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from ruslan_cli import kanban_db as _kb
    from ruslan_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Ruslan Agent CLI - Interactive AI Assistant
    ...
    os.environ["RUSLAN_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Запуск Ruslan Gateway (платформы обмена сообщениями)...")
    ...
    # Default to ruslan-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Ruslan): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside RuslanCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  RUSLAN_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("RUSLAN_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``ruslan chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("RUSLAN_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from ruslan_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Ruslan" box, no tool-gen
                        ...
                        if os.environ.get("RUSLAN_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("RUSLAN_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from ruslan_cli.kanban_db import (
                ...
                # Single-query mode (`ruslan chat -q "…"`): skip the welcome
