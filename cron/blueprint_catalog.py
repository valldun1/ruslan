...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``ruslan://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``ruslan://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"ruslan://blueprint/{quote(blueprint.key)}{qs}"
