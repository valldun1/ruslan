...
from ruslan_constants import get_default_ruslan_root, get_ruslan_home
...
RUSLAN_DIR = get_default_ruslan_root().resolve()
...
def _warn_if_orphaned_profile_store() -> None:
    ...
    if _WARNED_ORPHAN_STORE:
        return
    try:
        active = get_ruslan_home().resolve()
        if active == RUSLAN_DIR:
