...
def _get_lock_paths() -> tuple[Path, Path]:
    ...
    jobs store in cron.jobs (which uses get_default_ruslan_root). The tick lock
    ...
    config.yaml, scripts) stays profile-aware via _get_ruslan_home().
    """
    from ruslan_constants import get_default_ruslan_root
    lock_dir = (_ruslan_home or get_default_ruslan_root()) / "cron"
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_ruslan_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_ruslan_home() / ".env"), override=True, encoding="latin-1")
