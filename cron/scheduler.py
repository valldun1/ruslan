...
Uses a file-based lock (~/.ruslan/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `ruslan update` reloads
# the module) fail with ModuleNotFoundError for ruslan_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from ruslan_constants import get_ruslan_home
from ruslan_cli._subprocess_compat import windows_hide_flags
from ruslan_cli.config import load_config, _expand_env_vars
from ruslan_cli.fallback_config import get_fallback_chain
from ruslan_time import now as _ruslan_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy ruslan_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from ruslan_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``ruslan tools`` config for the ``cron`` platform.
    ...
    try:
        from ruslan_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``ruslan update`` / ``ruslan gateway stop`` / systemd restart, or an
...
_ruslan_home: Path | None = None


def _get_ruslan_home() -> Path:
    """Resolve Ruslan home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active RUSLAN_HOME at call time
    ...
    """
    return _ruslan_home or get_ruslan_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    ruslan_home = _get_ruslan_home()
    lock_dir = ruslan_home / "cron"
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Ruslan — {task_name}"
    try:
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("RUSLAN_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid RUSLAN_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within RUSLAN_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Ruslan-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against RUSLAN_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_ruslan_home() / "scripts"
    ...
    # escape — scripts MUST reside within RUSLAN_HOME/scripts/.
    try:
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _ruslan_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    #
    # Bounded with its own timeout (separate from RUSLAN_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("RUSLAN_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid RUSLAN_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_ruslan_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_ruslan_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["RUSLAN_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed RUSLAN_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads RUSLAN_SESSION_PLATFORM
    #     and RUSLAN_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via RUSLAN_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     RUSLAN_SESSION_PLATFORM == "telegram".
    ...
    #     RUSLAN_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read RUSLAN_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the RUSLAN_CRON_AUTO_DELIVER_* vars set
    # below, so clearing RUSLAN_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "RUSLAN_CRON_AUTO_DELIVER_PLATFORM",
        "RUSLAN_CRON_AUTO_DELIVER_CHAT_ID",
        "RUSLAN_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        # load_ruslan_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this RUSLAN_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_ruslan_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from ruslan_cli.env_loader import (
            load_ruslan_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_ruslan_dotenv(ruslan_home=_get_ruslan_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["RUSLAN_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["RUSLAN_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["RUSLAN_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > RUSLAN_MODEL env >
        ...
        model = job.get("model") or os.getenv("RUSLAN_MODEL") or ""
        ...
        try:
            import yaml
            _cfg_path = str(_get_ruslan_home() / "config.yaml")
        ...
        prefill_file = (
            os.getenv("RUSLAN_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_ruslan_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject RUSLAN_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # RUSLAN_HOME. When a workdir is configured, also inject project
        ...
        # override via RUSLAN_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("RUSLAN_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid RUSLAN_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_ruslan_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_ruslan_now().strftime('%b %d %H:%M')}"
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_ruslan_home())
        )
    ...
    *,
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _ruslan_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _ruslan_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set RUSLAN_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("RUSLAN_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid RUSLAN_CRON_MAX_PARALLEL value; defaulting to unbounded")
