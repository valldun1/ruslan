...
Ruslan supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.ruslan/config.yaml`:
...
Or set the env var: `RUSLAN_KANBAN_DISPATCH_IN_GATEWAY=false`
