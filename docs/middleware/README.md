...
Ruslan middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Ruslan'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Ruslan calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Ruslan retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Ruslan guardrails,
...
- `telemetry_schema_version`: currently `ruslan.observer.v1`
- `middleware_schema_version`: currently `ruslan.middleware.v1`
...
Ruslan stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Ruslan runs
them as a nested chain in registration order. Middleware failures are fail-open:
Ruslan logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Ruslan applies middleware in this order:
...
For each tool call, Ruslan applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Ruslan pre-execution path against the effective arguments:
...
```bash
ruslan plugins enable <plugin-name>
```

For isolated local testing, use one `RUSLAN_HOME` for plugin enablement and the
agent run:

```bash
export RUSLAN_HOME=/tmp/ruslan-middleware-test
mkdir -p "$RUSLAN_HOME"
ruslan plugins enable <plugin-name>
ruslan chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run ruslan plugins enable <plugin-name>
uv run ruslan chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["ruslan_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/ruslan-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Ruslan expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Ruslan treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Ruslan preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Ruslan does not convert downstream
