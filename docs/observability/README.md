# Ruslan Observer Hooks

Ruslan observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Ruslan' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Ruslan catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Ruslan only builds
