...
`ruslan config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.ruslan/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `RUSLAN_HOME`, secret scope, and
