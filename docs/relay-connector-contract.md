...
This document is the formal interface between the **Ruslan gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `ruslan gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`ruslan gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `ruslan` CLI.
