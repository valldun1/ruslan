...
When running Ruslan inside Docker, the default `network_mode: host` gives the
...
The Ruslan [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   RUSLAN_UID=$(id -u) RUSLAN_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=ruslan,ruslan-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://ruslan-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Ruslan trust model and vulnerability reporting
