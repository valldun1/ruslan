...
The canonical transcript store is SQLite via `SessionDB` (from `ruslan_state`). The
...
Counts consecutive restarts via a JSON file (`{RUSLAN_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `ruslan update`, `ruslan gateway restart`,
or `/restart`.
