...
window before ``ruslan update``'s graceful restart fires), a first-time lazy
...
def _fingerprint() -> str | None:
    ...
    ``ruslan_cli.main`` is always already imported in a gateway process (it's
    the entry point), so this import is free and avoids duplicating the
    worktree-aware ref resolution.
    """
    try:
        from ruslan_cli.main import _read_git_revision_fingerprint
