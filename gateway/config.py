...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_ruslan_home() / "sessions")
    ...
    # gateway behaves exactly as before — single RUSLAN_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.ruslan/config.yaml (primary user-facing config)
    3. ~/.ruslan/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_ruslan_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through ruslan_cli.config.load_config, so without this a managed
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `ruslan config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``ruslan config set gateway.streaming.*``
                streaming_cfg = (
