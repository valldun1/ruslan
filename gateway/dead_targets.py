...
The store is a small JSON file under the active profile's RUSLAN_HOME so each
...
class DeadTargetRegistry:
    ...
    def __init__(self, path: Optional[Path] = None) -> None:
        ...
        if path is not None:
            ...
            self._path = get_ruslan_home() / "gateway" / "dead_targets.json"
        self._load()
    ...
    def all_dead(self) -> Dict[str, Dict[str, object]]:
        """Snapshot of the current dead set (for diagnostics / `ruslan` CLI)."""
        with self._lock:
