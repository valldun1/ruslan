...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{RUSLAN_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``RUSLAN_HOME`` is a **durable** store — on Ruslan
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``ruslan gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting RUSLAN_HOME."""
    base = home if home is not None else get_ruslan_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable RUSLAN_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (RUSLAN_HOME is a
    durable Fly volume on Ruslan Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable RUSLAN_HOME volume is
