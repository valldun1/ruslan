...
from ruslan_constants import get_ruslan_dir, get_ruslan_home
...
PAIRING_DIR = get_ruslan_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from ruslan_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{RUSLAN_HOME}/pairing`` while newer code/docs may
    write ``{RUSLAN_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<RUSLAN_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<RUSLAN_HOME>/pairing/``
    directory (backward-compat for the ``ruslan pairing`` CLI).
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp RUSLAN_HOME
        # and PairingStore may be constructed before the env is set.
        if profile:
            from ruslan_constants import get_ruslan_home
            self._dir = get_ruslan_home() / "profiles" / profile / "pairing"
        else:
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `ruslan` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> ruslan pairing approve ...` as root, "
                    "re-run with `docker exec -u ruslan <container> ...` and "
                    "chown the existing file to the ruslan user, or restart the "
