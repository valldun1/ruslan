...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``ruslan setup`` display).
    ...
    # built-ins).  Used by ``ruslan gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `ruslan`
        # invocation -- including plain `ruslan chat`, which never touches any
        ...
        # `ruslan setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``ruslan setup``/``gateway status``, channel
