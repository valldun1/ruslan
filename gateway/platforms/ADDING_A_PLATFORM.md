...
There are two ways to add a platform to the Ruslan gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.ruslan/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Ruslan code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `ruslan gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `ruslan_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Ruslan core.
...
And add it to the `ruslan-gateway` composite:

```python
"ruslan-gateway": {
    "includes": [..., "ruslan-your-platform"]
}
```
...
## 12. Status Display (`ruslan_cli/status.py`)
...
## 13. Gateway Setup Wizard (`ruslan_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ ruslan_cli/ toolsets.py \
  --include="*.py" -l | sort -u
