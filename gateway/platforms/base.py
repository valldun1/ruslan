...
# Audio file extensions Ruslan recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Ruslan' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Ruslan-created Telegram private-chat
...
from ruslan_constants import get_default_ruslan_root, get_ruslan_dir, get_ruslan_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.ruslan/.env manually."
)
...
IMAGE_CACHE_DIR = get_ruslan_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_ruslan_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_ruslan_dir(new_subpath, old_name)
...
AUDIO_CACHE_DIR = get_ruslan_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_ruslan_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_ruslan_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_ruslan_dir("cache/screenshots", "browser_screenshots")
...
_RUSLAN_ROOT = get_default_ruslan_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "RUSLAN_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "RUSLAN_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "RUSLAN_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "RUSLAN_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _RUSLAN_HOME / "image_cache",
    _RUSLAN_HOME / "audio_cache",
    _RUSLAN_HOME / "video_cache",
    _RUSLAN_HOME / "document_cache",
    _RUSLAN_HOME / "browser_screenshots",
    ...
    _RUSLAN_HOME / "cache" / "images",
    _RUSLAN_HOME / "cache" / "audio",
    _RUSLAN_HOME / "cache" / "videos",
    _RUSLAN_HOME / "cache" / "documents",
    _RUSLAN_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Ruslan root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* RUSLAN_HOME's cache, so a gateway running at
    the root (e.g. ``RUSLAN_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when RUSLAN_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _RUSLAN_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("RUSLAN_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _RUSLAN_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Ruslan profile and shared Ruslan root both contain control
    ...
    # RUSLAN_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.ruslan
    ...
    for ruslan_root in (_RUSLAN_HOME, _RUSLAN_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(ruslan_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Ruslan
    secrets (``~/.ruslan/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``RUSLAN_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Ruslan-managed cache, under an operator-allowlisted root
    (``RUSLAN_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Ruslan root (~/.ruslan/.env,
    ...
    # ``MEDIA:~/.ruslan/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?ruslan\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+ruslan[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``RUSLAN_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Ruslan commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("RUSLAN_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "RUSLAN_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "RUSLAN_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Ruslan sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Ruslan home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Ruslan home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.ruslan/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from ruslan_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          RUSLAN_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          RUSLAN_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          RUSLAN_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("RUSLAN_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("RUSLAN_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("RUSLAN_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _ruslan_run_generation is set on
    ...
    def build_source(
        ...
        per-profile RUSLAN_HOME isolation.
        """
