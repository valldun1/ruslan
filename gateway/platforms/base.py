...
# Default location: {RUSLAN_HOME}/cache/images/ (legacy: image_cache/)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    for ruslan_root in (_RUSLAN_HOME, _RUSLAN_ROOT):
        denied.append(ruslan_root / ".env")
        denied.append(ruslan_root / "auth.json")
        denied.append(ruslan_root / "credentials")
        denied.append(ruslan_root / "config.yaml")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.ruslan/.env,
    # ~/.ruslan/auth.json, etc. — so the obvious prompt-injection sites
