...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.ruslan to the active profile home."""
    ...
    if raw == "~/.ruslan":
        return ruslan_home
    if raw.startswith("~/.ruslan/"):
        return ruslan_home / raw.removeprefix("~/.ruslan/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return ruslan_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under RUSLAN_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from ruslan_constants import get_ruslan_home

    scripts_root = (get_ruslan_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.ruslan" or raw_text.startswith("~/.ruslan/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__ruslan_ignore__") is True
        ):
            return False, None
