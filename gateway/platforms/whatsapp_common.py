...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Ruslan Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to RUSLAN_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/ruslan), this function
    mirrors the bridge source to a writable RUSLAN_HOME location and returns that
    ...
    # Try RUSLAN_HOME location first
    ruslan_home = get_ruslan_home()
    ruslan_home_bridge = ruslan_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to RUSLAN_HOME if needed
    if ruslan_home_bridge.exists():
        return ruslan_home_bridge

    # Mirror the bridge source to RUSLAN_HOME
    try:
        ruslan_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            ruslan_home_bridge,
            dirs_exist_ok=False,
