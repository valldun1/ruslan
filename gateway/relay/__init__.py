"""Relay/connector support package for the Ruslan gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``ruslan gateway enroll`` writes them to
    ``~/.ruslan/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``RUSLAN_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``ruslan gateway enroll``).
...
def _resolve_relay_identity_token() -> str:
    ...
    ``ruslan gateway enroll`` CLI. Two modes, in precedence order:
    ...
    if not token_url:
        ...
        from ruslan_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.ruslan/.env``.
    ...
    sets neither ``RUSLAN_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``ruslan gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'ruslan'}"
