...
Defenses 1 and 2 (the ``_RUSLAN_GATEWAY`` guard on ``ruslan gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.ruslan.gateway")``,
...
State lives in ``<RUSLAN_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_ruslan_home() / "gateway" / "restart_loop.json"
