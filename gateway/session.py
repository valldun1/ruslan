...
# ``RUSLAN_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``RUSLAN_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
...
# ``sessions_dir / f"{session_id}.json"`` in ruslan_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `ruslan tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `ruslan tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_ruslan_home()}/cron/output/)"
    )
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.ruslan/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.ruslan/state.db "
                "and are shown by `ruslan sessions list` and `/sessions`. "
