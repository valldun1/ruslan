...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Руслан ожидает OAuth callback на {redirect_uri}")
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Руслан не подключён к Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Руслан не подключён к Nous Portal.",
    ...
    *,
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Руслан создаст свою сессию, чтобы не конфликтовать с Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Руслан будет работать в отдельной сессии, чтобы не конфликтовать с Codex CLI / VS Code.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Руслан будет использовать OpenRouter для инференса.")
        elif should_reset_config:
            print("Запусти `hermes model` или настрой API-ключ для использования Руслана.")
        else:
