...
def _localized_telegram_desc(key: str, fallback: str) -> str:
    """Resolve a Telegram command description via i18n, falling back to English."""
    try:
        from agent.i18n import t, get_language
        result = t(f"telegram_cmd.{key}", lang=get_language())
        # If the key itself is returned, the catalog is missing an entry.
        if result == f"telegram_cmd.{key}":
            return fallback
        return result
    except Exception:
        return fallback
...
def telegram_bot_commands() -> list[tuple[str, str]]:
    ...
    Descriptions are localized via the agent.i18n system when
    ``display.language`` is set (e.g. to ``ru``) in config.yaml.
    """
    overrides = _resolve_config_gates()
    ...
    for cmd in COMMAND_REGISTRY:
        ...
        if tg_name:
            desc = _localized_telegram_desc(cmd.name, cmd.description)
    ...
    for name, description, args_hint in _iter_plugin_command_entries():
        ...
        if tg_name:
            desc = _localized_telegram_desc(name, description)
            result.append((tg_name, desc))
    return result
