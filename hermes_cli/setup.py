...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Руслан может хранить несколько ключей для одного провайдера и ротировать между"
            )
            print_info(
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Руслан может показать, что будет импортировано, без внесения изменений.")
    print()
...
def run_setup_wizard(args):
    ...
    print(
        color(
            "│             🛡 Настройка Руслана                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Давай настроим Руслана.                      │", Colors.MAGENTA
        )
    )
    print(
        color(
