"""
Ruslan MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Ruslan-specific extra)

Usage:
    ruslan mcp serve
    ruslan mcp serve --verbose
...
logger = logging.getLogger("ruslan.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using RUSLAN_HOME."""
    try:
        from ruslan_constants import get_ruslan_home
        return get_ruslan_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("RUSLAN_HOME", Path.home() / ".ruslan")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from ruslan_constants import get_ruslan_home
        directory_file = get_ruslan_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("RUSLAN_HOME", Path.home() / ".ruslan")
...
class EventBridge:
    ...
    This is the Ruslan equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from ruslan_constants import get_ruslan_home
            db_file = get_ruslan_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("RUSLAN_HOME", Path.home() / ".ruslan")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Ruslan MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "ruslan",
        instructions=(
            "Ruslan Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Ruslan MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
