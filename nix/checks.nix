...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "ruslan-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "ruslan-build-package" { } ''
          echo "PASS: package built at ${ruslan-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "ruslan-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "ruslan-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${ruslan-agent}/bin/ruslan || (echo "FAIL: ruslan binary missing"; exit 1)
          test -x ${ruslan-agent}/bin/ruslan-agent || (echo "FAIL: ruslan-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${ruslan-agent}/bin/ruslan version 2>&1 | grep -qi "ruslan" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "ruslan-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in ruslan ruslan-agent ruslan-acp; do
            test -x ${ruslan-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "ruslan-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking ruslan --help ==="
          ${ruslan-agent}/bin/ruslan --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${ruslan-agent}/bin/ruslan --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "ruslan-bundled-skills" { } ''
          set -e
        ...
        bundled-plugins = pkgs.runCommand "ruslan-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${ruslan-agent}/share/ruslan-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${ruslan-agent}/share/ruslan-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "RUSLAN_BUNDLED_PLUGINS" ${ruslan-agent}/bin/ruslan || \
            (echo "FAIL: RUSLAN_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: RUSLAN_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "ruslan-bundled-locales" { } ''
          set -e
          ...
          test -f ${ruslan-agent}/share/ruslan-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "RUSLAN_BUNDLED_LOCALES" ${ruslan-agent}/bin/ruslan || \
            (echo "FAIL: RUSLAN_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: RUSLAN_BUNDLED_LOCALES set in wrapper"
          ...
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without RUSLAN_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "ruslan-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${ruslan-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${ruslan-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "RUSLAN_TUI_DIR" ${ruslan-agent}/bin/ruslan || \
            (echo "FAIL: RUSLAN_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: RUSLAN_TUI_DIR set in wrapper"
        ...
        # Verify RUSLAN_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        ruslan-node = pkgs.runCommand "ruslan-node-version" { } ''
          set -e
          echo "=== Checking RUSLAN_NODE in wrapper ==="
          grep -q "RUSLAN_NODE" ${ruslan-agent}/bin/ruslan || \
            (echo "FAIL: RUSLAN_NODE not set in wrapper"; exit 1)
          echo "PASS: RUSLAN_NODE present in wrapper"

          RUSLAN_NODE=$(sed -n "s/^export RUSLAN_NODE='\(.*\)'/\1/p" ${ruslan-agent}/bin/ruslan)
          test -x "$RUSLAN_NODE" || (echo "FAIL: RUSLAN_NODE=$RUSLAN_NODE not executable"; exit 1)
          echo "PASS: RUSLAN_NODE executable at $RUSLAN_NODE"

          NODE_MAJOR=$("$RUSLAN_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All RUSLAN_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify RUSLAN_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "ruslan-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(RUSLAN_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking RUSLAN_MANAGED guards ==="
          check_blocked "config set" ${ruslan-agent}/bin/ruslan config set model foo
          check_blocked "config edit" ${ruslan-agent}/bin/ruslan config edit
        ...
        in pkgs.runCommand "ruslan-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${ruslan-agent}/bin/ruslan; then
        ...
        in pkgs.runCommand "ruslan-config-roundtrip" {
