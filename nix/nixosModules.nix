# nix/nixosModules.nix — NixOS module for ruslan-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: ruslan runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $RUSLAN_HOME/.env
# and read by ruslan at startup — no container recreation needed for env changes.
#
# Tool resolution: the ruslan wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.ruslan-agent;
    effectivePackage =
    ...
    ruslan-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-ruslan-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "ruslan-config-attrs";
      description = "Ruslan YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    # RUSLAN_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "ruslan-documents" { } (
      ''
        mkdir -p $out
    ...
    # ruslan user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "ruslan-container-entrypoint" ''
      set -eu

      RUSLAN_UID="''${RUSLAN_UID:?RUSLAN_UID must be set}"
      RUSLAN_GID="''${RUSLAN_GID:?RUSLAN_GID must be set}"

      # ── Group: ensure a group with GID=$RUSLAN_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$RUSLAN_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="ruslan"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$RUSLAN_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$RUSLAN_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$RUSLAN_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$RUSLAN_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="ruslan"
        TARGET_HOME="/home/ruslan"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$RUSLAN_UID" -g "$RUSLAN_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$RUSLAN_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$RUSLAN_UID:$RUSLAN_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure RUSLAN_HOME is owned by the target user.
      ...
      if [ -n "''${RUSLAN_HOME:-}" ] && [ -d "$RUSLAN_HOME" ]; then
        find "$RUSLAN_HOME" \! -user "$RUSLAN_UID" -exec chown "$RUSLAN_UID:$RUSLAN_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/ruslan-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/ruslan-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/ruslan ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/ruslan
        chmod 0440 /etc/sudoers.d/ruslan
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$RUSLAN_UID" --regid="$RUSLAN_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $RUSLAN_HOME/.env.
    ...
    # Default: /var/lib/ruslan/workspace → /data/workspace.
  ...
  in {
    options.services.ruslan-agent = with lib; {
      enable = mkEnableOption "Ruslan Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = ruslan-agent;
        description = "The ruslan-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/ruslan";
        description = "State directory. Contains .ruslan/ subdir (RUSLAN_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Ruslan config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $RUSLAN_HOME/.env at activation time.
          Ruslan reads this file on every startup via load_ruslan_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $RUSLAN_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $RUSLAN_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `ruslan gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the ruslan user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the ruslan plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Ruslan discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          ruslan_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as ruslan (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in ruslan-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the ruslan CLI to environment.systemPackages and export
          RUSLAN_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.ruslan symlink to the service
            stateDir. These users are automatically added to the ruslan group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.ruslan-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the ruslan CLI to system PATH and export RUSLAN_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.ruslan/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.RUSLAN_HOME = "${cfg.stateDir}/.ruslan";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.ruslan-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed ruslan
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure ruslan is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.ruslan        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ruslan/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ruslan/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ruslan/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ruslan/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ruslan/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."ruslan-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.ruslan
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.ruslan ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.ruslan ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.ruslan -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.ruslan/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.ruslan/$_subdir"
            chmod 2770 "${cfg.stateDir}/.ruslan/$_subdir"
            find "${cfg.stateDir}/.ruslan/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # ruslan-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.ruslan/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.ruslan/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ruslan/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.ruslan/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.ruslan/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ruslan/.managed
          chmod 0644 ${cfg.stateDir}/.ruslan/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.ruslan/.container-mode <<'RUSLAN_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    ruslan_bin=${containerDataDir}/current-package/bin/ruslan
    RUSLAN_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ruslan/.container-mode
            chmod 0644 ${cfg.stateDir}/.ruslan/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.ruslan/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.ruslan";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.ruslan" ]; then
                  rm -f "${symlinkPath}"
                  echo "ruslan-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.ruslan -> stateDir/.ruslan for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.ruslan";
                target = "${cfg.stateDir}/.ruslan";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "ruslan-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.ruslan/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.ruslan/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.ruslan/auth.json
              fi
            ''}
          ''}
          ...
          # Ruslan reads $RUSLAN_HOME/.env at startup via load_ruslan_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.ruslan/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'RUSLAN_NIX_ENV_EOF'
    ${envFileContent}
    RUSLAN_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.ruslan/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.ruslan/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ruslan/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.ruslan-agent = {
          description = "Ruslan Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $RUSLAN_HOME/.env by the activation script. load_ruslan_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/ruslan"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the ruslan group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.ruslan-agent = {
          description = "Ruslan Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              RUSLAN_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              RUSLAN_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env RUSLAN_UID="$RUSLAN_UID" \
                --env RUSLAN_GID="$RUSLAN_GID" \
