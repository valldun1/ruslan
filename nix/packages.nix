# nix/packages.nix — Ruslan Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      minimal = pkgs.callPackage ./ruslan-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.ruslanTui;
        web = full.ruslanWeb;
        desktop = full.ruslanDesktop;

        update-npm-lockfile = full.ruslanNpmLib.updateNpmLockfile;
      };
    };
}
