...
let
  ...
  # $RUSLAN_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$RUSLAN_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        ruslan-agent = prev.ruslan-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "ruslan-agent-env" {
    ruslan-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "ruslan-agent-editable-env" {
    ruslan-agent = dependency-groups;
  };
}
