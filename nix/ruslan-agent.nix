...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    cp -r ${bundledSkills} $out/share/ruslan-agent/skills
    cp -r ${bundledPlugins} $out/share/ruslan-agent/plugins
    cp -r ${bundledLocales} $out/share/ruslan-agent/locales
    cp -r ${hermesWeb} $out/share/ruslan-agent/web_dist

    mkdir -p $out/ui-tui
    cp -r ${hermesTui}/lib/ruslan-tui/* $out/ui-tui/
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          ...
          --set RUSLAN_PYTHON ${hermesVenv}/bin/python3 \
          --set RUSLAN_NODE ${lib.getExe nodejs} \
          ${lib.optionalString (rev != null) ''--set RUSLAN_REVISION ${rev} \''}
  ...
  passthru = {
    ...
    devShellHook = ''
      STAMP=".nix-stamps/ruslan-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "ruslan-agent: installing Python dependencies..."
        ...
        source .venv/bin/activate
        export RUSLAN_PYTHON=${hermesVenv}/bin/python3
      fi
    '';
  };
