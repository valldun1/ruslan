...
the actual `setup.sh` that creates the project workspace, configures Ruslan
...
> [NousResearch/kanban-video-pipeline](https://github.com/valldun1/kanban-video-pipeline).
...
2. **Create profiles** — `ruslan profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.ruslan/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `ruslan kanban create` assigned to the director
...
```bash
ruslan profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.ruslan/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.ruslan/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Ruslan
...
Each profile gets a `SOUL.md` at `~/.ruslan/profiles/<name>/SOUL.md` that
...
```bash
ruslan kanban create "Direct production of <video title>" \
    --assignee director \
...
the Ruslan `.env` (`${RUSLAN_HOME:-$HOME/.ruslan}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _ruslan_env="${RUSLAN_HOME:-$HOME/.ruslan}/.env"
    if grep -q "^${var}=" "$_ruslan_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_ruslan_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_ruslan_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY ruslan ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY ruslan OPENROUTER_API_KEY || exit 1
# ...
```
