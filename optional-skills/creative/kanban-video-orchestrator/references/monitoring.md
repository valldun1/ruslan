...
ruslan kanban watch --tenant <project-slug>

# Snapshot of the board
ruslan kanban list --tenant <project-slug>
ruslan kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
ruslan kanban stats --tenant <project-slug>

# Visual dashboard (browser)
ruslan dashboard

# Inspect a specific task (includes comments + events)
ruslan kanban show <task-id>

# Follow a single task's event stream
ruslan kanban tail <task-id>
```

Verify available subcommands with `ruslan kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `ruslan kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `ruslan kanban block <id>` then `ruslan kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `ruslan kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `ruslan kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `ruslan profile list` (profile exists?), `ruslan status` (gateway/dispatcher up?), and verify tenant. |
...
ruslan kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
ruslan kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(ruslan kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
ruslan kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
ruslan kanban block <task-id>
ruslan kanban archive <task-id>

# Diagnose what happened
ruslan kanban show <task-id>      # task body, comments, recent events
ruslan kanban tail <task-id>      # follow the live event stream
ruslan kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `ruslan kanban create` for the director
...
while true; do
    clear
    ruslan kanban list --tenant <slug>
    echo "---"
    ruslan kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `ruslan kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `ruslan profile list` and start
