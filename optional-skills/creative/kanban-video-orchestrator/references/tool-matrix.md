...
Maps each role archetype to the Ruslan skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public ruslan-agent
...
## Ruslan skills relevant to video production

### Visual / rendering skills (`ruslan-agent/skills/creative/`)
...
### Design / pre-production skills (`ruslan-agent/skills/creative/`)
...
### Audio / media skills (`ruslan-agent/skills/creative/` + `skills/media/`)
...
These are **not** Ruslan skills but external CLIs / APIs that profiles invoke.
...
## Built-in Ruslan tools for media review

These are native Ruslan tools — not invoked via terminal but through their own
...
key is present in `${RUSLAN_HOME:-~/.ruslan}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Ruslan profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${RUSLAN_HOME:-~/.ruslan}/.env` → environment variable.
...
When a new Ruslan-public video skill ships:
