...
Usage (via cron with --no-agent):
    ...
    ruslan cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$RUSLAN_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Ruslan-Watcher/1.0"})
