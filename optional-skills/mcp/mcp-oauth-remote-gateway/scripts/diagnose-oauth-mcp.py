...
Usage:
  ...
  <server_name> matches the files in $RUSLAN_HOME/mcp-tokens/<server>.json etc.
...
def _ruslan_home():
    # Prefer Ruslan' own resolver (profile-safe); fall back to env then ~/.ruslan.
    try:
        from ruslan_constants import get_ruslan_home
        return str(get_ruslan_home())
    except Exception:
        return os.environ.get("RUSLAN_HOME") or os.path.expanduser("~/.ruslan")


def _tokens_dir():
    return os.path.join(_ruslan_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "ruslan-diag", "version": "1.0"}},
        },
        headers={
