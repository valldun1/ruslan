#!/usr/bin/env python3
"""OpenClaw -> Ruslan migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Ruslan Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Ruslan.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Ruslan memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Ruslan memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Ruslan-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Ruslan-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Ruslan command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.ruslan/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.ruslan/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Ruslan .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Ruslan .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Ruslan .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Ruslan .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Ruslan .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Ruslan config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Ruslan config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Ruslan.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Ruslan MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Ruslan config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'ruslan cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Ruslan config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Ruslan session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Ruslan custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Ruslan config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Ruslan config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Ruslan config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Ruslan config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Ruslan in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Ruslan`` (prose), but lowercase matches
# like ``openclaw`` → ``ruslan`` (so filesystem paths like ``~/.openclaw``
# become ``~/.ruslan`` — the real Ruslan home — not the broken ``~/.Ruslan``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Ruslan'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Ruslan'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Ruslan'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Ruslan`` but maps ``openclaw`` → ``ruslan`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.ruslan/config.yaml`` (the real Ruslan home) instead of the broken
    ``~/.Ruslan/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Ruslan.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Ruslan Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-ruslan/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Ruslan config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-ruslan/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Ruslan session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Ruslan config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Ruslan
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Ruslan-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Ruslan-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: ruslan config set",
                        )
                    continue
        ...
        for oc_key, ruslan_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and ruslan_key not in secret_additions:
                secret_additions[ruslan_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, ruslan_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and ruslan_key not in secret_additions:
                        secret_additions[ruslan_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        ruslan_config = load_yaml_file(destination)
        current_model = ruslan_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = ruslan_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                ruslan_config["model"] = {"default": model_str}
            dump_yaml_file(destination, ruslan_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Ruslan still uses "edge"
        ...
        ruslan_config = load_yaml_file(destination)
        existing_tts = ruslan_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Ruslan destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Ruslan-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Ruslan-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        ruslan_cfg_path = self.target_root / "config.yaml"
        ruslan_cfg = load_yaml_file(ruslan_cfg_path)
        existing_mcp = ruslan_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Ruslan config")
                continue

            ruslan_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                ruslan_srv["command"] = srv["command"]
                if srv.get("args"):
                    ruslan_srv["args"] = srv["args"]
                if srv.get("env"):
                    ruslan_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    ruslan_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                ruslan_srv["url"] = srv["url"]
                if srv.get("headers"):
                    ruslan_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    ruslan_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                ruslan_srv["enabled"] = False
            if srv.get("timeout"):
                ruslan_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                ruslan_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                ruslan_srv["tools"] = {}
                if tools_cfg.get("include"):
                    ruslan_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    ruslan_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(ruslan_cfg_path)
            ruslan_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(ruslan_cfg_path, ruslan_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'ruslan cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = ruslan_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Ruslan reasoning_effort
        ...
        if compaction:
            compression = ruslan_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'ruslan gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("RUSLAN_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        ruslan_cfg_path = self.target_root / "config.yaml"
        ruslan_cfg = load_yaml_file(ruslan_cfg_path)
        sr = ruslan_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        ruslan_cfg_path = self.target_root / "config.yaml"
        ruslan_cfg = load_yaml_file(ruslan_cfg_path)
        custom_providers = ruslan_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(ruslan_cfg_path)
            ruslan_cfg["custom_providers"] = custom_providers
            dump_yaml_file(ruslan_cfg_path, ruslan_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Ruslan config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            ruslan_cfg_path = self.target_root / "config.yaml"
            ruslan_cfg = load_yaml_file(ruslan_cfg_path)
            discord_ruslan = ruslan_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_ruslan["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_ruslan["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                ruslan_cfg["discord"] = discord_ruslan
                dump_yaml_file(ruslan_cfg_path, ruslan_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        ruslan_cfg_path = self.target_root / "config.yaml"
        ruslan_cfg = load_yaml_file(ruslan_cfg_path)
        browser_ruslan = ruslan_cfg.get("browser") or {}
        changed = False

        # Map fields that have Ruslan equivalents
        if browser.get("cdpUrl"):
            browser_ruslan["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_ruslan["headless"] = browser["headless"]
            changed = True

        if changed:
            ruslan_cfg["browser"] = browser_ruslan
            if self.execute:
                self.maybe_backup(ruslan_cfg_path)
                dump_yaml_file(ruslan_cfg_path, ruslan_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            ruslan_mode = mode_map.get(mode, "manual")
            ruslan_cfg.setdefault("approvals", {})["mode"] = ruslan_mode
            if self.execute:
                self.maybe_backup(ruslan_cfg_path)
                dump_yaml_file(ruslan_cfg_path, ruslan_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{ruslan_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Ruslan Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Ruslan. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Ruslan Config Not Overwritten)",
                "",
                "These items already existed in your Ruslan config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Ruslan agent discovers these",
            "directories, it may read/write to them instead of the Ruslan state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `ruslan claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `ruslan claw cleanup` to fix it.",
            "",
            "## Ruslan-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `ruslan claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `ruslan setup` to configure any remaining settings",
            "- Run `ruslan mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `ruslan cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `ruslan cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `ruslan gateway install` if you need the gateway service",
            "- Review `~/.ruslan/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Ruslan Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("RUSLAN_HOME") or str(Path.home() / ".ruslan"), help="Ruslan home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Ruslan targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Ruslan-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Ruslan Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.ruslan/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.ruslan/config.yaml")
        print("    2. Run: ruslan mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: ruslan cron")
