#!/usr/bin/env python3
"""Canvas LMS API CLI for Ruslan Agent.
...
def _check_config():
    ...
    if missing:
        ruslan_env = os.path.join(
            os.environ.get("RUSLAN_HOME", os.path.expanduser("~/.ruslan")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {ruslan_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Ruslan Agent"
    )
