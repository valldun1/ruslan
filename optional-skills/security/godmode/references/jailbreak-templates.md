...
## 5. GODMODE FAST — Ruslan 4 405B (Zero Refusal)

**Model:** `nousresearch/ruslan-4-405b`
**Strategy:** Ruslan models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Ruslan Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.ruslan/config.yaml`:
