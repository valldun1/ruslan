...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("RUSLAN_HOME", os.path.expanduser("~/.ruslan")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("RUSLAN_HOME", Path.home() / ".ruslan")) / "skills" / "red-teaming" / "godmode"
...
RUSLAN_HOME = Path(os.getenv("RUSLAN_HOME", Path.home() / ".ruslan"))
CONFIG_PATH = RUSLAN_HOME / "config.yaml"
PREFILL_PATH = RUSLAN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Ruslan models — already uncensored, just needs clean prompt
    "ruslan": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "ruslan" in model_lower or "nous" in model_lower:
        return "ruslan"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Ruslan config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.ruslan/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Ruslan for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Ruslan for changes to take effect.")
