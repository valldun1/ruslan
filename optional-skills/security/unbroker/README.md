...
The design is **Ruslan-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
ruslan skills install official/security/unbroker
```

Then start a new Ruslan session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Ruslan session:
...
- **PII handling.** Dossiers live under the Ruslan home directory (`0600`, optionally
