...
All per-subject data lives under PDD_DATA_DIR (default: $RUSLAN_HOME/unbroker),
which is the same trust boundary Ruslan uses for .env and OAuth tokens.
"""
...
def ruslan_home() -> Path:
    return Path(os.environ.get("RUSLAN_HOME") or (Path.home() / ".ruslan"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else ruslan_home() / "unbroker"
