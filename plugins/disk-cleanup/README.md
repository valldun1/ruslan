...
Auto-tracks and cleans up ephemeral files created during Ruslan Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$RUSLAN_HOME` and `/tmp/ruslan-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `RUSLAN_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `RUSLAN_HOME` or `/tmp/ruslan-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$RUSLAN_HOME/disk-cleanup/` is itself excluded
- `$RUSLAN_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
