"""disk_cleanup — ephemeral file cleanup for Ruslan Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under RUSLAN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly RUSLAN_HOME and /tmp/ruslan-*
Never touches: ~/.ruslan/logs/ or any system directory.
"""
...
try:
    ...
    def get_ruslan_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("RUSLAN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".ruslan").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$RUSLAN_HOME/logs/``."""
    return get_ruslan_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$RUSLAN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under RUSLAN_HOME or ``/tmp/ruslan-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    ruslan_home = get_ruslan_home()
    try:
        path.resolve().relative_to(ruslan_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/ruslan-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("ruslan-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "ruslan-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $RUSLAN_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so RUSLAN_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        ruslan_home = get_ruslan_home()
        for parent in ("cron", "cronjobs"):
            base = ruslan_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside RUSLAN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under RUSLAN_HOME, but never recurse into known
    # durable state trees.  Some installs place the Ruslan checkout, venv,
    # and desktop build under RUSLAN_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    ruslan_home = get_ruslan_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in ruslan_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    ruslan_home = get_ruslan_home()
    try:
        rel = path.resolve().relative_to(ruslan_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "ruslan-agent",
        }:
            return None
        ...
        # Path isn't under RUSLAN_HOME (e.g. /tmp/ruslan-*) — fall through.
        pass
