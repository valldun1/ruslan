...
Let the ruslan agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where ruslan runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `ruslan meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `ruslan meet` CLI |
...
| `cli.py` | `ruslan meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$RUSLAN_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `ruslan meet node {run,list,approve,remove,status,ping}` |
...
ruslan meet install                                      # pip + Chromium
ruslan meet setup                                        # preflight
ruslan meet auth                                         # optional
ruslan meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
ruslan meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.ruslan/.env
ruslan meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
ruslan meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
ruslan meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.ruslan/.env
ruslan meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, ruslan will **not** switch your system audio input automatically — the
...
ruslan plugins enable google_meet
ruslan meet node run --display-name my-mac --host 0.0.0.0 --port 18789
