...
session-scoped state file under ``$RUSLAN_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $RUSLAN_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_ruslan_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this ruslan install, leave it first —
    ...
    env = os.environ.copy()
    env["RUSLAN_MEET_URL"] = url
    env["RUSLAN_MEET_OUT_DIR"] = str(out)
    env["RUSLAN_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["RUSLAN_MEET_HEADED"] = "1"
    if auth_state:
        env["RUSLAN_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["RUSLAN_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if RUSLAN_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["RUSLAN_MEET_MODE"] = mode
    if realtime_model:
        env["RUSLAN_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["RUSLAN_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["RUSLAN_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["RUSLAN_MEET_REALTIME_KEY"] = realtime_api_key
