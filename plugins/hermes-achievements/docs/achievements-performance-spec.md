# Ruslan Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: ruslan-achievements plugin
...
### `GET /api/plugins/ruslan-achievements/achievements`
...
### `POST /api/plugins/ruslan-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/ruslan-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/ruslan-achievements/overview`
...
- Persist plugin data under:
  - `~/.ruslan/plugins/ruslan-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.ruslan/plugins/ruslan-achievements/scan_checkpoint.json`
...
- `~/.ruslan/plugins/ruslan-achievements/`
