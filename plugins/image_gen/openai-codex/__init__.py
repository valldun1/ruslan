...
Output is saved as PNG under ``$RUSLAN_HOME/cache/images/``. Source images for
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `ruslan auth codex` (or `ruslan setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`ruslan auth codex` (or `ruslan setup` → Codex) to sign in."
                ),
