/**
 * Ruslan Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.ruslan/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__RUSLAN_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__RUSLAN_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "ruslan-kanban-dot-triage",
    todo: "ruslan-kanban-dot-todo",
    ready: "ruslan-kanban-dot-ready",
    running: "ruslan-kanban-dot-running",
    blocked: "ruslan-kanban-dot-blocked",
    done: "ruslan-kanban-dot-done",
    archived: "ruslan-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-ruslan-task";
  ...
  const DOCS_URL = "https://ruslan-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://ruslan-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "ruslan.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="ruslan-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])ruslan-kanban-md-code\1/i.test(attrs)) {
      return ' class="ruslan-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "ruslan-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "ruslan-kanban-md",
  ...
  // standard `drop` event and our `ruslan-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("ruslan-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("ruslan-kanban-column--drop");
          if (target) target.classList.add("ruslan-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("ruslan-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("ruslan-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("ruslan-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__RUSLAN_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "ruslan-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "ruslan-kanban-attention",
        "ruslan-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "ruslan-kanban-attention-bar" },
        h("span", { className: "ruslan-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "ruslan-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "ruslan-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "ruslan-kanban-attention-row",
                  "ruslan-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "ruslan-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "ruslan-kanban-attention-row-id" }, task.id),
                h("span", { className: "ruslan-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "ruslan-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "ruslan-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "ruslan-kanban-diag-action-btn",
      action.suggested ? "ruslan-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " ruslan-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".ruslan-kanban-drawer-comment-row input, .ruslan-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "ruslan-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("ruslan-kanban-diag", sevClass) },
      h("div", { className: "ruslan-kanban-diag-header" },
        h("span", { className: "ruslan-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "ruslan-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "ruslan-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "ruslan-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "ruslan-kanban-diag-data-row" },
                  h("span", { className: "ruslan-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "ruslan-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "ruslan-kanban-diag-data-row" },
                h("span", { className: "ruslan-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "ruslan-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "ruslan-kanban-diag-reassign-row" },
            h("span", { className: "ruslan-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "ruslan-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "ruslan-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "ruslan-kanban-diag-msg",
              msg.ok ? "ruslan-kanban-diag-msg--ok" : "ruslan-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "ruslan-kanban-section" },
      h("div", { className: "ruslan-kanban-section-head-row" },
        h("span", { className: "ruslan-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "ruslan-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "ruslan-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "ruslan-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "ruslan-kanban-docs-link",
      title: "Open Ruslan Kanban docs in a new tab",
      "aria-label": "Ruslan Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `ruslan kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "ruslan-kanban-msg-ok" : "ruslan-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "ruslan-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "ruslan-kanban-boardswitcher" },
      h("div", { className: "ruslan-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "ruslan-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "ruslan-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "ruslan-kanban-dialog-title" },
        ...
        h("div", { className: "ruslan-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Ruslan profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "ruslan-kanban-bulk" },
      h("span", { className: "ruslan-kanban-bulk-count" },
      ...
      h("div", { className: "ruslan-kanban-bulk-priority",
      ...
      h("div", { className: "ruslan-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Ruslan profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "ruslan-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("ruslan-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("ruslan-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "ruslan-kanban-trash",
        dragOver ? "ruslan-kanban-trash--drop" : "",
        props.draggingTaskId ? "ruslan-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "ruslan-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "ruslan-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".ruslan-kanban-card")) return true;
      if (target.closest && target.closest(".ruslan-kanban-column-add")) return true;
      if (target.closest && target.closest(".ruslan-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("ruslan-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("ruslan-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".ruslan-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "ruslan-kanban-columns",
        isScrollable ? "ruslan-kanban-columns--scrollable" : "",
        isPanning ? "ruslan-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("ruslan-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("ruslan-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "ruslan-kanban-column",
        dragOver ? "ruslan-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "ruslan-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "ruslan-kanban-col-check",
        ...
        h("span", { className: cn("ruslan-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "ruslan-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "ruslan-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "ruslan-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "ruslan-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "ruslan-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "ruslan-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "ruslan-kanban-lane" },
                  h("div", { className: "ruslan-kanban-lane-head" },
                    h("span", { className: "ruslan-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "ruslan-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "ruslan-kanban-card--stale-red";
    if (age >= tier.amber) return "ruslan-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".ruslan-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "ruslan-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "ruslan-kanban-card",
        props.selected ? "ruslan-kanban-card--selected" : "",
        props.failed ? "ruslan-kanban-card--failed" : "",
        props.draggingSource ? "ruslan-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "ruslan-kanban-card-content" },
          h("div", { className: "ruslan-kanban-card-row" },
            h("label", {
              className: "ruslan-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "ruslan-kanban-card-check",
            ...
            h("span", { className: "ruslan-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or ruslan kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "ruslan-kanban-warning-badge",
                    "ruslan-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "ruslan-kanban-needs-assignee",
          ...
          h("div", { className: "ruslan-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "ruslan-kanban-card-row ruslan-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "ruslan-kanban-assignee",
                            title: `Assigned to Ruslan profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "ruslan-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "ruslan-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "ruslan-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "ruslan-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Ruslan profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Ruslan profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "ruslan-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "ruslan-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "ruslan-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "ruslan-kanban-drawer-close",
        ...
        data ? h("div", { className: "ruslan-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "ruslan-kanban-section" },
      h("div", { className: "ruslan-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "ruslan-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "ruslan-kanban-drawer-body" },
      h("div", { className: "ruslan-kanban-drawer-title" },
        h("span", { className: cn("ruslan-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "ruslan-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "ruslan-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "ruslan-kanban-section" },
            h("div", { className: "ruslan-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "ruslan-kanban-section" },
            h("div", { className: "ruslan-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "ruslan-kanban-done-no-result ruslan-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "ruslan-kanban-section" },
            h("div", { className: "ruslan-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "ruslan-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "ruslan-kanban-section" },
        h("div", { className: "ruslan-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "ruslan-kanban-comment" },
            h("div", { className: "ruslan-kanban-comment-head" },
              h("span", { className: "ruslan-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "ruslan-kanban-diag-action-btn",
      ...
      h("div", { className: "ruslan-kanban-section" },
        h("div", { className: "ruslan-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "ruslan-kanban-comment" },
                h("div", { className: "ruslan-kanban-comment-head" },
                  h("span", { className: "ruslan-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "ruslan-kanban-comment-ago" },
      ...
      h("div", { className: "ruslan-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "ruslan-kanban-event",
              isDiag ? "ruslan-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "ruslan-kanban-event-header" },
                  h("span", { className: "ruslan-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "ruslan-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "ruslan-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "ruslan-kanban-event-header-plain" },
                  h("span", { className: "ruslan-kanban-event-kind" }, e.kind),
                  h("span", { className: "ruslan-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "ruslan-kanban-event-phantom-row" },
                  h("span", { className: "ruslan-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "ruslan-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "ruslan-kanban-event-payload" },
    ...
    return h("div", { className: "ruslan-kanban-section" },
      h("div", { className: "ruslan-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "ruslan-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `ruslan-kanban-run--${r.outcome || r.status || "ended"}`
          : "ruslan-kanban-run--active";
        return h("div", { key: r.id, className: cn("ruslan-kanban-run", outcomeClass) },
          h("div", { className: "ruslan-kanban-run-head" },
            h("span", { className: "ruslan-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "ruslan-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "ruslan-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "ruslan-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "ruslan-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "ruslan-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "ruslan-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "ruslan-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "ruslan-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "ruslan-kanban-pre ruslan-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "ruslan-kanban-meta-row" },
      h("span", { className: "ruslan-kanban-meta-label" }, props.label),
      h("span", { className: "ruslan-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "ruslan-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "ruslan-kanban-meta-row" },
        h("span", { className: "ruslan-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "ruslan-kanban-meta-value ruslan-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "ruslan-kanban-meta-row" },
        h("span", { className: "ruslan-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "ruslan-kanban-meta-value ruslan-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "ruslan-kanban-section" },
      h("div", { className: "ruslan-kanban-section-head-row" },
        h("span", { className: "ruslan-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "ruslan-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "ruslan-kanban-section" },
      h("div", { className: "ruslan-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "ruslan-kanban-deps-row" },
        h("span", { className: "ruslan-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "ruslan-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "ruslan-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "ruslan-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "ruslan-kanban-dep-chip-x",
      ...
      h("div", { className: "ruslan-kanban-deps-row" },
        h("span", { className: "ruslan-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "ruslan-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "ruslan-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "ruslan-kanban-msg-ok"
          : "ruslan-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "ruslan-kanban-section" },
      ...
      h("div", { className: "ruslan-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "ruslan-kanban-home-sub ruslan-kanban-home-sub--on"
              : "ruslan-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__RUSLAN_PLUGINS__ && typeof window.__RUSLAN_PLUGINS__.register === "function") {
    window.__RUSLAN_PLUGINS__.register("kanban", KanbanPage);
  }
})();
