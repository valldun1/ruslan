...
``ruslan://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Ruslan OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "ruslan-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``ruslan-desktop`` / ``ruslan-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Ruslan.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local RUSLAN_HOME override can't reach.
