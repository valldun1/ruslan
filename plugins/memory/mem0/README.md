...
```bash
ruslan memory setup    # select "mem0"
```
...
Behavioral settings live in `$RUSLAN_HOME/mem0.json` (set them via `ruslan memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.ruslan/.env`.
...
| `user_id` | `ruslan-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Ruslan session and call `mem0_search` — it connects to your server.
...
```bash
ruslan memory setup mem0 --mode oss \
...
```bash
ruslan memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$RUSLAN_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.ruslan/mem0_qdrant"}}
  }
}
```
...
```bash
ruslan memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$RUSLAN_HOME/mem0.json`).
