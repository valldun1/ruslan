...
Configuration
-------------
Secret (lives in $RUSLAN_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $RUSLAN_HOME/mem0.json, set via `ruslan memory
setup`):
  ...
  agent_id           — Agent identifier (default: ruslan)
...
def _load_config() -> dict:
    """Load config from env vars, with $RUSLAN_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "ruslan"),
        "oss": {},
    }
    ...
    config_path = get_ruslan_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, ruslan_home):
        """Write config to $RUSLAN_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $RUSLAN_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "ruslan")
