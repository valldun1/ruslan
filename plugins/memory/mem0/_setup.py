...
def _save_mem0_json(ruslan_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(ruslan_home) / "mem0.json"
    existing = {}
...
def _setup_platform(ruslan_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "ruslan-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "ruslan"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.ruslan/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.ruslan/.env (or unset it) or Ruslan will keep "
    ...
    provider.save_config(provider_config, ruslan_home)

    if env_writes:
        _write_env(Path(ruslan_home) / ".env", env_writes)
...
def _setup_selfhosted(ruslan_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "ruslan-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "ruslan")
...
def _setup_oss(ruslan_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(ruslan_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "ruslan-user")
    ...
    if env_writes:
        _write_env(Path(ruslan_home) / ".env", env_writes)
    _save_mem0_json(ruslan_home, {"mode": "oss", "user_id": user_id, "agent_id": "ruslan", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, ruslan_home: str) -> str:
...
_PGVECTOR_CONTAINER = "ruslan-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "ruslan"
...
def _setup_oss_interactive(ruslan_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], ruslan_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], ruslan_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'ruslan-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "ruslan-user")

    agent_id = input("  Agent ID [ruslan]: ").strip()
    agent_id = agent_id or "ruslan"
    ...
    if env_writes:
        _write_env(Path(ruslan_home) / ".env", env_writes)
    _save_mem0_json(ruslan_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(ruslan_home: str, config: dict) -> None:
    """Entry point called by ruslan memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(ruslan_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(ruslan_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(ruslan_home, config, flags)
        return
