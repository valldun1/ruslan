...
- Supermemory API key from [app.supermemory.ai/integrations?connect=ruslan](http://app.supermemory.ai/integrations?connect=ruslan)
...
Config file: `$RUSLAN_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `ruslan` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `ruslan-{identity}` → `ruslan-coder`). |
...
All Supermemory API calls send `x-sm-source: ruslan`, and document writes stamp
`metadata.sm_source: ruslan`. This is a **functional routing key, not telemetry**:
it groups Ruslan-written memories into a dedicated "Ruslan" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Ruslan profile:

```json
{
  "container_tag": "ruslan-{identity}"
}
```

For a profile named `coder`, this resolves to `ruslan-coder`. The default profile resolves to `ruslan-default`. Without `{identity}`, all profiles share the same container.
