"""
Email platform adapter for the Ruslan gateway.

Allows users to interact with Ruslan by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "ruslan-agent" "version" "{_ruslan_version}" '
            '"vendor" "Valldun" '
            '"support-email" "ruslan@ruslan.team")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `ruslan config show` already read. Without the
    ...
    async def connect(self) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `ruslan gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Ruslan Agent")
        ...
        msg_id = f"<ruslan-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# ruslan_cli/gateway.py, and the _send_email dispatch in
