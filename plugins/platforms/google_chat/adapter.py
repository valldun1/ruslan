...
# ``model_tools`` import time, so ``ruslan status``, ``ruslan chat``, etc.
...
# ``ruslan_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Ruslan is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a ruslan MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "ruslan-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${RUSLAN_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _ruslan_home = _get_ruslan_home()
        except (ModuleNotFoundError, ImportError):
            _ruslan_home = _Path.home() / ".ruslan"
        self._thread_count_store = _ThreadCountStore(
            _ruslan_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Ruslan is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("RUSLAN_HOME", str(_Path.home() / ".ruslan"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${RUSLAN_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a ruslan MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Ruslan is thinking…' marker message.
        ...
        "Ruslan is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Ruslan is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Ruslan is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Ruslan is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Ruslan is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``ruslan setup``.

    The setup wizard at ``ruslan_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.ruslan/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. ruslan-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.ruslan/.env")
    print_info("Restart the gateway: ruslan gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``ruslan cron`` running as a
    separate process from ``ruslan gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Ruslan plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'ruslan-agent[google_chat]'",
        ...
        # the "Ruslan is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Ruslan is thinking…' marker message "
