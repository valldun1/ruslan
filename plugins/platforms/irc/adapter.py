"""
IRC Platform Adapter for Ruslan Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Ruslan agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "ruslan-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Ruslan Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Ruslan Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: ruslan_, ruslan_1, ruslan_2...
...
def check_requirements() -> bool:
    ...
    # ruslan setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `ruslan gateway setup` flow for the IRC platform.

    Lazy-imports ``ruslan_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Ruslan to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #ruslan — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.ruslan/.env")
    print_info("Restart the gateway for changes to take effect: ruslan gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "ruslan-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "ruslan-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Ruslan Agent (cron)")
