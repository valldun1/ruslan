"""
LINE Messaging API platform adapter for Ruslan Agent.
...
to Ruslan Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``RUSLAN_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            ruslan_home = Path.home().joinpath(".ruslan").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``ruslan status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``ruslan status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``ruslan setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.ruslan/.env`` via ``ruslan_cli.config``.
    """
    print()
    ...
    try:
        from ruslan_cli.config import get_env_var, set_env_var
    except ImportError:
        print("ruslan_cli.config not available; set LINE_* vars manually in ~/.ruslan/.env")
        return
