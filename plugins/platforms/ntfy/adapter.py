"""ntfy platform adapter (Ruslan plugin).
...
``plugins/platforms/ntfy/``. The Ruslan plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "ruslan-in"              # subscribe topic (incoming)
          publish_topic: "ruslan-out"     # optional — defaults to topic
...
_ECHO_TAG = "ruslan-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``ruslan cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Ruslan dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `ruslan gateway status` without
