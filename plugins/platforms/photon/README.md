...
This plugin connects Ruslan Agent to iMessage (and other Spectrum
...
TypeScript-only, Ruslan runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Ruslan-Sidecar-Token`.
...
There is no separate `login` command; like every other Ruslan channel,
...
Run `ruslan photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.ruslan/.env` (the same place every other
...
Management metadata lives in `~/.ruslan/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `ruslan photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `ruslan photon status`, a DM and a group roundtrip,
