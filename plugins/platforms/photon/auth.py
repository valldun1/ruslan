...
Credential storage mirrors every other Ruslan channel:

    * runtime SDK creds  -> ``~/.ruslan/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.ruslan/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a ruslan dependency
...
# registers Ruslan as its own client_id.
...
# Default name of the project Ruslan provisions for the operator.
DEFAULT_PROJECT_NAME = "Ruslan Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of ruslan-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.ruslan/auth.json`` honouring the active Ruslan profile."""
    try:
        from ruslan_constants import get_ruslan_home
        return Path(get_ruslan_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.ruslan")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.ruslan/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.ruslan/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.ruslan/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: ruslan_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `ruslan photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `ruslan photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `ruslan photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `ruslan photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `ruslan photon setup`)"
