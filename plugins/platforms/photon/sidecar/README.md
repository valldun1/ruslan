...
Small Node helper that bridges Ruslan Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Ruslan is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Ruslan-Sidecar-Token`)
...
The Ruslan plugin's `ruslan photon setup` command runs `npm install`
here automatically.
