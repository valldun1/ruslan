// Ruslan Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Ruslan): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Ruslan -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Ruslan-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `ruslan photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Ruslan can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Ruslan'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Ruslan allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Ruslan, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-ruslan-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Ruslan supplied them so a known-good
