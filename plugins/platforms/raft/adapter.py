...
and injects content-free wake hints into Ruslan' normal gateway session pipeline.
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # Ruslan only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Ruslan turn."""
        if not self._message_handler:
            return
...
def interactive_setup() -> None:
    """Interactive ``ruslan gateway setup`` flow for the Raft platform.

    Lazy-imports CLI helpers so the plugin stays importable in gateway runtime
    and test contexts. The flow persists ``RAFT_PROFILE`` to the Ruslan env
    ...
    print_info("Connect Ruslan to Raft as an external agent.")
