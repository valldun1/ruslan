"""SimpleX Chat platform adapter (Ruslan plugin).
...
This adapter ships as a Ruslan platform plugin under
``plugins/platforms/simplex/``. The Ruslan plugin loader scans the
...
Optional environment variables:
    ...
    RUSLAN_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``ruslan setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Ruslan — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("RUSLAN_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``ruslan setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.ruslan/.env`` via
    ``ruslan_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "ruslan_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.ruslan/.env"
        )
        return
