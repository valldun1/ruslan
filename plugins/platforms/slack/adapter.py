...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `ruslan slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_ruslan_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Ruslan does not currently need to route them into the agent loop.
            ...
            # /model work directly without /ruslan prefix). A single regex
            ...
            # manifest (see `ruslan slack manifest`). In Socket Mode, Slack
            ...
            from ruslan_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/ruslan$")

            @self._app.command(_slash_pattern)
            async def handle_ruslan_command(ack, command):
            ...
            for _action_id in (
                "ruslan_approve_once",
                "ruslan_approve_session",
                "ruslan_approve_always",
                "ruslan_deny",
            ):
            ...
            for _action_id in (
                "ruslan_confirm_once",
                "ruslan_confirm_always",
                "ruslan_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("ruslan_feedback")(self._handle_feedback_action)
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Ruslan handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Ruslan session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Ruslan owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        if original_text.startswith("!"):
            try:
                from ruslan_cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@ruslan`` still resolve.
        ...
        try:
            ...
            if not smart_denied:
                actions.append({
                    ...
                    "action_id": "ruslan_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "ruslan_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "ruslan_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "ruslan_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "ruslan_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "ruslan_confirm_once": "once",
            "ruslan_confirm_always": "always",
            "ruslan_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "ruslan_approve_once": "once",
            "ruslan_approve_session": "session",
            "ruslan_approve_always": "always",
            "ruslan_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/ruslan <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/ruslan
        ...
        if slash_name in {"ruslan", ""}:
            # Legacy /ruslan <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from ruslan_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/ruslan   ``).
        ...
        # "Running /cmd…" ack shown by handle_ruslan_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/ruslan <question>" must produce public replies so
...
# ``ruslan_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``ruslan_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under RUSLAN_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from ruslan_cli.slack_cli import _build_full_manifest
            from ruslan_constants import get_ruslan_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Ruslan",
                bot_description="Your Ruslan agent on Slack",
            )
            target = Path(get_ruslan_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `ruslan slack manifest --write` anytime to refresh after "
                "Ruslan adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `ruslan update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://ruslan-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``ruslan_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'ruslan-agent[slack]'",
        # Interactive setup wizard — replaces ruslan_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in ruslan_cli/gateway.py.
