...
``{title, url, description, position}`` rows every other Ruslan web
provider produces.
...
prefers Ruslan-managed xAI Grok OAuth (via ``ruslan auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.ruslan/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``ruslan tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `ruslan auth` to sign in with "
