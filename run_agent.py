...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``ruslan -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("RUSLAN_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("RUSLAN_SESSION_SOURCE", "")
...
from ruslan_cli.timeouts import (
...
_loaded_env_paths = load_ruslan_dotenv(ruslan_home=_ruslan_home, project_env=_project_env)
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[ruslan-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Ruslan' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from ruslan_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``ruslan chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``RUSLAN_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``RUSLAN_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("RUSLAN_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``RUSLAN_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("RUSLAN_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        ruslan-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See ruslan-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Ruslan against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("RUSLAN_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("ruslan_cli.plugins.has_hook", ...)`` still
        ...
        try:
            from ruslan_cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Respects ``RUSLAN_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.ruslan/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects RUSLAN_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.ruslan/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``RUSLAN_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("RUSLAN_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (RUSLAN_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (RUSLAN_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset RUSLAN_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("RUSLAN_DEV_CREDITS"))
        ...
        if _dev:
            # RUSLAN_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `ruslan logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _ensure_primary_openai_client(self, *, reason: str) -> Any:
        ...
        logger.warning(
            "Обнаружен закрытый общий клиент OpenAI; выполняется повторное создание перед использованием (%s) %s",
            reason,
            self._client_log_context(),
        )
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``ruslan auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from ruslan_cli.copilot_auth import resolve_copilot_token
        ...
        logger.info("Учётные данные Copilot обновлены из %s", token_source)
        return True
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from ruslan_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from ruslan_cli.models import lmstudio_model_reasoning_options
...
def main(
    ...
    if list_tools:
        ...
        print("📋 Доступные инструменты и наборы инструментов:")
        print("-" * 50)
        ...
        print("\n💡 Usage Examples:")
        print("# Использование предопределённых наборов")
        print("python run_agent.py --enabled_toolsets=research --query='искать новости Python'")
        print("python run_agent.py --enabled_toolsets=development --query='отладить этот код'")
        print("python run_agent.py --enabled_toolsets=safe --query='анализировать без терминала'")
        print("  ")
        print("# Комбинирование нескольких наборов")
        print("python run_agent.py --enabled_toolsets=web,vision --query='анализировать веб-сайт'")
        print("  ")
        print("# Отключение наборов инструментов")
        print("python run_agent.py --disabled_toolsets=terminal --query='без выполнения команд'")
        print("  ")
        print("# Запуск с сохранением траекторий")
        print("python run_agent.py --save_trajectories --query='ваш вопрос здесь'")
        return
