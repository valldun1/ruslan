...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Ruslan-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.ruslan/sessions/``.  Without
...
class AIAgent:
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        # untrusted input (X-Ruslan-Session-Id header) and must not escape
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``RUSLAN_CA_BUNDLE`` settings.  It is passed on the client AND on
