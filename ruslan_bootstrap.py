"""Windows UTF-8 bootstrap for Ruslan entry points.
...
should be imported at the very top of every Ruslan entry point
(``ruslan``, ``ruslan-agent``, ``ruslan-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Ruslan modules.

    Ruslan ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Ruslan source root
    ...
    """
    root = src_root or os.environ.get("RUSLAN_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``RUSLAN_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("RUSLAN_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import ruslan_bootstrap``
# (or ``from ruslan_bootstrap import apply_windows_utf8_bootstrap``) at
