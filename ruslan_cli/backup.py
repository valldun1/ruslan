"""
Backup and import commands for ruslan CLI.

`ruslan backup` creates a zip archive of the entire ~/.ruslan/ directory
(excluding the ruslan-agent repo and transient files).

`ruslan import` restores from a backup zip, overlaying onto the current
RUSLAN_HOME root.
"""
...
from ruslan_constants import get_default_ruslan_root, get_ruslan_home, display_ruslan_home
...
# ``ruslan-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/ruslan-agent/``
...
# RUSLAN_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "ruslan-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under RUSLAN_HOME) —
...
# File names that ``ruslan import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE RUSLAN_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside RUSLAN_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to ruslan root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``ruslan-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/ruslan-agent/`` — must be preserved.
        if part == "ruslan-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside RUSLAN_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Ruslan home directory."""
    ruslan_root = get_default_ruslan_root()

    if not ruslan_root.is_dir():
        print(f"Error: Ruslan home directory not found at {ruslan_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"ruslan-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"ruslan-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_ruslan_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(ruslan_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(ruslan_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``ruslan-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "ruslan-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(ruslan_root)
    ...
    # outside RUSLAN_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_ruslan_home()}."
        )
    ...
    print(f"\nRestore with: ruslan import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Ruslan backup.
    ...
    # Look for telltale files that a ruslan home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Ruslan backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.ruslan/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a ruslan dir name
        if prefix in {".ruslan", "ruslan"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_ruslan_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (ruslan_root / "config.yaml").exists()
        has_env = (ruslan_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Ruslan configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under RUSLAN_HOME.
            ...
            target = ruslan_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(ruslan_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_ruslan_home()}."
            )
        ...
        profiles_dir = ruslan_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # ruslan_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: ruslan profile list  (after installing ruslan)")

        # Guidance
        print()
        if not (ruslan_root / "ruslan-agent").is_dir():
            print("Note: The ruslan-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: ruslan update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  ruslan -p {pname} gateway install")

        print("Done. Your Ruslan configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and ruslan backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to RUSLAN_HOME).
...
# — `ruslan update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $RUSLAN_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside RUSLAN_HOME and the entry is silently
...
def _quick_snapshot_root(ruslan_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(ruslan_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside RUSLAN_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``ruslan update``.
    ...
    Args:
        ...
        ruslan_home: Override for the Ruslan home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(ruslan_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for ruslan backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_ruslan_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, ruslan_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``ruslan_root`` to ``out_path``.
...
def _pre_update_backup_dir(ruslan_home: Optional[Path] = None) -> Path:
    home = ruslan_home or get_ruslan_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of RUSLAN_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<RUSLAN_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``ruslan update``) should continue even if the backup fails.
    """
    ruslan_root = ruslan_home or get_default_ruslan_root()
    if not ruslan_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(ruslan_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, ruslan_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `ruslan claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of RUSLAN_HOME under ``backups/`` before a
    ``ruslan claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``ruslan import <archive>``.  Writes to
    ``<RUSLAN_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `ruslan import` and the
