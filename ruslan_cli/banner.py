...
from ruslan_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def _fetch_pypi_latest(package: str = "ruslan-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Ruslan update is available.

    Two paths: if ``RUSLAN_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = ruslan_home / ".update_check"
    embedded_rev = os.environ.get("RUSLAN_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # RUSLAN_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `ruslan update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/ruslan/update/check`
    ...
    if embedded_rev:
        ...
        # $RUSLAN_HOME/ruslan-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = ruslan_home / "ruslan-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Ruslan git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$RUSLAN_HOME/ruslan-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``ruslan_cli/build_info.py``) and return it as a frozen
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Ruslan checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/ruslan-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Ruslan Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        from ruslan_cli.codex_runtime_switch import get_current_runtime
