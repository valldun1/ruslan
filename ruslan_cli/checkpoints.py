"""`ruslan checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.ruslan/checkpoints/``.  Actions:

    ruslan checkpoints               # same as `status`
    ruslan checkpoints status        # total size, project count, breakdown
    ruslan checkpoints list          # per-project checkpoint counts + workdir
    ruslan checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    ruslan checkpoints clear [-f]    # nuke the entire base (asks first)
    ruslan checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    ruslan checkpoints
    ruslan checkpoints prune --retention-days 3 --max-size-mb 200
    ruslan checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: ruslan checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``ruslan checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `ruslan checkpoints` → status
