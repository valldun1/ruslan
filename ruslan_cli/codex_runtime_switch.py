...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Ruslan'
...
The actual runtime resolution happens in ruslan_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "ruslan"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, ruslan-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Ruslan' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Ruslan for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from ruslan_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the ruslan-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "ruslan-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Ruslan tool callback are always-on production
            ...
            if "ruslan-tools" in mig_report.migrated:
                msg_lines.append(
                    "Ruslan tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Ruslan runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Ruslan tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Ruslan runtime.")
