"""Shell completion script generation for ruslan CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_ruslan_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Ruslan Agent bash completion
# Add to ~/.bashrc:
#   eval "$(ruslan completion bash)"

_ruslan_profiles() {{
    local profiles_dir="$HOME/.ruslan/profiles"
...
_ruslan_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_ruslan_profiles)" -- "$cur"))
        return
    fi
...
complete -F _ruslan_completion ruslan
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef ruslan
# Ruslan Agent zsh completion
# Add to ~/.zshrc:
...
_ruslan_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.ruslan/profiles" ]]; then
        profiles+=($HOME/.ruslan/profiles/*(N/:t))
    fi
...
_ruslan() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_ruslan_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'ruslan command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Ruslan Agent fish completion",
        "# Add to your config:",
        "#   ruslan completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __ruslan_profiles",
        "    echo default",
        "    if test -d $HOME/.ruslan/profiles",
        "        for d in $HOME/.ruslan/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c ruslan -f -s p -l profile"
        " -d 'Profile name' -xa '(__ruslan_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__ruslan_profiles)' -d 'Profile name'"
                )
