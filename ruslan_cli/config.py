...
from ruslan_cli.default_soul import DEFAULT_SOUL_MD
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``valldun1/ruslan`` image bakes a ``docker``
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade ruslan-agent"
    if method == "docker":
        return "docker pull valldun1/ruslan:latest"
    if method == "pip":
...
Ruslan Agent runs as a published image (valldun1/ruslan), not a
...
Verify the new version after restart:
  docker run --rm valldun1/ruslan:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/valldun1/ruslan/tags
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into RUSLAN_HOME if the user doesn't have one yet."""
...
DEFAULT_CONFIG = {
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://ruslan.team/docs/api/model-catalog.json",
    ...
    "updates": {
        ...
        # can be restored with ``ruslan import <path>``.  Defaults to true
        # after the #48200 incident: a ``ruslan update --yes`` run that
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if unknown:
        logger.warning(
            "providers.%s: неизвестные ключи конфигурации проигнорированы: %s",
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Ruslan versions) write ``models`` as
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if missing_config:
            ...
            print("Выполните 'ruslan config migrate', чтобы добавить их")
