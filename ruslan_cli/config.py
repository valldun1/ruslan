"""
Configuration management for Ruslan Agent.

Config files are stored in ~/.ruslan/ for easy access:
- ~/.ruslan/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.ruslan/.env         - API keys and secrets

This module provides:
- ruslan config          - Show current configuration
- ruslan config edit     - Open config in editor
- ruslan config get      - Print a resolved configuration value
- ruslan config set      - Set a specific value
- ruslan config unset    - Remove a user configuration value
- ruslan config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``ruslan config set`` (which rewrites
    ...
    ``config.yaml`` in place: ruslan never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.ruslan/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``ruslan logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  ruslan config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Ruslan makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Ruslan itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``ruslan update``, the TUI build.
...
#   on every plugin install / ``ruslan update``.
...
# * ``RUSLAN_HOME`` / ``RUSLAN_PROFILE`` / ``RUSLAN_CONFIG`` /
#   ``RUSLAN_ENV`` — Ruslan runtime location flags. Writing these into
...
#
# IMPORTANT: ``RUSLAN_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (RUSLAN_LANGFUSE_PUBLIC_KEY,
# RUSLAN_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Ruslan runtime location — never via dashboard env writer.
    # NOT a RUSLAN_* blanket: integration credentials (RUSLAN_GEMINI_*,
    # RUSLAN_LANGFUSE_*, RUSLAN_SPOTIFY_*, ...) ARE allowed.
    "RUSLAN_HOME", "RUSLAN_PROFILE", "RUSLAN_CONFIG", "RUSLAN_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Ruslan runtime location "
            "(RUSLAN_HOME, RUSLAN_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "RUSLAN_TOOL_PROGRESS", "RUSLAN_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `ruslan plugins enable
    # observability/langfuse` or `ruslan tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "RUSLAN_LANGFUSE_ENV",
    "RUSLAN_LANGFUSE_RELEASE",
    "RUSLAN_LANGFUSE_SAMPLE_RATE",
    "RUSLAN_LANGFUSE_MAX_CHARS",
    "RUSLAN_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from ruslan_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("RUSLAN_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_ruslan_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Ruslan is running in package-manager-managed mode.

    Two signals: the RUSLAN_MANAGED env var (set by the systemd service),
    or a .managed marker file in RUSLAN_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade ruslan-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``ruslan_cli/`` — i.e. the git checkout for source
    installs, ``/opt/ruslan`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$RUSLAN_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Ruslan was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$RUSLAN_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. RUSLAN_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.ruslan``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$RUSLAN_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.ruslan:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$RUSLAN_HOME``, a
    ...
    ``docker`` and ``ruslan update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/ruslan-agent`` image bakes a ``docker``
        stamp into ``/opt/ruslan`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $RUSLAN_HOME.
    try:
    ...
    #    host install whose shared $RUSLAN_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``ruslan update``.
    try:
        method = (
            (get_ruslan_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``ruslan_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$RUSLAN_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/ruslan`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Ruslan lives in a ``uv tool`` layout.

    ``uv tool install ruslan-agent`` places the install at
    ``.../uv/tools/ruslan-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``ruslan-agent`` happens to be uv-tool-installed on the machine while
    the *active* Ruslan is a regular pip/venv install, causing
    ``ruslan update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/ruslan-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade ruslan-agent"
    if method == "docker":
        return "docker pull nousresearch/ruslan-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade ruslan-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade ruslan-agent"
        return "pip install --upgrade ruslan-agent"
    return "ruslan update"
...
# banner, the TUI/desktop session info panel, and ``ruslan update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://ruslan-agent.nousresearch.com/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``ruslan update`` / ``ruslan update --check`` so the wording — and the
...
# Long-form text for ``ruslan update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# ruslan_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Ruslan, it doesn't update
...
✗ ``ruslan update`` doesn't apply inside the Docker container.

Ruslan Agent runs as a published image (nousresearch/ruslan-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/ruslan-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate ruslan-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/ruslan-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/ruslan-agent/tags
  • Your config and session history live under ``$RUSLAN_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``ruslan update`` inside Docker.
...
def format_managed_message(action: str = "modify this Ruslan installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("RUSLAN_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Ruslan installation is managed by NixOS "
            f"(RUSLAN_MANAGED={env_hint}).\n"
            "Edit services.ruslan-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Ruslan installation is managed by Homebrew "
            f"(RUSLAN_MANAGED={env_hint}).\n"
            "Use:\n"
    ...
    return (
        f"Cannot {action}: this Ruslan installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Ruslan."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from RUSLAN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, ruslan_bin
    or None if container mode is not active, we're already inside the
    container, or RUSLAN_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("RUSLAN_DEV") == "1":
        return None
    ...
    container_mode_file = get_ruslan_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "ruslan-agent")
    exec_user = info.get("exec_user", "ruslan")
    ruslan_bin = info.get("ruslan_bin", "/data/current-package/bin/ruslan")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "ruslan_bin": ruslan_bin,
    }
...
# Re-export from ruslan_constants — canonical definition lives there.
from ruslan_constants import get_ruslan_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_ruslan_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_ruslan_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_ruslan_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the RUSLAN_UID / RUSLAN_GID env vars set by Docker deployments.

    Docker containers running Ruslan commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level RUSLAN_HOME once, but
    subdirectories created at runtime by ``ensure_ruslan_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("RUSLAN_UID", "").strip()
    gid_str = os.environ.get("RUSLAN_GID", "").strip()
    try:
...
def _chown_to_ruslan_uid(path) -> None:
    """Chown ``path`` to ``RUSLAN_UID:RUSLAN_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_ruslan_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_ruslan_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the ruslan group can
    share state with the gateway service.

    The mode can be overridden via the RUSLAN_HOME_MODE environment variable
    (e.g. RUSLAN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse RUSLAN_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``RUSLAN_UID``/``RUSLAN_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("RUSLAN_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_ruslan_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Ruslan runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("RUSLAN_CONTAINER") or os.environ.get("RUSLAN_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set RUSLAN_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into RUSLAN_HOME, upgrading legacy empty templates.
...
def ensure_ruslan_home():
    """Ensure ~/.ruslan directory structure exists with secure permissions.
    ...
    # Named profiles must be created explicitly (e.g. ``ruslan profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old RUSLAN_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_ruslan_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_ruslan_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"RUSLAN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Ruslan-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Ruslan
        ...
        # identity slot (SOUL.md). Empty by default. The RUSLAN_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Ruslan adds a coding operating brief
    ...
    "terminal": {
        ...
        #             RUSLAN_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force RUSLAN_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Ruslan auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Ruslan sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Ruslan
        ...
        # bundled Ruslan image, which drops to the `ruslan` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Ruslan sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Ruslan should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``ruslan chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.ruslan/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: ruslan sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .ruslan.md, or .cursorrules before Ruslan applies
    ...
    # single read_file page exceeds these limits, Ruslan truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Ruslan' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); ruslan =
                                      # Ruslan' compression threshold triggers
    ...
    # See tools/kanban_tools.py and ruslan_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``ruslan dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``ruslan kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``ruslan kanban decompose`` and the kanban
        ...
        # ``ruslan profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `ruslan model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `ruslan` (and `ruslan chat`) launches by default:
        ...
        # REPL and `--tui` (or RUSLAN_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `ruslan --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `ruslan -c` muscle memory.  Default off so existing
        # users aren't surprised.  RUSLAN_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `ruslan --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to RUSLAN_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/ruslan`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `ruslan pets`. Disabled until
        ...
        "pet": {
            ...
            # get_ruslan_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``RUSLAN_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``RUSLAN_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``RUSLAN_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``RUSLAN_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``RUSLAN_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/ruslan``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Ruslan appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.ruslan/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.ruslan/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.ruslan/cache/delegation/
    ...
    # response. If not, Ruslan feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Ruslan auto-pauses the goal and
    ...
    "moa": {
        ...
        # <ruslan_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.ruslan/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${RUSLAN_SKILL_DIR} and ${RUSLAN_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `ruslan curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `ruslan curator run --consolidate` overrides
        ...
        # normally restored on every `ruslan update`, so pruning them only
        ...
        # skipped), snapshot ~/.ruslan/skills/ into
        # ~/.ruslan/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `ruslan curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for ruslan-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (RUSLAN_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Ruslan' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.ruslan/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or RUSLAN_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `ruslan doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``ruslan_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Ruslan to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $RUSLAN_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via RUSLAN_CRON_MAX_PARALLEL env var.
        ...
        # RUSLAN_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `ruslan -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `ruslan` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `ruslan kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     ruslan-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Ruslan tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.ruslan/logs/.
    ...
    # update model picker lists without shipping a ruslan-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://ruslan-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `ruslan model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal RUSLAN_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.ruslan/sessions/sessions.json entirely.
        ...
        # the RUSLAN_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.ruslan.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.ruslan/.env,
        ...
        # behavior: files must live under the Ruslan cache, under
        ...
        # user. Bridged to RUSLAN_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Ruslan
        # cache (~/.ruslan/cache/{documents,images,audio,video,screenshots})
        ...
        # to RUSLAN_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # RUSLAN_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to RUSLAN_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.ruslan/state.db.
    ...
    "sessions": {
        ...
        # default of ``ruslan sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.ruslan/sessions/session_{sid}.json`` on every turn
    ...
    # ``ruslan update`` behaviour.
    "updates": {
        # Run a full ``ruslan backup``-style zip of RUSLAN_HOME before every
        # ``ruslan update``.  Backups land in ``<RUSLAN_HOME>/backups/`` and
        # can be restored with ``ruslan import <path>``.  Off by default:
        # zipping a large RUSLAN_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``ruslan update
        ...
        "backup_keep": 5,
        # What `ruslan update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<RUSLAN_HOME>/lsp/bin/`` on first use.
    ...
    # `ruslan tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.ruslan/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.ruslan/.env (or your shell) and never in config.yaml.
            ...
            # ~/.ruslan/bin/ on first use.  When False you must install
            ...
            # `ruslan secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.ruslan/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Ruslan disables it for our users unless they
        # explicitly opt in here. When false (default), Ruslan sets
    ...
    # Ruslan Desktop (Electron app) launch options. These only affect
    # `ruslan desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the RUSLAN_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Ruslan mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'ruslan model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Ruslan auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Ruslan, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @ruslan:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. RUSLAN_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #ruslan)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: ruslan-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'ruslan-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Ruslan API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Ruslan API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Ruslan API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # RUSLAN_TOOL_PROGRESS and RUSLAN_TOOL_PROGRESS_MODE are deprecated —
    ...
    "RUSLAN_EPHEMERAL_SYSTEM_PROMPT": {
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `ruslan config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.ruslan.config`` entries, then checks
    ...
    try:
        ...
        # should never break `ruslan update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Ruslan' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Ruslan versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``ruslan_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'ruslan setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Ruslan won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'ruslan doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("RUSLAN_HOME", "~/.ruslan")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "ruslan update / ruslan -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("RUSLAN_TOOL_PROGRESS")
            old_mode = get_env_value("RUSLAN_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from RUSLAN_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from RUSLAN_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("RUSLAN_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from RUSLAN_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$RUSLAN_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_ruslan_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `ruslan plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `ruslan curator status` has no
    ...
    #   3. Creates `~/.ruslan/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_ruslan_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_ruslan_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `ruslan config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `ruslan config set`"
                    )
    ...
    # behavior set it to true explicitly via `ruslan config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from ruslan_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: ruslan config set <key> <value>")
    ...
    # `ruslan update`, but only the version bump is persisted.
    ...
    # metadata.ruslan.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_ruslan_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``ruslan config set``
    ...
    backends) — while display paths (``ruslan status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _ruslan_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.ruslan/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.ruslan/config.yaml.
    ...
    (which change ``RUSLAN_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/ruslan/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_ruslan_dotenv()
...
#   openai-codex (OAuth — ruslan auth) — OpenAI Codex
#   nous         (OAuth — ruslan auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.ruslan/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Ruslan writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.ruslan/.env.
    ...
    interactive menu render (`ruslan tools`, `ruslan setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `ruslan tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Ruslan env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.ruslan/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.ruslan/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.ruslan/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.ruslan/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Ruslan — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Ruslan vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.ruslan/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.ruslan/.env`` over ``os.environ``.

    Used for Ruslan-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    from ruslan_cli.auth import get_anthropic_key
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `ruslan config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `ruslan config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: ruslan config get <key> [--json]")
            print()
            print("Examples:")
            print("  ruslan config get model")
            print("  ruslan config get terminal.backend")
            print("  ruslan config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: ruslan config set <key> <value>")
            print()
            print("Examples:")
            print("  ruslan config set model anthropic/claude-sonnet-4")
            print("  ruslan config set terminal.backend docker")
            print("  ruslan config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: ruslan config unset <key>")
            print()
            print("Examples:")
            print("  ruslan config unset model")
            print("  ruslan config unset terminal.backend")
            print("  ruslan config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'ruslan config migrate' to add them")
        ...
        print("  ruslan config           Show current configuration")
        print("  ruslan config edit      Open config in editor")
        print("  ruslan config get <key>          Print a resolved config value")
        print("  ruslan config set <key> <value>   Set a config value")
        print("  ruslan config unset <key>        Remove a config value")
        print("  ruslan config check     Check for missing/outdated config")
        print("  ruslan config migrate   Update config with new options")
        print("  ruslan config path      Show config file path")
        print("  ruslan config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``ruslan config`` UI so users
