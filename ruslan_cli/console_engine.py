"""Safe Ruslan Console command engine.

This module backs ``ruslan console`` and is intentionally narrower than the
full Ruslan CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["RuslanConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'ruslan doctor'")
        and last.startswith("Run 'ruslan setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="ruslan", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `ruslan "):
        return ""
    return summary
...
# dashboard opens a fresh RuslanConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("ruslan_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace, ConsoleContext], None] | None = None,
) -> Callable[["RuslanConsoleEngine", list[str]], str]:
    def handler(_engine: RuslanConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    ...
    *,
    ...
    handler_factory: Callable[[Sequence[str]], Callable[["RuslanConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `ruslan {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `ruslan {usage}`?",
            contexts=ALL_CONTEXTS if child_key in hosted_paths else LOCAL_CONTEXTS,
        )


class RuslanConsoleEngine:
    """Curated line-command executor for Ruslan Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Ruslan Console does not run shell syntax. Use one supported "
                    "Ruslan command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Ruslan component status.", _status, contexts=ALL_CONTEXTS)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, contexts=ALL_CONTEXTS)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Ruslan logs.", _logs, contexts=ALL_CONTEXTS)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Ruslan configuration?",
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Ruslan Console."""

        extracted = {
            "version": (
                "ruslan_cli.subcommands.version",
            ...
            "prompt-size": (
                "ruslan_cli.subcommands.prompt_size",
            ...
            "insights": (
                "ruslan_cli.subcommands.insights",
            ...
            "security": (
                "ruslan_cli.subcommands.security",
            ...
            "import": (
                "ruslan_cli.subcommands.import_cmd",
            ...
            "plugins": (
                "ruslan_cli.subcommands.plugins",
            ...
            "pairing": (
                "ruslan_cli.subcommands.pairing",
            ...
            "webhook": (
                "ruslan_cli.subcommands.webhook",
            ...
            "profile": (
                "ruslan_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Ruslan configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "ruslan_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("ruslan_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "ruslan_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("ruslan_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("ruslan_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        for size in range(min(len(tokens), 3), 0, -1):
            ...
            if command:
                if self.context not in command.contexts:
                    raise ConsoleCommandError(
                        f"`ruslan {command.usage}` is not available in "
                        f"{self.context} Ruslan Console."
                    )
        ...
        raise ConsoleCommandError(f"Unsupported Ruslan Console command: {probe}.{suffix}")
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Ruslan Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`ruslan {first}` is not available in Ruslan Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Ruslan Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Ruslan Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Ruslan Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Ruslan Console.",
            ("skills", "publish"): "`skills publish` is not available in Ruslan Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Ruslan Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Ruslan Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Ruslan Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Ruslan Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Ruslan Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Ruslan Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Ruslan Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Ruslan Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Ruslan Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Ruslan Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Ruslan Console."
        return ""
...
def _enforce_hosted_line_policy(path: tuple[str, ...], args: Sequence[str]) -> None:
    if path == ("config", "set"):
        ...
        if key and not _hosted_config_key_allowed(key):
            raise ConsoleCommandError(
                f"`config set {key}` is not available in hosted Ruslan Console. "
    ...
    if path == ("mcp", "add"):
        if _flag_present(args, "--command") or _flag_present(args, "--args"):
            raise ConsoleCommandError(
                "Hosted Ruslan Console does not add stdio MCP servers. "
                "Use catalog install or an HTTP/SSE URL."
            )
        if _flag_present(args, "--preset"):
            raise ConsoleCommandError(
                "Hosted Ruslan Console does not add MCP presets directly. "
                "Use `mcp install <catalog-name>`."
            )
        url = _flag_value(args, "--url")
        if not url:
            raise ConsoleCommandError(
                "Hosted Ruslan Console requires `mcp add` to use --url with "
                "an HTTP/SSE endpoint."
            )
        scheme = urlparse(url).scheme.lower()
        if scheme not in {"http", "https"}:
            raise ConsoleCommandError(
                "Hosted Ruslan Console only accepts http:// or https:// MCP URLs."
            )
        return
...
def _apply_confirmed_defaults(args: argparse.Namespace, context: ConsoleContext) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Ruslan Console.")
...
def _status(_engine: RuslanConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: RuslanConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Ruslan Console.")
    ...
    from ruslan_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: RuslanConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: RuslanConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from ruslan_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from ruslan_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from ruslan_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config migrate")

    def _run() -> None:
        from ruslan_cli.config import migrate_config
...
def _sessions_export(_engine: RuslanConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: RuslanConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: RuslanConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from ruslan_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="ruslan_cli.subcommands.profile",
...
def _cron_list(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from ruslan_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from ruslan_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from ruslan console")
...
def _cron_resume(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: RuslanConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``ruslan console`` REPL."""
    ...
    engine = RuslanConsoleEngine()
    if interactive:
        print("Ruslan Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("ruslan> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
