...
Profile-aware location: ``$RUSLAN_HOME/logs/dashboard-auth.log``.
...
from ``ruslan_constants`` or other ruslan_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$RUSLAN_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``ruslan_constants.get_ruslan_home`` semantics: env var wins,
    else ``~/.ruslan``. A local copy avoids an import cycle with the
    middleware which lives below ``ruslan_cli``.
    """
    home = os.environ.get("RUSLAN_HOME") or str(Path.home() / ".ruslan")
