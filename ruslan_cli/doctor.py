"""
Doctor command for ruslan CLI.

Diagnoses issues with Ruslan Agent setup.
"""
...
from ruslan_cli.config import get_project_root, get_ruslan_home, get_env_path
from ruslan_cli.env_loader import load_ruslan_dotenv
from ruslan_constants import display_ruslan_home
from ruslan_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
RUSLAN_HOME = get_ruslan_home()
_DHH = display_ruslan_home()  # user-facing display path (e.g. ~/.ruslan or ~/.ruslan/profiles/coder)

# Load environment variables from ~/.ruslan/.env so API key checks work
_env_path = get_env_path()
load_ruslan_dotenv(ruslan_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from ruslan_cli.colors import Colors, color
from ruslan_cli.models import _RUSLAN_USER_AGENT
from ruslan_constants import OPENROUTER_MODELS_URL
...
from ruslan_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.ruslan/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from ruslan_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches ruslan_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``ruslan --version`` reporting a stale version while
    ...
    """
    try:
        from ruslan_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != ruslan_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'ruslan update', or set "
            "ruslan_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-ruslan and dashboard static services are up
    ...
    for static in ("main-ruslan", "dashboard"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from ruslan_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the RUSLAN_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("RUSLAN_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via RUSLAN_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `ruslan`.
    os.environ.setdefault("RUSLAN_INTERACTIVE", "1")

    # Handle `ruslan doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.ruslan/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`ruslan doctor --ack {hit.advisory.id}`."
                )
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from ruslan_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'ruslan config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from ruslan_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.ruslan/.env or run 'ruslan setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'ruslan setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'ruslan config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'ruslan setup' to migrate config")
                else:
        ...
        # Detect stale RUSLAN_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives RUSLAN_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from ruslan_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("RUSLAN_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"RUSLAN_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `ruslan setup` run)",
                )
                if should_fix:
                    if remove_env_value("RUSLAN_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale RUSLAN_MAX_ITERATIONS from .env "
                    ...
                    issues.append(
                        "Stale RUSLAN_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'ruslan doctor --fix'"
                    )
        except Exception:
            pass
    ...
    for subdir_name in expected_subdirs:
        subdir_path = ruslan_home / subdir_name
    ...
    if soul_path.exists():
        ...
        if should_fix:
            ...
            soul_path.write_text(
                ...
                "<!-- Edit this file to customize how Ruslan communicates. -->\n\n"
                "You are Ruslan, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = ruslan_home / "memories"
    ...
    state_db_path = ruslan_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from ruslan_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'ruslan doctor --fix' "
                        "(or 'ruslan sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from ruslan_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'ruslan doctor --fix' "
                        "(or 'ruslan sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = ruslan_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'ruslan doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "ruslan"
        ...
        _cmd_link = _cmd_link_dir / "ruslan"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(ruslan not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/ruslan → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/ruslan points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/ruslan → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/ruslan — run 'ruslan doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/ruslan exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/ruslan not found",
                    "(ruslan command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/ruslan → {_venv_bin}")
                    fixed_count += 1
    ...
    try:
        from ruslan_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `ruslan update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `ruslan doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable RUSLAN_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Ruslan one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'ruslan config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``ruslan doctor``.
    ...
    # never the legitimate source for `ruslan doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'ruslan setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = RUSLAN_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: ruslan memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: ruslan memory setup",
                    "No Honcho API key — run 'ruslan memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run ruslan memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: ruslan memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: ruslan memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from ruslan_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "ruslan -p" in content:
                            _m = _re.search(r"ruslan -p (\S+)", content)
