"""
Dump command for ruslan CLI.

Outputs a compact, plain-text summary of the user's Ruslan setup
...
from ruslan_cli.config import get_ruslan_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.ruslan/.env.
    ...
    exports. ``ruslan debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.ruslan_build_sha`` by the Dockerfile's
    ``RUSLAN_GIT_SHA`` build-arg (see ``ruslan_cli/build_info.py``).
    ...
    try:
        from ruslan_cli.build_info import get_build_sha
...
def _redact(value: str) -> str:
    ...
    ``ruslan dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from ruslan_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(ruslan_home: Path) -> int:
...
def _cron_summary(ruslan_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = ruslan_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"ruslan_home:      {display_ruslan_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.ruslan/.env: a managed
        ...
        # A credential added via `ruslan auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `ruslan auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["ruslan-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(ruslan_home)}")
    lines.append(f"  skills:             {_count_skills(ruslan_home)}")
