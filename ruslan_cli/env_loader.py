"""Helpers for loading Ruslan .env files consistently across entrypoints."""
...
# load_ruslan_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_ruslan_dotenv().
# Used by setup / `ruslan model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# RUSLAN_HOME paths we've already pulled external secrets for during this
# process.  ``load_ruslan_dotenv()`` is called at module-import time from
# several hot modules (cli.py, ruslan_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_ruslan_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which RUSLAN_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `ruslan setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``ruslan_cli.config._sanitize_env_lines`` which
    already knows all valid Ruslan env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from ruslan_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_ruslan_dotenv(
    *,
    ruslan_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Ruslan environment files with user config taking precedence.

    Behavior:
    - `~/.ruslan/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(ruslan_home or os.getenv("RUSLAN_HOME", Path.home() / ".ruslan"))
    ...
    #   EnvironmentFile=-/path/to/.ruslan/.op.env
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of RUSLAN_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Ruslan reads
    ...
    owns the once-per-RUSLAN_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_ruslan_dotenv()`` runs at import
    time from several hot modules (cli.py, ruslan_cli/main.py,
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `ruslan model`
