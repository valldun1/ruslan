"""
ruslan fallback — manage the fallback provider chain.
...
Subcommands:
  ...
  ruslan fallback add      Pick provider + model via the same picker as `ruslan model`,
                           then append the selection to the chain
  ruslan fallback remove   Pick an entry to delete from the chain
  ruslan fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.ruslan/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from ruslan_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `ruslan model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `ruslan model`, then append the selection to the chain."""
    from ruslan_cli.main import _require_tty, select_provider_and_model
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``ruslan fallback [subcommand]``."""
