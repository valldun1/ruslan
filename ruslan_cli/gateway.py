...
def _sync_ruslan_home_from_systemd_unit(system: bool) -> None:
    ...
    unit_home = env.get("RUSLAN_HOME", "").strip()
    if not unit_home:
        return
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Ruslan can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo ruslan gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo ruslan gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo ruslan gateway start --system")
            return scope, False
...
# (the documented `nohup ruslan gateway run` workaround). See #23387.
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           🛡️ Ruslan Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
def gateway_setup():
    ...
    print(
        color(
            "│             🛡️ Gateway Setup                            │", Colors.MAGENTA
        )
    )
    print(
        color(
