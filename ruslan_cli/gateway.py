"""
Gateway subcommand for ruslan CLI.

Handles: ruslan gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_ruslan_home is imported lazily at call sites to avoid ImportError
# when ruslan_constants is cached from a pre-update version during `ruslan update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "ruslan CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``ruslan gateway status`` from falsely counting the ``ruslan`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``ruslan gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_ruslan_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"ruslan_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. RUSLAN_HOME
        ...
        # RUSLAN_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "ruslan_home=" in command_lc
            and f"ruslan_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``ruslan update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Ruslan profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Ruslan profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from ruslan_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "ruslan_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m ruslan_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``ruslan update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Ruslan's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose RUSLAN_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``ruslan update`` users
    # with no running gateway until they re-invoke ``ruslan gateway``
    ...
    from ruslan_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from ruslan_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / RUSLAN_HOME) the windowless
        # base interpreter needs to import ruslan_cli.  Empty on POSIX, where
...
def _ruslan_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``RUSLAN_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("RUSLAN_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_ruslan_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``RUSLAN_HOME``.

    Under ``sudo``, ``RUSLAN_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_ruslan_home` falls back to ``/root/.ruslan`` — the wrong
    profile. The unit file pins ``RUSLAN_HOME`` for the actual gateway
    ...
    unit_home = (_ruslan_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("RUSLAN_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("RUSLAN_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["RUSLAN_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}ruslan gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}ruslan gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from ruslan_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `ruslan gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``ruslan gateway status`` output so users with
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (ruslan_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (RUSLAN_HOME-scoped).
...
from ruslan_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Ruslan Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``ruslan gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``RUSLAN_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("RUSLAN_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "ruslan-gateway"
SERVICE_DESCRIPTION = "Ruslan Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current RUSLAN_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (RUSLAN_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from ruslan_constants import get_default_ruslan_root

    home = get_ruslan_home().resolve()
    default = get_default_ruslan_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary RUSLAN_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(ruslan_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when RUSLAN_HOME is a named profile.

    For ``~/.ruslan/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        ruslan_home: Optional explicit RUSLAN_HOME path. Defaults to the current
            ``get_ruslan_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Ruslan root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_ruslan_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from ruslan_constants import get_default_ruslan_root

    home = Path(ruslan_home or str(get_ruslan_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_ruslan_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(ruslan_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".ruslan"
    try:
        Path(ruslan_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(ruslan_home, default_root=target_root)
    except ValueError:
        return _profile_arg(ruslan_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this RUSLAN_HOME.

    Default ``~/.ruslan`` returns ``ruslan-gateway`` (backward compatible).
    Profile ``~/.ruslan/profiles/coder`` returns ``ruslan-gateway-coder``.
    Any other RUSLAN_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Ruslan installs that predate the
# ruslan-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (ruslan-gateway-*.service) and unrelated third-party
# "ruslan" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("ruslan.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "ruslan_cli.main gateway",
    "ruslan_cli/main.py gateway",
    "gateway/run.py",
    " ruslan gateway ",
    "/ruslan gateway ",
)
...
def _find_legacy_ruslan_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Ruslan gateway units.

    Detects unit files installed by older Ruslan versions that used a
    different service name (e.g. ``ruslan.service`` before the rename to
    ``ruslan-gateway.service``). When both a legacy unit and the current
    ``ruslan-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``ruslan-gateway-coder.service`` and unrelated third-party
      ``ruslan-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``ruslan.service`` running an unrelated
...
def has_legacy_ruslan_units() -> bool:
    """Return True when any legacy Ruslan gateway unit files exist."""
    return bool(_find_legacy_ruslan_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Ruslan gateway unit files if any are installed.
    ...
    print_info("  These run alongside the current ruslan-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    ruslan gateway migrate-legacy")


def remove_legacy_ruslan_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Ruslan gateway unit files.

    Iterates over whatever ``_find_legacy_ruslan_units()`` returns — which is
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    ruslan gateway uninstall")
    print_info("    sudo ruslan gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: ruslan gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Ruslan often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.ruslan`` → ``ai.ruslan.gateway.plist`` (backward compatible).
    Profile ``~/.ruslan/profiles/coder`` → ``ai.ruslan.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.ruslan.gateway-{suffix}" if suffix else "ai.ruslan.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.ruslan/ruslan-agent  -> /home/alice/.ruslan/ruslan-agent
      /opt/ruslan                 -> /opt/ruslan  (kept as-is)
...
def _ruslan_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current RUSLAN_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_ruslan_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.ruslan                    → /home/alice/.ruslan
      /root/.ruslan/profiles/coder     → /home/alice/.ruslan/profiles/coder
      /opt/custom-ruslan               → /opt/custom-ruslan  (kept as-is)
    """
    current_ruslan = get_ruslan_home().resolve()
    current_default = (Path.home() / ".ruslan").resolve()
    target_default = Path(target_home_dir) / ".ruslan"

    # Default ~/.ruslan → remap to target user's default
    if current_ruslan == current_default:
        return str(target_default)

    # Profile or subdir of ~/.ruslan → preserve the relative structure
    try:
        relative = current_ruslan.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.ruslan) — keep as-is
        return str(current_ruslan)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    ruslan_node = ruslan_home / "node" / "bin"
    if _is_dir(ruslan_node):
        candidates.append(str(ruslan_node))
    ruslan_nm = ruslan_home / "node_modules" / ".bin"
    if _is_dir(ruslan_nm):
        candidates.append(str(ruslan_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m ruslan_cli.main``, so module
    ...
    dir, or a clone that ``ruslan update`` later relocates/removes — the path
    ...
    ``RUSLAN_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if RUSLAN_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        ruslan_home = _ruslan_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(ruslan_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's RUSLAN_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(ruslan_home) if ruslan_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m ruslan_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="RUSLAN_HOME={ruslan_home}"
Restart=always
RestartSec=5
...
"""
    ...
    ruslan_home = str(get_ruslan_home().resolve())
    profile_arg = _profile_arg(ruslan_home)
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned RUSLAN_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo ruslan gateway … --system``, RUSLAN_HOME is often stripped
    # and falls back to ``/root/.ruslan``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / RUSLAN_HOME comparisons
    ...
    _sync_ruslan_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir RUSLAN_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved RUSLAN_HOME
    ...
    exported a throwaway ``RUSLAN_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``ruslan gateway restart``
    with ``RUSLAN_HOME=/tmp/ruslan-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="RUSLAN_HOME=..."`` lines and launchd
    ``<key>RUSLAN_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'RUSLAN_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>RUSLAN_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp RUSLAN_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: RUSLAN_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported RUSLAN_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single RUSLAN_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only RUSLAN_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp RUSLAN_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../ruslan_test`` path into
    # ``Environment="RUSLAN_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/ruslan_test"' in new_unit
        or "/ruslan_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # RUSLAN_HOME into the unit (manual E2E homes like /tmp/ruslan-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Ruslan install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo ruslan gateway uninstall --system")
    print_info("         ruslan gateway install")
    print_info("         ruslan gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (ruslan.service from pre-rename installs)
    # before installing the new ruslan-gateway.service. If both remain, they
    ...
    if has_legacy_ruslan_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_ruslan_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin RUSLAN_HOME; adopt it before any
    ...
    # ``sudo ruslan gateway install --system --force`` would bake /root/.ruslan
    ...
    print(
        f"  {'sudo ' if system else ''}ruslan gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}ruslan gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # RUSLAN_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `ruslan gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `ruslan gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}ruslan gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}ruslan gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}ruslan gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}ruslan gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.ruslan.gateway-{suffix}" if suffix else "ai.ruslan.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``ruslan gateway start/stop/status`` call).
...
#      background process (the `nohup ruslan gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_ruslan_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_ruslan_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m ruslan_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active RUSLAN_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "ruslan_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup ruslan gateway run --replace`
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.ruslan/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See ruslan-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_ruslan_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Ruslan install"
    )
    return True
...
def launchd_stop():
    ...
    # `ruslan gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Ruslan PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Ruslan PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Ruslan install")
    else:
        print("⚠ Service definition is stale relative to the current Ruslan install")
        print("  Run: ruslan gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: ruslan gateway stop")
            else:
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``RUSLAN_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``ruslan gateway run`` is always a misconfiguration, so we
    ...
    try:
        from ruslan_constants import get_default_ruslan_root
        default_root = get_default_ruslan_root()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``ruslan gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``ruslan gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other RUSLAN_HOME roots.
    """
    ...
    print("  Use 'ruslan gateway restart' to replace it,")
    print("  or 'ruslan gateway stop' first.")
    print("  Or use 'ruslan gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("RUSLAN_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Ruslan gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'ruslan' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/ruslan/docker/entrypoint.sh before the Ruslan command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$RUSLAN_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set RUSLAN_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `ruslan gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set RUSLAN_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `ruslan gateway restart` which already
    ...
    # `ruslan gateway start/restart` — leaving the gateway vulnerable to
    ...
    # RUSLAN_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("RUSLAN_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. ruslan) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Ruslan delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Ruslan connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: ruslan pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Ruslan will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``ruslan setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run ruslan under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.ruslan/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_ruslan_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from ruslan_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'ruslan pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: ruslan pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'ruslan gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Ruslan will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Ruslan will store the returned account_id/token in ~/.ruslan/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `ruslan gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_ruslan_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `ruslan pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `ruslan pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "RuslanAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``ruslan_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from ruslan_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.ruslan/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.ruslan/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
        ...
        if service_running:
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: ruslan gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo ruslan gateway install --system"
                        )
                    print_info("  Or run in foreground:  ruslan gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: ruslan gateway run")
                print_info(
                    "  For persistence:   tmux new -s ruslan 'ruslan gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: ruslan gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup ruslan gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'ruslan gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`ruslan_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from ruslan_cli.service_manager import (
    ...
    if profile is None:
        ...
        # RUSLAN_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``ruslan gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``ruslan gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``ruslan gateway run`` are unaffected.
      2. ``RUSLAN_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``ruslan gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``RUSLAN_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("RUSLAN_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("RUSLAN_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${RUSLAN_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if RUSLAN_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or RUSLAN_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-ruslan service's
    # pattern in docker/s6-rc.d/main-ruslan/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: ruslan gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: ruslan gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s ruslan 'ruslan gateway run'"
                )
                print()
            ...
            print()
            print(
                "  ruslan gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s ruslan 'ruslan gateway run'         # persistent via tmux"
            )
            print(
                "  nohup ruslan gateway run > ~/.ruslan/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  ruslan profile create <name>     # creates the s6 service slot")
                print("  ruslan -p <name> gateway start   # bring it up via s6")
                print("  ruslan status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: ruslan gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: ruslan gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: ruslan gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  ruslan profile delete <name>     # tears down the s6 service slot")
                print("  ruslan -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `ruslan -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: ruslan gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_RUSLAN_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `ruslan gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_RUSLAN_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `ruslan gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: ruslan gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  ruslan gateway install")
                else:
                    print("To install as a service:")
                    print("  ruslan gateway install")
                    print("  sudo ruslan gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  ruslan gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup ruslan gateway run > ~/.ruslan/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s ruslan 'ruslan gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  ruslan gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  ruslan gateway install  # Install as user service")
                    print(
                        "  sudo ruslan gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Ruslan gateway unit files from
        # pre-rename installs (e.g. ruslan.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_ruslan_units(interactive=not yes, dry_run=dry_run)
