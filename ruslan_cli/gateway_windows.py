...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m ruslan_cli.main gateway run --replace`` invocation) and
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    source checkout only if ``RUSLAN_HOME`` cannot be resolved yet.
    """
    from ruslan_cli.config import get_ruslan_home
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from ruslan_cli.config import get_ruslan_home
        print(f"    type {Path(get_ruslan_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_ruslan_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from ruslan_cli.config import get_ruslan_home

    ruslan_home = Path(get_ruslan_home()).resolve()
    print()
    print("Next steps:")
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_ruslan_home()).resolve()
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from ruslan_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
...
def restart() -> None:
    ...
    """
    _assert_windows()
    from ruslan_cli.gateway import kill_gateway_processes
