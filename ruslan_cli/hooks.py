"""ruslan hooks — inspect and manage shell-script hooks.

Usage::

    ruslan hooks list
    ruslan hooks test <event> [--for-tool X] [--payload-file F]
    ruslan hooks revoke <command>
    ruslan hooks doctor

Consent records live under ``~/.ruslan/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.ruslan/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``ruslan hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: ruslan hooks {list|test|revoke|doctor}")
        print("Run 'ruslan hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.ruslan/config.yaml.")
        print("See `ruslan hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `ruslan hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `ruslan hooks test` and `ruslan hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Ruslan wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `ruslan hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `ruslan hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `ruslan hooks doctor`.")
