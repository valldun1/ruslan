...
Used by ``ruslan kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``ruslan_cli/goals.py`` — same aux
...
RUSLAN_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("RUSLAN_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Ruslan Agent board.
...
def _profile_author() -> str:
    """Mirror of ``ruslan_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("RUSLAN_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=RUSLAN_KANBAN_SPECIFY_MAX_TOKENS,
