"""``ruslan logs`` — view and filter Ruslan log files.
...
Usage examples::
    ...
    ruslan logs                    # last 50 lines of agent.log
    ruslan logs -f                 # follow agent.log in real time
    ruslan logs errors             # last 50 lines of errors.log
    ruslan logs gateway -n 100    # last 100 lines of gateway.log
    ruslan logs gui -f            # follow gui.log (dashboard/pty/ws)
    ruslan logs desktop -f        # follow desktop.log (Electron app boot/backend)
    ruslan logs --level WARNING    # only WARNING+ lines
    ruslan logs --session abc123   # filter by session ID substring
    ruslan logs --component tools  # only tool-related lines
    ruslan logs --since 1h         # lines from the last hour
    ruslan logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_ruslan_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Ruslan runs — try 'ruslan chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_ruslan_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_ruslan_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_ruslan_home()}/logs/")
        return

    print(f"Log files in {display_ruslan_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'ruslan chat' to generate logs)")
