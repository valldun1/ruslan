#!/usr/bin/env python3
"""
Ruslan CLI - Main entry point.

Usage:
    ruslan                     # Interactive chat (default)
    ruslan chat                # Interactive chat
    ruslan gateway             # Run gateway in foreground
    ruslan gateway start       # Start gateway as service
    ruslan gateway stop        # Stop gateway service
    ruslan gateway status      # Show gateway status
    ruslan gateway install     # Install gateway service
    ruslan gateway uninstall   # Uninstall gateway service
    ruslan setup               # Interactive setup wizard
    ruslan logout              # Clear stored authentication
    ruslan status              # Show status of all components
    ruslan cron                # Manage cron jobs
    ruslan cron list           # List cron jobs
    ruslan cron status         # Check if cron scheduler is running
    ruslan doctor              # Check configuration and dependencies
    ruslan honcho setup                    # Configure Honcho AI memory integration
    ruslan honcho status                   # Show Honcho config and connection status
    ruslan honcho sessions                 # List directory → session name mappings
    ruslan honcho map <name>               # Map current directory to a session name
    ruslan honcho peer                     # Show peer names and dialectic settings
    ruslan honcho peer --user NAME         # Set user peer name
    ruslan honcho peer --ai NAME           # Set AI peer name
    ruslan honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    ruslan honcho mode                     # Show current memory mode
    ruslan honcho mode [hybrid|honcho|local]  # Set memory mode
    ruslan honcho tokens                   # Show token budget settings
    ruslan honcho tokens --context N       # Set session.context() token cap
    ruslan honcho tokens --dialectic N     # Set dialectic result char cap
    ruslan honcho identity                 # Show AI peer identity representation
    ruslan honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    ruslan honcho migrate                  # Step-by-step migration guide: OpenClaw native → Ruslan + Honcho
    ruslan version             Show version
    ruslan update              Update to latest version
    ruslan uninstall           Uninstall Ruslan Agent
    ruslan acp                 Run as an ACP server for editor integration
    ruslan sessions browse     Interactive session picker with search

    ruslan claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: ruslan_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``ruslan_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``ruslan update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``ruslan_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, ruslan crashes on import and the user can't run
# ``ruslan update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import ruslan_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'ruslan' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``ruslan tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``ruslan.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("ruslan")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"ruslan", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"ruslan")
        # Windows: the .exe name is already ``ruslan.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* ruslan_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("RUSLAN_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".ruslan", "config.yaml")
...
# `entry.tsx`; this is just the earlier cousin. ``RUSLAN_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("RUSLAN_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`ruslan --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``ruslan --version`` before config/logging imports on Termux."""
    if os.environ.get("RUSLAN_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from ruslan_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from ruslan_cli.subcommands.cron import build_cron_parser
from ruslan_cli.subcommands.gateway import build_gateway_parser
from ruslan_cli.subcommands.profile import build_profile_parser
from ruslan_cli.subcommands.model import build_model_parser
from ruslan_cli.subcommands.setup import build_setup_parser
from ruslan_cli.subcommands.postinstall import build_postinstall_parser
from ruslan_cli.subcommands.whatsapp import build_whatsapp_parser
from ruslan_cli.subcommands.slack import build_slack_parser
from ruslan_cli.subcommands.login import build_login_parser
from ruslan_cli.subcommands.logout import build_logout_parser
from ruslan_cli.subcommands.auth import build_auth_parser
from ruslan_cli.subcommands.status import build_status_parser
from ruslan_cli.subcommands.webhook import build_webhook_parser
from ruslan_cli.subcommands.hooks import build_hooks_parser
from ruslan_cli.subcommands.doctor import build_doctor_parser
from ruslan_cli.subcommands.security import build_security_parser
from ruslan_cli.subcommands.dump import build_dump_parser
from ruslan_cli.subcommands.debug import build_debug_parser
...
from ruslan_cli.subcommands.dashboard import build_dashboard_parser
from ruslan_cli.subcommands.gui import build_gui_parser
from ruslan_cli.subcommands.logs import build_logs_parser
from ruslan_cli.subcommands.prompt_size import build_prompt_size_parser
from ruslan_cli.subcommands.memory import build_memory_parser
from ruslan_cli.subcommands.acp import build_acp_parser
from ruslan_cli.subcommands.tools import build_tools_parser
from ruslan_cli.subcommands.insights import build_insights_parser
from ruslan_cli.subcommands.skills import build_skills_parser
from ruslan_cli.subcommands.pairing import build_pairing_parser
from ruslan_cli.subcommands.plugins import build_plugins_parser
from ruslan_cli.subcommands.mcp import build_mcp_parser
from ruslan_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (ruslan tools, ruslan setup, ruslan model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'ruslan {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any ruslan module import.
#
# Many modules cache RUSLAN_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("RUSLAN_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.ruslan/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set RUSLAN_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `ruslan mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Ruslan' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo ruslan -p <name>` against the invoking user's home.
        ...
        candidate = home / ".ruslan" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`ruslan chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors ruslan_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If RUSLAN_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.ruslan/profiles/coder or
    # /opt/data/profiles/coder).  If RUSLAN_HOME points to the ruslan root
    # instead (e.g. systemd hardcodes RUSLAN_HOME=/root/.ruslan), we must
    # still read active_profile — the user may have switched profiles via
    # `ruslan profile use` and the gateway should honour that choice.
    # See issue #22502.
    ruslan_home_env = os.environ.get("RUSLAN_HOME", "")
    if profile_name is None and ruslan_home_env:
        if Path(ruslan_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the ruslan root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as RUSLAN_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``ruslan gateway run`` to mean
    # "the root RUSLAN_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("RUSLAN_S6_SUPERVISED_CHILD"):
        try:
            from ruslan_constants import get_default_ruslan_root

            active_path = get_default_ruslan_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set RUSLAN_HOME
    if profile_name is not None:
        try:
            from ruslan_cli.profiles import resolve_profile_env

            ruslan_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            ruslan_home = _resolve_sudo_user_profile_env(profile_name)
            if not ruslan_home:
            ...
            # A bug in profiles.py must NEVER prevent ruslan from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["RUSLAN_HOME"] = ruslan_home
...
# Load .env from ~/.ruslan/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from ruslan_cli.config import get_ruslan_home
from ruslan_cli.env_loader import load_ruslan_dotenv

load_ruslan_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → RUSLAN_REDACT_SECRETS env
# var BEFORE ruslan_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    if _cfg_path.exists():
        ...
        try:
            from ruslan_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "RUSLAN_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["RUSLAN_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `ruslan` subcommands
...
try:
    from ruslan_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from ruslan_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if ruslan_constants not importable yet
...
from ruslan_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to ruslan_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (ruslan_cli.main._model_flow_*) keep resolving unchanged.
from ruslan_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_ruslan_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("RUSLAN_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("RUSLAN_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from ruslan_cli.config import get_env_path, get_ruslan_home, load_config
    from ruslan_cli.auth import get_auth_status

    # Determine whether Ruslan itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_ruslan_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_ruslan_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Ruslan has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Ruslan to use their tokens.
    if _has_ruslan_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, ruslan_bin
        cli_args: the original CLI arguments (everything after 'ruslan')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    ruslan_bin = container_info["ruslan_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo ruslan {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo ruslan {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @ruslan/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@ruslan" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/ruslan-ink/package.json",
    "packages/ruslan-ink/index.js",
    "packages/ruslan-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``RUSLAN_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("RUSLAN_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``RUSLAN_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("RUSLAN_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    ruslan_home = os.environ.get("RUSLAN_HOME") or str(Path.home() / ".ruslan")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "RUSLAN_HOME": ruslan_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(ruslan_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(ruslan_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if ruslan_cli_dir is None:
        ruslan_cli_dir = Path(__file__).parent
    bundled = ruslan_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``ruslan update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Ruslan checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `ruslan update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Ruslan checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `ruslan --tui`\n"
        "If the checkout is still inconsistent, run `ruslan update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (RUSLAN_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("RUSLAN_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from ruslan_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("RUSLAN_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with RUSLAN_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset RUSLAN_TUI_DIR (e.g. `unset RUSLAN_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @ruslan/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @ruslan/ink resolves through
        # packages/ruslan-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "ruslan-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from ruslan_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from ruslan_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="ruslan-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["RUSLAN_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["RUSLAN_PYTHON_SRC_ROOT"] = os.environ.get(
        "RUSLAN_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("RUSLAN_PYTHON", sys.executable)
    env.setdefault("RUSLAN_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["RUSLAN_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["RUSLAN_MODEL"] = model
        env["RUSLAN_INFERENCE_MODEL"] = model
    if provider:
        env["RUSLAN_TUI_PROVIDER"] = provider
        env["RUSLAN_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["RUSLAN_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["RUSLAN_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["RUSLAN_TUI_SKILLS"] = value
    if query:
        env["RUSLAN_TUI_QUERY"] = query
    if image:
        env["RUSLAN_TUI_IMAGE"] = image
    if checkpoints:
        env["RUSLAN_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["RUSLAN_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["RUSLAN_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["RUSLAN_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["RUSLAN_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["RUSLAN_ACCEPT_HOOKS"] = "1"
    ...
    # RUSLAN_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `ruslan --tui` try to
    ...
    # still set RUSLAN_TUI_RESUME themselves.
    env.pop("RUSLAN_TUI_RESUME", None)
    if resume_session_id:
        env["RUSLAN_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `ruslan update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``RUSLAN_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``ruslan kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``ruslan kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("RUSLAN_KANBAN_BOARD"):
        return
    try:
        from ruslan_cli.kanban_db import get_current_board

        os.environ["RUSLAN_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.ruslan/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Ruslan — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Ruslan still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    try:
        from ruslan_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from ruslan_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'ruslan doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if _termux_should_prefetch_update_check():
        try:
            from ruslan_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["RUSLAN_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from ruslan_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from ruslan_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `ruslan gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: ruslan gateway")
            return
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``ruslan whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``ruslan_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from ruslan_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from ruslan_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from ruslan_cli.config import stamp_install_method
    from ruslan_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from ruslan_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``ruslan model``) and the setup wizard
    ...
    from ruslan_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("RUSLAN_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from ruslan_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'ruslan model' for "
                "available providers, or run 'ruslan doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in ruslan_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.ruslan/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.ruslan/.env if the active provider is not 'custom'.
    ...
    """
    from ruslan_cli.config import get_env_value, save_env_value, load_config
...
#
# Ruslan uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `ruslan model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `ruslan model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`ruslan_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from ruslan_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``ruslan model`` flow, then route aux tasks to them here.
    """
    from ruslan_cli.config import load_config
    from ruslan_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from ruslan_cli.auth import _prompt_model_selection
    from ruslan_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Выберите провайдера:"):
    ...
    """
    try:
        from ruslan_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from ruslan_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Ruslan URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# downstream call sites read `ruslan_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `ruslan` invocation — including
# fast paths like `ruslan --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from ruslan_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``ruslan setup`` / ``ruslan model``.
    ...
    recover from a malformed paste without editing ``~/.ruslan/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from ruslan_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from ruslan_cli.config import save_env_value
    from ruslan_cli.secret_prompt import masked_secret_prompt
    ...
    from ruslan_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `ruslan setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from ruslan_constants import display_ruslan_home as _dhh_fn

            print(
                f"    Ruslan will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Ruslan CLI with a provider."""
    from ruslan_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from ruslan_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from ruslan_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from ruslan_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from ruslan_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from ruslan_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``ruslan slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: ruslan slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `ruslan slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from ruslan_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from ruslan_cli.kanban import kanban_command
...
def cmd_doctor(args):
    ...
    from ruslan_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `ruslan security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from ruslan_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from ruslan_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from ruslan_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from ruslan_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Ruslan home directory to a zip file."""
    if getattr(args, "quick", False):
        from ruslan_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from ruslan_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Ruslan backup from a zip file."""
    from ruslan_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from ruslan_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Ruslan Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from ruslan_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from ruslan_cli.uninstall import run_gui_uninstall
    ...
    from ruslan_cli.uninstall import run_uninstall
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``ruslan`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="ruslan-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``ruslan update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``ruslan_cli/web_dist/`` (per vite.config.ts
    outDir: "../ruslan_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "ruslan_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``ruslan`` launcher present but ``ruslan_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the ruslan venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — ruslan venv python3, no nix-shell overhead
    ...
    # a ruslan venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``ruslan update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``ruslan web``).
    ...
    # build path usable on a stock `py -m ruslan_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_ruslan_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (ruslan web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``ruslan update`` can unconditionally call ``ruslan desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``ruslan desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $RUSLAN_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $RUSLAN_HOME."""
    from ruslan_constants import get_ruslan_home
    return get_ruslan_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``ruslan update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Ruslan.app/Contents/MacOS/Ruslan"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Ruslan.exe",
            release_dir / "win-ia32-unpacked" / "Ruslan.exe",
            release_dir / "win-arm64-unpacked" / "Ruslan.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "ruslan",
            release_dir / "linux-unpacked" / "Ruslan",
            release_dir / "linux-arm64-unpacked" / "ruslan",
            release_dir / "linux-arm64-unpacked" / "Ruslan",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Ruslan'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_ruslan_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Ruslan.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Ruslan.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Ruslan.exe: Access is
    ...
    an unrelated "Ruslan" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Ruslan is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Ruslan.app/Contents/MacOS/Ruslan  ->  app bundle = .../Ruslan.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Ruslan Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from ruslan_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["RUSLAN_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["RUSLAN_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "ruslan_root", None):
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Ruslan is
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``ruslan dashboard`` processes other than ourselves.

    ``ruslan dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``ruslan update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "ruslan.*dashboard"`) keeps us consistent
            # with `ruslan_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `ruslan update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``ruslan update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``ruslan update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: ruslan curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``ruslan dashboard`` processes.

    Called at the end of ``ruslan update`` (default ``reason``) and also
    from ``ruslan dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Ruslan Desktop Electron app spawns this dashboard as a
    # backend child, it sets RUSLAN_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("RUSLAN_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.ruslan-update-staging"
    backup = f"{dst}.ruslan-update-old"
...
def _update_via_zip(args):
    """Update Ruslan Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `ruslan update "
            f"--branch {branch}`, or update against main with `ruslan update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for ruslan-agent itself
        ...
        # GitHub ZIPs extract to ruslan-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"ruslan-agent-{branch}")
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: ruslan skills list-modified  "
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes ruslan completely unrunnable (SyntaxError on import).
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Ruslan couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Ruslan couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `ruslan update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_ruslan_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_ruslan_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``ruslan update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from ruslan_constants import get_default_ruslan_root

    default_home = get_default_ruslan_root()
...
# Install-scoped breadcrumb dropped right before ``ruslan update`` mutates the
...
# survives and the next ``ruslan`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $RUSLAN_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``ruslan update`` left half-done.
    ...
    protocol stream (``ruslan acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `ruslan update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``ruslan update`` is still progressing even if pip/uv itself is silent.
    """
...
def _ruslan_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_ruslan_instances(
    ...
    The Ruslan Desktop Electron app spawns ``ruslan.EXE`` as a backend child,
    so during ``ruslan update`` the user-invoked process and the desktop's
    ...
    shims (``ruslan.exe`` / ``ruslan-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``ruslan update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``ruslan.exe``) is a separate process from the Python
    ...
    for shim in _ruslan_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated ruslan.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``ruslan update``
    ...
    #      ruslan.exe sitting *under* a non-Ruslan parent (e.g. a Ruslan
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "ruslan.exe"
    lines = ["✗ Another ruslan.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Ruslan Desktop, exit any open `ruslan` REPLs, and")
    lines.append("  stop the gateway (`ruslan gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `ruslan update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_ruslan_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``ruslan.exe``.
    ...
    when ``ruslan update`` runs, ``ruslan.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``ruslan.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next ruslan invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Ruslan Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Ruslan Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _ruslan_exe_shims(scripts_dir):
        ...
        print(
            "    Close Ruslan Desktop, exit other `ruslan` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `ruslan update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_ruslan_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``ruslan.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``ruslan.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``ruslan`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_ruslan_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``ruslan.exe.old.*`` left by prior updates.

    Called early on every ruslan invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``ruslan update`` runs ``uv pip install -e .[all]`` which
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``ruslan.exe`` / ``ruslan-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_ruslan_exe`` for the rationale.
    """
    ...
    # hours later inside ``ruslan update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``ruslan update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``ruslan.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``ruslan`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from ruslan_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `ruslan update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``ruslan update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.ruslan/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``ruslan update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``ruslan update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.ruslan/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``ruslan update --gateway``) the update is already
    ...
    try:
        ...
        # ruslan_cli.config.get_ruslan_home to simulate setup failure.
        from ruslan_cli.config import get_ruslan_home as _get_ruslan_home

        logs_dir = _get_ruslan_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "docker":
        ...
        from ruslan_cli.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``ruslan update`` without requiring a reinstall.  The
    ...
    ``ruslan`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/ruslan.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v ruslan'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/ruslan, code at /usr/local/lib/ruslan-agent).
    fhs_link = Path("/usr/local/bin/ruslan")
    ...
    path_comment = (
        "# Ruslan Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of RUSLAN_HOME before running the update.
    ...
    RUSLAN_HOME directories.  The ``--backup`` flag on ``ruslan update``
    ...
    try:
        from ruslan_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_ruslan_home so the user sees ~/.ruslan/...
    try:
        from ruslan_constants import get_ruslan_home, display_ruslan_home

        home = get_ruslan_home()
        try:
            display_path = f"{display_ruslan_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    ruslan.exe concurrent-instance guard does not see them. They still import
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m ruslan_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``ruslan gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from ruslan_cli import gateway_windows
        from ruslan_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``ruslan update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Ruslan Agent to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `ruslan update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Ruslan via pip (for PyPI installs)."""
    from ruslan_cli import __version__
    ...
    if is_uv_tool_install():
        ...
        cmd = [pipx, "upgrade", "ruslan-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "ruslan-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "ruslan-agent"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_ruslan_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $RUSLAN_HOME, this is cheap
        ...
        try:
            from ruslan_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in ruslan_cli/config.py bricked
        # every user who ran ``ruslan update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``ruslan
        ...
        # the old bytecode (e.g. get_ruslan_home added to ruslan_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``ruslan`` launch finishes the
        ...
        # build.  ``ruslan desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``ruslan desktop`` shouldn't be forced into a full
        ...
        # current — so copy it straight over ~/.ruslan/cache/model_catalog.json
        ...
        # modules in this process.  Reload ruslan_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_ruslan_home() added since the last release.
        try:
            import importlib
            import ruslan_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit RUSLAN_HOME so
        # it is not affected by sync_skills()'s module-level RUSLAN_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's RUSLAN_HOME env var points at the default or a named profile.
        try:
            from ruslan_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from ruslan_cli.profiles import backfill_profile_envs
        ...
        # check `ruslan curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``ruslan update`` gives users a
        ...
        # When running as ``ruslan update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_ruslan_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... ruslan-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... ruslan-gateway*)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. ruslan-gateway.service or ruslan-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `ruslan update` restart it automatically, allow\n"
                            ...
                            # path in `ruslan gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: ruslan gateway status"
                        )
        ...
        # Warn if legacy Ruslan gateway unit files are still installed.
        # When both ruslan.service (from a pre-rename install) and the
        # current ruslan-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `ruslan update` surfaces the issue until the user migrates.
        try:
            from ruslan_cli.gateway import (
                has_legacy_ruslan_units,
                _find_legacy_ruslan_units,
                supports_systemd_services,
            )
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``ruslan -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from ruslan_constants import display_ruslan_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `ruslan profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_ruslan_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → ruslan -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  ruslan profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     ruslan -p {name} chat")
                else:
        ...
        from ruslan_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from ruslan_constants import get_ruslan_home as _hh
        ...
        from ruslan_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `ruslan profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → ruslan -p {name}  ({wrapper})")
        print()
        ...
        from ruslan_cli.profiles import rename_profile
        ...
        from ruslan_cli.profiles import export_profile
        ...
        from ruslan_cli.profiles import import_profile
        ...
        import tempfile
        from ruslan_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="ruslan_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  ruslan -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      ruslan -p {plan.manifest.name} chat")
        ...
        from ruslan_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `ruslan profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"ruslan -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from ruslan_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("ruslan_requires"):
            print(f"Requires:     Ruslan {data['ruslan_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from ruslan_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.ruslan_requires:
        print(f"  Requires: Ruslan {mf.ruslan_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``ruslan dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``ruslan dashboard --status``
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``ruslan dashboard register`` for OAuth.
    ...
    try:
        from ruslan_cli.web_server import should_require_auth
    ...
    try:
        from ruslan_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    # launches the dashboard (`worker dashboard` → RUSLAN_HOME points into
    ...
    try:
        from ruslan_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("RUSLAN_DESKTOP") != "1"
    ):
        ...
        # RUSLAN_HOME.  We must resolve the root explicitly instead of just
        # dropping RUSLAN_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV RUSLAN_HOME=/opt/data`), so an unset
        # RUSLAN_HOME falls back to $HOME/.ruslan = /opt/data/.ruslan — an
        ...
        # update-button guard also misfires).  get_default_ruslan_root()
        # returns the root for both layouts: ~/.ruslan for a standard install
        ...
        try:
            from ruslan_constants import get_default_ruslan_root
            env["RUSLAN_HOME"] = str(get_default_ruslan_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop RUSLAN_HOME) rather than block the reroute.
            env.pop("RUSLAN_HOME", None)
    ...
    # the same logs directory as every other Ruslan surface.
    try:
        from ruslan_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "RUSLAN_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["RUSLAN_WEB_DIST"])
            if "RUSLAN_WEB_DIST" in os.environ
            else PROJECT_ROOT / "ruslan_cli" / "web_dist"
        )
    ...
    try:
        from ruslan_cli.mcp_startup import start_background_mcp_discovery
    ...
    from ruslan_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from ruslan_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from ruslan_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from ruslan_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from ruslan_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Ruslan log files."""
    from ruslan_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from ruslan_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``ruslan -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``ruslan_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``ruslan -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``ruslan`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("RUSLAN_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from ruslan_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from ruslan_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["RUSLAN_DEFER_AGENT_STARTUP"] = "1"
            os.environ["RUSLAN_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["RUSLAN_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `ruslan --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_ruslan_home()}/memories/\n"
            )
            return
        ...
        from ruslan_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Ruslan Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from ruslan_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from ruslan_cli.tools_config import run_post_setup_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from ruslan_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from ruslan_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from ruslan_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from ruslan_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from ruslan_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from ruslan_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for ruslan CLI."""
    # Cosmetic: make the process show up as 'ruslan' instead of 'python3.11'
    ...
    # Sweep stale ``ruslan.exe.old.*`` quarantine files left by previous
    # ``ruslan update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_ruslan_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``ruslan update``
    ...
    # ``ruslan skills install update``) merely defers recovery one launch;
    # under-matching (missing ``ruslan -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in ruslan_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from ruslan_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `ruslan model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    # migrate command
    # =========================================================================
    from ruslan_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in ruslan_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in ruslan_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in ruslan_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in ruslan_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `ruslan whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in ruslan_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from ruslan_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in ruslan_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in ruslan_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in ruslan_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in ruslan_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in ruslan_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in ruslan_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from ruslan_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from ruslan_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # doctor command  (parser built in ruslan_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in ruslan_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in ruslan_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in ruslan_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in ruslan_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.ruslan/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo ruslan uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from ruslan_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in ruslan_cli/subcommands/import_cmd.py)
    # =========================================================================
    ...
    # =========================================================================
    # skills command  (parser built in ruslan_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from ruslan_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in ruslan_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``ruslan --help``, ``ruslan version``, ``ruslan logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from ruslan_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from ruslan_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # tools command  (parser built in ruslan_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `ruslan computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `ruslan tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `ruslan computer-use doctor` to run cua-driver's\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from ruslan_cli.tools_config import _cua_driver_cmd
            # Honor RUSLAN_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from ruslan_cli.tools_config import _cua_driver_env
    ...
    # =========================================================================
    # mcp command  (parser built in ruslan_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    # =========================================================================
    # insights command  (parser built in ruslan_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in ruslan_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in ruslan_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in ruslan_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in ruslan_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in ruslan_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in ruslan_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in ruslan_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Ruslan-Setup.exe success screen tells users to
    # run `ruslan desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in ruslan_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in ruslan_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in ruslan_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``ruslan -c Pokemon Agent Dev`` → ``ruslan -c 'Pokemon Agent Dev'``
    ...
    from ruslan_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'ruslan -c model' where 'model' is consumed as the
