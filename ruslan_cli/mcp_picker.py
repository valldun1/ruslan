"""MCP picker — interactive `ruslan mcp picker` (also the default `ruslan mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``ruslan mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `ruslan plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from ruslan_cli.cli_output import prompt_yes_no
from ruslan_cli.curses_ui import curses_single_select
from ruslan_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Ruslan session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from ruslan_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `ruslan mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: ruslan mcp install <name>    Picker: ruslan mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Ruslan is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Ruslan — run `ruslan update` "
...
def show_catalog() -> None:
    """`ruslan mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`ruslan mcp picker` (and default `ruslan mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`ruslan mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `ruslan mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
