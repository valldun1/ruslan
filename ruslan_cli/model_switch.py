...
_RUSLAN_MODEL_WARNING = (
    "Valldun Ruslan 3 & 4 models are NOT agentic and are not designed "
    "for use with Ruslan Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Valldun Ruslan 3 / Ruslan 4 chat families.
...
def switch_model(
    ...
    # Mirrors the same logic in ruslan_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    *,
    ...
    # https://ruslan.team/docs/api/model-catalog.json so
    ...
    for ruslan_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                ...
                if store and store.get("credential_pool", {}).get(ruslan_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Ruslan writes ``models:`` as a dict keyed by model id
            # (see ruslan_cli/main.py::_save_custom_provider); older
