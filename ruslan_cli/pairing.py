...
Usage:
    ruslan pairing list              # Show all pending + approved users
    ruslan pairing approve <platform> <code>  # Approve a pairing code
    ruslan pairing revoke <platform> <user_id> # Revoke user access
    ruslan pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle ruslan pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: ruslan pairing {list|approve|revoke|clear-pending}")
        print("Run 'ruslan pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.ruslan/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'ruslan pairing list' to see pending codes.\n")
