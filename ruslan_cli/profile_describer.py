...
Used by ``ruslan profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``ruslan_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Ruslan Agent kanban board.
...
Rules:
  ...
  - Never write "Ruslan Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_ruslan_home())
        else:
