"""Profile distributions — shareable, packaged Ruslan profiles via git.

A distribution is a Ruslan profile published as a git repository (or
...
* ``ruslan profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``ruslan skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``ruslan profile``, not a parallel tree):

    ruslan profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "ruslan-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            ruslan_requires=str(data.get("ruslan_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.ruslan_requires:
            out["ruslan_requires"] = self.ruslan_requires
        if self.author:
...
def check_ruslan_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Ruslan {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``ruslan profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Ruslan profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Ruslan distribution."
        )

    # Version check up-front so we fail fast
    check_ruslan_requires(manifest.ruslan_requires, ruslan_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.ruslan).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="ruslan_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `ruslan profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`ruslan profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="ruslan_dist_update_") as tmp:
        plan = plan_install(
