"""
Profile management for multiple isolated Ruslan instances.

Each profile is a fully independent RUSLAN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.ruslan/profiles/<name>/`` by default.

The "default" profile is ``~/.ruslan`` itself — backward compatible,
zero migration needed.

Usage::

    ruslan profile create coder          # fresh profile + bundled skills
    ruslan profile create coder --clone  # also copy config, .env, SOUL.md, skills
    ruslan profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    ruslan -p coder chat                 # or via flag
    ruslan profile use coder             # set as sticky default
    ruslan profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See ruslan_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.ruslan``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   ruslan-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `ruslan backup` archives
...
# Marker file written by `ruslan profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `ruslan
...
# `ruslan skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Ruslan infrastructure directories that only the default profile
         (``~/.ruslan``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_ruslan_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.ruslan) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "ruslan-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when RUSLAN_HOME equals the
...
# into the export. The set below identifies the *known Ruslan profile
# artifacts* at the root of RUSLAN_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``ruslan_constants``.
...
_RESERVED_NAMES = frozenset({
    "ruslan", "default", "test", "tmp", "root", "sudo",
})

# Ruslan subcommands that cannot be used as profile names/aliases
_RUSLAN_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the ruslan root, NOT to the current RUSLAN_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where RUSLAN_HOME points outside
    ``~/.ruslan``, profiles live under ``RUSLAN_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_ruslan_home() / "profiles"


def _get_default_ruslan_home() -> Path:
    """Return the default (pre-profile) RUSLAN_HOME path.

    In standard deployments this is ``~/.ruslan``.
    In Docker/custom deployments where RUSLAN_HOME is outside ``~/.ruslan``
    (e.g. ``/opt/data``), returns RUSLAN_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_ruslan_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Ruslan installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its RUSLAN_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_ruslan_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, ruslan subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _RUSLAN_SUBCOMMANDS:
        return f"'{canon}' conflicts with a ruslan subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            ruslan_exe = shutil.which("ruslan") or "ruslan"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(ruslan_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Ruslan. If we
    ...
    try:
        ...
        from ruslan_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `ruslan doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``ruslan -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``ruslan profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``ruslan -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``ruslan profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``RUSLAN_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Ruslan config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_ruslan_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, ruslan_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own RUSLAN_HOME.
    ...
    The returned ``ruslan_home`` is the path to pass to
    ``set_ruslan_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_ruslan_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``ruslan update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.ruslan)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.ruslan/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `ruslan -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Ruslan profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `ruslan update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`ruslan profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Ruslan (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `ruslan profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `ruslan -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches RUSLAN_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``ruslan profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "RUSLAN_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_ruslan_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Ruslan *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Ruslan backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``RUSLAN_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `ruslan -p <canon> profile
    ...
    ruslan_markers = ("ruslan_cli.main", "ruslan-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Ruslan process: either an entrypoint marker in argv, or
            # a resolved executable named `ruslan`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_ruslan = (
                any(marker in joined for marker in ruslan_markers)
                or exe_name == "ruslan"
                or exe_name.startswith("ruslan")
            )
            if not is_ruslan:
                continue
            ...
            # ...or by RUSLAN_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("RUSLAN_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.ruslan).\n"
            "To remove everything, use: ruslan uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_ruslan_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``RUSLAN_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from ruslan_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set RUSLAN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("RUSLAN_HOME")
    try:
        os.environ["RUSLAN_HOME"] = str(profile_dir)
        from ruslan_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["RUSLAN_HOME"] = old_home
        elif "RUSLAN_HOME" in os.environ:
            del os.environ["RUSLAN_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.ruslan/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: ruslan profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from RUSLAN_HOME.

    Returns ``"default"`` if RUSLAN_HOME is not set or points to ``~/.ruslan``.
    Returns the profile name if RUSLAN_HOME points into ``~/.ruslan/profiles/<name>``.
    Returns ``"custom"`` if RUSLAN_HOME is set to an unrecognized path.
    """
    from ruslan_constants import get_ruslan_home
    ruslan_home = get_ruslan_home()
    resolved = ruslan_home.resolve()

    default_resolved = _get_default_ruslan_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      RUSLAN_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Ruslan file the
      user may have lying alongside RUSLAN_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.ruslan itself — its parent is ~/ and its
        # directory name is ".ruslan", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: ruslan profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.ruslan itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.ruslan). "
            "Specify a different name: ruslan profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="ruslan_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"ruslan_{old_name}"
    legacy_old_host = f"ruslan.{old_name}"
    new_host = f"ruslan_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_ruslan_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("ruslan_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a RUSLAN_HOME path string.

    Called early in the CLI entry point, before any ruslan modules
    are imported, to set the RUSLAN_HOME environment variable.
    """
