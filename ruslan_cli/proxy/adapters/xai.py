...
# xAI's public API is OpenAI-compatible for the endpoints Ruslan commonly
# uses. The Responses endpoint is included because Ruslan' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Ruslan-managed OAuth credentials."""

    auth_hint = "ruslan auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`ruslan auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`ruslan auth reset xai-oauth` or re-authenticate with "
                    "`ruslan auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `ruslan auth add xai-oauth --type oauth`."
            )
