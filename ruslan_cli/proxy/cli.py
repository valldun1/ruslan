"""CLI handlers for the ``ruslan proxy`` subcommand."""
...
from ruslan_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "ruslan proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"ruslan auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Ruslan proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Ruslan proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: ruslan proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``ruslan proxy <subcommand>``."""
    ...
    print(
        "ruslan proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  ruslan proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
