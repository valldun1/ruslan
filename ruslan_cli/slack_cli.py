"""``ruslan slack ...`` CLI subcommands.

Today only ``ruslan slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ ruslan slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Ruslan. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Ruslan deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Ruslan on Slack's older Assistant messaging
    ...
    """
    from ruslan_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Ruslan in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Ruslan in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Ruslan agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``ruslan_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$RUSLAN_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Ruslan")
    ...
    """
    name = getattr(args, "name", None) or "Ruslan"
    description = getattr(args, "description", None) or "Your Ruslan agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("RUSLAN_HOME") or str(Path.home() / ".ruslan")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Ruslan app\n"
