...
on Japanese locales, etc.).  Ruslan's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``ruslan_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Ruslan is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``RUSLAN_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("RUSLAN_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Ruslan-managed Git install directories so
    ...
    # already-running shells don't see the change — which means ruslan
    ...
    # install without Ruslan-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Ruslan's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Ruslan's
    ``ruslan config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Ruslan (User env var in Windows Settings, or
    export in a PowerShell profile) and Ruslan picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Ruslan-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\ruslan\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``ruslan`` from right after install) retain their old PATH.

    Any subprocess Ruslan spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Ruslan-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "ruslan", "git", "cmd"),
        os.path.join(local_appdata, "ruslan", "git", "bin"),
        os.path.join(local_appdata, "ruslan", "git", "usr", "bin"),
        # Ruslan venv Scripts directory — host of the ruslan.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes ruslan, but harmless to include.
        os.path.join(local_appdata, "ruslan", "ruslan-agent", "venv", "Scripts"),
