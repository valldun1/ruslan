"""``ruslan dashboard`` / ``ruslan serve`` subcommand parsers.
...
``ruslan_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `ruslan dashboard` / `ruslan serve` cmdlines and SIGTERM them directly —
    # the same path `ruslan update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Ruslan web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Ruslan web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Ruslan desktop app shells (<= 0.15.x) spawn the
    # backend as `ruslan dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Ruslan couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Ruslan Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Ruslan backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Ruslan backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports RUSLAN_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `ruslan dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.ruslan/.env.
    # Nested subparser so bare `ruslan dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "RUSLAN_DASHBOARD_OAUTH_CLIENT_ID into ~/.ruslan/.env, and prints "
            "how to engage the login gate. Requires being logged in (ruslan setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://ruslan.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via RUSLAN_DASHBOARD_PORTAL_URL. Mainly for "
