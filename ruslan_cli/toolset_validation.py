...
the tool registry or launching Ruslan (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``ruslan-cli`` to the non-existent ``ruslan``. ``resolve_toolset('ruslan')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``ruslan-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``ruslan`` instead of ``ruslan-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `ruslan tools` to reconfigure."
        )
    return warnings
