"""
Ruslan Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.ruslan/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Ruslan PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing ruslan-agent or ruslan PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Ruslan Agent" comment and following line
                if '# Ruslan Agent' in line or '# ruslan-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('ruslan' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing ruslan
                if 'ruslan' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the ruslan wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "ruslan",
        Path("/usr/local/bin/ruslan"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains ruslan_cli reference)
                content = wrapper.read_text()
                if 'ruslan_cli' in content or 'ruslan-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(ruslan_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``ruslan`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Ruslan home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (ruslan_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``ruslan gateway run`` processes
    ...
    try:
        from ruslan_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from ruslan_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``RUSLAN_HOME`` and ``RUSLAN_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\ruslan\git\cmd``, ``%LOCALAPPDATA%\ruslan\git\bin``,
#      ``%LOCALAPPDATA%\ruslan\git\usr\bin``, ``%LOCALAPPDATA%\ruslan\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\ruslan\git\`` and Node to
#      ``%LOCALAPPDATA%\ruslan\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``ruslan dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\ruslan\gateway-service\`` and sometimes
...
def _ruslan_path_markers(ruslan_home: Path) -> list[str]:
    """Path-entry substrings that identify Ruslan-owned User-PATH entries."""
    root = str(ruslan_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare ruslan-agent install dir.
...
def remove_path_from_windows_registry(ruslan_home: Path) -> list[str]:
    """Strip Ruslan-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _ruslan_path_markers(ruslan_home)
            kept: list[str] = []
...
def remove_ruslan_env_vars_windows() -> list[str]:
    """Delete RUSLAN_HOME and RUSLAN_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("RUSLAN_HOME", "RUSLAN_GIT_BASH_PATH"):
                try:
...
def _is_default_ruslan_home(ruslan_home: Path) -> bool:
    """Return True when ``ruslan_home`` points at the default (non-profile) root."""
    try:
        from ruslan_constants import get_default_ruslan_root
        return ruslan_home.resolve() == get_default_ruslan_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its RUSLAN_HOME directory.

    We shell out to ``ruslan -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current RUSLAN_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m ruslan_cli.main` so we don't depend on a `ruslan`
    #    wrapper that may be half-removed mid-uninstall.
    ruslan_invocation = [_sys.executable, "-m", "ruslan_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                ruslan_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's RUSLAN_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``ruslan uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$RUSLAN_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from ruslan_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(ruslan_home)
    ...
    if not summary["gui_installed"]:
        print("No Ruslan Chat GUI installation was found.")
        print(f"  Checked: {ruslan_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Ruslan agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(ruslan_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Ruslan agent at {ruslan_home / 'ruslan-agent'}")
        print(f"  • Your config, sessions, and secrets under {ruslan_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Ruslan agent is still installed. Run 'ruslan' to use the CLI,")
    print("or 'ruslan uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.ruslan/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.ruslan/ for future reinstall
    """
    ...
    is_default_profile = _is_default_ruslan_home(ruslan_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.ruslan data); otherwise keep-data. Named profiles
    ...
    print(f"  Config:  {ruslan_home / 'config.yaml'}")
    print(f"  Secrets: {ruslan_home / '.env'}")
    print(f"  Data:    {ruslan_home / 'cron/'}, {ruslan_home / 'sessions/'}, {ruslan_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their RUSLAN_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Ruslan data!", Colors.RED, Colors.BOLD))
...
def _perform_uninstall(
    *,
    ...
    ``ruslan`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$RUSLAN_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(ruslan_home))))
    ...
    log_info("Removing Ruslan-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(ruslan_home)
    ...
    #     step-5 rmtree(ruslan_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE RUSLAN_HOME and
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.ruslan/, just remove the ruslan-agent subdir
            if ruslan_home in project_root.parents or project_root.parent == ruslan_home:
    ...
    #     under RUSLAN_HOME but they're install tooling, not config — safe to
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile RUSLAN_HOME dirs live under
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m ruslan_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import ruslan_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``ruslan_constants`` + ``ruslan_cli.colors``
    (and lazily ``ruslan_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m ruslan_cli.uninstall")
    parser.add_argument(
        "--mode",
