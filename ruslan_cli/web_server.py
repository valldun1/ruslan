...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # SPA's StatusPage can show "OAuth gate ON via Valldun" or
...
def _spawn_ruslan_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    cmd = [sys.executable, "-m", "ruslan_cli.main", *subcommand]
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `ruslan model`
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://ruslan.team/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "weixin": {
        ...
        "docs_url": "https://ruslan.team/docs/user-guide/messaging/weixin/",
    ...
    "webhook": {
        ...
        "docs_url": "https://ruslan.team/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.ruslan.team"
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        _RUSLAN_OAUTH_FILE = None  # type: ignore
    ...
    if ruslan_creds and ruslan_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "ruslan_pkce",
            "source_label": f"Ruslan PKCE ({_RUSLAN_OAUTH_FILE})",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://ruslan.team/docs/guides/xai-grok-oauth",
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _RUSLAN_OAUTH_FILE
                if _RUSLAN_OAUTH_FILE.exists():
                    _RUSLAN_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _RUSLAN_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _RUSLAN_OAUTH_FILE.with_name(
        f"{_RUSLAN_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _RUSLAN_OAUTH_FILE)
        try:
            _RUSLAN_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
# the tokens exactly like `ruslan auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `ruslan auth add xai-oauth`'s credential-pool insert.
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `ruslan sessions prune`)."""
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process RUSLAN_HOME at import time. The dashboard is a single
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from ruslan_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from ruslan_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # RUSLAN_HOME override (see mcp_tool._wrap_with_home_override), so
...
# `ruslan memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `ruslan memory setup` to configure a new one.",
            )
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``ruslan tools`` picker uses, so the GUI and CLI stay in
...
def _discover_dashboard_plugins() -> list:
    ...
    2. User plugins:    ~/.ruslan/plugins/<name>/dashboard/manifest.json
...
def start_server(
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"RUSLAN_DASHBOARD_READY port={actual_port}", flush=True)
