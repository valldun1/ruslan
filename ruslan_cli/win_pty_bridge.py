"""Windows ConPTY bridge for the `ruslan dashboard` chat tab.

Drop-in counterpart to ``ruslan_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``ruslan_cli.web_server`` consumes: ``spawn``, ``read``,
