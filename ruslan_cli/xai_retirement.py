...
Pure logic: walks a Ruslan config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `ruslan doctor` and a future `ruslan migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Ruslan config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Ruslan config and return retirement issues.
...
def apply_migration(
    ...
    from ruslan_cli.config import require_readable_config_before_write
