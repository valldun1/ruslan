"""Centralized logging setup for Ruslan Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.ruslan/logs/`` (profile-aware via ``get_ruslan_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``ruslan_cli.web_server``,
    ``ruslan_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Ruslan (TUI, gateway, ``hy_memory`` server, MCP
...
from ruslan_constants import get_config_path, get_ruslan_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_ruslan_session_injector", False):
    ...
    _session_record_factory._ruslan_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``ruslan logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``ruslan logs --component gateway``.
    "gateway": ("gateway", "ruslan_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("ruslan_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "ruslan_cli.web_server",
        "ruslan_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ruslan_home: Optional[Path] = None,
    ...
    """Configure the Ruslan logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    ruslan_home
        Override for the Ruslan home directory.  Falls back to
        ``get_ruslan_home()`` (profile-aware).
    log_level
    ...
    home = ruslan_home or get_ruslan_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_ruslan_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._ruslan_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the ruslan group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from ruslan_cli.config import is_managed
...
# the module header): when several Ruslan processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._ruslan_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="ruslan-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_ruslan_queue", False):
