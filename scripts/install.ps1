# ============================================================================
# Ruslan Agent Installer for Windows
# ============================================================================
...
param(
    ...
    # builds apps/desktop into a launchable Ruslan.exe.
    #
    # Why opt-in:
    ...
    #     `ruslan desktop` already builds on demand.
...
function Resolve-UvCmd {
    ...
    # sibling process and RUSLAN_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Ruslan uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\ruslan\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Ruslan needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Ruslan copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\ruslan\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``RUSLAN_GIT_BASH_PATH`` (User scope) so Ruslan can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Ruslan features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Ruslan needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``RUSLAN_GIT_BASH_PATH`` (User env scope) so Ruslan can find it even before
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("RUSLAN_GIT_BASH_PATH", $candidate, "User")
            $env:RUSLAN_GIT_BASH_PATH = $candidate
            Write-Info "Set RUSLAN_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Ruslan may not find Git Bash."
    Write-Info "If needed, set RUSLAN_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    Write-Info "Installing Ruslan-managed Node.js $NodeVersion LTS..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\ruslan-node-extract"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\ruslan-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `ruslan
                    # update` path (#4735).
                    ...
                    $stashName = "ruslan-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Ruslan behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running ruslan processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\ruslan-agent-extract"
    ...
    # next `ruslan update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `ruslan.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install ruslan-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and ruslan.exe broken with
    ...
    # here, not 30 seconds later when the user runs `ruslan`.
    if (-not $NoVenv) {
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `ruslan dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `ruslan dashboard` should now work."
            } else {
...
function Write-BootstrapMarker {
    ...
    # Ruslan-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `ruslan update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".ruslan-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # (the default PS5 behaviour), and Ruslan's prompt-injection scanner
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Ruslan-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'ruslan setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\ruslan-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\ruslan-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\ruslan-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Ruslan` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Ruslan.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_ruslan_desktop command
    # resolves apps/desktop/release/win-unpacked/Ruslan.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Ruslan-managed
    ...
    # The --dir mode produces an unpacked Ruslan.exe in
    ...
    # are NOT trying to work around). The Ruslan icon + product name are
    # stamped onto Ruslan.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\ruslan-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Ruslan.exe",
        "$desktopDir\release\win-arm64-unpacked\Ruslan.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Ruslan.exe was found under $desktopDir\release\*-unpacked\"
    }
    ...
    #    Ruslan.exe. We deliberately do NOT point them at `ruslan desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Ruslan.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Ruslan.lnk')
        )
        ...
        # with the Ruslan icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.ruslan/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'ruslan setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m ruslan_cli.main setup
    } else {
        python -m ruslan_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'ruslan whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: ruslan gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: ruslan gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: ruslan gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   ruslan              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   ruslan setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   ruslan config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   ruslan config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   ruslan gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   ruslan update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Ruslan repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Ruslan-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Ruslan to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
