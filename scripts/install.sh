...
# Configuration
REPO_URL_SSH="git@github.com:valldun1/ruslan.git"
REPO_URL_HTTPS="https://github.com/valldun1/ruslan.git"
...
print_banner() {
    ...
    echo "│             ⚕ Ruslan Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Valldun.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://ruslan.team/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Ruslan Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Ruslan will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (ruslan --tui may not work)"
        }
...
ensure_browser() {
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$RUSLAN_HOME/node" --silent --ignore-scripts \
