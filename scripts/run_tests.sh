#!/usr/bin/env bash
# Canonical test runner for ruslan-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.ruslan/...)
#
# Usage:
...
# (RUSLAN_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.ruslan/ruslan-agent/venv"; do
...
elif [ -n "${RUSLAN_PYTHON:-}" ] && [ -x "$RUSLAN_PYTHON" ] \
    && "$RUSLAN_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: RUSLAN_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `ruslan` binary rather
  # than the devShell hook.
  PYTHON="$RUSLAN_PYTHON"
  echo "▶ no local venv — using Nix dev venv via RUSLAN_PYTHON: $PYTHON"
else
  echo "error: no virtualenv found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and RUSLAN_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  exit 1
fi
...
if [ -f "$HOME/.ruslan/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.ruslan"
...
exec env -i \
  ...
  LC_ALL=C.UTF-8 \
  PYTHONHASHSEED=0 \
  ${RUSLAN_RUN_SLOW_PET_TESTS:+RUSLAN_RUN_SLOW_PET_TESTS="$RUSLAN_RUN_SLOW_PET_TESTS"} \
