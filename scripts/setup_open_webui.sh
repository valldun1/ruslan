...
# Bootstrap Open WebUI against Ruslan Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.ruslan/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-ruslan.sh
...
#   RUSLAN_API_PORT=8642
#   RUSLAN_API_HOST=127.0.0.1
#   RUSLAN_API_MODEL_NAME='Ruslan Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Ruslan Agent WebUI}"
...
RUSLAN_ENV_FILE="${RUSLAN_ENV_FILE:-$HOME/.ruslan/.env}"
RUSLAN_API_PORT="${RUSLAN_API_PORT:-8642}"
RUSLAN_API_HOST="${RUSLAN_API_HOST:-127.0.0.1}"
RUSLAN_API_CONNECT_HOST="${RUSLAN_API_CONNECT_HOST:-127.0.0.1}"
RUSLAN_API_MODEL_NAME="${RUSLAN_API_MODEL_NAME:-Ruslan Agent}"
RUSLAN_API_BASE_URL="http://${RUSLAN_API_CONNECT_HOST}:${RUSLAN_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-ruslan.sh"
LOG_DIR="$HOME/.ruslan/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$RUSLAN_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$RUSLAN_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.ruslan.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.ruslan</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.ruslan"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.ruslan"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-ruslan.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Ruslan Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-ruslan.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.ruslan/logs/openwebui.log
StandardError=append:%h/.ruslan/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-ruslan.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$RUSLAN_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Ruslan API server is configured...'
  upsert_env API_SERVER_ENABLED true "$RUSLAN_ENV_FILE"
  upsert_env API_SERVER_HOST "$RUSLAN_API_HOST" "$RUSLAN_ENV_FILE"
  upsert_env API_SERVER_PORT "$RUSLAN_API_PORT" "$RUSLAN_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$RUSLAN_API_MODEL_NAME" "$RUSLAN_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$RUSLAN_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Ruslan gateway so API server settings take effect...'
  ruslan gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${RUSLAN_API_CONNECT_HOST}:${RUSLAN_API_PORT}/health" >/dev/null; then
    log 'Ruslan API server did not answer on the first check. Trying to start gateway in the background...'
    nohup ruslan gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${RUSLAN_API_CONNECT_HOST}:${RUSLAN_API_PORT}/health" >/dev/null
  ...
  log "Ruslan API endpoint: ${RUSLAN_API_BASE_URL}"
