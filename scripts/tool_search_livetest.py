#!/usr/bin/env python3
"""Live test harness for Ruslan Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any ruslan imports.
ORIGINAL_HOME = os.environ.get("RUSLAN_HOME")
ORIGINAL_AUTH = Path.home() / ".ruslan" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.ruslan/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.ruslan/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="ruslan_ts_live_"))
    ruslan_home = home_dir / ".ruslan"
    ruslan_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, ruslan_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".ruslan" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, ruslan_home / ".env")
        ...
        load_ruslan_dotenv(ruslan_home=str(Path.home() / ".ruslan"))
    ...
    (ruslan_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return ruslan_home
...
def reset_module_state():
    """Drop cached modules so the new RUSLAN_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "ruslan_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["RUSLAN_HOME"] = str(home)
...
def main():
    ...
    # Restore original RUSLAN_HOME
    if ORIGINAL_HOME is not None:
        os.environ["RUSLAN_HOME"] = ORIGINAL_HOME
    else:
