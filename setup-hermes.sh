...
# 5. Symlinks the 'ruslan' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Ruslan Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/ruslan-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink ruslan into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up ruslan command..."

RUSLAN_BIN="$SCRIPT_DIR/venv/bin/ruslan"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$RUSLAN_BIN" "$COMMAND_LINK_DIR/ruslan"
echo -e "${GREEN}✓${NC} Symlinked ruslan → $COMMAND_LINK_DISPLAY_DIR/ruslan"
...
# ============================================================================
# Seed bundled skills into ~/.ruslan/skills/
# ============================================================================

RUSLAN_SKILLS_DIR="${RUSLAN_HOME:-$HOME/.ruslan}/skills"
mkdir -p "$RUSLAN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.ruslan/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$RUSLAN_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  ruslan status        # Check configuration"
if is_termux; then
    echo "  ruslan gateway       # Run gateway in foreground"
else
    echo "  ruslan gateway install # Install gateway service (messaging + cron)"
fi
echo "  ruslan cron list     # View scheduled jobs"
echo "  ruslan doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m ruslan_cli.main setup
fi
