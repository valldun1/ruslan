...
## 5. Ruslan Configuration

### Config location

`$RUSLAN_HOME/config.yaml` (defaults to `~/.ruslan/config.yaml` when `RUSLAN_HOME` is unset)
...
Restart the Ruslan session for changes to take effect. The MCP connection is
