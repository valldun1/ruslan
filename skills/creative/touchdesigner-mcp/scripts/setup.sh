...
RUSLAN_HOME_DIR="${RUSLAN_HOME:-$HOME/.ruslan}"
RUSLAN_CFG="${RUSLAN_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Ruslan config has twozero_td MCP entry ──
if [[ ! -f "$RUSLAN_CFG" ]]; then
    echo -e " ${FAIL} Ruslan config not found at ${RUSLAN_CFG}"
    manual_steps+=("Create ${RUSLAN_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$RUSLAN_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Ruslan config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Ruslan config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${RUSLAN_CFG} manually"); }
    manual_steps+=("Restart Ruslan session to pick up config change")
fi
