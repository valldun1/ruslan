#!/usr/bin/env bash
# GitHub environment detection helper for Ruslan Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _ruslan_env="${RUSLAN_HOME:-$HOME/.ruslan}/.env"; [ -f "$_ruslan_env" ] && grep -q "^GITHUB_TOKEN=" "$_ruslan_env" 2>/dev/null; then
