...
# Ruslan Desktop Plugins Skill

Write plugins for the Ruslan desktop app: statusbar items, layout panes,
...
changes. This skill does not cover backend plugins (`~/.ruslan/plugins/`);
...
- The Ruslan desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$RUSLAN_HOME/desktop-plugins/` (usually
  `~/.ruslan/desktop-plugins/`).

## How to Run

1. Create `$RUSLAN_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (relative to this skill directory) — that's
   `~/.ruslan/...` by default, or `~/.ruslan/profiles/<profile>/...` under a
...
The ONLY import surface is `@ruslan/plugin-sdk` (plus `react` /
...
- Do not import anything except `@ruslan/plugin-sdk`, `react`, and
