#!/usr/bin/env python3
"""Google Workspace API CLI for Ruslan Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Ruslan-facing JSON contract and falls back to the Python client
...
from _ruslan_home import get_ruslan_home
...
def _gws_binary() -> str | None:
    override = os.getenv("RUSLAN_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Ruslan Agent")
