...
Damien's incident (Discord, 2026-05-28): a long Ruslan session in a Discord
...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``ruslan_state`` module, whose ``SessionDB``
    ...
    namespace = {"__name__": "ruslan_state"}
    source = '''
class SessionDB:
...
_NominalSessionDBImpostor.__module__ = "ruslan_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``ruslan_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = ruslan_state.SessionDB
    monkeypatch.setattr(ruslan_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(ruslan_state, "SessionDB", real_session_db_type)
