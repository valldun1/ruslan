...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Ruslan Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_ruslan_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path / ".ruslan"))

    ruslan_env = tmp_path / ".ruslan" / ".env"
    ruslan_env.parent.mkdir(parents=True)
    ruslan_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.ruslan/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.ruslan/auth.json` and read the
    ...
    oauth = ruslan_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = ruslan_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.ruslan/auth.json and @file:.ruslan/.anthropic_oauth.json "
        "and @file:.ruslan/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
