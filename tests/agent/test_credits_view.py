...
from ruslan_cli.nous_account import NousPortalAccountInfo, NousPaidServiceAccessInfo
...
def test_view_fetch_failure_is_logged_out(monkeypatch):
    ...
    monkeypatch.setattr("ruslan_cli.nous_account.get_nous_portal_account_info", _boom)
...
def test_credits_command_registered():
    from ruslan_cli.commands import resolve_command, COMMAND_REGISTRY
...
def test_cli_show_credits_non_interactive_renders_text_not_modal(monkeypatch, capsys):
    ...
    monkeypatch.setattr(RuslanCLI, "_prompt_text_input_modal", _boom_modal, raising=False)
