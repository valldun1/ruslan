...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated RUSLAN_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="ruslan_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".ruslan"))
    prev_home = os.environ.get("RUSLAN_HOME")
    os.environ["RUSLAN_HOME"] = os.path.join(test_home, ".ruslan")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("ruslan_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["RUSLAN_HOME"] = prev_home
