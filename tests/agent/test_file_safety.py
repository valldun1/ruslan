...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_ruslan_env(self):
        """Ruslan' own .env inside RUSLAN_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: ruslan internal .env is in ~/.ruslan/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.ruslan/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.ruslan/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Ruslan cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        ruslan_home = tmp_path / ".ruslan"
        cache = ruslan_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._ruslan_home_path", return_value=ruslan_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Ruslan cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        ruslan_home = tmp_path / ".ruslan"
        hub = ruslan_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_ruslan_home(self, tmp_path):
        """The env basename guard does not depend on RUSLAN_HOME resolution."""
        ruslan_home = tmp_path / ".ruslan"
        ruslan_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        ruslan_home = tmp_path / ".ruslan"
        cache = ruslan_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
