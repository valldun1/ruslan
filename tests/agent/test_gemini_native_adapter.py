...
def test_translate_native_response_surfaces_reasoning_and_tool_calls():
        ...
        "candidates": [
            {
                "content": {
                    "parts": [
                        ...
                        {"functionCall": {"name": "search", "args": {"q": "ruslan"}}},
                    ]
                },
                "finishReason": "STOP",
            }
    ...
    assert json.loads(choice.message.tool_calls[0].function.arguments) == {"q": "ruslan"}
...
def test_max_tokens_none_defaults_to_gemini_output_ceiling():
    ...
    maxOutputTokens is absent, truncating tool calls mid-stream. Ruslan passes
...
def test_x_goog_api_client_header_is_set():
    ...
    assert "ruslan-agent/" in headers["X-Goog-Api-Client"], (
        "ruslan-agent not found in X-Goog-Api-Client header"
    )


def test_x_goog_api_client_header_format():
    """Header value should be 'ruslan-agent/<version>' matching the package version."""
    from agent.gemini_native_adapter import GeminiNativeClient, _RUSLAN_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    expected = f"ruslan-agent/{_RUSLAN_VERSION}"
    assert headers["X-Goog-Api-Client"] == expected


def test_user_agent_contains_version():
    """User-Agent should include the ruslan-agent version."""
    from agent.gemini_native_adapter import GeminiNativeClient, _RUSLAN_VERSION

    client = GeminiNativeClient(api_key="fake-key", model="gemini-2.0-flash")
    headers = client._headers()

    assert f"ruslan-agent/{_RUSLAN_VERSION}" in headers["User-Agent"]


def test_ruslan_version_is_valid():
    """_RUSLAN_VERSION should be a non-empty string."""
    from agent.gemini_native_adapter import _RUSLAN_VERSION

    assert isinstance(_RUSLAN_VERSION, str)
    assert len(_RUSLAN_VERSION) > 0
    assert _RUSLAN_VERSION != "0.0.0", (
        "Version should resolve from ruslan_cli.__version__, not the fallback"
    )
