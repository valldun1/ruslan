...
class TestBuildLearnPrompt:
    ...
    def test_teaches_the_full_hardline_standards(self):
        ...
        # author is always the literal Ruslan, never the host/OS identity (#52368).
        assert "author: always the literal value `ruslan`" in std
        assert "never fill it from the host" in std
        # #2 Ruslan-tool framing names the wrapped tools, not shell utilities.
