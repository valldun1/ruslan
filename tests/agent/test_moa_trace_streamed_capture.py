...
path via ``ruslan chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp RUSLAN_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point RUSLAN_HOME at a temp dir and turn moa.save_traces on."""
    ruslan_home = tmp_path / ".ruslan"
    ruslan_home.mkdir()
    monkeypatch.setenv("RUSLAN_HOME", str(ruslan_home))

    # save_moa_turn reads config via ruslan_cli.config.load_config; stub it to
