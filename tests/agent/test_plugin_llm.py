...
""",
        ...
        from ruslan_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        ruslan_home = tmp_path / ".ruslan"
        ruslan_home.mkdir()
        (ruslan_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("RUSLAN_HOME", str(ruslan_home))
        from ruslan_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from ruslan_cli.plugins import PluginContext, PluginManifest, PluginManager
