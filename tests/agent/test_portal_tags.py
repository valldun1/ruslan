...
def test_ruslan_client_tag_includes_current_version():
    """The client tag must reflect ruslan_cli.__version__ verbatim."""
    from ruslan_cli import __version__
    from agent.portal_tags import ruslan_client_tag

    assert ruslan_client_tag() == f"client=ruslan-client-v{__version__}"


def test_ruslan_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import ruslan_client_tag

    tag = ruslan_client_tag()
    assert tag.startswith("client=ruslan-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import ruslan_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=ruslan-agent" in tags
    assert ruslan_client_tag() in tags
    assert len(tags) == 2
