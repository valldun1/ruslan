...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate RUSLAN_HOME under tmp_path."""
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path / ".ruslan"))
    (tmp_path / ".ruslan").mkdir()

    # Force the constants/image cache helpers to re-read RUSLAN_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("ruslan_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_ruslan_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under RUSLAN_HOME — gateway cleanup
