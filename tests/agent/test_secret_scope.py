...
class TestMultiplexActiveFailClosed:
    ...
    def test_global_env_still_reads_environ_under_multiplex(self, monkeypatch):
        ...
        ss.set_multiplex_active(True)
        # No scope, multiplex on — but RUSLAN_HOME is global, so no raise.
        assert ss.get_secret("RUSLAN_HOME") == "/opt/data"

    def test_kanban_prefix_is_global(self, monkeypatch):
        monkeypatch.setenv("RUSLAN_KANBAN_DB", "/x/kanban.db")
        ss.set_multiplex_active(True)
        assert ss.get_secret("RUSLAN_KANBAN_DB") == "/x/kanban.db"
