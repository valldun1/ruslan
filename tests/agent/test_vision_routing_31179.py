...
def isolated_home(monkeypatch):
    """Temp RUSLAN_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="ruslan_test_31179_")
    ruslan_home = os.path.join(test_home, ".ruslan")
    os.makedirs(ruslan_home)
...
def _fresh_modules():
    """Drop cached ruslan modules so each test reloads against current env."""
