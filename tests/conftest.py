"""Shared fixtures for the ruslan-agent test suite.
...
2. **Isolated RUSLAN_HOME.** RUSLAN_HOME points to a per-test tempdir so
   code reading ``~/.ruslan/*`` via ``get_ruslan_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".ruslan"`` instead of the canonical
   ``get_ruslan_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No RUSLAN_SESSION_* inheritance** — the agent's current gateway
...
# RUSLAN_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_RUSLAN_BEHAVIORAL_VARS = frozenset({
    "RUSLAN_YOLO_MODE",
    "RUSLAN_INTERACTIVE",
    "RUSLAN_QUIET",
    "RUSLAN_TOOL_PROGRESS",
    "RUSLAN_TOOL_PROGRESS_MODE",
    "RUSLAN_MAX_ITERATIONS",
    "RUSLAN_SESSION_PLATFORM",
    "RUSLAN_SESSION_CHAT_ID",
    "RUSLAN_SESSION_CHAT_NAME",
    "RUSLAN_SESSION_THREAD_ID",
    "RUSLAN_SESSION_SOURCE",
    "RUSLAN_SESSION_KEY",
    "RUSLAN_GATEWAY_SESSION",
    "RUSLAN_CRON_SESSION",
    "_RUSLAN_GATEWAY",
    "RUSLAN_PLATFORM",
    "RUSLAN_MODEL",
    "RUSLAN_INFERENCE_MODEL",
    "RUSLAN_INFERENCE_PROVIDER",
    "RUSLAN_TUI_PROVIDER",
    "RUSLAN_MANAGED",
    "RUSLAN_MANAGED_DIR",
    "RUSLAN_DEV",
    ...
    "RUSLAN_TIMEZONE",
    "RUSLAN_REDACT_SECRETS",
    "RUSLAN_BACKGROUND_NOTIFICATIONS",
    "RUSLAN_EXEC_ASK",
    "RUSLAN_HOME_MODE",
    "RUSLAN_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.ruslan/kanban.db instead of the per-test RUSLAN_HOME.
    "RUSLAN_KANBAN_DB",
    "RUSLAN_KANBAN_BOARD",
    "RUSLAN_KANBAN_HOME",
    "RUSLAN_KANBAN_WORKSPACES_ROOT",
    "RUSLAN_KANBAN_LOGS_ROOT",
    "RUSLAN_KANBAN_TASK",
    "RUSLAN_KANBAN_WORKSPACE",
    "RUSLAN_KANBAN_RUN_ID",
    "RUSLAN_KANBAN_CLAIM_LOCK",
    "RUSLAN_KANBAN_DISPATCH_IN_GATEWAY",
    "RUSLAN_TENANT",
    ...
    "RUSLAN_DASHBOARD_OAUTH_CLIENT_ID",
    "RUSLAN_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and RUSLAN_HOME to per-test tempdirs so code that
    reads ``~/.ruslan/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral RUSLAN_* vars that could change test semantics.
    for name in _RUSLAN_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect RUSLAN_HOME to a per-test tempdir. Code that reads
    #    ``~/.ruslan/*`` via ``get_ruslan_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.ruslan/*`` via
    #    ``Path.home() / ".ruslan"`` instead of ``get_ruslan_home()``
    #    is a bug to fix at the callsite.
    fake_ruslan_home = tmp_path / "ruslan_test"
    fake_ruslan_home.mkdir()
    (fake_ruslan_home / "sessions").mkdir()
    (fake_ruslan_home / "cron").mkdir()
    (fake_ruslan_home / "memories").mkdir()
    (fake_ruslan_home / "skills").mkdir()
    monkeypatch.setenv("RUSLAN_HOME", str(fake_ruslan_home))
    ...
    #    ~/.ruslan/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import ruslan_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_ruslan_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal ruslan config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``ruslan-gateway`` process
...
# correlated with a ``tests/ruslan_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> ruslan-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart ruslan-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting ruslan/python patterns are also blocked.
    """
    ...
    def _matches_ruslan_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _RUSLAN_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_ruslan_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch ruslan-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m ruslan_cli.main".
                if (
                    "ruslan" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live ruslan-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting ruslan/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `ruslan update` (or the
        # equivalent `python -m ruslan_cli.main update`).  These commands
        ...
        if "update" in low and (
            # ruslan update / ruslan update --gateway / setsid bash -c ... ruslan update
            ("ruslan" in low and "update" in low.split())
            or
            # python -m ruslan_cli.main update --gateway
            ("ruslan_cli" in low and "update" in low.split())
            or
            # venv/bin/ruslan update  (absolute path variant used in tests)
            (".venv/bin/ruslan" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`ruslan update` against the real checkout, fetching "
