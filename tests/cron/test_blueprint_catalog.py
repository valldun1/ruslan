...
the shared /blueprint command handler (ruslan_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated RUSLAN_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("ruslan://blueprint/morning-brief?")
    ...
    def test_catalog_entry_has_all_surfaces(self):
        ...
        assert entry["appUrl"].startswith("ruslan://")
...
class TestCommandHandler:
    ...
    def test_name_match_is_forgiving(self, isolated_home):
        from ruslan_cli.blueprint_cmd import handle_blueprint_command, match_blueprint
