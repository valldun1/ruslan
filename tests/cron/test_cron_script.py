...
- Path resolution (absolute, relative to RUSLAN_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp RUSLAN_HOME."""
    ruslan_home = tmp_path / ".ruslan"
    ruslan_home.mkdir()
    (ruslan_home / "cron").mkdir()
    (ruslan_home / "cron" / "output").mkdir()
    (ruslan_home / "scripts").mkdir()
    monkeypatch.setenv("RUSLAN_HOME", str(ruslan_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "RUSLAN_DIR", ruslan_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", ruslan_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", ruslan_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", ruslan_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Ruslan provider env (SECURITY.md §2.3)."""
        ...
        blocked_var = sorted(_RUSLAN_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.ruslan/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("RUSLAN_SESSION_PLATFORM") is None
        assert os.environ.get("RUSLAN_SESSION_CHAT_ID") is None
        assert os.environ.get("RUSLAN_SESSION_CHAT_NAME") is None
