...
Override the image with ``RUSLAN_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("RUSLAN_TEST_IMAGE", "ruslan-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``RUSLAN_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("RUSLAN_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged ruslan user
# ---------------------------------------------------------------------------
#
# Background: every Ruslan runtime path inside the container drops to UID
# 10000 (the ``ruslan`` user) via ``s6-setuidgid ruslan``. ``docker exec``
...
# fails (PermissionError swallowed) for ruslan, so a test that ran as root
...
# helpers below run every probe under ``-u ruslan`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: ruslan).
