"""Regression test: ``ruslan dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``ruslan dump`` used to report
...
* If ``/opt/ruslan/.ruslan_build_sha`` exists in the image, assert that
  ``ruslan dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's ruslan user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"ruslan dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/ruslan/.ruslan_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/ruslan/.ruslan_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``RUSLAN_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``ruslan dump``
