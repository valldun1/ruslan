"""Runtime smoke test for Docker $RUSLAN_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the ruslan user after container boot.
...
registered later runs its log service as the dropped ruslan user and
...
def test_logs_gateways_seeded_and_ruslan_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by ruslan after boot."""
    ...
    assert "logs=ruslan" in r.stdout, (
        f"logs/ not owned by ruslan: {r.stdout}"
    )
    assert "gateways=ruslan" in r.stdout, (
        f"logs/gateways/ not owned by ruslan: {r.stdout}"
    )
