...
Guards against the 60-vs-500 bug where a stale `.env RUSLAN_MAX_ITERATIONS=60`
...
def _run_gateway_import(ruslan_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    env["RUSLAN_HOME"] = str(ruslan_home)
...
@pytest.fixture
def ruslan_home(tmp_path: Path) -> Path:
    home = tmp_path / ".ruslan"
    home.mkdir()
    return home


def test_config_max_turns_wins_over_stale_env(ruslan_home: Path) -> None:
    """Regression: config.yaml:agent.max_turns=500 must beat .env=60."""
    _write_config(ruslan_home, agent_cfg={"max_turns": 500})
    _write_env(ruslan_home, {"RUSLAN_MAX_ITERATIONS": "60"})

    env = _run_gateway_import(ruslan_home, initial_env={})

    assert env.get("RUSLAN_MAX_ITERATIONS") == "500", (
        f"expected config.yaml max_turns=500 to win; got {env.get('RUSLAN_MAX_ITERATIONS')!r}. "
        "Stale .env value is shadowing config — the bridge lost its override."
    )


def test_config_gateway_timeout_wins_over_stale_env(ruslan_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(ruslan_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
    })
    _write_env(ruslan_home, {
        "RUSLAN_AGENT_TIMEOUT": "60",
        "RUSLAN_AGENT_TIMEOUT_WARNING": "30",
    })

    env = _run_gateway_import(ruslan_home, initial_env={})

    assert env.get("RUSLAN_AGENT_TIMEOUT") == "1800"
    assert env.get("RUSLAN_AGENT_TIMEOUT_WARNING") == "900"


def test_config_display_busy_input_mode_wins_over_stale_env(ruslan_home: Path) -> None:
    _write_config(ruslan_home, display_cfg={"busy_input_mode": "interrupt"})
    _write_env(ruslan_home, {"RUSLAN_GATEWAY_BUSY_INPUT_MODE": "queue"})

    env = _run_gateway_import(ruslan_home, initial_env={})

    assert env.get("RUSLAN_GATEWAY_BUSY_INPUT_MODE") == "interrupt"


def test_config_display_busy_text_mode_wins_over_stale_env(ruslan_home: Path) -> None:
    _write_config(ruslan_home, display_cfg={"busy_text_mode": "queue"})
    _write_env(ruslan_home, {"RUSLAN_GATEWAY_BUSY_TEXT_MODE": "interrupt"})

    env = _run_gateway_import(ruslan_home, initial_env={})

    assert env.get("RUSLAN_GATEWAY_BUSY_TEXT_MODE") == "queue"


def test_config_timezone_wins_over_stale_env(ruslan_home: Path) -> None:
    _write_config(ruslan_home, timezone="America/Los_Angeles")
    _write_env(ruslan_home, {"RUSLAN_TIMEZONE": "UTC"})

    env = _run_gateway_import(ruslan_home, initial_env={})

    assert env.get("RUSLAN_TIMEZONE") == "America/Los_Angeles"


def test_env_value_survives_when_config_omits_key(ruslan_home: Path) -> None:
    ...
    """
    _write_config(ruslan_home, agent_cfg={})  # no max_turns
    _write_env(ruslan_home, {"RUSLAN_MAX_ITERATIONS": "123"})
