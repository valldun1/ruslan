...
def adapter(tmp_path):
    ...
    ~/.ruslan/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.ruslan/...)
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the Ruslan host. The relay flattens Chat
...
class TestSend:
    ...
    async def test_send_clarify_posts_choice_card(self, adapter):
        ...
        assert buttons[0]["onClick"]["action"]["function"] == "ruslan_clarify"
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'Ruslan is thinking…' stuck in chat. Race fix via
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/ruslan-chat"
            ),
        ...
        monkeypatch.setattr("ruslan_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("ruslan_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("ruslan_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "ruslan_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "ruslan_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "ruslan_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "ruslan_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
