...
def test_kanban_notifier_dedupes_board_slugs_pointing_to_same_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("RUSLAN_KANBAN_DB", str(db_path))
    kb.init_db()
...
def test_kanban_db_path_is_test_isolated_from_real_home():
    ruslan_home = Path(kb.kanban_home())
    production_db = Path.home() / ".ruslan" / "kanban.db"
    ...
    assert kb.kanban_db_path().resolve().is_relative_to(ruslan_home.resolve())
    assert kb.kanban_db_path().resolve() != production_db.resolve()
