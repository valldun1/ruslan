...
closure the PR changed, against a real temp ``RUSLAN_HOME``.
"""
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "ruslan_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_ruslan_home() / config.yaml`` — point it here.
    monkeypatch.setattr("ruslan_constants.get_ruslan_home", lambda: ruslan_home)
    monkeypatch.setattr("ruslan_cli.config.get_ruslan_home", lambda: ruslan_home)
    return cfg_path
