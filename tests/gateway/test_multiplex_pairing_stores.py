...
def test_secondary_profile_pairing_stores_created(tmp_path, monkeypatch):
    ...
    with patch("ruslan_cli.profiles.profiles_to_serve", return_value=[
        ("coder", tmp_path / ".ruslan" / "profiles" / "coder"),
    ]), patch("ruslan_cli.profiles.get_active_profile_name", return_value="default"):
...
def test_pairing_store_scoped_to_profile_dir(tmp_path, monkeypatch):
    ...
    with patch("ruslan_cli.profiles.profiles_to_serve", return_value=[
        ("ops", tmp_path / ".ruslan" / "profiles" / "ops"),
    ]), patch("ruslan_cli.profiles.get_active_profile_name", return_value="default"):
