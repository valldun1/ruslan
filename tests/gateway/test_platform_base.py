...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.ruslan/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("RUSLAN_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("RUSLAN_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("RUSLAN_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("RUSLAN_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        self._patch_roots(monkeypatch)
        monkeypatch.setenv("RUSLAN_KANBAN_HOME", str(tmp_path / "ruslan"))
        monkeypatch.setenv("RUSLAN_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "ruslan" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("RUSLAN_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("RUSLAN_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("RUSLAN_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("RUSLAN_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("RUSLAN_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("RUSLAN_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.ruslan/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_ruslan_credentials(self, tmp_path, monkeypatch):
        """~/.ruslan/.env and ~/.ruslan/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        ruslan_dir = fake_home / ".ruslan"
        ruslan_dir.mkdir(parents=True)
        env_file = ruslan_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._RUSLAN_HOME",
            ruslan_dir,
        )
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.ruslan/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        ruslan_dir = fake_home / ".ruslan"
        (ruslan_dir / "mcp-tokens").mkdir(parents=True)
        secret = ruslan_dir / rel
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._RUSLAN_ROOT",
            ruslan_dir,
        )

        assert BasePlatformAdapter.validate_media_delivery_path(str(secret)) is None

    def test_denylist_blocks_ruslan_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        ruslan_dir = fake_home / ".ruslan"
        ruslan_dir.mkdir(parents=True)
        config_file = ruslan_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_ruslan_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Ruslan root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".ruslan" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        ruslan_root = fake_home / ".ruslan"
        config_file = ruslan_root / "config.yaml"
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the RUSLAN_HOME root (google_token.json)
        ...
        ruslan_dir = fake_home / ".ruslan"
        ruslan_dir.mkdir(parents=True)
        token = ruslan_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._RUSLAN_HOME", ruslan_dir)
        monkeypatch.setattr("gateway.platforms.base._RUSLAN_ROOT", ruslan_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.ruslan/pairing/ (platform pairing tokens) are
    ...
    def test_ruslan_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        ruslan_dir = fake_home / ".ruslan"
        cache_dir = ruslan_dir / "cache" / "documents"
    ...
    def test_denylist_blocks_non_cache_file_under_ruslan_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.ruslan
        ...
        ruslan_dir = fake_home / ".ruslan"
        ruslan_dir.mkdir(parents=True)
        artifact = ruslan_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting RUSLAN_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``RUSLAN_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("RUSLAN_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_ruslan_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.ruslan/.env`` stays blocked under the $HOME exception — it is a
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose RUSLAN_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.ruslan) emits
        ...
        ruslan_root = denied_root / ".ruslan"
        prof_cache = ruslan_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._RUSLAN_ROOT", ruslan_root
        )
    ...
    def test_profile_scoped_credential_still_blocked_under_root(self, tmp_path, monkeypatch):
        ...
        denied_root = tmp_path / "root"
        ruslan_root = denied_root / ".ruslan"
        prof_dir = ruslan_root / "profiles" / "myprof"
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"RUSLAN_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("RUSLAN_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"RUSLAN_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "RUSLAN_HUMAN_DELAY_MODE": "natural",
            "RUSLAN_HUMAN_DELAY_MIN_MS": "oops",
            "RUSLAN_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "RUSLAN_HUMAN_DELAY_MODE": "custom",
            "RUSLAN_HUMAN_DELAY_MIN_MS": "100",
            "RUSLAN_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"RUSLAN_MEDIA_DELIVERY_STRICT": "1",
                                     "RUSLAN_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("RUSLAN_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Ruslan
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../ruslan/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.ruslan/cache/media/sensitive_host_path_abc123.bin"
