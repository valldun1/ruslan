...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("ruslan_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("ruslan_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("ruslan_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/ruslan/profiles/from-source")
                    result = mock_runner._resolve_profile_home_for_source(discord_source)
                    
                    assert result == Path("/ruslan/profiles/from-source")
    ...
    def test_routing_wins_over_active_profile(self, mock_runner, discord_source):
        ...
        with patch("ruslan_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("ruslan_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("ruslan_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/ruslan/profiles/routed")
                    ...
                    assert result == Path("/ruslan/profiles/routed")
    ...
    def test_active_profile_fallback(self, mock_runner, discord_source):
        ...
        with patch("ruslan_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("ruslan_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/ruslan/profiles/active")
                ...
                assert result == Path("/ruslan/profiles/active")
    ...
    def test_default_fallback_when_no_active(self, mock_runner, discord_source):
        ...
        with patch("ruslan_cli.profiles.get_active_profile_name", return_value=None):
            with patch("ruslan_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/ruslan")
                
                mock_runner._profile_name_for_source = MagicMock(return_value=None)
                
                result = mock_runner._resolve_profile_home_for_source(discord_source)
                
                assert result == Path("/ruslan")
...
class TestMissingProfileWarning:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("ruslan_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("ruslan_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/ruslan/profiles/nonexistent")
                with patch("ruslan_cli.profiles.profile_exists", return_value=False):
                    with patch("ruslan_constants.get_ruslan_home", return_value=Path("/ruslan")):
                        with caplog.at_level(logging.WARNING):
                            result = mock_runner._resolve_profile_home_for_source(discord_source)
                            
                            # Should fall back to global RUSLAN_HOME
                            assert result == Path("/ruslan")
    ...
    def test_existing_profile_no_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("ruslan_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("ruslan_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/ruslan/profiles/existing")
                with patch("ruslan_cli.profiles.profile_exists", return_value=True):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
                        
                        assert result == Path("/ruslan/profiles/existing")
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("ruslan_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("ruslan_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("ruslan_constants.get_ruslan_home", return_value=Path("/ruslan")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
    ...
    def test_exception_with_no_profile_name(self, mock_runner, discord_source, caplog):
        ...
        with patch("ruslan_cli.profiles.get_active_profile_name", return_value=None):
            with patch("ruslan_cli.profiles.get_profile_dir", side_effect=RuntimeError("Filesystem error")):
                with patch("ruslan_constants.get_ruslan_home", return_value=Path("/ruslan")):
                    mock_runner._profile_name_for_source = MagicMock(return_value=None)
