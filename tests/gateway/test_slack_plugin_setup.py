...
``ruslan_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``ruslan_cli.config`` (get_env_value / save_env_value) and
``ruslan_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch those
source modules.
"""
import ruslan_cli.config as config_mod
import ruslan_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved):
    ...
    # Manifest writing reaches out to ruslan_cli.slack_cli + filesystem; stub it.
    import ruslan_cli.slack_cli as slack_cli_mod
