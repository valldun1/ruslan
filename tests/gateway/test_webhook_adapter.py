...
class TestPayloadFilters:
    ...
    async def test_filter_accepts_nested_any_and_in_file(self, tmp_path, monkeypatch):
        """Nested any groups can match dynamic watchlists under RUSLAN_HOME."""
                        ...
                        "any": [
                            {
                                ...
                                "in_file": "~/.ruslan/data/watchlist.json",
                            },
                            {
    ...
    @pytest.mark.asyncio
    async def test_script_tilde_ruslan_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        """~/.ruslan/scripts paths must resolve through RUSLAN_HOME for profiles."""
        monkeypatch.setenv("RUSLAN_HOME", str(tmp_path))
        scripts = tmp_path / "scripts"
        scripts.mkdir()
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.ruslan/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)
