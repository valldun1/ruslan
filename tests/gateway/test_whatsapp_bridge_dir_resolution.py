...
(/opt/ruslan/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable RUSLAN_HOME location instead.
"""
import importlib
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "ruslan_constants.get_ruslan_home", lambda: ruslan_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into RUSLAN_HOME.
    assert not (ruslan_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_ruslan_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable RUSLAN_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = ruslan_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the RUSLAN_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    ruslan_home = tmp_path / "ruslan_home"
    mirror = ruslan_home / "scripts" / "whatsapp-bridge"
