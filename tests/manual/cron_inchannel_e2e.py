...
Uses a throwaway RUSLAN_HOME so it never touches ~/.ruslan. No real names.
"""
...
def _fresh_home():
    """Point RUSLAN_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_ruslan_home() at import time)."""
...
# Import AFTER RUSLAN_HOME is set.
...
# a different get_ruslan_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"RUSLAN_HOME (throwaway): {HOME}")
