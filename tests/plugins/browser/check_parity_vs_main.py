...
Run from the PR worktree:
    ...
    cd ~/.ruslan/ruslan-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.ruslan/ruslan-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.ruslan/ruslan-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a ruslan-agent checkout"
)
...
# Isolated RUSLAN_HOME for the config write.
