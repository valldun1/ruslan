...
def test_moa_slots_routed_through_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "ruslan_cli.runtime_provider.resolve_runtime_provider", fake_resolve
    )
...
def test_moa_slot_runtime_falls_back_on_resolution_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "ruslan_cli.runtime_provider.resolve_runtime_provider", boom
    )
...
def test_reference_messages_drops_system_but_renders_tools_as_text():
    ...
    messages = [
        {"role": "system", "content": "huge ruslan system prompt"},
    ...
    assert all("huge ruslan system prompt" not in m["content"] for m in view)
...
def test_reference_messages_flattens_cache_decorated_content():
    ...
    plain = [
        {"role": "system", "content": "ruslan system prompt"},
        {"role": "user", "content": "Can we get codex usage resets into ruslan?"},
    ]
