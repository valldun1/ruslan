...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    ruslan_state = ModuleType("ruslan_state")
    ruslan_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "ruslan_state", ruslan_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Ruslan"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Ruslan"
