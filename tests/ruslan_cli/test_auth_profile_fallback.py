...
When ``RUSLAN_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.ruslan/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.ruslan            (has its own auth.json fixture)
    * Profile     -> tmp_path/.ruslan/profiles/coder   (active, RUSLAN_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Ruslan is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from ruslan_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    ruslan_home = tmp_path / "classic"
    ruslan_home.mkdir()
    monkeypatch.setenv("RUSLAN_HOME", str(ruslan_home))

    _write(ruslan_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (RUSLAN_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.ruslan as the "real home" — which is NOT equal
    # to the RUSLAN_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    ruslan_home = tmp_path / "classic"
    ...
    _write(ruslan_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from ruslan_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: RUSLAN_HOME is set to a custom path that is NOT under
    # ~/.ruslan/profiles/ — get_default_ruslan_root() returns RUSLAN_HOME
...
def test_provider_state_transaction_locks_global_fallback_before_use(
    ...
    import ruslan_cli.auth as auth
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_ruslan_home_override(profile_b)
        try:
