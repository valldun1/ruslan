...
def test_xai_oauth_authorize_url_includes_referrer_ruslan_agent():
    """Attribution: xAI's OAuth server can identify Ruslan-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "ruslan-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "ruslan auth add xai-oauth" in body
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`ruslan auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `ruslan auth remove`.
