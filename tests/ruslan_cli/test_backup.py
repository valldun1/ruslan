...
class TestRunPreUpdateBackup:
    ...
    def test_default_enabled_creates_backup(self, ruslan_home, capsys):
        """With the new safe default (``pre_update_backup: true``), every
        ``ruslan update`` creates a backup before any destructive step
        runs — the cost is a few minutes of zip time vs. the alternative
        of silent total data loss of ``~/.ruslan/`` observed in #48200
