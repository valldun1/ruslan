...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (RUSLAN_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via RUSLAN_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in RUSLAN_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from ruslan_cli.providers import RUSLAN_OVERLAYS
        assert "bedrock" in RUSLAN_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from ruslan_cli.providers import RUSLAN_OVERLAYS
        assert RUSLAN_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from ruslan_cli.providers import RUSLAN_OVERLAYS
        assert RUSLAN_OVERLAYS["bedrock"].auth_type == "aws_sdk"
