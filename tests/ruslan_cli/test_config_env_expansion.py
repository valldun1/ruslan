...
import pytest
from ruslan_cli.config import _expand_env_vars, load_config
...
class TestLoadConfigExpansion:
    def test_load_config_expands_env_vars(self, tmp_path, monkeypatch):
        ...
        # ruslan_cli.config, making string-target monkeypatches hit a different
...
class TestLoadConfigCacheEnvStaleness:
    ...
    environment (#58514): a load before load_ruslan_dotenv() runs, or an env
...
class TestLoadCliConfigExpansion:
    ...
    def test_cli_config_expands_auxiliary_api_key(self, tmp_path, monkeypatch):
        ...
        # Patch the ruslan home so load_cli_config finds our test config
        monkeypatch.setattr("cli._ruslan_home", tmp_path)
