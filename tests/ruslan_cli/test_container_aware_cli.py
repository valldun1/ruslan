...
def container_env(tmp_path, monkeypatch):
    """Set up a fake RUSLAN_HOME with .container-mode file."""
    ruslan_home = tmp_path / ".ruslan"
    ruslan_home.mkdir()
    monkeypatch.setenv("RUSLAN_HOME", str(ruslan_home))
    monkeypatch.delenv("RUSLAN_DEV", raising=False)

    container_mode = ruslan_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=ruslan-agent\n"
        "exec_user=ruslan\n"
        "ruslan_bin=/data/current-package/bin/ruslan\n"
    )
    return ruslan_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("ruslan_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "ruslan-agent"
    assert info["exec_user"] == "ruslan"
    assert info["ruslan_bin"] == "/data/current-package/bin/ruslan"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("ruslan_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_ruslan_dev(container_env, monkeypatch):
    """Returns None when RUSLAN_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("RUSLAN_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_ruslan_dev_zero(container_env, monkeypatch):
    """RUSLAN_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("RUSLAN_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ruslan_home = Path(tmpdir) / ".ruslan"
        ruslan_home.mkdir()
        (ruslan_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("ruslan_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_ruslan_home": lambda: ruslan_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("RUSLAN_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=ruslan-custom\n"
        "exec_user=myuser\n"
        "ruslan_bin=/opt/ruslan/bin/ruslan\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "ruslan-custom"
    assert info["exec_user"] == "myuser"
    assert info["ruslan_bin"] == "/opt/ruslan/bin/ruslan"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from ruslan_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "ruslan"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "ruslan-agent" in cmd
    assert "/data/current-package/bin/ruslan" in cmd
    assert "chat" in cmd
