"""Regression tests: `ruslan dashboard` validates RUSLAN_WEB_DIST before serving.

A custom RUSLAN_WEB_DIST without --skip-build previously skipped BOTH the
...
@pytest.fixture()
def main_mod():
    import ruslan_cli.main as main
    return main
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """RUSLAN_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("RUSLAN_WEB_DIST", str(empty_dist))
    ...
    assert "RUSLAN_WEB_DIST" in out and str(empty_dist) in out


def test_env_dist_with_index_starts_server(main_mod, monkeypatch, tmp_path):
    """A valid RUSLAN_WEB_DIST (has index.html) proceeds to start_server
    without building."""
    _wire_common(main_mod, monkeypatch)
    dist = tmp_path / "dist"
    dist.mkdir()
    (dist / "index.html").write_text("<html></html>", encoding="utf-8")
    monkeypatch.setenv("RUSLAN_WEB_DIST", str(dist))
...
def test_env_dist_tilde_expanded_for_web_server(main_mod, monkeypatch, tmp_path):
    """A '~/...' RUSLAN_WEB_DIST must be written back expanded so
    ...
    monkeypatch.setenv("RUSLAN_WEB_DIST", "~/mydist")
    ...
    import os
    assert os.environ["RUSLAN_WEB_DIST"] == str(dist)
