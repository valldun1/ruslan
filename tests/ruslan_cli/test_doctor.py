...
import ruslan_cli.doctor as doctor
import ruslan_cli.gateway as gateway_cli
from ruslan_cli import doctor as doctor_mod
from ruslan_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `ruslan doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "RUSLAN_HOME", ruslan_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("RUSLAN_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("RUSLAN_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_ruslan_home(self, tmp_path, provider=""):
        """Create a minimal RUSLAN_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_ruslan_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "RUSLAN_HOME", home)
        ...
        try:
            from ruslan_cli import auth as _auth_mod
...
def test_run_doctor_accepts_ruslan_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp RUSLAN_HOME.

        ``run_doctor`` reads the module-level ``RUSLAN_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("RUSLAN_HOME")`` alone
        leaves doctor probing the REAL ~/.ruslan. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from ruslan_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("ruslan_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "ruslan_cli.doctor", raising=False)
        from ruslan_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale RUSLAN_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"RUSLAN_MAX_ITERATIONS={ghost}\n")
        (ruslan_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "RUSLAN_HOME", ruslan_home)
        monkeypatch.setattr(doctor_mod, "get_ruslan_home", lambda: ruslan_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("RUSLAN_MAX_ITERATIONS", str(os_environ_value))
        else:
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale RUSLAN_MAX_ITERATIONS" in out
        env_after = (ruslan_home / ".env").read_text(encoding="utf-8")
        assert "RUSLAN_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`ruslan doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Ruslan install.
