"""Tests for the ``ruslan hooks`` CLI subcommand."""
...
from ruslan_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("RUSLAN_ACCEPT_HOOKS", raising=False)
...
class TestHooksTest:
    def test_synthetic_payload_matches_production_shape(self, tmp_path):
        """`ruslan hooks test` must feed the script stdin in the same
        shape invoke_hook() would at runtime.  Prior to this fix,
        run_once bypassed _serialize_payload and the two paths diverged —
        scripts tested with `ruslan hooks test` saw different top-level
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `ruslan hooks doctor` used to run every
