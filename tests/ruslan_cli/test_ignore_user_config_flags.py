"""Tests for --ignore-user-config and --ignore-rules flags on `ruslan chat`.
...
files. In Ruslan the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.ruslan/config.yaml`` in ``load_cli_config()``
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("RUSLAN_IGNORE_USER_CONFIG", "RUSLAN_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``RUSLAN_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<ruslan_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._ruslan_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        """With RUSLAN_IGNORE_USER_CONFIG=1, user config.yaml is ignored.
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``ruslan_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("RUSLAN_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("RUSLAN_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("RUSLAN_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("RUSLAN_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("RUSLAN_IGNORE_USER_CONFIG") == "1"
        assert "RUSLAN_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "RUSLAN_IGNORE_USER_CONFIG" not in os.environ
        assert "RUSLAN_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real ruslan parser accepts both flags."""
        ...
        import inspect
        import ruslan_cli.main as hm
        src = inspect.getsource(hm)
        assert "RUSLAN_IGNORE_USER_CONFIG" in src
        assert "RUSLAN_IGNORE_RULES" in src
