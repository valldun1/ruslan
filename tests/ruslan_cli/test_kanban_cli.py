"""Tests for the kanban CLI surface (ruslan_cli.kanban)."""
...
from ruslan_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`ruslan kanban list --session <id>` filters by the originating
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="ruslan", add_help=False)
...
def test_kanban_in_autocomplete_table():
    from ruslan_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    kb.create_board("alpha")
    kb.create_board("beta")
    monkeypatch.setenv("RUSLAN_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("RUSLAN_KANBAN_BOARD") == "beta"
