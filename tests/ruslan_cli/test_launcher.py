"""Tests for the top-level `./ruslan` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./ruslan` should use `ruslan_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "ruslan"
    called = []

    fake_main_module = types.ModuleType("ruslan_cli.main")

    def fake_main():
        called.append("ruslan_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "ruslan_cli.main", fake_main_module)
