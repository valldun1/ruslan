...
ruslan_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `ruslan config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_ruslan_home = get_ruslan_home()`` at import time (module
    singleton), so monkeypatching RUSLAN_HOME after import doesn't move it.
    ...
    this only matters for tests that swap RUSLAN_HOME.)
    """
    import cli

    cli._ruslan_home = home
