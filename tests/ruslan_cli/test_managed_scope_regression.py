...
def ruslan_home(tmp_path, monkeypatch):
    ...
    # /etc/ruslan on the dev/CI box can't influence the test.
    monkeypatch.setenv("RUSLAN_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(ruslan_home, monkeypatch):
    from ruslan_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(ruslan_home, monkeypatch):
    from ruslan_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(ruslan_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_ruslan_dotenv(ruslan_home=str(home))
