...
Execution via the ``RUSLAN_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``ruslan_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.ruslan/plugins/`` source via
   ``os.environ.get("RUSLAN_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``ruslan_cli.web_server._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — RUSLAN_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.ruslan/plugins``
        and isolate the user-plugins dir to an empty tmp tree."""
        monkeypatch.setenv("RUSLAN_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("RUSLAN_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("RUSLAN_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"RUSLAN_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestDiscoveryScrubsApiField:
    ...
    def test_project_safe_api_path_is_scrubbed(self, tmp_path, monkeypatch):
        monkeypatch.setenv("RUSLAN_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
        monkeypatch.setenv("RUSLAN_ENABLE_PROJECT_PLUGINS", "1")
    ...
    def test_bundled_safe_api_path_survives(self, tmp_path, monkeypatch):
        ...
        ruslan_home.mkdir()
        monkeypatch.setenv("RUSLAN_BUNDLED_PLUGINS", str(tmp_path / "bundled"))
        dashboard = _write_plugin_manifest(
            tmp_path / "bundled",
            "safe-bundled",
            {
...
class TestEndToEndPocBlocked:
    ...
    ``RUSLAN_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("RUSLAN_ENABLE_PROJECT_PLUGINS", "0")
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "ruslan_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "ruslan_dashboard_plugin_evil" not in sys.modules
