"""Tests for the per-profile Projects store (ruslan_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Ruslan Agent", folders=["/tmp/ruslan"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "ruslan-agent"
    assert proj.name == "Ruslan Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/ruslan"
    assert [f.path for f in proj.folders] == ["/tmp/ruslan"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "ruslan-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Ruslan Agent")
    pdb.create_project(conn, name="Ruslan Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["ruslan-agent", "ruslan-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' RUSLAN_HOME.
...
def test_db_path_under_ruslan_home():
    # Resolves under RUSLAN_HOME (set by the autouse isolation fixture).
