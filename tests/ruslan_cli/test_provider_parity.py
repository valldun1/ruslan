...
the SAME provider universe as ``ruslan model`` (the CLI/TUI picker).
...
def test_every_ruslan_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `ruslan model` universe."""
    ...
    assert not missing, (
        "providers shown in `ruslan model` but not configurable in the desktop "
