"""Tests for `ruslan chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("RUSLAN_SAFE_MODE", "RUSLAN_IGNORE_USER_CONFIG", "RUSLAN_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in ruslan_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("RUSLAN_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under RUSLAN_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("RUSLAN_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("RUSLAN_SAFE_MODE", raising=False)
        from ruslan_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under RUSLAN_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `ruslan chat`."""
