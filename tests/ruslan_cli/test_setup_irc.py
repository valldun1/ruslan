"""Tests for IRC gateway configuration via `ruslan setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Ruslan install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Ruslan install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#ruslan")
            monkeypatch.setenv("IRC_NICKNAME", "ruslan-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `ruslan setup gateway` experience with IRC present."""
