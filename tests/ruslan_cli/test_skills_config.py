"""Tests for ruslan_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("ruslan_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from ruslan_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("ruslan_cli.config.load_config")
    ...
    @patch("ruslan_cli.config.load_config")
    @patch.dict("os.environ", {"RUSLAN_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """RUSLAN_SESSION_PLATFORM should be used when RUSLAN_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_ruslan_platform_takes_precedence(self, tmp_path, monkeypatch):
        """RUSLAN_PLATFORM should win over RUSLAN_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from ruslan_cli.skills_config import _get_categories
        skills = [
