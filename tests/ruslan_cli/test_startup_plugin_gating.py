...
``ruslan_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``ruslan --help``, ``ruslan version``,
``ruslan logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``ruslan --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from ruslan_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["ruslan", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ruslan", "-w", "--tui"], None),
        (["ruslan", "version"], "version"),
        (["ruslan", "--tui", "chat"], "chat"),
        (["ruslan", "-w", "logs"], "logs"),
        (["ruslan", "chat", "hello world"], "chat"),
        (["ruslan", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["ruslan", "-m", "gpt5", "chat"], "chat"),
        (["ruslan", "--model", "gpt5", "chat", "hi"], "chat"),
        (["ruslan", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["ruslan", "-z", "hello world"], None),
        (["ruslan", "-z", "hello", "chat"], "chat"),
        (["ruslan", "--model=gpt5", "chat"], "chat"),     # inline form
        (["ruslan", "--", "chat"], "chat"),               # -- terminator
        (["ruslan", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["ruslan", "some-plugin-cmd"], "some-plugin-cmd"),
        (["ruslan", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["ruslan"],                          # bare → chat
        ["ruslan", "--help"],                # top-level help
        ["ruslan", "-h"],
        ["ruslan", "version"],               # known built-in
        ["ruslan", "logs"],
        ["ruslan", "gateway", "run"],
        ["ruslan", "--tui"],
        ["ruslan", "-w", "--tui"],
        ["ruslan", "chat", "hi"],
        ["ruslan", "help"],                  # accepted built-in-ish
        ["ruslan", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["ruslan", "meet", "join"],          # potential google_meet plugin
        ["ruslan", "honcho", "status"],      # potential memory plugin
        ["ruslan", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``ruslan help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"ruslan_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
