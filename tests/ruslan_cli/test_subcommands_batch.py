...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="ruslan")
...
def test_config_get_unset_subcommands_parse():
    """`ruslan config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="ruslan")
    sub = parser.add_subparsers(dest="command")
    handler = _h("config")
...
# ── deprecated `ruslan login` fails gracefully, not with argparse error ────
#
# `ruslan login` is a removed command; its handler (`login_command` in
# `ruslan_cli/auth.py`) prints a deprecation notice pointing at `ruslan auth` /
# `ruslan model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `ruslan --help`.
