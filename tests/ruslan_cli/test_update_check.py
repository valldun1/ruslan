"""Tests for the update check mechanism in ruslan_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "ruslan-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("ruslan_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    """
    import ruslan_cli.banner as banner

    # No local git checkout -> the PyPI path is exercised (pip-install class).
    fake_banner = tmp_path / "ruslan_cli" / "banner.py"
    ...
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path))
    monkeypatch.delenv("RUSLAN_REVISION", raising=False)
    with patch("ruslan_cli.banner.subprocess.run") as mock_run, \
         patch("ruslan_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path))
    with patch("ruslan_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path))
    with patch("ruslan_cli.banner.subprocess.run") as mock_run:
        with patch("ruslan_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when RUSLAN_HOME has no git repo."""
    import ruslan_cli.banner as banner
    ...
    # Point RUSLAN_HOME at a temp dir with no ruslan-agent/.git
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path))
    with patch("ruslan_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    RUSLAN_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `ruslan update` correctly refusing to run there. The guard
    ...
    with patch("ruslan_cli.config.detect_install_method", return_value="docker"), \
         patch("ruslan_cli.banner.subprocess.run") as mock_run, \
         patch("ruslan_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("ruslan_cli.config.detect_install_method", return_value="pip"), \
         patch("ruslan_cli.banner.subprocess.run") as mock_run, \
         patch("ruslan_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from ruslan_cli.main import _invalidate_update_cache

    # Build a fake ~/.ruslan with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"RUSLAN_HOME": str(default_home)}):
