...
def forced_files_client(monkeypatch, tmp_path):
    root = tmp_path / "data"
    monkeypatch.setenv("RUSLAN_DASHBOARD_FILES_ROOT", str(root))
...
def local_files_client(monkeypatch, tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.delenv("RUSLAN_DASHBOARD_FILES_ROOT", raising=False)
...
def test_gated_local_mode_still_defaults_to_home(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("RUSLAN_HOME", str(home / ".ruslan"))
...
def test_other_credential_store_basenames_blocked(forced_files_client):
    ...
    point the managed root at RUSLAN_HOME itself (#57505), which contains
