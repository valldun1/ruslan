...
Before the fix, ``ruslan whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``ruslan gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    ruslan = home / ".ruslan"
    ruslan.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("RUSLAN_HOME", str(ruslan))
...
def _env_value(ruslan_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from ruslan_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("ruslan_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``ruslan whatsapp`` with an existing paired session and
