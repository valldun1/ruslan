...
# Resolve the skill's scripts dir across layouts: standalone dev repo (tests/) and ruslan-agent
...
_CANDIDATES = [
    ...
    _HERE.parent.parent.parent.joinpath(*_REL),          # ruslan-agent (tests/skills/)
]
...
def test_cdp_default_profile_uses_ruslan_home():
    prev = os.environ.get("RUSLAN_HOME")
    with tempfile.TemporaryDirectory() as d:
        os.environ["RUSLAN_HOME"] = d
        try:
...
def test_dotenv_env_fills_missing_creds_and_shell_wins():
    prev_home = os.environ.get("RUSLAN_HOME")
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            for k, v in (("RUSLAN_HOME", prev_home), ("BROWSERBASE_API_KEY", prev_key)):
                if v is None:
