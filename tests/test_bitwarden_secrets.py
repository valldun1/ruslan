...
binary download are exercised manually by `ruslan secrets bitwarden
...
def ruslan_home(tmp_path, monkeypatch):
    """Point Ruslan at an isolated home directory."""
    home = tmp_path / ".ruslan"
    home.mkdir()
    monkeypatch.setenv("RUSLAN_HOME", str(home))
    # Some modules cache get_ruslan_home; clear if needed.
    import ruslan_constants
    if hasattr(ruslan_constants, "_RUSLAN_HOME_CACHE"):
        ruslan_constants._RUSLAN_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(ruslan_home, monkeypatch):
...
def test_install_bws_happy_path(ruslan_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(ruslan_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(ruslan_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from ruslan_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under ruslan_home/cache/."""
    home = tmp_path / ".ruslan"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
