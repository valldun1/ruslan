"""Guards for the multi-container Ruslan WebUI install surface."""
...
def test_setup_uses_temporary_outputs_when_source_tree_is_read_only(
    ...
    """WebUI installs from read-only /opt/ruslan must not write build metadata."""
    ...
    assert Path(build_cmd.build_base).name.startswith("ruslan-agent-build")
    ...
    assert Path(source_relative_build.build_base).name.startswith("ruslan-agent-build")
    ...
    assert Path(egg_info_cmd.egg_base).name.startswith("ruslan-agent-egg-info")
