"""Tests for ruslan_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Ruslan entry point
(ruslan, ruslan-agent, ruslan-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Ruslan entry point imports ruslan_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported ruslan_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("ruslan_bootstrap", None)
    import ruslan_bootstrap  # noqa: WPS433
    return ruslan_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Ruslan CLI
...
class TestEntryPointsImportBootstrap:
    """Every Ruslan entry point must import ruslan_bootstrap as its
    ...
    # Entry points that invoke Ruslan as a process.  Each one must
    # import ruslan_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "ruslan_cli/main.py",   # ruslan CLI (console_script)
        "run_agent.py",          # ruslan-agent (console_script)
        "acp_adapter/entry.py",  # ruslan-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import ruslan_bootstrap' and that
        ...
        half-finished ``ruslan update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``ruslan_bootstrap`` as a top-level module) leaves ruslan
        ...
        """
        # Resolve relative to the ruslan-agent repo root.  Tests live
        # at tests/test_ruslan_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # ruslan start even when ruslan_bootstrap hasn't been
        ...
        assert first_import_name == "ruslan_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'ruslan_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import ruslan_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Ruslan's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("RUSLAN_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["RUSLAN_PYTHON_SRC_ROOT"] = env
            elif "RUSLAN_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["RUSLAN_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/ruslan")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("RUSLAN_PYTHON_SRC_ROOT", None)
            else:
                os.environ["RUSLAN_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/ruslan", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Ruslan root.  The guard must relocate Ruslan to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/ruslan"])
        assert result[0] == "/opt/ruslan"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Ruslan root.
        assert result.index("/opt/ruslan") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/ruslan", "/opt/ruslan", ""])
        assert result.count("/opt/ruslan") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["RUSLAN_PYTHON_SRC_ROOT"] = "/env/ruslan"
            hb.harden_import_path()
            assert sys.path[0] == "/env/ruslan"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("RUSLAN_PYTHON_SRC_ROOT", None)
