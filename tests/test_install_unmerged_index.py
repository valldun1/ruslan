...
The ``ruslan update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m ruslan_cli.main gateway run`` — image name
    ``pythonw``, not ``ruslan.exe`` — so the ``taskkill /IM ruslan.exe`` guard
    ...
    # The ruslan.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM ruslan.exe' in text
