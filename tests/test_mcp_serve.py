"""
Tests for mcp_serve — Ruslan MCP server.
...
def _isolate_ruslan_home(tmp_path, monkeypatch):
    """Redirect RUSLAN_HOME to a temp directory."""
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path))
    try:
        import ruslan_constants
        monkeypatch.setattr(ruslan_constants, "get_ruslan_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking ruslan_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <RUSLAN_HOME>/state.db for its mtime gate; the autouse
        # fixture points RUSLAN_HOME at tmp_path.
        db_path = tmp_path / "state.db"
