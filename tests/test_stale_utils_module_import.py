...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.ruslan/ruslan-agent/utils.py)
...
2. ``ruslan update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``ruslan_cli/main.py`` (the ``ruslan update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``ruslan_constants`` after the
