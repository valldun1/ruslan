"""
Tests for timezone support (ruslan_time module + integration points).
...
def _reset_ruslan_time_cache():
    """Reset the ruslan_time module cache (replacement for removed reset_cache)."""
    ruslan_time._cached_tz = None
    ruslan_time._cached_tz_name = None
    ruslan_time._cache_resolved = False
...
class TestRuslanTimeNow:
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["RUSLAN_TIMEZONE"] = "Asia/Kolkata"
        result = ruslan_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["RUSLAN_TIMEZONE"] = "UTC"
        result = ruslan_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["RUSLAN_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="ruslan_time"):
            result = ruslan_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["RUSLAN_TIMEZONE"] = "Europe/London"
        tz = ruslan_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["RUSLAN_TIMEZONE"] = "Not/A/Timezone"
        tz = ruslan_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When RUSLAN_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) RUSLAN_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("RUSLAN_TIMEZONE=" + os.environ.get("RUSLAN_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "RUSLAN_TIMEZONE=NOT_SET" in result["output"], (
            "RUSLAN_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When RUSLAN_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("RUSLAN_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=ruslan_tz) which shifted
        absolute time when system-local tz != Ruslan tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_ruslan_tz(self):
        """Already-aware datetimes should be normalized to Ruslan tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Ruslan tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Ruslan tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Ruslan is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Ruslan tz
        ...
        # Use a Ruslan timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _ruslan_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["RUSLAN_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_ruslan_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Ruslan timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["RUSLAN_TIMEZONE"] = "US/Eastern"
        _reset_ruslan_time_cache()
