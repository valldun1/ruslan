...
class TestToolsetConsistency:
    ...
    def test_ruslan_platforms_share_core_tools(self):
        """All ruslan-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on ruslan-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["ruslan-cli", "ruslan-telegram", "ruslan-discord", "ruslan-whatsapp", "ruslan-slack", "ruslan-signal", "ruslan-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_ruslan_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("ruslan-whatsapp")

    def test_ruslan_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("ruslan-api-server")
