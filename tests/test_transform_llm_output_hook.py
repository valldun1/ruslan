...
from ruslan_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(ruslan_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <ruslan_home>/plugins/<name> and opt it in."""
    plugin_dir = ruslan_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path / "ruslan_empty"))
