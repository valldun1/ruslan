...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("ruslan_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("ruslan_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("RUSLAN_SESSION_KEY", session_key)
        monkeypatch.setenv("RUSLAN_EXEC_ASK", "1")
        monkeypatch.delenv("RUSLAN_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("ruslan-verify-", "ruslan-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "ruslan-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/ruslan-verify-example.py",
            "rm -f /tmp/ruslan-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/ruslan-verify-example.py",
            "rm -f /tmp/nested/../ruslan-verify-example.py",
            "rm -f /tmp/./ruslan-verify-example.py",
            "rm -f /tmp//ruslan-verify-example.py",
            "rm -f /tmp/a/../../tmp/ruslan-verify-example.py",
            "rm -f /var/tmp/ruslan-verify-example.py",
            "rm -f /tmp/unrelated.py",
            "rm -f /tmp/ruslan-verify-*",
            "rm -f /tmp/ruslan-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/ruslan-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/ruslan-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\ruslan-victim\file.txt"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\ruslan-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\ruslan-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\ruslan-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
    ...
    def test_powershell_bare_remove_item_requires_approval(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            r"powershell Remove-Item -Recurse -Force C:\tmp\ruslan-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows PowerShell destructive delete"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"RUSLAN_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_ruslan_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.ruslan/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_ruslan_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $RUSLAN_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_ruslan_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$RUSLAN_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestRuslanConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.ruslan/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.ruslan/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.ruslan/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.ruslan/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.ruslan/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.ruslan/config.yaml")
        assert dangerous is True
        assert "ruslan config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.ruslan/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_ruslan_home_config(self):
    ...
    def test_sed_in_place_absolute_ruslan_home_env(self):
        env_path = get_ruslan_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_ruslan_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $RUSLAN_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.ruslan/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_ruslan_home_config(self):
        config_path = get_ruslan_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.ruslan/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_ruslan_home_env(self):
        env_path = get_ruslan_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.ruslan/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.ruslan/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.ruslan/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.ruslan/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.ruslan/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Ruslan config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_ruslan_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $RUSLAN_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.ruslan/config.yaml/.env must require approval — the
    ...
    def test_cp_to_ruslan_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.ruslan/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Ruslan-home prefixes must fold to ~/ and
    ~/.ruslan/ in dangerous-command detection.
    ...
    SSH, and Ruslan config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_ruslan_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/RUSLAN_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_ruslan_home_config_folds(self, monkeypatch):
        # Ruslan home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("RUSLAN_HOME", r"C:\Users\tester\.ruslan")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.ruslan\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(
        ...
        monkeypatch.setenv("RUSLAN_LANGUAGE", lang)
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.ruslan/ruslan-agent && source venv/bin/activate && python -m ruslan_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m ruslan_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m ruslan_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "ruslan_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart ruslan-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_ruslan_gateway_stop_detected(self):
        cmd = "ruslan gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_ruslan_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `ruslan` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "ruslan -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_ruslan_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "ruslan --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_ruslan_gateway_multiple_flags_detected(self):
        cmd = "ruslan -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_ruslan_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "ruslan -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_ruslan_gateway_start_not_flagged(self):
        cmd = "ruslan gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_ruslan_detected(self):
        """pkill targeting ruslan/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_ruslan_detected(self):
        cmd = "killall ruslan"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Ruslan security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.ruslan/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep ruslan) bypasses the pkill/killall name-matching
    ...
    def test_pkill_ruslan_still_detected(self):
    ...
    def test_kill_dollar_pidof_detected(self):
        """`kill $(pidof ruslan)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        cmd = "kill -TERM $(pidof ruslan_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()

    def test_kill_backtick_pidof_detected(self):
        cmd = "kill -9 `pidof ruslan`"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True


class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Ruslan service
    label achieves the same effect as `ruslan gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_ruslan_detected(self):
        cmd = "launchctl stop ai.ruslan.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "ruslan" in desc.lower()

    def test_launchctl_kickstart_ruslan_detected(self):
        cmd = "launchctl kickstart -k system/ai.ruslan.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_ruslan_detected(self):
        cmd = "launchctl bootout system/ai.ruslan.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_ruslan_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.ruslan.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Ruslan label is out of scope for the
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("RUSLAN_GATEWAY_SESSION", "RUSLAN_CRON_SESSION",
                      "RUSLAN_YOLO_MODE",
                      "RUSLAN_SESSION_KEY", "RUSLAN_INTERACTIVE")
        }
        os.environ.pop("RUSLAN_YOLO_MODE", None)
        os.environ.pop("RUSLAN_INTERACTIVE", None)
        # RUSLAN_CRON_SESSION takes priority over RUSLAN_GATEWAY_SESSION in
        ...
        os.environ.pop("RUSLAN_CRON_SESSION", None)
        os.environ["RUSLAN_GATEWAY_SESSION"] = "1"
        os.environ["RUSLAN_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("ruslan_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"RUSLAN_INTERACTIVE": "1"}, clear=False):
