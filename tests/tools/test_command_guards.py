...
def _clean_state():
    ...
    saved = {}
    for k in ("RUSLAN_INTERACTIVE", "RUSLAN_GATEWAY_SESSION", "RUSLAN_EXEC_ASK", "RUSLAN_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["RUSLAN_INTERACTIVE"] = "1"
        session_key = os.getenv("RUSLAN_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No RUSLAN_INTERACTIVE or RUSLAN_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
