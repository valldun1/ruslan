...
Older images made /opt/ruslan/ui-tui and /opt/ruslan/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV RUSLAN_TUI_DIR=/opt/ruslan/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R ruslan:ruslan /opt/ruslan/ui-tui" not in text
    assert "chown -R ruslan:ruslan /opt/ruslan/node_modules" not in text
