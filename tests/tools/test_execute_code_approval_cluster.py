...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad RUSLAN_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: RUSLAN_GATEWAY_SESSION set, a bound session
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setattr(A, "_YOLO_MODE_FROZEN", False)
    monkeypatch.setenv("RUSLAN_CRON_SESSION", "1")
    monkeypatch.delenv("RUSLAN_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_ruslan_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "RUSLAN_HOME": "/h", "RUSLAN_PROFILE": "p",
        "RUSLAN_CONFIG": "/c.yaml", "RUSLAN_ENV": "/e",
        # other RUSLAN_* → dropped (broad prefix removed)
        "RUSLAN_BASE_URL": "https://x", "RUSLAN_INTERACTIVE": "1",
        "RUSLAN_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("RUSLAN_HOME", "RUSLAN_PROFILE", "RUSLAN_CONFIG", "RUSLAN_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "RUSLAN_BASE_URL", "RUSLAN_INTERACTIVE", "RUSLAN_KANBAN_DB",
...
def test_env_scrub_logs_dropped_ruslan_vars(caplog):
    """Dropping a non-allowlisted, non-secret RUSLAN_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset RUSLAN_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "RUSLAN_HOME": "/h",          # allowlisted → kept, not logged
        "RUSLAN_BASE_URL": "https://x",   # dropped → logged
        "RUSLAN_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "RUSLAN_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "RUSLAN_HOME" in out and "PATH" in out
    assert "RUSLAN_BASE_URL" not in out and "RUSLAN_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "RUSLAN_BASE_URL" in msgs and "RUSLAN_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "RUSLAN_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped RUSLAN_* vars."""
    import logging
