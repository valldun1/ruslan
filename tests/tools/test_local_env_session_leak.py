"""Cross-session RUSLAN_SESSION_* leak guard for the local terminal backend.

Regression coverage for the bug where a terminal subprocess could observe a
*different concurrent session's* ``RUSLAN_SESSION_KEY`` (and the other
``RUSLAN_SESSION_*`` vars).
...
subprocess-env bridge resolves each ``RUSLAN_SESSION_*`` from the ContextVar and,
...
from tools.environments.local import _make_run_env, _sanitize_subprocess_env, ruslan_subprocess_env
...
def test_engaged_unset_contextvar_strips_foreign_session_key(monkeypatch):
    ...
    os.environ["RUSLAN_SESSION_KEY"], this task's ContextVar is unset, and the
    ...
    assert "RUSLAN_SESSION_KEY" not in env, (
        "Foreign concurrent session key leaked into subprocess env: "
        f"{env.get('RUSLAN_SESSION_KEY')!r}"
    )
...
def test_set_session_vars_engages_and_overrides_foreign_global(monkeypatch):
    ...
    assert env.get("RUSLAN_SESSION_KEY") == "agent:main:discord:group:MY_BUGS_ROOT:111"


def test_engaged_strips_all_session_vars_when_unset(monkeypatch):
    """The strip covers every RUSLAN_SESSION_* mirror, not just the key."""
    _engage()
    monkeypatch.setenv("RUSLAN_SESSION_KEY", "foreign-key")
    monkeypatch.setenv("RUSLAN_SESSION_THREAD_ID", "foreign-thread")
    monkeypatch.setenv("RUSLAN_SESSION_CHAT_ID", "foreign-chat")
    monkeypatch.setenv("RUSLAN_SESSION_USER_ID", "foreign-user")
...
def test_unengaged_process_preserves_os_environ_fallback(monkeypatch):
    ...
    Pure single-process CLI/one-shot sets RUSLAN_SESSION_* directly in os.environ
    ...
    monkeypatch.setenv("RUSLAN_SESSION_KEY", "cli-session-key")
    monkeypatch.setenv("RUSLAN_SESSION_ID", "cli-session-id")

    env = _make_run_env({})

    assert env.get("RUSLAN_SESSION_KEY") == "cli-session-key"
    assert env.get("RUSLAN_SESSION_ID") == "cli-session-id"
...
def test_engaged_explicit_empty_contextvar_clears(monkeypatch):
    ...
    """
    monkeypatch.setenv("RUSLAN_SESSION_KEY", "foreign-after-clear")
    ...
    assert env.get("RUSLAN_SESSION_KEY", "") == "", (
        f"Foreign key survived an explicit clear: {env.get('RUSLAN_SESSION_KEY')!r}"
    )
...
def test_explicit_empty_thread_id_overrides_stale_value(monkeypatch):
    ...
    no thread id binds RUSLAN_SESSION_THREAD_ID="" and that empty value must win
    over an older non-empty value left in os.environ.
    """
    monkeypatch.setenv("RUSLAN_SESSION_THREAD_ID", "stale-thread-from-prior-turn")
    ...
    assert env.get("RUSLAN_SESSION_THREAD_ID") == "", (
        "Bound-empty thread id did not override the stale value: "
        f"{env.get('RUSLAN_SESSION_THREAD_ID')!r}"
    )
    assert env.get("RUSLAN_SESSION_KEY") == "mm:chan"
...
def test_sanitize_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "RUSLAN_SESSION_KEY": "agent:main:discord:thread:FOREIGN_BG:FOREIGN_BG",
        "RUSLAN_SESSION_THREAD_ID": "FOREIGN_BG",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert "RUSLAN_SESSION_KEY" not in sanitized, (
        f"Background subprocess inherited foreign key: {sanitized.get('RUSLAN_SESSION_KEY')!r}"
    )
    assert "RUSLAN_SESSION_THREAD_ID" not in sanitized
...
def test_sanitize_subprocess_env_set_contextvar_wins_when_engaged():
    ...
    assert sanitized.get("RUSLAN_SESSION_KEY") == "agent:main:discord:group:REAL_BG:222"
...
def test_sanitize_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "RUSLAN_SESSION_KEY": "cli-bg-key",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert sanitized.get("RUSLAN_SESSION_KEY") == "cli-bg-key"


# --------------------------------------------------------------------------- #
# Non-terminal spawn surface (ruslan_subprocess_env) — sibling path
# --------------------------------------------------------------------------- #

def test_ruslan_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    """ruslan_subprocess_env (browser/ACP/CLI/TUI-host spawns) must not leak a
...
def test_ruslan_subprocess_env_bound_contextvar_wins(monkeypatch):
    ...
    try:
        env = ruslan_subprocess_env()
        assert env.get("RUSLAN_SESSION_KEY") == "agent:main:discord:group:MINE:111"
    finally:
        clear_session_vars(tokens)


def test_ruslan_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    """A pure single-process CLI (never engaged) keeps the inherited fallback."""
    monkeypatch.setenv("RUSLAN_SESSION_KEY", "cli-fallback-key")
    # not engaged (autouse fixture leaves _session_context_engaged False)
    env = ruslan_subprocess_env()
    assert env.get("RUSLAN_SESSION_KEY") == "cli-fallback-key"
