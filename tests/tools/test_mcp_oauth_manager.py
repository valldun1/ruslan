...
def test_ruslan_provider_subclass_exists():
    """RuslanMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _RUSLAN_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _RUSLAN_PROVIDER_CLS is not None
    assert issubclass(_RUSLAN_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_ruslan_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns RuslanMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _RUSLAN_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("RUSLAN_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _RUSLAN_PROVIDER_CLS is not None
    assert isinstance(provider, _RUSLAN_PROVIDER_CLS)
    assert provider._ruslan_server_name == "srv"
