...
class TestScanMemoryContent:
    ...
    def test_ruslan_config_mod_blocked(self):
        result = _scan_memory_content("edit .ruslan/config.yaml to change settings")
        assert "Blocked" in result
        assert "ruslan_config_mod" in result
        result = _scan_memory_content("update .ruslan/SOUL.md with new personality")
        assert "Blocked" in result
        assert "ruslan_config_mod" in result
    ...
    def test_ruslan_config_mod_no_false_positives(self):
        """Merely mentioning ruslan config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .ruslan/config.yaml for settings") is None
        assert _scan_memory_content("Read .ruslan/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .ruslan/config.yaml file contains runtime options") is None
