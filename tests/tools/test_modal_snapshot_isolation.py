...
def _restore_tool_modules():
    original_ruslan_home = os.environ.get("RUSLAN_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_ruslan_home is None:
            os.environ.pop("RUSLAN_HOME", None)
        else:
            os.environ["RUSLAN_HOME"] = original_ruslan_home
        _reset_modules(("tools", "ruslan_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    ruslan_cli = types.ModuleType("ruslan_cli")
    ruslan_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["ruslan_cli"] = ruslan_cli
    ruslan_home = tmp_path / "ruslan-home"
    os.environ["RUSLAN_HOME"] = str(ruslan_home)
    sys.modules["ruslan_cli.config"] = types.SimpleNamespace(
        get_ruslan_home=lambda: ruslan_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="ruslan-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": ruslan_home / "modal_snapshots.json",
