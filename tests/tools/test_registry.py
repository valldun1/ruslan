...
class TestDeregisterAuthorization:
    ...
    def test_plugin_cannot_deregister_unowned_tool_without_opt_in(self):
        reg = self._reg()
        reg.register_plugin_override_policy("ruslan_plugins.evil", False)
        with patch.object(ToolRegistry, "_caller_module", return_value="ruslan_plugins.evil"):
            import pytest
    ...
    def test_plugin_with_opt_in_can_deregister_unowned_tool(self):
        reg = self._reg()
        reg.register_plugin_override_policy("ruslan_plugins.allowed", True)
        with patch.object(ToolRegistry, "_caller_module", return_value="ruslan_plugins.allowed"):
            reg.deregister("protected")
    ...
    def test_plugin_can_deregister_its_own_tool(self):
        ...
        reg = ToolRegistry()
        reg.register_plugin_override_policy("ruslan_plugins.myplug", False)
        handler = eval("lambda *a, **k: 'own'", {"__name__": "ruslan_plugins.myplug"})
        reg.register(
            name="own_tool", toolset="myplug-ts",
            schema={"name": "own_tool", "description": "", "parameters": {"type": "object", "properties": {}}},
            handler=handler,
        )
        with patch.object(ToolRegistry, "_caller_module", return_value="ruslan_plugins.myplug"):
    ...
    def test_plugin_root_module_can_deregister_submodule_handler(self):
        ...
        ruslan_plugins.pkg (root cleanup code) must be allowed to deregister a
        tool whose handler was defined in ruslan_plugins.pkg.handlers.  The
        exact module strings differ, but they share the same plugin package root
        (ruslan_plugins.pkg) — ownership is bound to the package, not the leaf
        module (egilewski review, #55840).
        """
        reg = ToolRegistry()
        reg.register_plugin_override_policy("ruslan_plugins.pkg", False)
        handler = eval("lambda *a, **k: 'sub'", {"__name__": "ruslan_plugins.pkg.handlers"})
        reg.register(
            name="sub_tool", toolset="pkg-ts",
            schema={"name": "sub_tool", "description": "", "parameters": {"type": "object", "properties": {}}},
            handler=handler,
        )
        # Caller is the plugin root (ruslan_plugins.pkg), handler is in a
        # submodule (ruslan_plugins.pkg.handlers) — must be allowed.
        with patch.object(ToolRegistry, "_caller_module", return_value="ruslan_plugins.pkg"):
    ...
    def test_opted_in_plugin_submodule_can_deregister(self):
        ...
        root (``ruslan_plugins.allowed``).  If the caller is a submodule
        (``ruslan_plugins.allowed.cleanup``), the old code looked up
        ``_plugin_override_policy.get("ruslan_plugins.allowed.cleanup")`` →
        ...
        reg.register_plugin_override_policy("ruslan_plugins.allowed", True)
        with patch.object(ToolRegistry, "_caller_module", return_value="ruslan_plugins.allowed.cleanup"):
            reg.deregister("protected")
        assert reg._tools.get("protected") is None
    ...
    def test_core_code_deregister_always_allowed(self):
        """Non-plugin callers (core Ruslan code) are never gated."""
        reg = self._reg()
    ...
    def test_full_bypass_blocked(self):
        ...
        with pytest.raises(PermissionError):
            evil_handler = eval("lambda *a, **k: 'hijacked'", {"__name__": "ruslan_plugins.evil"})
