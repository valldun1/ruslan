...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._ruslan_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(ruslan_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(ruslan_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_ruslan_bin(self):
        """After _INSTALL_FAILED, manual install in RUSLAN_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        ruslan_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(ruslan_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(ruslan_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._ruslan_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == ruslan_bin
            assert _tirith_mod._resolved_path == ruslan_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# RUSLAN_HOME isolation
# ---------------------------------------------------------------------------

class TestRuslanHomeIsolation:
    def test_ruslan_bin_dir_respects_ruslan_home(self):
        """_ruslan_bin_dir must use RUSLAN_HOME, not hardcoded ~/.ruslan."""
        from tools.tirith_security import _ruslan_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"RUSLAN_HOME": tmpdir}):
            result = _ruslan_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_ruslan_home(self):
        """_failure_marker_path must use RUSLAN_HOME, not hardcoded ~/.ruslan."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"RUSLAN_HOME": "/custom/ruslan"}):
            result = _failure_marker_path()
        assert result == "/custom/ruslan/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets RUSLAN_HOME; verify it's active."""
        ruslan_home = os.getenv("RUSLAN_HOME")
        assert ruslan_home is not None, "RUSLAN_HOME should be set by conftest"
        assert "ruslan_test" in ruslan_home, "Should point to test temp dir"

    def test_get_ruslan_home_fallback(self):
        """Without RUSLAN_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_ruslan_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove RUSLAN_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".ruslan")
            result = _get_ruslan_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._ruslan_bin_dir",
