...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_ruslan_home_override(self, tmp_path):
        """Auth lookup should read from RUSLAN_HOME/auth.json, not ~/.ruslan/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".ruslan").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``ruslan tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_null_web_section_does_not_crash(self):
        ...
        with patch("ruslan_cli.config.load_config", return_value={"web": None}):
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "ruslan_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from ruslan_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.ruslan/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.ruslan/.env), not bare os.getenv. Parametrized over the four
