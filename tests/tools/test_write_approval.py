...
and the shared slash-command handlers (ruslan_cli/write_approval_commands.py).
...
def ruslan_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="ruslan_wa_test_")
    home = os.path.join(d, ".ruslan")
    os.makedirs(home)
    monkeypatch.setenv("RUSLAN_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import ruslan_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(ruslan_home):
...
def test_invalid_subsystem_is_off(ruslan_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(ruslan_home):
...
def test_memory_gate_on_no_interactive_stages(ruslan_home):
...
def test_memory_gate_on_then_apply(ruslan_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(ruslan_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(ruslan_home, monkeypatch):
    ...
    monkeypatch.setattr("ruslan_cli.config.load_config", _boom)
...
def test_skill_gate_off_allows_create(ruslan_home):
...
def test_skill_gate_on_always_stages(ruslan_home):
...
def test_skill_gate_on_then_apply_writes_file(ruslan_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's RUSLAN_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(ruslan_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(ruslan_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(ruslan_home):
...
def test_handle_approve_all(ruslan_home):
    from ruslan_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(ruslan_home):
    from ruslan_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(ruslan_home):
    from ruslan_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(ruslan_home):
    from ruslan_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(ruslan_home):
...
def test_handle_approval_invalid(ruslan_home):
    from ruslan_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(ruslan_home):
    from ruslan_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(ruslan_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(ruslan_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(ruslan_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(ruslan_home, monkeypatch):
    ...
    from tools import write_approval as wa
    _set_approval("memory", True)
    monkeypatch.setenv("RUSLAN_GATEWAY_SESSION", "1")
...
def test_skills_never_prompt_inline_even_with_callback(ruslan_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(ruslan_home):
