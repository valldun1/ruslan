...
class TestWriteDenyExactPaths:
    ...
    def test_ruslan_env(self):
        # ``.env`` under the active RUSLAN_HOME (profile-aware, not just
        # ``~/.ruslan``) must be write-denied. The hermetic test conftest
        # points RUSLAN_HOME at a tempdir — resolve via get_ruslan_home()
        # to match the denylist.
        from ruslan_constants import get_ruslan_home
        path = str(get_ruslan_home() / ".env")
        assert _is_write_denied(path) is True

    def test_ruslan_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.ruslan/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "ruslan_root"
        ...
        # Sanity check: RUSLAN_HOME does point to the profile dir, not the root.
        from ruslan_constants import get_ruslan_home, get_default_ruslan_root
        assert get_ruslan_home() == profile_home
        assert get_default_ruslan_root() == root
...
class TestWriteAllowed:
    ...
    def test_ruslan_control_files_requested_writable(self):
        from ruslan_constants import get_ruslan_home
