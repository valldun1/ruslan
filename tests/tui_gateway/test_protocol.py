...
def server():
    with patch.dict("sys.modules", {
        "ruslan_constants": MagicMock(get_ruslan_home=MagicMock(return_value="/tmp/ruslan_test")),
        "ruslan_cli.env_loader": MagicMock(),
        "ruslan_cli.banner": MagicMock(),
        "ruslan_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `RUSLAN_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("RUSLAN_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("RUSLAN_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_sync_session_key_after_compress_reanchors_active_session_lease(
    ...
    from ruslan_cli.active_sessions import (
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/ruslan-agent-dev": {"name": "ruslan-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "ruslan-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "ruslan_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "ruslan-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "ruslan-agent-dev"
