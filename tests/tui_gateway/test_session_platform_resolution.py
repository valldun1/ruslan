...
The desktop app's chat panel uses ``ruslan serve`` (the ``tui_gateway``
...
``tui_gateway`` session-creation time:
  ...
  RUSLAN_DESKTOP=1, RUSLAN_DESKTOP_TERMINAL unset  -> platform="desktop"
  RUSLAN_DESKTOP=1, RUSLAN_DESKTOP_TERMINAL=1     -> platform="tui"  (embedded pane)
...
class TestResolveSessionPlatform:
    ...
    def test_truthy_variants_recognized(self, clean_env, val):
        clean_env.setenv("RUSLAN_DESKTOP", val)
        _srv = _reload_resolver()
        assert _srv._resolve_session_platform() == "desktop"
    ...
    def test_embedded_terminal_overrides_desktop_when_both_set(self, clean_env):
        ...
        clean_env.setenv("RUSLAN_DESKTOP", "1")
        clean_env.setenv("RUSLAN_DESKTOP_TERMINAL", "true")
        _srv = _reload_resolver()
        assert _srv._resolve_session_platform() == "tui"
