...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart ruslan gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "ruslan update (restarts gateway, kills running agents)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.ruslan/ form so the Ruslan config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/ruslan/.ruslan/config.yaml`) rather than
    # ~, $HOME, or $RUSLAN_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live RUSLAN_HOME even when that is set
...
def _rewrite_resolved_ruslan_home(command: str) -> str:
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate RUSLAN_HOME (e.g. "/" or "") rewriting
        ...
        # /home/ruslan/.ruslan or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.ruslan/")
    return command
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("RUSLAN_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set RUSLAN_INTERACTIVE or RUSLAN_GATEWAY_SESSION to require approval.",
