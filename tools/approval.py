...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("RUSLAN_YOLO_MODE", ""))
...
# os.environ["RUSLAN_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export RUSLAN_INTERACTIVE.
_ruslan_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "ruslan_interactive",
    default=None,
)


def set_ruslan_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["RUSLAN_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``RUSLAN_INTERACTIVE`` env var for legacy callers.
    """
    return _ruslan_interactive_ctx.set("1" if interactive else "")


def reset_ruslan_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_ruslan_interactive_context`."""
    _ruslan_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``RUSLAN_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _ruslan_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("RUSLAN_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("RUSLAN_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("RUSLAN_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set RUSLAN_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind RUSLAN_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds RUSLAN_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("RUSLAN_CRON_SESSION"):
        return False
    if env_var_enabled("RUSLAN_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $RUSLAN_HOME, or by the resolved absolute
# active profile home path such as /home/ruslan/.ruslan/config.yaml. The
# resolved-absolute form is folded into the ~/.ruslan/ patterns at detection
...
# go stale when RUSLAN_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_RUSLAN_ENV_PATH = (
    r'(?:~\/\.ruslan/|'
    r'(?:\$home|\$\{home\})/\.ruslan/|'
    r'(?:\$ruslan_home|\$\{ruslan_home\})/)'
    r'\.env\b'
)
# ~/.ruslan/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _RUSLAN_ENV_PATH; matches the RUSLAN_HOME override form as
# well as ~/.ruslan/.
_RUSLAN_CONFIG_PATH = (
    r'(?:~\/\.ruslan/|'
    r'(?:\$home|\$\{home\})/\.ruslan/|'
...
DANGEROUS_PATTERNS = [
    ...
    # `ruslan` and `gateway` (e.g. `ruslan -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bruslan\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart ruslan gateway (kills running agents)"),
    (r'\bruslan\s+update\b', "ruslan update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `ruslan gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart ruslan-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart ruslan-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(ruslan|gateway|cli\.py)\b', "kill ruslan/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill ruslan` but not
    # `kill -9 $(pgrep -f ruslan)` because the substitution is opaque
    ...
    # the `ruslan gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.ruslan.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(ruslan|ai\.ruslan)\b', "stop/restart ruslan launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Ruslan file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.ruslan/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Ruslan-managed security file (~/.ruslan/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_RUSLAN_CONFIG_PATH}|{_RUSLAN_ENV_PATH})', "in-place edit of Ruslan config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_RUSLAN_CONFIG_PATH}|{_RUSLAN_ENV_PATH})', "in-place edit of Ruslan config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_RUSLAN_CONFIG_PATH}|{_RUSLAN_ENV_PATH})', "in-place edit of Ruslan config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.ruslan/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / RUSLAN_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Ruslan home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\ruslan), so folding the user home
    # first would eat the prefix the Ruslan-home fold needs.
    command = _rewrite_resolved_ruslan_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / RUSLAN_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.ruslan``); the matched
...
def _rewrite_resolved_ruslan_home(command: str) -> str:
    """Rewrite the resolved absolute Ruslan home prefix to ``~/.ruslan/``.

    Resolves the active ``RUSLAN_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.ruslan/`` so the static ``_RUSLAN_CONFIG_PATH`` / ``_RUSLAN_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/ruslan/.ruslan/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$RUSLAN_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from ruslan_constants import get_ruslan_home
        home = get_ruslan_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.ruslan")
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Ruslan ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"ruslan-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["RUSLAN_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "RUSLAN_SPINNER_PAUSE" in os.environ:
            del os.environ["RUSLAN_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Ruslan approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``RUSLAN_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            RUSLAN_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("RUSLAN_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set RUSLAN_INTERACTIVE or "
                "RUSLAN_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set RUSLAN_INTERACTIVE or "
            "RUSLAN_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("RUSLAN_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``RUSLAN_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("RUSLAN_EXEC_ASK")
        ...
        if env_var_enabled("RUSLAN_CRON_SESSION"):
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from ruslan_cli.config import load_config as _load_cfg
