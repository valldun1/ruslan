...
`ruslan skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.ruslan.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.ruslan.blueprint`` is a mapping containing
    ...
    ruslan = meta.get("ruslan") if isinstance(meta, dict) else None
    blueprint = ruslan.get("blueprint") if isinstance(ruslan, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.ruslan.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.ruslan.blueprint`` block) they can hand
    to ``ruslan skills publish`` to share. ``body`` is the plain-language
