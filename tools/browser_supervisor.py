...
One ``CDPSupervisor`` runs per Ruslan ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Ruslan logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "ruslan-dialog-bridge.invalid"
...
(() => {
  if (window.__ruslanDialogBridgeInstalled) return;
  window.__ruslanDialogBridgeInstalled = true;
  const ENDPOINT = "http://ruslan-dialog-bridge.invalid/";
