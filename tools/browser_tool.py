...
from ruslan_constants import agent_browser_runnable, get_ruslan_home
...
# means a compromised transitive dependency could read every Ruslan secret
...
def _build_browser_env() -> dict:
    ...
    Strips Ruslan-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``ruslan_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = ruslan_subprocess_env(inherit_credentials=False)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    # node_modules binary, which disappears on the next ``ruslan update`` and
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_ruslan_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_ruslan_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_ruslan_home() / "node")),
            ]
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/ruslan-agent:latest")
                else:
