...
from ruslan_cli.config import DEFAULT_CONFIG, cfg_get
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    ruslan_node_bin = str(ruslan_home / "node" / "bin")
    ruslan_node_root = str(ruslan_home / "node")
    ruslan_nm_bin = str(ruslan_home / "node_modules" / ".bin")
    return [ruslan_node_bin, ruslan_node_root, ruslan_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _resolve_cdp_override(cdp_url: str) -> str:
    ...
    try:
        ...
        logger.warning("Не удалось разрешить CDP endpoint %s через %s: %s", raw, version_url, exc)
        return raw
...
# plugins drop in under ``~/.ruslan/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.ruslan/plugins/browser/<vendor>/``) participate
    ...
    try:
        ...
        if provider_key:
            try:
                ...
                logger.warning(
                    "Не удалось создать экземпляр явного cloud_provider %r; при следующем вызове будет выполнена повторная попытка",
                    provider_key,
                    exc_info=True,
                )
                return None
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Ruslan silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Ruslan CLI where sys.stdout and sys.stderr both report
    ...
    try:
        ...
        if not nav.get("success"):
            logger.warning("Запасной вариант Chrome: не удалось выполнить навигацию: %s", nav.get("error"))
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-ruslan_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed ruslan processes — this way every clean ruslan exit sweeps
    ...
    if _active_sessions:
        ...
        try:
            ...
            logger.error("Ошибка экстренной очистки: %s", e)
        finally:
            with _cleanup_lock:
    ...
    # Sweep orphans from other crashed ruslan processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live ruslan processes.
    try:
...
def _cleanup_inactive_browser_sessions():
    ...
    for task_id in sessions_to_cleanup:
        try:
            ...
            logger.info("Очистка неактивного сеанса для задачи: %s (неактивен %s с)", task_id, elapsed)
            cleanup_browser(task_id)
            with _cleanup_lock:
                if task_id in _session_last_activity:
                    del _session_last_activity[task_id]
        except Exception as e:
            logger.warning("Ошибка очистки неактивного сеанса %s: %s", task_id, e)


def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current ruslan PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live ruslan process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning ruslan process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced ruslan PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent ruslan instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-ruslan_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live ruslan process.
            continue
        ...
        try:
            ...
            logger.info("Завершён осиротевший демон браузера PID %d (сеанс %s)",
                        daemon_pid, session_name)
            reaped += 1
...
def _browser_cleanup_thread_worker():
    ...
    try:
        ...
        logger.warning("Ошибка завершения осиротевшего процесса: %s", e)

    while _cleanup_running:
        try:
            _cleanup_inactive_browser_sessions()
        except Exception as e:
            logger.warning("Ошибка потока очистки: %s", e)
...
def _start_browser_cleanup_thread():
    ...
    with _cleanup_lock:
        if _cleanup_thread is None or not _cleanup_thread.is_alive():
            ...
            logger.info("Запущен поток очистки неактивных сеансов (тайм-аут: %s с)", BROWSER_SESSION_INACTIVITY_TIMEOUT)
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Ruslan-managed
    ...
    # Build an extended search PATH including Ruslan-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                ruslan_nm = str(get_ruslan_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=ruslan_nm)
            if not recheck:
                ruslan_node_bin = str(get_ruslan_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=ruslan_node_bin)
            if not recheck:
                ruslan_node_root = str(get_ruslan_home() / "node")
                recheck = shutil.which("agent-browser", path=ruslan_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        logger.warning("agent-browser CLI не найден: %s", e)
        return {"success": False, "error": str(e)}
    ...
    try:
        ...
        logger.warning("Не удалось создать сеанс браузера для задачи=%s: %s", task_id, e)
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this ruslan PID as the session owner (cross-process safe
        ...
        try:
            ...
            proc.kill()
            proc.wait()
            logger.warning("Браузер '%s' превысил время ожидания через %ds (task=%s, socket_dir=%s)",
            ...
            if not stdout_text and returncode == 0 and command not in _EMPTY_OK_COMMANDS:
                logger.warning("Браузер '%s' вернул пустой вывод (rc=0)", command)
                ...
                try:
                    ...
                    logger.warning("Браузер '%s' вернул вывод не в формате JSON (rc=%s): %s",
                    ...
                    if command == "screenshot":
                        ...
                        if recovered_path and Path(recovered_path).exists():
                            logger.info(
                                "Браузер 'screenshot' восстановил файл из вывода не в формате JSON: %s",
                                recovered_path,
                            )
                            result = {
                                "success": True,
                ...
                logger.warning("Браузер '%s' завершился ошибкой (rc=%s): %s", command, returncode, error_msg[:300])
                result = {"success": False, "error": error_msg}
            else:
                result = {"success": True, "data": {}}

    except Exception as e:
        logger.warning("Исключение браузера '%s': %s", command, e, exc_info=True)
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = ruslan_home / "browser_recordings"
        ...
        if result.get("success"):
            ...
            logger.info("Автоматическая запись сеанса браузера %s в %s", task_id, recording_path)
        else:
...
def _maybe_stop_recording(task_id: str):
    ...
    try:
        ...
        if result.get("success"):
            ...
            logger.info("Сохранена запись браузера для сеанса %s: %s", task_id, path)
    except Exception as e:
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Ruslan
    ...
    screenshots_dir = get_ruslan_dir("cache/screenshots", "browser_screenshots")
    ...
    if engine == "lightpanda" and _should_inject_engine(engine):
        ...
        if fb_result.get("success"):
            ...
            logger.warning("Сбой создания снимка экрана Lightpanda Chrome fallback vision: %s", fb_result.get("error"))
    ...
    try:
        ...
        logger.warning("Ошибка browser_vision: %s", e, exc_info=True)
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if session_info:
        ...
        try:
            ...
            logger.warning("Не удалось закрыть agent-browser для задачи %s: %s", task_id, e)
        ...
        if bb_session_id:
            ...
            if provider is not None:
                try:
                    ...
                    logger.warning("Не удалось закрыть сеанс облачного браузера: %s", e)
...
if __name__ == "__main__":
    ...
    if check_browser_requirements():
        ...
        print("❌ Отсутствуют требования:")
        try:
            browser_cmd = _find_agent_browser()
            if _requires_real_termux_browser_install(browser_cmd):
                print("- обнаружен резервный bare npx (недостаточно для локального режима Termux)")
                print(f"     Install: {_browser_install_hint()}")
            elif _cp is None and not _chromium_installed():
                print("- бинарный файл браузера Chromium не найден")
                ...
                if _running_in_docker():
                    ...
                    print("docker pull ghcr.io/valldun1/ruslan-agent:latest")
                else:
                    print("Установите с помощью:")
                    print("npx agent-browser install --with-deps")
                    print("Или:  npx playwright install --with-deps chromium")
        except FileNotFoundError:
            print("- CLI agent-browser не найден")
            print(f"     Install: {_browser_install_hint()}")
        if _cp is not None and not _cp.is_configured():
            print(f"   - {_cp.provider_name()} credentials not configured")
            print("Совет: установите browser.cloud_provider на 'local', чтобы вместо этого использовать бесплатный локальный режим")
