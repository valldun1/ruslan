#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Ruslan desktop GUI.
...
``read_terminal`` it is gated on ``RUSLAN_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    ...
    return env_var_enabled("RUSLAN_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Ruslan desktop GUI (the tabs mirroring terminal(background=true) runs). "
