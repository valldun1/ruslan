...
# Must match ruslan_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``ruslan-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. ruslan-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.ruslan/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_ruslan_dir("cache/delegation", "delegation_cache")
...
def delegate_task(
    ...
    if background:
        ...
        try:
            ...
            _source = get_session_env("RUSLAN_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("RUSLAN_UI_SESSION_ID", "")
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and RUSLAN_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'ruslan auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Ruslan config.

    Prefer the shared persistent loader because it follows the active
    RUSLAN_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``RUSLAN_IGNORE_USER_CONFIG=1`` (``ruslan chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("RUSLAN_IGNORE_USER_CONFIG") == "1"
