...
def _is_ruslan_provider_credential(name: str) -> bool:
    """True if ``name`` is a Ruslan-managed provider credential (API key,
    token, or similar) per ``_RUSLAN_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Ruslan API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Ruslan credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Ruslan-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_ruslan_internal_secret(name):
        return True
    return name in _RUSLAN_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Ruslan-managed provider credentials (from
    ``_RUSLAN_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Ruslan-managed
    ...
    Non-Ruslan third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_ruslan_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Ruslan provider "
                "credential %r (blocked by _RUSLAN_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Ruslan-managed provider credentials must not be passed
                ...
                if _is_ruslan_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Ruslan "
                        "provider credential %r from config.yaml (blocked "
                        "by _RUSLAN_PROVIDER_ENV_BLOCKLIST). Operator "
