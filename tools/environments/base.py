"""Base class for all Ruslan execution environment backends.
...
# RUSLAN_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("RUSLAN_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {RUSLAN_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_ruslan_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__RUSLAN_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Ruslan backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/ruslan-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/ruslan-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        bootstrap = (
            ...
            f"__ruslan_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__ruslan_fns\" ] && declare -f $__ruslan_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("__ruslan_ec=$?")
        # Restrict Ruslan metadata files without changing the user's command
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"RUSLAN_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        # --- Debug tracing (opt-in via RUSLAN_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __RUSLAN_CWD_{session}__ marker from stdout output.
