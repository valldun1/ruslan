...
* Never raises. Any I/O error (disk full, permission denied, missing
  RUSLAN_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_ruslan_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: RUSLAN_HOME env var, then ~/.ruslan
