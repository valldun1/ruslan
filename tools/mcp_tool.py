...
transport, discovers their tools, and registers them into the ruslan-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.ruslan/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.ruslan/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Ruslan' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            ruslan_home = os.path.expanduser(
                os.getenv(
                    "RUSLAN_HOME", os.path.join(os.path.expanduser("~"), ".ruslan")
                )
            )
            candidates = [
                os.path.join(ruslan_home, "node", "bin", resolved_command),
                ...
                # image (which the Ruslan Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Ruslan MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Ruslan gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Ruslan document cache; returns a marker with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Ruslan' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (RUSLAN_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit RUSLAN_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Ruslan' logging (agent.log via ruslan_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ungraceful exit of this Ruslan process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Ruslan' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Ruslan is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `ruslan mcp login {server_name}` (or delete the tokens "
            f"file under ~/.ruslan/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local RUSLAN_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_ruslan_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_ruslan_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local RUSLAN_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_ruslan_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Ruslan config file.
    ...
    ``os.environ`` (which includes ``~/.ruslan/.env`` loaded at startup).
    """
    try:
        from ruslan_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("RUSLAN_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Ruslan' MEDIA: tag convention and out to messaging
            ...
            # Ruslan' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Ruslan's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Ruslan uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Ruslan registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``ruslan tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Ruslan registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple ruslan
