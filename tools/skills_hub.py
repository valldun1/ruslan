#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Ruslan Skills Hub.
...
Used by ruslan_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _ruslan_home() -> Path:
    return get_ruslan_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _ruslan_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            ruslan_meta = metadata.get("ruslan", {})
            if isinstance(ruslan_meta, dict):
                tags = ruslan_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "NousResearch/ruslan-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                ruslan_meta = meta_block.get("ruslan", {})
                if isinstance(ruslan_meta, dict):
                    tags = ruslan_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Ruslan centralized index source
# ---------------------------------------------------------------------------

RUSLAN_INDEX_URL = "https://ruslan-agent.nousresearch.com/docs/api/skills-index.json"
RUSLAN_INDEX_TTL = 6 * 3600  # 6 hours


def _ruslan_index_cache_file() -> Path:
    return _index_cache_dir() / "ruslan-index.json"


def _load_ruslan_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for RUSLAN_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    ruslan_index_cache_file = _ruslan_index_cache_file()
    if ruslan_index_cache_file.exists():
        try:
            age = time.time() - ruslan_index_cache_file.stat().st_mtime
            if age < RUSLAN_INDEX_TTL:
                return json.loads(ruslan_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Ruslan index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Ruslan index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Ruslan index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ruslan_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        ruslan_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class RuslanIndexSource(SkillSource):
    """Skill source backed by the centralized Ruslan Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_ruslan_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "ruslan-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "ruslan-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        RuslanIndexSource(auth=auth), # Centralized index (search + resolved install paths)
