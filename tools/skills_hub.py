...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Valldun) but not activated
...
RUSLAN_INDEX_URL = "https://ruslan.team/docs/api/skills-index.json"
RUSLAN_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "ruslan-index.json"
...
def _load_ruslan_index() -> Optional[dict]:
    ...
    try:
        resp = httpx.get(RUSLAN_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
