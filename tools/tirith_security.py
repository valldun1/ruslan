...
it is automatically downloaded from GitHub releases to $RUSLAN_HOME/bin/tirith.
...
def _get_ruslan_home() -> str:
    """Return the Ruslan home directory, respecting RUSLAN_HOME env var."""
    return str(get_ruslan_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_ruslan_home(), ".tirith-install-failed")
...
def _ruslan_bin_dir() -> str:
    """Return $RUSLAN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_ruslan_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $RUSLAN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_ruslan_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $RUSLAN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $RUSLAN_HOME/bin/tirith
    ...
    ruslan_bin = os.path.join(_ruslan_bin_dir(), "tirith")
    if os.path.isfile(ruslan_bin) and os.access(ruslan_bin, os.X_OK):
        _resolved_path = ruslan_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return ruslan_bin
