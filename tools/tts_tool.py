...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.ruslan/config.yaml``. Ruslan
...
Configuration is loaded from ~/.ruslan/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``ruslan_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from ruslan_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import ruslan_xai_user_agent
...
# Base URL now resolved via ruslan_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_ruslan_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.ruslan/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.ruslan/config.yaml.
    ...
    try:
        ...
        logger.debug("ruslan_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``ruslan tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into Ruslan without any Python code
...
#           output_format: wav
#
# Ruslan writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__RUSLAN_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
# file, so retired models disappear from ruslan the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``ruslan_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = (get_env_value("DEEPINFRA_API_KEY") or "").strip()
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `ruslan setup` to configure, "
    ...
    from ruslan_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `ruslan model` or set XAI_API_KEY.")
    ...
    # output_format when Ruslan actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": ruslan_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import ruslan_cli as _ruslan_cli

            _ruslan_version = str(_ruslan_cli.__version__)
        except Exception:
            _ruslan_version = "0.0.0"
        # Include Ruslan client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Ruslan caches Piper voice models.

    Resolves to ``~/.ruslan/cache/piper-voices/`` under the active
    RUSLAN_HOME so voice downloads follow profile boundaries.
    """
    from ruslan_constants import get_ruslan_dir
    root = Path(get_ruslan_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.ruslan/config.yaml (tts: section).
    ...
    platform = get_session_env("RUSLAN_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'ruslan-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run ruslan setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'ruslan setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'ruslan tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_ruslan_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
