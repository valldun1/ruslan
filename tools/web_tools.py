...
Backend is selected during ``ruslan tools`` setup (web.backend in config.yaml).
When available, Ruslan can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Ruslan config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Ruslan' config/.env layer (``ruslan config set``, ``ruslan tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.ruslan/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``ruslan tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `ruslan tools` repaint.
        try:
...
async def process_content_with_llm(
    ...
    try:
        ...
        if content_len > MAX_CONTENT_SIZE:
            ...
            logger.warning("Содержимое слишком большое (%.", size_mb)
        ...
        logger.info("Обработка содержимого с помощью LLM (%d символов)", content_len)
...
async def _process_large_content_chunked(
    ...
    logger.info("Разделено на %d фрагментов по ~%d символов каждый", len(chunks), chunk_size)
    ...
    logger.info("Получено %d/%d сводок фрагментов", len(summaries), len(chunks))
    ...
    logger.info("Синтезирование %d сводок...", len(summaries))
...
Create a single, unified markdown summary."""
    ...
    try:
        ...
        logger.info("Синтез завершён: %d -> %d символов (%.2f%%)", original_len, final_len, compression * 100)
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `ruslan tools` to set one up."
                ),
            }
        else:
            logger.info(
...
async def web_extract_tool(
    ...
    try:
        logger.info("Извлечение содержимого из %d URL", len(normalized_urls))
        ...
        logger.info("Извлечено содержимое с %d страниц", pages_extracted)
...
if __name__ == "__main__":
    ...
    """
    print("🌐 Автономный модуль веб-инструментов")
    print("=" * 40)
    ...
    if web_available:
        ...
        print("❌ Не настроен бэкенд веб-поиска")
        print(
    ...
    if not nous_available:
        print("❌ Нет доступной вспомогательной модели для обработки содержимого LLM")
        print("Установите OPENROUTER_API_KEY, настройте Nous Portal или задайте OPENAI_BASE_URL + OPENAI_API_KEY")
        print("⚠️  Без вспомогательной модели обработка содержимого LLM будет отключена")
    else:
    ...
    print("🛠️  Веб-инструменты готовы к использованию!")
    ...
    if _debug.active:
        ...
        print("🐛 Режим отладки отключён (установите WEB_TOOLS_DEBUG=true для включения)")
    
    print("\nBasic usage:")
    print("  from web_tools import web_search_tool, web_extract_tool")
    print("  import asyncio")
    print("")
    print("# Поиск (синхронный)")
    print("results = web_search_tool('Python tutorials')")
    print("")
    print("# Извлечение (асинхронное)")
    ...
    if nous_available:
        print("\nLLM-enhanced usage:")
        print("# Содержимое автоматически обрабатывается для страниц >5000 символов (по умолчанию)")
        ...
        print("  )")
        print("")
        print("# Отключение обработки LLM")
        print("raw_content = await web_extract_tool(['https://example.com'], use_llm_processing=False)")
    
    print("\nDebug mode:")
    print("# Включить отладочное логирование")
    print("export WEB_TOOLS_DEBUG=true")
    print("# Логи отладки содержат:")
    print("# - Все вызовы инструментов с параметрами")
    print("# - Оригинальные ответы API")
    print("# - Метрики сжатия LLM")
    print("# - Финальные обработанные результаты")
    print("# Логи сохранены в: ./logs/web_tools_debug_UUID.json")
