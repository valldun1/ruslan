...
_RUSLAN_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on RUSLAN_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (RUSLAN_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `ruslan tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (RUSLAN_KANBAN_TASK env "
    ...
    # Coding posture (base Ruslan — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in ruslan_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Ruslan toolsets (CLI + messaging platforms)
    #
    ...
    # the `ruslan send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "ruslan-cli": {
        ...
        "tools": _RUSLAN_CORE_TOOLS,
        "includes": []
    },

    "ruslan-cron": {
        # Mirrors ruslan-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `ruslan tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as ruslan-cli; gated by `ruslan tools`",
        "tools": _RUSLAN_CORE_TOOLS,
        "includes": []
    },
    ...
    "ruslan-email": {
        "description": "Email bot toolset - interact with Ruslan via email (IMAP/SMTP)",
        "tools": _RUSLAN_CORE_TOOLS,
        "includes": []
    },
    ...
    "ruslan-sms": {
        "description": "SMS bot toolset - interact with Ruslan via SMS (Twilio)",
        "tools": _RUSLAN_CORE_TOOLS,
        "includes": []
    },
    ...
    "ruslan-webhook": {
        ...
        "tools": _RUSLAN_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "ruslan-gateway": {
        ...
        "tools": [],
        "includes": ["ruslan-telegram", "ruslan-discord", "ruslan-whatsapp", "ruslan-slack", "ruslan-signal", "ruslan-bluebubbles", "ruslan-homeassistant", "ruslan-email", "ruslan-sms", "ruslan-mattermost", "ruslan-matrix", "ruslan-dingtalk", "ruslan-feishu", "ruslan-wecom", "ruslan-wecom-callback", "ruslan-weixin", "ruslan-qqbot", "ruslan-webhook", "ruslan-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``ruslan-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_RUSLAN_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``ruslan-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (ruslan-<name>).
        # Gives them _RUSLAN_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("ruslan-"):
            platform_name = name[len("ruslan-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_RUSLAN_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
