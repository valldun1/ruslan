...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("RUSLAN_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over RUSLAN_MODEL / RUSLAN_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set RUSLAN_INFERENCE_MODEL in the
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from ruslan_cli.browser_connect import try_launch_chrome_debug
