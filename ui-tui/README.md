...
React + Ink terminal UI for Ruslan. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `RUSLAN_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `ruslan setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.ruslan/.ruslan_history` or under `RUSLAN_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `ruslan setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/ruslan-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external ruslan setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.ruslan/tui-parent.log
    ...
    types/
      ruslan-ink.d.ts            type declarations for @ruslan/ink
