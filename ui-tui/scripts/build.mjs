...
await build({
  ...
  // Skip the prebuilt @ruslan/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@ruslan/ink': resolve(root, 'packages/ruslan-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The ruslan_cli launcher
