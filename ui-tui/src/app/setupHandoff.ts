import type { RunExternalProcess } from '@ruslan/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`ruslan ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching ruslan: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`ruslan ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
