import { forceRedraw, type MouseTrackingMode } from '@ruslan/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update Ruslan Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
      ...
      // Exit code 42 signals the Python wrapper to exec `ruslan update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try RUSLAN_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Ruslan #${i + 1}`
