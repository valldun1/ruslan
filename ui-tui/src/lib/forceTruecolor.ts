...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.RUSLAN_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Ruslan on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via RUSLAN_TUI_TRUECOLOR=1.
