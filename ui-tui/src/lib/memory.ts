...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because RUSLAN_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.RUSLAN_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.ruslan', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `ruslan-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.RUSLAN_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.RUSLAN_HEAPDUMP_MAX_BYTES?.trim()
