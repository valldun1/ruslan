...
const logDir = join(process.env.RUSLAN_HOME?.trim() || join(homedir(), '.ruslan'), 'logs')
...
// a real ~/.ruslan (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('ruslan-tui: parent lifecycle log unavailable\n')
    }
  }
}
