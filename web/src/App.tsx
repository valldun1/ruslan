...
const SIDEBAR_COLLAPSED_KEY = "ruslan-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="ruslan-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="ruslan-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateRuslan = status?.can_update_ruslan === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'ruslan update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "ruslan update";
    return (
      t.status.updateRuslanConfirmMessage ??
      `This will run 'ruslan update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateRuslanConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateRuslan) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateRuslan,
      runningLabel: t.status.updatingRuslan,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Ruslan gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateRuslanConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateRuslanConfirmTitle ?? `${t.status.updateRuslan}?`}
    />
    </>
  );
}
