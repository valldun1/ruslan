...
interface RuslanConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function RuslanConsoleModal({ open, onClose }: RuslanConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("ruslan> ");
  const inputPromptRef = useRef("ruslan> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "ruslan> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Ruslan Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="ruslan-console-title"
    >
      <div
        className={cn(
          themedBody,
