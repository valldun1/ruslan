...
// (e.g. https://mission-control.tilos.com/ruslan/). The Python backend
// injects ``window.__RUSLAN_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__RUSLAN_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const RUSLAN_BASE_PATH = readBasePath();
const BASE = RUSLAN_BASE_PATH;
...
declare global {
  interface Window {
    __RUSLAN_SESSION_TOKEN__?: string;
    __RUSLAN_BASE_PATH__?: string;
    ...
    __RUSLAN_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Ruslan-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__RUSLAN_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``ruslan update``, ``ruslan gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__RUSLAN_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__RUSLAN_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("ruslan.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("ruslan.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__RUSLAN_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("ruslan.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__RUSLAN_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Ruslan dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``ruslan_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__RUSLAN_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Ruslan-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``ruslan_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__RUSLAN_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateHermes: () =>
    fetchJSON<ActionResponse>("/api/ruslan/update", { method: "POST" }),
  checkHermesUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/ruslan/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** ruslan-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``ruslan update``. */
  can_update_ruslan?: boolean;
  config_path: string;
