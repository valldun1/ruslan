...
The ACP adapter wraps Ruslan' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
ruslan acp / ruslan-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.ruslan/.env
  -> configure stderr logging
  -> construct RuslanACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'ruslan-agent[acp]==<version>' ruslan-acp`, pointed at the `ruslan-agent` PyPI release.
...
- `allow_once` -> Ruslan `once`
- `allow_always` -> Ruslan `always`
- reject options -> Ruslan `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Ruslan tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["ruslan-acp"])
...
Instead it reuses Ruslan' runtime resolver:

- `acp_adapter/auth.py`
- `ruslan_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Ruslan provider/credentials. It also always advertises a terminal setup auth method (`ruslan-setup`, args `--setup`) so first-run registry clients can open Ruslan' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.ruslan/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `ruslan-acp` toolset definition
- `ruslan_cli/main.py` — `ruslan acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `ruslan-acp` script
