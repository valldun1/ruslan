...
sidebar_position: 1
title: "Architecture"
description: "Ruslan Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Ruslan Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── ruslan_constants.py       # RUSLAN_HOME, profile-aware paths
...
│
├── ruslan_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `ruslan` subcommands (large file)
...
│   ├── skills_config.py      # ruslan skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # ruslan tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # ruslan gateway start/stop
│
...
Exposes Ruslan as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`ruslan -p <name>`) gets its own RUSLAN_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
