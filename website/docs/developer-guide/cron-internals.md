...
description: "How Ruslan stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `ruslan_cli/cron.py` | CLI `ruslan cron` subcommands |
...
Jobs are stored in `~/.ruslan/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `ruslan cron` commands are run or during active CLI sessions.
...
```python
# ~/.ruslan/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.ruslan/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `ruslan cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `ruslan cron` CLI provides direct job management:

```bash
ruslan cron list                    # Show all jobs
ruslan cron create                  # Interactive job creation (alias: add)
ruslan cron edit <job_id>           # Edit job configuration
ruslan cron pause <job_id>          # Pause a running job
ruslan cron resume <job_id>         # Resume a paused job
ruslan cron run <job_id>            # Trigger immediate execution
ruslan cron remove <job_id>         # Delete a job
```
