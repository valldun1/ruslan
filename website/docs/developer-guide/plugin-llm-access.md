...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Ruslan already has applies. The plugin never sees a
...
ruslan-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Ruslan provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Ruslan provider
        # (must be one Ruslan already knows about — same names as
        # `ruslan model` and config.yaml model.provider).
        ...
        # plugin sends — Ruslan does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.ruslan/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Ruslan' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Ruslan subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/ruslan-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/ruslan-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/ruslan-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/ruslan-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
