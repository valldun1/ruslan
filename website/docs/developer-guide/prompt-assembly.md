...
description: "How Ruslan builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Ruslan deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.ruslan.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.ruslan/SOUL.md)
You are Ruslan, an AI assistant created by Valldun.
...
The platform hint (Layer 10 above) is the per-surface guidance Ruslan
...
`SOUL.md` lives at `~/.ruslan/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_ruslan_md(cwd_path)       # 1. .ruslan.md / RUSLAN.md (walks to git root)
    ...
    # SOUL.md from RUSLAN_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.ruslan.md`, `RUSLAN.md` | CWD up to git root | Ruslan-native project config |
...
- **YAML frontmatter stripped** — `.ruslan.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Ruslan concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.ruslan.md` / `RUSLAN.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Ruslan already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.ruslan/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.ruslan/MEMORY.md` and `~/.ruslan/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.ruslan.md`, `RUSLAN.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Ruslan.
- Ephemeral overlays such as `RUSLAN_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Ruslan assembles prompts for everyone, change Python and treat it as a code contribution
