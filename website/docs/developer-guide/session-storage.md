...
Ruslan Agent uses a SQLite database (`~/.ruslan/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `ruslan_state.py`


## Architecture Overview

```
~/.ruslan/state.db (SQLite, WAL mode)
...
Multiple ruslan processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.ruslan/state.db
...
Default path: `~/.ruslan/state.db`

This is derived from `ruslan_constants.get_ruslan_home()` which resolves to
`~/.ruslan/` by default, or the value of `RUSLAN_HOME` environment variable.
