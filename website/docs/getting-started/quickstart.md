...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Ruslan Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Ruslan setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Ruslan Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Ruslan Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Ruslan for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Ruslan working on my machine | `ruslan setup` | Run a real chat and verify it responds |
| I already know my provider | `ruslan model` | Save the config, then start chatting |
| I want a bot or always-on setup | `ruslan gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `ruslan model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `ruslan model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Ruslan cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `ruslan model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `ruslan setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `ruslan update`. Re-enable anything later with `ruslan tools`, seed skills with `ruslan skills opt-in --sync`, or tune settings with `ruslan setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `ruslan model` |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `ruslan_cli/models.py` may change between releases) | `ruslan model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `ruslan model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `ruslan model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `ruslan model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `ruslan model` (requires `copilot` CLI + `copilot login`) |
...
Ruslan Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `ruslan model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Ruslan separates secrets from normal config:

- **Secrets and tokens** → `~/.ruslan/.env`
- **Non-secret settings** → `~/.ruslan/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
ruslan config set model anthropic/claude-opus-4.6
ruslan config set terminal.backend docker
ruslan config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
ruslan            # classic CLI
ruslan --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Ruslan ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `ruslan` vs `ruslan --tui`.
:::
...
```bash
ruslan --continue    # Resume the most recent session
ruslan -c            # Short form
```
...
```bash
ruslan gateway setup    # Interactive platform configuration
```
...
- `ruslan tools` — tune tool access per platform
- `ruslan skills` — browse and install reusable workflows
...
```bash
ruslan config set terminal.backend docker    # Docker isolation
ruslan config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Ruslan install directory (the curl installer placed it at
# ~/.ruslan/ruslan-agent on Linux/macOS or %LOCALAPPDATA%\ruslan\ruslan-agent on Windows):
...
Skills are on-demand instruction documents that teach Ruslan how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Ruslan ships with a catalog of bundled skills already installed in `~/.ruslan/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
ruslan skills browse                      # list everything available
ruslan skills search kubernetes           # find skills by keyword
ruslan skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `ruslan skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Ruslan ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.ruslan/ruslan-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Ruslan opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `ruslan model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `ruslan gateway setup` and check `ruslan gateway status` |
| `ruslan --continue` can't find old session | Switched profiles or session never saved | Check `ruslan sessions list` and confirm you're in the right profile |
...
| `ruslan doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `ruslan model`
3. `ruslan setup`
4. `ruslan sessions list`
5. `ruslan --continue`
6. `ruslan gateway status`
...
| `ruslan tools` | Configure which tools are enabled per platform |
| `ruslan setup` | Full setup wizard (configures everything at once) |
| `ruslan doctor` | Diagnose issues |
| `ruslan update` | Update to latest version |
| `ruslan gateway` | Start the messaging gateway |
| `ruslan --continue` | Resume last session |
