...
description: "Run Ruslan Agent directly on an Android phone with Termux"
---

# Ruslan on Android with Termux

This is the tested path for running Ruslan Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Ruslan from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Ruslan now ships a Termux-aware installer path:
...
- links `ruslan` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `ruslan` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/ruslan" "$PREFIX/bin/ruslan"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `ruslan` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.ruslan/.env`.
...
### `ruslan doctor` says ripgrep or Node is missing
