...
Copy-paste blueprints for common automation patterns. Each blueprint uses Ruslan's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`ruslan webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `ruslan webhook subscribe`) |
...
```bash
ruslan webhook subscribe github-pr-review \
  --events "pull_request" \
...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (ruslan_cli/commands.py, ruslan_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (ruslan_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
ruslan cron create "0 6 * * *" \
  "Run a dependency security audit on the ruslan-agent project.

1. cd ~/.ruslan/ruslan-agent && source .venv/bin/activate
...
```bash
ruslan webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
ruslan webhook subscribe alert-triage \
...
```python title="~/.ruslan/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Ruslan-Monitor/1.0"})
...
ruslan cron create "every 30m" \
  ...
  --script ~/.ruslan/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
ruslan cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Ruslan Agent development." \
...
```bash
ruslan webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
ruslan cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the ruslan-agent codebase.
