...
description: "Diagnose and fix common Ruslan cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`ruslan gateway` for foreground, or `ruslan gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `ruslan cron tick`.
...
```bash
date
ruslan cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.ruslan/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.ruslan/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.ruslan/.env` |
...
| `local` | Write access to `~/.ruslan/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `ruslan cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `ruslan skills install <skill-name>` or via `/skills` in the CLI.
...
2. `~/.ruslan/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `ruslan cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Ruslan config directory). Verify:
```bash
ls ~/.ruslan/scripts/your-script.py   # Must exist
ruslan cron edit <job_id> --script ~/.ruslan/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `ruslan skills install <skill-name>`.
...
Jobs are stored in `~/.ruslan/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.ruslan/cron/jobs.json
chmod 600 ~/.ruslan/cron/jobs.json   # Your user should own it
```
...
```bash
ruslan cron list                    # Show all jobs, states, next_run times
ruslan cron run <job_id>            # Schedule for next tick (for testing)
ruslan cron edit <job_id>           # Fix configuration issues
ruslan logs                         # View recent Ruslan logs
ruslan skills list                  # Verify installed skills
```
