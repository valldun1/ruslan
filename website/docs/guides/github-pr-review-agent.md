...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Ruslan Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Ruslan instantly when PRs are opened or updated.
:::
...
- **Ruslan Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  ruslan gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.ruslan/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Ruslan can access GitHub. Start a chat:
...
Still in the chat, ask Ruslan to review a real PR:
...
A skill gives Ruslan consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.ruslan/skills/code-review
```

Create `~/.ruslan/skills/code-review/SKILL.md`:
...
Verify it loaded — start `ruslan` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Ruslan your team's standards:
...
2. Teach Ruslan your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
ruslan gateway status    # Is the gateway running?
ruslan cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-ruslan-plugin)** — wrap the review logic into a shareable plugin
