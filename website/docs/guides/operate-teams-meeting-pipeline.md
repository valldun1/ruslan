...
```bash
ruslan teams-pipeline validate
```
...
```bash
ruslan teams-pipeline token-health
ruslan teams-pipeline token-health --force-refresh
```
...
```bash
ruslan teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
ruslan teams-pipeline maintain-subscriptions
ruslan teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Ruslan cron (recommended if you already run the Ruslan gateway)

Ruslan ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.ruslan/scripts/`. First create the script:

```bash
mkdir -p ~/.ruslan/scripts
cat > ~/.ruslan/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec ruslan teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.ruslan/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
ruslan cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/ruslan-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Ruslan Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=ruslan
EnvironmentFile=/etc/ruslan/env
ExecStart=/usr/local/bin/ruslan teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/ruslan-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Ruslan Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now ruslan-teams-pipeline-maintain.timer
systemctl list-timers ruslan-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/ruslan teams-pipeline maintain-subscriptions >> /var/log/ruslan/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.ruslan/.env` at the top of a wrapper script that crontab calls.
...
```bash
ruslan teams-pipeline subscriptions   # should show expirationDateTime advanced
ruslan teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
ruslan teams-pipeline list
ruslan teams-pipeline list --status failed
ruslan teams-pipeline show <job-id>
```

### Replay a stored job

```bash
ruslan teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
ruslan teams-pipeline fetch --meeting-id <meeting-id>
ruslan teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `ruslan teams-pipeline maintain-subscriptions --dry-run`
- inspect `ruslan teams-pipeline list --status failed`
...
- run `ruslan teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `ruslan teams-pipeline run`
...
- [ ] `ruslan teams-pipeline validate` returns an OK snapshot
- [ ] `ruslan teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Ruslan cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
