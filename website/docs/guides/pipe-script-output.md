...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `ruslan send`."
---

# Pipe Script Output to Messaging Platforms

`ruslan send` is a small, scriptable CLI that pushes a message to any
messaging platform Ruslan is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | ruslan send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `ruslan
...
ruslan send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
ruslan send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
ruslan send --list

# Filter by platform
ruslan send --list telegram
```
...
Any platform Ruslan ships adapters for works as a target:
...
`ruslan send` resolves the message body in this order:

1. **Positional argument** — `ruslan send --to telegram "hi"`
2. **`--file PATH`** — `ruslan send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | ruslan send --to telegram`

When stdin is a TTY (no pipe), Ruslan does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  ruslan send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `ruslan send` reuses your Ruslan config, the same script works on
any host where Ruslan is installed — no need to export bot tokens into
...
`ruslan send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  ruslan send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | ruslan send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.ruslan/bin/ruslan send \
...
```bash
./train.py --epochs 200 && \
  ruslan send --to telegram "training done" || \
  ruslan send --to telegram "training failed (exit $?)"
```
...
ruslan send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(ruslan send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `ruslan send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `ruslan send` calls
the platform's REST endpoint directly using credentials from
`~/.ruslan/.env` and `~/.ruslan/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `ruslan gateway start` and retry.
...
# Machine-readable
ruslan send --list --json
```

The listing is built from `~/.ruslan/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`ruslan gateway
...
| Approach | Multi-platform | Reuses Ruslan creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `ruslan send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`ruslan send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `ruslan send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `ruslan send` shares with cron delivery.
