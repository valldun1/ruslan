...
3. Sets `model.provider: nous` in `~/.ruslan/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Ruslan is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
ruslan setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
ruslan auth add nous --type oauth --manual-paste
# Then re-run `ruslan setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Ruslan Agent release notes" and summarize the top 3 hits.
```

You should see Ruslan call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`ruslan setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
ruslan config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Ruslan-4 for agent work

Ruslan-4-70B and Ruslan-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.nousresearch.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Ruslan Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Ruslan-side opinion.
...
These rows appear in `ruslan tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Ruslan runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Ruslan will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
ruslan cron create "every day at 9am" \
...
If you use [Ruslan profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.ruslan/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `ruslan portal info` shows "not logged in" after `ruslan setup --portal`
...
```bash
ruslan config set model.provider nous
```
...
Re-verify with `ruslan portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Ruslan doesn't replay it endlessly. Just log in again:

```bash
ruslan auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/valldun1/ruslan/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`ruslan portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Ruslan profiles where you're using the wrong one (check `ruslan profile list`)

### Want to revoke and start clean

```bash
ruslan auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Ruslan tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Ruslan configurations
