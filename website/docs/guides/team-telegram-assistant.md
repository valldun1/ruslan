...
This tutorial walks you through setting up a Telegram bot powered by Ruslan Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Ruslan Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.ruslan/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Ruslan itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Ruslan Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_ruslan_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Ruslan Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Ruslan Gateway...
...
# View live logs
journalctl --user -u ruslan-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `ruslan gateway install` to update the plist.
:::
...
```bash
ruslan gateway stop && ruslan gateway start
```
...
ruslan pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.ruslan/.env`:
...
Control how much detail the bot shows when using tools. In `~/.ruslan/config.yaml`:
...
Customize how the bot communicates by editing `~/.ruslan/SOUL.md`:

For a full guide, see [Use SOUL.md with Ruslan](/guides/use-soul-with-ruslan).
...
```bash
# From the CLI
ruslan cron list          # View all scheduled jobs
ruslan cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u ruslan-gateway` (Linux) or `~/.ruslan/logs/gateway.log` (macOS) |
| Cron job output | `~/.ruslan/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.ruslan/cron/jobs.json` |
| Pairing data | `~/.ruslan/pairing/` |
| Session history | `~/.ruslan/sessions/` |
