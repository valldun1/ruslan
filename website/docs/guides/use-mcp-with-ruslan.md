---
sidebar_position: 6
title: "Use MCP with Ruslan"
description: "A practical guide to connecting MCP servers to Ruslan Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Ruslan

This guide shows how to actually use MCP with Ruslan Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Ruslan tool
- you want Ruslan to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Ruslan to internal APIs, databases, or company systems without modifying Ruslan core

Do not use MCP when:
- a built-in Ruslan tool already solves the job well
...
- Ruslan remains the agent
- MCP servers contribute tools
- Ruslan discovers those tools at startup or reload time
...
If you installed Ruslan with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Ruslan banner/status should show MCP integration when configured
- ask Ruslan what tools it has available
...
## WSL2: bridge Ruslan in WSL to Windows Chrome

This is the practical setup when:

- Ruslan runs inside WSL2
...
In this setup, Ruslan does **not** connect to Chrome directly. Instead:

- Ruslan runs in WSL
- Ruslan starts a local stdio MCP server
...
```text
Ruslan (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Ruslan stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Ruslan core browser transport
...
```bash
ruslan mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
ruslan mcp test chrome-devtools-win
```

Then start a fresh Ruslan session or run:
...
Once loaded, Ruslan can use the MCP-prefixed browser tools directly. For example:
...
If Ruslan runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Ruslan from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Ruslan from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Ruslan:
...
2. Ruslan-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Ruslan will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Ruslan to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Ruslan to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Ruslan remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
ruslan mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
ruslan mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `ruslan mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Ruslan to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Ruslan to merge, publish, deploy, or spawn runtimes.
...
Now Ruslan can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Ruslan core.
...
Because Ruslan now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
