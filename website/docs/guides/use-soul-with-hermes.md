---
sidebar_position: 7
title: "Use SOUL.md with Ruslan"
description: "How to use SOUL.md to shape Ruslan Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Ruslan

`SOUL.md` is the **primary identity** for your Ruslan instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Ruslan to feel like the same assistant every time you talk to it — or if you want to replace the Ruslan persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Ruslan should be
- what Ruslan should avoid stylistically
- how Ruslan should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Ruslan is and how Ruslan speaks
...
Ruslan now uses only the global SOUL file for the current instance:

```text
~/.ruslan/SOUL.md
```

If you run Ruslan with a custom home directory, it becomes:
...
Ruslan automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Ruslan does not overwrite it
- if the file exists but is empty, Ruslan adds nothing from it to the prompt

## How Ruslan uses it

When Ruslan starts a session, it reads `SOUL.md` from `RUSLAN_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Ruslan feels.
...
Ruslan already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Ruslan should sound.

# Avoid
What Ruslan should not do.

# Defaults
How Ruslan should behave when ambiguity appears.
```
...
Then restart Ruslan or start a new session.
...
### I edited SOUL.md but Ruslan still sounds the same

Check:
- you edited `~/.ruslan/SOUL.md` or `$RUSLAN_HOME/SOUL.md`
...
### Ruslan is ignoring parts of my SOUL.md
