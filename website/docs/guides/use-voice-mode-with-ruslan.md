---
sidebar_position: 8
title: "Use Voice Mode with Ruslan"
description: "A practical guide to setting up and using Ruslan voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Ruslan
...
- you want Ruslan sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Ruslan.
...
## Step 1: make sure normal Ruslan works first
...
Ruslan supports both local and cloud speech stacks.
...
### If you use `ruslan setup`

If you choose NeuTTS in the setup wizard, Ruslan checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Ruslan transcribes and responds
...
- asking Ruslan to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Ruslan loop.

## Tuning CLI behavior

### Silence threshold

If Ruslan starts/stops too aggressively, tune:
...
Ruslan stays a normal chat bot, but can speak replies.
...
- you want Ruslan to function like a portable research or ops assistant
...
Ruslan joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Ruslan detects speech boundaries
- transcripts are posted in the associated text channel
- Ruslan responds in text and audio
...
1. get text Ruslan working
2. install `ruslan-agent[voice]`
