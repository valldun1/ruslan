...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Ruslan new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Ruslan how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Ruslan installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Ruslan to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Ruslan but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
ruslan skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.ruslan/skills/`
...
For how to ship skills in your own plugin, see [Build a Ruslan Plugin → Bundle skills](/guides/build-a-ruslan-plugin#bundle-skills).
...
When a skill with config is first loaded, Ruslan prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
ruslan skills config gif-search

# View all skill config
ruslan config show | grep '^skills\.config'
```
...
```bash
mkdir -p ~/.ruslan/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.ruslan/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
ruslan chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.ruslan/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Ruslan may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Ruslan will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.ruslan/skills/devops/`, `~/.ruslan/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Ruslan to update the skill with what you learned. Skills that aren't maintained become liabilities.
