...
# → Ruslan opens or prints an accounts.x.ai verification URL
...
On servers, containers, browser-only consoles (Cloud Shell, Codespaces, EC2 Instance Connect), or SSH sessions where Ruslan cannot open a browser locally, Ruslan prints the xAI verification URL and user code. Open the URL in any browser on your laptop or in the cloud console, enter the code if prompted, and Ruslan will keep polling until xAI approves the login. No SSH tunnel or local callback listener is required.
...
The same device-code flow applies when you sign in from the web dashboard or the desktop app: Ruslan shows the verification URL and user code, then polls in the background until you approve access.

## How the Login Works

1. Ruslan requests a device code from `auth.x.ai`.
2. You open the verification URL, sign in, enter the displayed code if prompted, and approve access.
3. Ruslan polls xAI until approval, then saves tokens to `~/.ruslan/auth.json`.
...
Device-code approval has a finite expiry window (xAI sets `expires_in` on the device-code response, typically on the order of tens of minutes). If you do not approve the login in time, Ruslan raises a timeout error.

**Fix:** re-run `ruslan auth add xai-oauth` (or `ruslan model`). The flow starts fresh.

### Logging in from a remote server

On SSH or container sessions Ruslan prints the verification URL and user code instead of opening a browser. Open that URL in a browser on your laptop or in a cloud console — no SSH port forward is needed for xAI Grok OAuth.
...
This is **not** a stale-token problem — re-running `ruslan model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/ruslan-agent/issues/26847)).
