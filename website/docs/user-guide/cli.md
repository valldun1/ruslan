...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Ruslan Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Ruslan Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `ruslan setup --portal` — and you're ready to `ruslan chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Ruslan also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `ruslan --tui` — see the [TUI](tui.md) guide.
:::
...
ruslan chat --model "anthropic/claude-sonnet-4"

# With a specific provider
ruslan chat --provider nous        # Use Nous Portal
ruslan chat --provider openrouter  # Force OpenRouter
...
ruslan chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
ruslan --continue             # Resume the most recent CLI session (-c)
ruslan --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
ruslan chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
ruslan -w                         # Interactive mode in worktree
ruslan -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Ruslan CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Ruslan CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `RUSLAN_YOLO_MODE` is on (either `ruslan --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Ruslan to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Ruslan supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Ruslan replies |
...
```bash
ruslan -s ruslan-agent-dev,github-auth
ruslan chat -s github-pr-workflow -s github-auth
```

Ruslan loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.ruslan/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.ruslan/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Ruslan..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Ruslan recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Ruslan — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Ruslan is working, Ruslan prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Ruslan to the background — just like any terminal process. The shell prints a confirmation:

```
Ruslan Agent has been suspended. Run `fg` to bring Ruslan Agent back.
```
...
Resume this session with:
  ruslan --resume 20260225_143052_a1b2c3
...
```bash
ruslan --continue                          # Resume the most recent CLI session
ruslan -c                                  # Short form
ruslan -c "my project"                     # Resume a named session (latest in lineage)
ruslan --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
ruslan --resume "refactoring auth"         # Resume by title
ruslan -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `ruslan sessions rename <id> <title>` from the command line. Use `ruslan sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Ruslan's SQLite state database under `~/.ruslan/state.db`. The database keeps:
...
Ruslan immediately confirms the task and gives you back the prompt:
