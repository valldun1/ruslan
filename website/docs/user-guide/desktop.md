...
sidebar_position: 3
title: "Desktop App"
description: "The native Ruslan desktop app — a polished experience for chatting with Ruslan, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Ruslan desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Ruslan Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `ruslan` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Ruslan has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`ruslan`) and **[TUI](./tui.md)** (`ruslan --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`ruslan dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Ruslan Desktop](../getting-started/installation.md).

If you already have Ruslan installed, simply run
...
- **The same conversation history** as every other Ruslan surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Ruslan instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Ruslan Desktop to a remote backend](./features/web-dashboard.md#connecting-ruslan-desktop-to-a-remote-backend).
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `ruslan desktop --cwd <path>` (or the `RUSLAN_DESKTOP_CWD` environment variable).

### Voice

Talk to Ruslan and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `ruslan model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Ruslan management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#ruslan-cron).
- **Profiles** — switch between [Ruslan profiles](./profiles.md) (isolated config/skills/sessions).
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Ruslan agent, your config, and your chats stay. (Same as `ruslan uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `ruslan uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `ruslan uninstall --full`.)
...
You can do the same from the terminal — `ruslan uninstall --gui` for the GUI alone, or `ruslan uninstall` / `ruslan uninstall --full` for the agent too.

:::note
Running `ruslan uninstall --gui` from a **source checkout** (a `ruslan desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `ruslan desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `ruslan desktop`

To launch via the CLI, simply run `ruslan desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `ruslan update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `RUSLAN_DESKTOP_CWD`)           |
| `--ruslan-root PATH` | Override the Ruslan source root the app uses (sets `RUSLAN_DESKTOP_RUSLAN_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `ruslan` CLI already on `PATH` during backend resolution      |
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `RUSLAN_DASHBOARD_BASIC_AUTH_USERNAME` / `RUSLAN_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.ruslan/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `RUSLAN_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Ruslan Desktop to a remote backend](./features/web-dashboard.md#connecting-ruslan-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Ruslan Desktop](../reference/environment-variables.md#web-dashboard--ruslan-desktop).

## Troubleshooting

Boot logs land in `RUSLAN_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.ruslan/ruslan-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
- [TUI](./tui.md) — the modern terminal UI used by `ruslan --tui` and the dashboard chat tab
