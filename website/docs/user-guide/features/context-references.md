...
Type `@` followed by a reference to inject content directly into your message. Ruslan expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Ruslan env: `$RUSLAN_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$RUSLAN_HOME/skills/.hub/`
