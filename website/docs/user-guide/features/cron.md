...
Schedule tasks to run automatically with natural language or cron expressions. Ruslan exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Ruslan itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
At creation, an unpinned job (one you don't give an explicit `provider`/`model`) follows the global default selected by `ruslan model` — and Ruslan **snapshots** that provider and model on the job. If the global default later changes, the job **fails closed**: it skips the run, makes no inference call, and sends an alert telling you to pin the provider/model explicitly (`cronjob action=update job_id=… provider=… model=…`) to proceed. This prevents an unattended job from silently inheriting a switch to a paid provider/model and spending money you didn't intend (#44585). To make a job deliberately track your global default, pin it to the new values after changing them. `ruslan setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Ruslan disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
ruslan cron create "every 2h" "Check server status"
ruslan cron create "every 1h" "Summarize new feed items" --skill blogwatcher
ruslan cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ruslan will use the unified `cronjob` tool internally.
...
ruslan cron create "every 1d at 09:00" \
...
```bash
ruslan cron edit <job_id> --schedule "every 4h"
ruslan cron edit <job_id> --prompt "Use the revised task"
ruslan cron edit <job_id> --skill blogwatcher --skill maps
ruslan cron edit <job_id> --add-skill maps
ruslan cron edit <job_id> --remove-skill blogwatcher
ruslan cron edit <job_id> --clear-skills
```
...
```bash
ruslan cron list
ruslan cron pause <job_id_or_name>
ruslan cron resume <job_id_or_name>
ruslan cron run <job_id_or_name>
ruslan cron remove <job_id_or_name>
ruslan cron edit <job_id_or_name> [...flags]
ruslan cron status
ruslan cron tick
```
...
```bash
ruslan gateway install     # Install as a user service
sudo ruslan gateway install --system   # Linux: boot-time system service for servers
ruslan gateway             # Or run in foreground
...
1. loads jobs from `~/.ruslan/cron/jobs.json`
...
A file lock at `~/.ruslan/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.ruslan/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.ruslan/cron/output/`), but no message is sent to the delivery target.
...
Or set the `RUSLAN_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.ruslan/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Ruslan directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Ruslan will write the check script to `~/.ruslan/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.ruslan/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.ruslan/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.ruslan/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.ruslan/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Ruslan reads Job 1's most recent output from `~/.ruslan/cron/output/{job1_id}/*.md`
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `ruslan tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `ruslan tools` cron-platform config wins; otherwise Ruslan falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Ruslan should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.ruslan/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.ruslan/scripts/.feed-changed.last"
...
:::tip
Ruslan's own `~/.ruslan/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/ruslan-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.ruslan/cron/jobs.json`. Output from job runs is saved to `~/.ruslan/cron/output/{job_id}/{timestamp}.md`.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `ruslan cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `RUSLAN_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Ruslan resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
