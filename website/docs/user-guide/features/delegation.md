...
When a background delegation finishes, Ruslan stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If Ruslan restarts after completion but before delivery, the pending
...
Ruslan cannot prove whether its external side effects happened. Pending and
...
With a hard cap configured, if a subagent times out having made **zero** API calls (usually: provider unreachable, auth failure, or tool-schema rejection), `delegate_task` writes a structured diagnostic to `~/.ruslan/logs/subagent-timeout-<session>-<timestamp>.log` containing the subagent's config snapshot, credential-resolution trace, and any early error messages. Much easier to root-cause than the previous silent-timeout behavior.
:::
...
By default, `delegate_task` runs **inside the parent's current turn** and blocks until every child finishes. With `background=true`, the child may continue after that turn returns while the owning session and Ruslan process remain alive:
...
- A Ruslan process restart does **not** resume a running child. Its attempt becomes `unknown` because Ruslan cannot prove which side effects happened.
