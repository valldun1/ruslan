...
When Ruslan Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.ruslan/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.ruslan/config.yaml` (switchable per session
via `/personality`).
...
If you use Ruslan' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.ruslan/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Ruslan Agent's deliverable
