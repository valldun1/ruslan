...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Ruslan's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Ruslan instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
ruslan memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `ruslan` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Ruslan syncs them back at session init.
...
The `ruslan honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `ruslan memory setup honcho` (or run `ruslan memory setup` and pick it from the list); the `ruslan honcho` subcommand then appears on the next invocation.

```bash
ruslan memory setup honcho    # Configure Honcho directly (works before activation)
ruslan honcho status          # Connection status, config, and key settings
ruslan honcho setup           # Redirects to `ruslan memory setup` (post-activation alias)
ruslan honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
ruslan honcho peer            # Show or update peer names + dialectic reasoning level
ruslan honcho mode            # Show or set recall mode (hybrid/context/tools)
ruslan honcho tokens          # Show or set token budget for context and dialectic
ruslan honcho identity        # Seed or show the AI peer's Honcho identity
ruslan honcho sync            # Sync Honcho config to all existing profiles
ruslan honcho peers           # Show peer identities across all profiles
ruslan honcho sessions        # List known Honcho session mappings
ruslan honcho map             # Map current directory to a Honcho session name
ruslan honcho enable          # Enable Honcho for the active profile
ruslan honcho disable         # Disable Honcho for the active profile
ruslan honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `ruslan honcho`

If you previously used the standalone `ruslan honcho setup`:
...
No re-login or re-setup needed. Run `ruslan memory setup` and select "honcho" — the wizard detects your existing config.
