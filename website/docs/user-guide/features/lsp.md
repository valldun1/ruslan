...
Ruslan runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Ruslan
...
1. Ruslan captures a baseline of current diagnostics for the file.
...
…). Ruslan auto-detects the binary on PATH or in
`<RUSLAN_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Ruslan at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<RUSLAN_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`ruslan lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Ruslan installs both packages together when you
run `ruslan lsp install typescript` or auto-install fires on first
use.

## CLI

```
ruslan lsp status          # service state + per-server install status
ruslan lsp list            # registry, optionally --installed-only
ruslan lsp install <id>    # eagerly install one server
ruslan lsp install-all     # try every server with a known recipe
ruslan lsp restart         # tear down running clients
ruslan lsp which <id>      # print resolved binary path
```

`ruslan lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <RUSLAN_HOME>/lsp/bin
...
When `install_strategy: auto`, Ruslan installs binaries into
`<RUSLAN_HOME>/lsp/bin/`. NPM packages land in
`<RUSLAN_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Ruslan-owned and is
...
Servers are kept alive for the life of the Ruslan process. There's
...
**`ruslan lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<RUSLAN_HOME>/lsp/bin/`. Run
`ruslan lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `ruslan lsp status`**
...
When `ruslan lsp status` shows a `Backend warnings` section, install
...
`~/.ruslan/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.ruslan/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `ruslan lsp restart` to clear the set;
