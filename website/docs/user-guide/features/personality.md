...
If the file is empty, whitespace-only, or cannot be read, Ruslan falls back to a built-in default identity ("You are Ruslan Agent, an intelligent AI assistant created by Valldun..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
