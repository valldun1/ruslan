...
description: "Customize Ruslan Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Ruslan Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `RUSLAN_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Ruslan is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Ruslan now seeds a default `SOUL.md` automatically in:

```text
~/.ruslan/SOUL.md
```

More precisely, it uses the current instance's `RUSLAN_HOME`, so if you run Ruslan with a custom home directory, it will use:
...
- Ruslan creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Ruslan loads `SOUL.md` only from `RUSLAN_HOME`
- Ruslan does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Ruslan falls back to a built-in default identity
...
If Ruslan loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `RUSLAN_HOME`, the personality belongs to the Ruslan instance itself.

That also makes it easier to teach users:
- "Edit `~/.ruslan/SOUL.md` to change Ruslan' default personality."
...
- how Ruslan should handle uncertainty, disagreement, or ambiguity
...
## What Ruslan injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Ruslan falls back to a built-in default identity ("You are Ruslan Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Ruslan ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Ruslan, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Ruslan its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.ruslan/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.ruslan/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-ruslan)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Ruslan speaks
- `display.skin` and `/skin` affect how Ruslan looks in the terminal
