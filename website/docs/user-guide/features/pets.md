...
Ruslan can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<RUSLAN_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Ruslan renders the sprite at full fidelity when your
...
ruslan pets list
ruslan pets list cat

# Install a pet and make it active in one step
ruslan pets install boba --select
...
## `ruslan pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `ruslan pets list [query] [--limit N]` |
| List installed pets | `ruslan pets list --installed` |
| Install a pet | `ruslan pets install <slug> [--select] [--force]` |
| Set the active pet | `ruslan pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `ruslan pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `ruslan pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `ruslan pets off` |
| Remove an installed pet | `ruslan pets remove <slug>` |
| Diagnose setup | `ruslan pets doctor` |

`ruslan pets show` flags:
...
Beyond installing pre-made pets from the gallery, Ruslan can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `ruslan pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `ruslan tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `RUSLAN_PET_IMAGE_PROVIDER` env var (e.g. `RUSLAN_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Ruslan is
...
Run `ruslan pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Ruslan uses its own
  profile-scoped `<RUSLAN_HOME>/pets/` instead — install through `ruslan pets`.
