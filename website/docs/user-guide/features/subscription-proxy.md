...
chat completions — use your Ruslan-managed provider subscription as their
...
| Use case | "Use Ruslan as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Ruslan stores
the refresh token in `~/.ruslan/auth.json` — the same place all Ruslan
...
Model:      Ruslan-4-70B    # or Ruslan-4.3-36B, Ruslan-4-405B
```
...
interface in `ruslan_cli/proxy/adapters/`.
...
If you see `not logged in`, run `ruslan portal`. If you see
...
`ruslan_cli/proxy/adapters/<provider>.py` and registering it in
