...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Ruslan Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Ruslan Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Ruslan](/guides/use-voice-mode-with-ruslan).
...
:::tip
The `~/.ruslan/` directory and default `config.yaml` are created automatically the first time you run `ruslan`. You only need to create `~/.ruslan/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `ruslan setup --portal` wires both up at once.
:::
...
Voice mode is available in both the **classic CLI** (`ruslan chat`) and the **TUI** (`ruslan --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.ruslan/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
ruslan                # Start the interactive CLI
```
...
1. Start the CLI with `ruslan` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.ruslan/config.yaml` (default: `ctrl+b`).
:::
...
```bash
ruslan gateway        # Start the gateway (connects to configured platforms)
ruslan gateway setup  # Interactive setup wizard for first-time configuration
```
...
```bash
ruslan gateway        # Start with existing configuration
```
...
If you are running Ruslan inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.ruslan/.env`
...
- Review gateway logs: `tail -f ~/.ruslan/logs/gateway.log`
