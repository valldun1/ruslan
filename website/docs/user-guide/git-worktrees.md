...
title: "Git Worktrees"
description: "Run multiple Ruslan agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Ruslan Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Ruslan so each session has a clean, isolated working directory.

## Why Use Worktrees with Ruslan?

Ruslan treats the **current working directory** as the project root:

- CLI: the directory where you run `ruslan` or `ruslan chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.ruslan/config.yaml`
...
git worktree add ../repo-feature feature/ruslan-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/ruslan-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Ruslan there:

```bash
cd ../repo-feature

# Start Ruslan in the worktree
ruslan
```
...
git worktree add ../repo-experiment-a feature/ruslan-a
git worktree add ../repo-experiment-b feature/ruslan-b
```
...
- Works on its own branch (`feature/ruslan-a` vs `feature/ruslan-b`).
...
- Ruslan checkpoint data under `~/.ruslan/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Ruslan experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/ruslan-checkpoints-docs`, `feature/ruslan-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Ruslan from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `ruslan -w` (Automatic Worktree Mode)

Ruslan has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `ruslan/ruslan-<hash>`).
...
```bash
ruslan -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `ruslan -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Ruslan session its own clean checkout.
