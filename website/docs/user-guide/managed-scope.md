...
`~/.ruslan/config.yaml`, `~/.ruslan/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/ruslan`:

```text
/etc/ruslan/
├── config.yaml     # managed config layer (wins over ~/.ruslan/config.yaml)
└── .env            # managed env layer (wins over ~/.ruslan/.env + shell)
```
...
The location can be relocated with the `RUSLAN_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `RUSLAN_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Ruslan.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export RUSLAN_MANAGED_DIR=/opt/org/ruslan-policy
```

:::warning
A user who can set `RUSLAN_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `ruslan doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/ruslan/config.yaml` (managed) | `/etc/ruslan/.env` (managed) |
| 2 | `~/.ruslan/config.yaml` (user) | `~/.ruslan/.env` (user) |
...
```bash
ruslan config        # shows a header naming the managed source + the pinned keys
ruslan doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Ruslan refuses and names the source:

```bash
$ ruslan config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/ruslan/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `ruslan config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/ruslan

# Pin some config values for every user on this machine
sudo tee /etc/ruslan/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/ruslan/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/ruslan
sudo chmod 0644 /etc/ruslan/config.yaml /etc/ruslan/.env
```

Changes take effect on the next Ruslan start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`ruslan doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Ruslan as `root`), managed scope is advisory.
