...
description: "Chat with Ruslan from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Ruslan from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Ruslan](/guides/use-voice-mode-with-ruslan).
...
```mermaid
flowchart TB
    subgraph Gateway["Ruslan Gateway"]
...
For group chats, hooks, and automation flows, Ruslan supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Ruslan keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Ruslan does not hide failures just because the text resembles a silence token.
...
```bash
ruslan gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
ruslan gateway              # Run in foreground
ruslan gateway setup        # Configure messaging platforms interactively
ruslan gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo ruslan gateway install --system   # Linux only: install a boot-time system service
ruslan gateway start        # Start the default service
ruslan gateway stop         # Stop the default service
ruslan gateway status       # Check default service status
ruslan gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Ruslan Agent to the latest version |
...
with the `session_reset` section in `~/.ruslan/config.yaml`:
...
Configure per-platform overrides in `~/.ruslan/gateway.json`:
...
ruslan pairing approve telegram XKGH5N7P

# Other pairing commands:
ruslan pairing list          # View pending + approved users
ruslan pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Ruslan appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.ruslan/config.yaml`:
...
Off by default. When enabled, Ruslan prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.ruslan/config.yaml`:
...
```bash
RUSLAN_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
ruslan gateway install               # Install as user service
ruslan gateway start                 # Start the service
ruslan gateway stop                  # Stop the service
ruslan gateway status                # Check status
journalctl --user -u ruslan-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo ruslan gateway install --system
sudo ruslan gateway start --system
sudo ruslan gateway status --system
journalctl -u ruslan-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Ruslan installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit ruslan-gateway` (or `sudo systemctl edit ruslan-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `ruslan update`. When `ruslan update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart ruslan-gateway` command (it never blocks on an interactive password prompt).
...
```bash
ruslan gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `ruslan update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo ruslan update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/ruslan-gateway`:

```
ruslan ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed ruslan-gateway*, /usr/bin/systemctl --no-ask-password start ruslan-gateway*, /usr/bin/systemctl --no-ask-password restart ruslan-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Ruslan will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Ruslan installations on the same machine (with different `RUSLAN_HOME` directories), each gets its own systemd service name. The default `~/.ruslan` uses `ruslan-gateway`; other installations use `ruslan-gateway-<hash>`. The `ruslan gateway` commands automatically target the correct service for your current `RUSLAN_HOME`.
:::

### macOS (launchd)

```bash
ruslan gateway install               # Install as launchd agent
ruslan gateway start                 # Start the service
ruslan gateway stop                  # Stop the service
ruslan gateway status                # Check status
tail -f ~/.ruslan/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.ruslan.gateway.plist`. It includes three environment variables:
...
- **RUSLAN_HOME** — scopes the gateway to your Ruslan installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `ruslan gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `RUSLAN_HOME` directory gets its own launchd label. The default `~/.ruslan` uses `ai.ruslan.gateway`; other installations use `ai.ruslan.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `ruslan-cli` | Full access |
| Telegram | `ruslan-telegram` | Full tools including terminal |
| Discord | `ruslan-discord` | Full tools including terminal |
| WhatsApp | `ruslan-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `ruslan-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `ruslan-slack` | Full tools including terminal |
| Google Chat | `ruslan-google_chat` | Full tools including terminal |
| Signal | `ruslan-signal` | Full tools including terminal |
| SMS | `ruslan-sms` | Full tools including terminal |
| Email | `ruslan-email` | Full tools including terminal |
| Home Assistant | `ruslan-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `ruslan-mattermost` | Full tools including terminal |
| Matrix | `ruslan-matrix` | Full tools including terminal |
| DingTalk | `ruslan-dingtalk` | Full tools including terminal |
| Feishu/Lark | `ruslan-feishu` | Full tools including terminal |
| WeCom | `ruslan-wecom` | Full tools including terminal |
| WeCom Callback | `ruslan-wecom-callback` | Full tools including terminal |
| Weixin | `ruslan-weixin` | Full tools including terminal |
| BlueBubbles | `ruslan-bluebubbles` | Full tools including terminal |
| QQBot | `ruslan-qqbot` | Full tools including terminal |
| Yuanbao | `ruslan-yuanbao` | Full tools including terminal |
| Microsoft Teams | `ruslan-teams` | Full tools including terminal |
| API Server | `ruslan-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `ruslan-webhook` | Full tools including terminal |
| Raft | `ruslan-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.ruslan/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
