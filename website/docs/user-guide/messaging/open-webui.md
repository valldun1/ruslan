...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to Ruslan Agent via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With Ruslan Agent's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
flowchart LR
    ...
    B["ruslan-agent<br/>gateway API server<br/>port 8642"]
    A -->|POST /v1/chat/completions| B
    B -->|SSE streaming response| A
```

Open WebUI connects to Ruslan Agent's API server just like it would connect to OpenAI. Ruslan handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

:::important Runtime location
The API server is a **Ruslan agent runtime**, not a pure LLM proxy. For each request, Ruslan creates a server-side `AIAgent` on the API-server host. Tool calls run where that API server is running.

For example, if a laptop points Open WebUI or another OpenAI-compatible client at a Ruslan API server on a remote machine, `pwd`, file tools, browser tools, local MCP tools, and other workspace tools run on the remote API-server host, not on the laptop.
:::

Open WebUI talks to Ruslan server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.
...
```bash
ruslan config set API_SERVER_ENABLED true
ruslan config set API_SERVER_KEY your-secret-key
```

`ruslan config set` auto-routes the flag to `config.yaml` and the secret to `~/.ruslan/.env`. If the gateway is already running, restart it so the change takes effect:

```bash
ruslan gateway stop && ruslan gateway
```

### 2. Start Ruslan Agent gateway
...
# {"object":"list","data":[{"id":"ruslan-agent", ...}]}
```
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **ruslan-agent** for the default profile). Start chatting!
...
6. Enter:
   ...
   - **API Key**: the exact same value as `API_SERVER_KEY` in Ruslan
7. Click the **checkmark** to verify the connection
8. **Save**

Your agent model should now appear in the model dropdown (named after your profile, or **ruslan-agent** for the default profile).
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and Ruslan Agent responds accordingly. Each request includes the full conversation history.
...
2. Edit your ruslan-agent connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and Ruslan Agent can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, Ruslan also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. Ruslan Agent creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | Ruslan Agent's API URL (include `/v1`) |
...
- **Check model listing**: `curl -H "Authorization: Bearer your-secret-key" http://localhost:8642/v1/models` should return a list with `ruslan-agent`
...
- **Empty Ollama backend shadowing the picker**: If you omitted `ENABLE_OLLAMA_API=false`, Open WebUI shows an empty Ollama section above your Ruslan models. Restart the container with `-e ENABLE_OLLAMA_API=false` or disable Ollama in **Admin Settings → Connections**.
...
Ruslan Agent may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in Ruslan Agent.
...
To run separate Ruslan instances per user — each with their own config, memory, and skills — use [profiles](/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated Ruslan agent.
...
```bash
ruslan -p alice config set API_SERVER_MODEL_NAME "Alice's Agent"
```
:::
