...
`simplex:group:12` as a cron `deliver=` target or in a `ruslan send` call.
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`ruslan send` CLI](/guides/pipe-script-output):

```bash
ruslan send simplex:<contact-id> "Done!"
```
