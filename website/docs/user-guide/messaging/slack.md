...
sidebar_position: 4
title: "Slack"
description: "Set up Ruslan Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Ruslan Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Ruslan instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Ruslan uses the modern
...
The fastest path is to paste a manifest Ruslan generates for you. It
...
### Option A: From a Ruslan-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   ruslan slack manifest --agent-view --write
   ```
   This writes `~/.ruslan/slack-manifest.json` and prints paste-in
...
4. Enter an app name (e.g., "Ruslan Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Ruslan can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `ruslan-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Ruslan configuration issue.
:::
...
Ruslan uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
ruslan gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
ruslan gateway              # Foreground
ruslan gateway install      # Install as a user service
sudo ruslan gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Ruslan now
...
Every Ruslan command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Ruslan command with its description.

Under the hood: Ruslan ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/ruslan-agent/blob/main/ruslan_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing Ruslan
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Ruslan can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Ruslan only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When Ruslan adds new commands (e.g. after `ruslan update`), regenerate
the manifest and update your Slack app:

```bash
ruslan slack manifest --write
```
...
3. Paste the new contents of `~/.ruslan/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/ruslan <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/ruslan btw run the tests` — Ruslan routes it the same way as `/btw
run the tests`. Free-form questions also work: `/ruslan what's the
...
Slack never delivers them to Ruslan.

As a workaround, Ruslan recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Ruslan treats it identically to the slash form and replies in the same
thread.
...
Ruslan falls back to a text prompt, the prompt instructs you to reply
...
```bash
ruslan slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Ruslan behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Ruslan Agent what time is it?`). In channels, Ruslan replies in a thread attached to that message. |
| **Threads** | If you @mention Ruslan inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.ruslan/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Ruslan in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Ruslan will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Ruslan will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Ruslan Agent`).

---

## Multi-Workspace Support

Ruslan can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Ruslan also loads tokens from an **OAuth token file** at:

```
~/.ruslan/slack_tokens.json
```
...
- When a message arrives, Ruslan uses the correct workspace-specific client to respond.
...
Ruslan supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Ruslan Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Ruslan Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Ruslan now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Ruslan Agent`)
...
- Tokens should be stored in `~/.ruslan/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Ruslan config directory
