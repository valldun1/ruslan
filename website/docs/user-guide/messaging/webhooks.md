...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Ruslan Agent — Webhooks Tutorial"
...
1. Enable via `ruslan gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `ruslan webhook subscribe`
...
| `script` | No | Filter/transform script under `~/.ruslan/scripts/`. The webhook payload is passed as JSON on stdin. JSON object stdout replaces the payload before templating; text stdout is exposed as `script_output`; empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. |
          ...
          filters:
            ...
            - any:
                ...
                - field: "payload.project_id"
                  in_file: "~/.ruslan/data/todoist/watchlist.json"
...
Use `script` when declarative filters are not enough. Scripts must live under `~/.ruslan/scripts/` for the active profile; relative paths resolve there, and path traversal outside that directory is blocked. `.sh` and `.bash` scripts run with bash, and all other extensions run with the current Python interpreter.

The route payload is sent to stdin as JSON:

```python
# ~/.ruslan/scripts/todoist-ruslan-label.py
import json
import sys

payload = json.load(sys.stdin)
labels = payload.get("payload", {}).get("labels", [])
if "ruslan" not in labels:
    print("[SILENT]")
    raise SystemExit(0)
...
- Empty stdout, exact `[SILENT]`, `{"__ruslan_ignore__": true}`, timeout, missing script, or nonzero exit code returns HTTP 200 with `{"status":"ignored","reason":"script"}`.
...
Add the `github-pr` route to your `~/.ruslan/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Ruslan processes the event, and posts a review comment on the PR.
...
```bash
ruslan webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `ruslan webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.

### Create a subscription

```bash
ruslan webhook subscribe github-issues \
  --events "issues" \
...
```bash
ruslan webhook remove github-issues
```

### Test a subscription

```bash
ruslan webhook test github-issues
ruslan webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.ruslan/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `ruslan webhook subscribe` command.
...
This is the same trust model that applies to everything the agent reads: web pages, files, and tool output are all untrusted input. Ruslan does not — and cannot reliably — sanitize untrusted text with a blocklist; phrasing, encoding, and translation make that trivially bypassable. **The trust boundary is the agent's capability surface, not the input channel.** Harden there:
...
- Run the gateway in foreground to see logs: `ruslan gateway run`
