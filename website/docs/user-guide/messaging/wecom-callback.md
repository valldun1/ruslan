...
Connect Ruslan to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Ruslan supports two WeCom integration modes:
...
> Run `ruslan gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Ruslan decrypts the message, queues it for the agent
...
(Use `ruslan gateway start` only after `ruslan gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Ruslan and the token the
...
in `~/.ruslan/.env` values around `=` will also break signature checks. After
fixing, restart `ruslan gateway run`.
...
Check `ruslan gateway run` logs for the bound host/port. If the adapter bound to
