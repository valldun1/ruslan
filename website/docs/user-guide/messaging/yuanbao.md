...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Ruslan Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Ruslan to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.ruslan/.env`:
...
All standard Ruslan commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.ruslan/logs/gateway.log`
...
4. Consider enabling verbose logging: `RUSLAN_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Ruslan automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
RUSLAN_LOG_LEVEL=debug ruslan gateway
```
...
```bash
ruslan chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
