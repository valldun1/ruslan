...
If you only run one Ruslan agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Ruslan agents that should all
...
(`ai.ruslan.gateway-<name>.plist`) or systemd user service
(`ruslan-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
ruslan profile create coder
ruslan profile create personal-bot
ruslan profile create research
...
```bash
ruslan config set gateway.multiplex_profiles true
ruslan gateway restart
```

Equivalently, in the default profile's `~/.ruslan/config.yaml`:
...
You do **not** run `ruslan gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `ruslan gateway start` / `run` is a
...
home). `ruslan status` reports the multiplexer and the profiles it serves;
`ruslan status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/ruslan-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: ruslan-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    ruslan gateway "$action"
  else
    ruslan -p "$profile" gateway "$action"
  fi
}
...
```bash
ruslan-gateways start      # start every configured profile
ruslan-gateways stop       # stop every configured profile
ruslan-gateways restart    # restart all
ruslan-gateways status     # status across all
ruslan-gateways list       # delegates to `ruslan gateway list`
```

:::tip
The `default` profile is targeted with `ruslan gateway <action>` (no `-p`),
not `ruslan -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `ruslan -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.ruslan.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/ruslan-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.ruslan.gateway.plist` /
`ruslan-gateway.service`.
...
```bash
# Default profile
tail -f ~/.ruslan/logs/gateway.log
tail -f ~/.ruslan/logs/gateway.error.log

# Named profile
tail -f ~/.ruslan/profiles/<name>/logs/gateway.log
tail -f ~/.ruslan/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.ruslan/logs/gateway.log ~/.ruslan/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
ruslan logs -f                  # follow default profile
ruslan -p coder logs -f         # follow one profile
ruslan logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
ruslan profile list             # profiles + model + gateway state
ruslan-gateways status          # full status across every profile
launchctl list | grep ruslan    # macOS — PIDs and labels
systemctl --user list-units 'ruslan-gateway-*'   # Linux — units
```
...
```
~/.ruslan/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.ruslan/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
ruslan config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.ruslan/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=ruslan --why="gateways running" \
  sleep infinity &
...
`ruslan-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.ruslan/.env ~/.ruslan/profiles/*/.env
```

## Updating the code

`ruslan update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `ruslan gateway start` after a previous `ruslan gateway stop`. The
...
```bash
ps -ef | grep "ruslan_cli.*-p <profile>"
cat ~/.ruslan/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.ruslan.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.ruslan.gateway-<profile>.plist

# Linux
systemctl --user restart ruslan-gateway-<profile>.service
```

### Health check

```bash
ruslan doctor                  # default profile
ruslan -p <profile> doctor     # one profile
```
