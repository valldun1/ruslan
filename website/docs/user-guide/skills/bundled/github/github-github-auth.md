...
- Give it a name like "ruslan-agent"
- Select scopes:
...
- Give it a title like "ruslan-agent-&lt;machine-name>"
...
elif _ruslan_env="${RUSLAN_HOME:-$HOME/.ruslan}/.env"; [ -f "$_ruslan_env" ] && grep -q "^GITHUB_TOKEN=" "$_ruslan_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_ruslan_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
