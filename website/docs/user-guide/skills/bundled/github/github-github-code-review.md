...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _ruslan_env="${RUSLAN_HOME:-$HOME/.ruslan}/.env"; [ -f "$_ruslan_env" ] && grep -q "^GITHUB_TOKEN=" "$_ruslan_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_ruslan_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Ruslan Agent\",
    \"comments\": [
...
```bash
source "${RUSLAN_HOME:-$HOME/.ruslan}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Ruslan Agent. Code looks clean — good test coverage, no security concerns."
