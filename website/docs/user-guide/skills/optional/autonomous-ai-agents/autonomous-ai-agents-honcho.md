...
description: "Configure and use Honcho memory with Ruslan -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Ruslan -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `ruslan skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Ruslan

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Ruslan profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/ruslan#running-honcho-locally-with-ruslan

### Verify

```bash
ruslan honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Ruslan profile's AI peer
...
Honcho models conversations as interactions between **peers**. Ruslan creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Ruslan instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Ruslan run |
| `global` | Single session across all directories |

Manual override: `ruslan honcho map my-project-name`
...
Each Ruslan profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block ruslan.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `ruslan.coder` host block in `honcho.json`
...
```bash
ruslan honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Ruslan when Honcho memory is active.
...
Config file: `$RUSLAN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `ruslan honcho setup`. Ensure `memory.provider: honcho` is in `~/.ruslan/config.yaml`.

### Memory not persisting across sessions
Check `ruslan honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `ruslan profile create <name> --clone`. For existing profiles: `ruslan honcho sync`.
...
| Command | Description |
|---------|-------------|
| `ruslan honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `ruslan honcho status` | Show resolved config, connection test, peer info for active profile |
| `ruslan honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `ruslan honcho disable` | Disable Honcho for the active profile |
| `ruslan honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `ruslan honcho peers` | Show peer identities across all profiles |
| `ruslan honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `ruslan honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `ruslan honcho sessions` | List known directory-to-session-name mappings |
| `ruslan honcho map <name>` | Map current working directory to a Honcho session name |
| `ruslan honcho identity` | Seed AI peer identity or show both peer representations |
| `ruslan honcho sync` | Create host blocks for all Ruslan profiles that don't have one yet |
| `ruslan honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Ruslan + Honcho |
| `ruslan memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `ruslan memory status` | Show active memory provider and config |
| `ruslan memory off` | Disable external memory provider |
