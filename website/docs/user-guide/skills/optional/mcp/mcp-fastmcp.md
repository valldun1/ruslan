...
| | |
|---|---|
| Source | Optional — install with `ruslan skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Ruslan or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Ruslan. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.ruslan/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.ruslan/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Ruslan integration, either:

- configure the server in `~/.ruslan/config.yaml` using the `native-mcp` skill, or
...
### Ruslan cannot see the deployed server

The server-building part may be correct while the Ruslan config is not. Load the `native-mcp` skill and configure the server in `~/.ruslan/config.yaml`, then restart Ruslan.
