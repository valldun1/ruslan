---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Ruslan Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Ruslan Agent"
---
...
Migrate a user's OpenClaw customization footprint into Ruslan Agent. Imports Ruslan-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Related skills | [`ruslan-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-ruslan-agent) |
...
# OpenClaw -> Ruslan Migration

Use this skill when a user wants to move their OpenClaw setup into Ruslan Agent with minimal manual cleanup.
...
```bash
ruslan claw migrate              # Full interactive migration
...
ruslan claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `ruslan setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_ruslan.py` to:

- import `SOUL.md` into the Ruslan home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Ruslan memory entries
- merge OpenClaw command approval patterns into Ruslan `command_allowlist`
- migrate Ruslan-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Ruslan working-directory configuration
- copy OpenClaw skills into `~/.ruslan/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Ruslan workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.ruslan/tts/`
- archive non-secret docs that do not have a direct Ruslan destination
...
- `scripts/openclaw_to_ruslan.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.ruslan/skills/migration/openclaw-migration/scripts/openclaw_to_ruslan.py`

Do not guess a shorter path like `~/.ruslan/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.ruslan/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Ruslan-compatible secrets, currently:
...
Ruslan CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Ruslan. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Ruslan workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Ruslan skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.ruslan/skills/migration/openclaw-migration/scripts/openclaw_to_ruslan.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.ruslan/skills/migration/openclaw-migration/scripts/openclaw_to_ruslan.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.ruslan/skills/migration/openclaw-migration/scripts/openclaw_to_ruslan.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.ruslan/skills/migration/openclaw-migration/scripts/openclaw_to_ruslan.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.ruslan/skills/migration/openclaw-migration/scripts/openclaw_to_ruslan.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.ruslan/skills/migration/openclaw-migration/scripts/openclaw_to_ruslan.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Ruslan unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Ruslan targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Ruslan destination. Unsupported auth blobs must still be reported as skipped.
...
- Ruslan persona state imported
- Ruslan memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.ruslan/skills/openclaw-imports/`
