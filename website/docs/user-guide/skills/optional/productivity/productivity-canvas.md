...
| | |
|---|---|
| Source | Optional — install with `ruslan skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Ruslan Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${RUSLAN_HOME:-~/.ruslan}/.env`:
...
```bash
CANVAS="python $RUSLAN_HOME/skills/productivity/canvas/scripts/canvas_api.py"
