...
| | |
|---|---|
| Source | Optional — install with `ruslan skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${RUSLAN_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${RUSLAN_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${RUSLAN_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client ruslan
```
...
```bash
PUBLISH="${RUSLAN_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client ruslan
```
...
```bash
DRIVE="${RUSLAN_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `ruslan`)    |
