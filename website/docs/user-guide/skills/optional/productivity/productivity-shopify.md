...
| | |
|---|---|
| Source | Optional — install with `ruslan skills install official/productivity/shopify` |
...
4. Save to `${RUSLAN_HOME:-~/.ruslan}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Ruslan","productType":"Apparel","tags":["test"]}}'
```
