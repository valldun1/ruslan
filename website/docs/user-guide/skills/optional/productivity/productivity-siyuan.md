...
| | |
|---|---|
| Source | Optional — install with `ruslan skills install official/productivity/siyuan` |
...
3. Store it in `${RUSLAN_HOME:-~/.ruslan}/.env`:
   ```
...
```yaml
# In ~/.ruslan/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
