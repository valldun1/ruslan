...
| | |
|---|---|
| Source | Optional — install with `ruslan skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Ruslan core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Ruslan native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Ruslan native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Ruslan native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Ruslan usage patterns
...
- Do not silently choose Parallel when Ruslan native tools are already sufficient.
