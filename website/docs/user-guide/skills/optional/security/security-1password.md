...
| | |
|---|---|
| Source | Optional — install with `ruslan skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Ruslan Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Ruslan terminal calls (desktop app flow only)
...
### Service Account (recommended for Ruslan)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${RUSLAN_HOME:-~/.ruslan}/.env` (the skill will prompt for this on first load).
...
## Ruslan Execution Pattern (desktop app flow)

Ruslan terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/ruslan-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/ruslan-op.sock"
