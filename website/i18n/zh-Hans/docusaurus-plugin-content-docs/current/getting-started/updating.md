...
想在拉取前确认是否有更新？运行 `ruslan update --check` — 它会获取并与 `origin/main` 比较提交。不修改任何文件，不重启 gateway。适合在以"是否有更新"为条件的脚本和 cron 任务中使用。
...
与 [GitHub releases 页面](https://github.com/NousResearch/ruslan-agent/releases) 上的最新版本进行比较。
