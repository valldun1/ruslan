...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/ruslan-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    ruslan_meta = (fm.get("metadata") or {}).get("ruslan") or {}
    tags = ruslan_meta.get("tags") or []
    related = ruslan_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `ruslan skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Ruslan Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Ruslan ships with a large built-in skill library copied into `~/.ruslan/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Ruslan also syncs bundled skills on `ruslan update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.ruslan/skills/` tree, it is still shipped with Ruslan; restore it with `ruslan skills reset <name> --restore`.",
        "",
